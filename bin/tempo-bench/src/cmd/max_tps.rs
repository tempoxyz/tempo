...
pub struct MaxTpsArgs {
    ...
    /// Batch size in seconds for signing transactions with expiring nonces.
    ///
    ...
    /// transactions to generate/sign before sending. Default: 15 seconds.
    #[arg(long)]
...
impl MaxTpsArgs {
    ...
    pub async fn run(self) -> eyre::Result<()> {
        ...
        info!(
            ...
            expiry_secs, "Creating signers (with expiring nonces)"
        );
    ...
    async fn run_with_manager<F: TxFiller<TempoNetwork> + 'static>(
        ...
        let mut pending_txs = all_pending_txs;
...
/// Sends pre-signed transactions at the target TPS rate with external counters for batch mode.
