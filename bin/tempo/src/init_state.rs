...
pub struct InitFromBinaryDump<C: reth_cli::chainspec::ChainSpecParser = TempoChainSpecParser> {
    ...
    /// Use `-` to read the dump from stdin.
    ...
    InitFromBinaryDump<C>
{
        ...
        N: CliNodeTypes<ChainSpec = C::ChainSpec>,
    {
        ...
        let input: Box<dyn Read> = if is_stdin_path(&self.state) {
            info!(target: "tempo::cli", "Loading binary state dump from stdin");
            Box::new(io::stdin())
        } else {
        ...
        let mut reader = BufReader::with_capacity(64 * 1024 * 1024, input);
