use std::{ffi::OsString, fs::OpenOptions, path::PathBuf, sync::Arc};
...
pub(crate) enum TempoSubcommand {
    ...
    /// Any unrecognized subcommand is dispatched to a `tempo-<name>` binary.
...
impl ExtendedCommand for TempoSubcommand {
    fn execute(self, runner: CliRunner) -> eyre::Result<()> {
        match self {
            ...
            Self::External(args) => plugin::dispatch_external(args),
        }
    }
}
