...
# an Actions artifact. If the nightly Docker build is stale or unchanged, the
...
LAST_FEATURE_REF=$(node "$(dirname "${BASH_SOURCE[0]}")/bench-read-state.js" "$REPO" "${STATE_FILE##*/}" bench-replay-scheduled.yml)
