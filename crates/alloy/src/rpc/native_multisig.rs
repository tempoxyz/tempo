...
fn assemble_mock_multisig_signature(
    ...
    use tempo_primitives::transaction::MultisigWeightAccumulator;
    ...
    let mut weight =
        MultisigWeightAccumulator::new(config.threshold).map_err(|error| error.to_string())?;
...
fn record_simulation_owner(
    ...
    if weight.has_quorum() && !is_last {
        return Err(MultisigQuorumError::ExcessSignatures.to_string());
    }
    Ok(())
}
...
mod tests {
    ...
    #[cfg(feature = "revm")]
    #[test]
    fn simulation_signature_reports_account_mismatch_values() {
        let owner = address!("0x1111111111111111111111111111111111111111");
        ...
        let actual = address!("0x2222222222222222222222222222222222222222");
        let spec = MultisigSimulationSpec {
            config,
            approvals: vec![MultisigSimulationApproval::Primitive(
                MultisigSimulationPrimitiveApproval {
        ...
        assert_eq!(
            create_mock_native_multisig_signature(actual, &spec).unwrap_err(),
            format!("initial multisig account mismatch: expected {expected}, actual {actual}")
        );
    }
