...
use alloy_primitives::{
    Address, TxHash,
    map::{AddressMap, B256Set, HashSet},
};
...
///
/// Note: Transaction expiry (`valid_before` and key expiry) is now tracked directly
/// by the pool via the shared `ExpiryTracker`. This state only holds non-expiry
/// maintenance data.
#[derive(Default)]
        ...
        + 'static,
{
    ...
    // Get shared expiry tracker - tracks valid_before and key expiry
    // Pool add/remove already updates this, so no startup population needed
    let expiry_tracker = pool.expiry_tracker();
    ...
    loop {
        tokio::select! {
            ...
            Some(event) = chain_events.next() => {
                ...
                // Collect mined transaction hashes for use later (expiring nonce tx removal).
                ...
                // 2. Drain expired transactions from the shared expiry tracker
                // The pool already untracks removed transactions, so we just need to
                // drain expired ones and filter by pool.contains() to catch any that
                // were removed between tracking and now.
                let expired_start = Instant::now();
                let expired = expiry_tracker.drain_expired(tip_timestamp);

                // Evict valid_before expired transactions
                let valid_before_expired: Vec<TxHash> = expired
                    .valid_before_expired
                    .into_iter()
                    .filter(|h| pool.contains(h))
                    .collect();
                let valid_before_count = valid_before_expired.len();
                if valid_before_count > 0 {
                    debug!(
                        target: "txpool",
                        count = valid_before_count,
                        tip_timestamp,
                        "Evicting expired AA transactions (valid_before)"
                    );
                    pool.remove_transactions(valid_before_expired.clone());
                    metrics.expired_transactions_evicted.increment(valid_before_count as u64);
                }
                updates.expired_txs = valid_before_expired;

                // Evict key expired transactions
                let key_expired: Vec<TxHash> = expired
                    .key_expired
                    .into_iter()
                    .filter(|h| pool.contains(h))
                    .collect();
                ...
                if state.pending_staleness.should_check(tip_timestamp) {
                    ...
                    if !stale_to_evict.is_empty() {
                        ...
                        // For 2D pool txs, removal will push to removed_txs which we drain next block.
                        // For protocol pool txs, they're handled lazily via pool.contains() filtering.
