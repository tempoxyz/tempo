...
fn into_origin_pooled_transaction(
    tx: Arc<ValidPoolTransaction<TempoPooledTransaction>>,
) -> (TransactionOrigin, TempoPooledTransaction) {
    match Arc::try_unwrap(tx) {
        Ok(tx) => (tx.origin, tx.transaction),
        Err(tx) => (tx.origin, tx.transaction.clone()),
    }
}
                    ...
                    if !hashes.is_empty() {
                        ...
                        tokio::spawn(async move {
                            let txs: Vec<_> = removed_txs
                                .into_iter()
                                .map(into_origin_pooled_transaction)
                                .collect();
