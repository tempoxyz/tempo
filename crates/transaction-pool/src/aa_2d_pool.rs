...
impl AA2dPool {
    ...
    ///
    /// ## Limitations
    /// * This currently assumes that the account's nonce key is already tracked in
    ///   [`AA2dNonceKeys`], if not then this transaction is considered pending.
    pub(crate) fn add_transaction(
        &mut self,
        transaction: Arc<ValidPoolTransaction<TempoPooledTransaction>>,
    ) -> PoolResult<AddedTransaction<TempoPooledTransaction>> {
        debug_assert!(transaction.transaction.is_aa_2d(), "only AA transactions are supported");
        ...
        let on_chain_nonce = self
            .nonce_keys
            .current_nonce(transaction.transaction.sender_ref(), &tx_id.seq_id.nonce_key)
            .unwrap_or_default();
    ...
    /// [`NONCE_PRECOMPILE_ADDRESS`](tempo_precompiles::NONCE_PRECOMPILE_ADDRESS).
    ///
...
impl AA2dNonceKeys {
    ...
    /// Update the tracked nonces for the slots and return the new on chain identifiers for the
    /// changed slots.
    ///
    ...
    pub fn tracked_counts(&self) -> (usize, usize) {
        ...
        let num_nonce_keys = inner.address_to_nonce_keys.values().map(|keys| keys.len()).sum();
...
impl AA2dNonceKeysInner {
    ...
    fn current_nonce(&self, address: &Address, nonce_key: &U256) -> Option<u64> {
        self.address_to_nonce_keys.get(address)?.get(nonce_key).copied()
    }
