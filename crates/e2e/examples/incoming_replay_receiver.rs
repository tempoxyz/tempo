...
//! SOURCE RECEIVER GENESIS FROM TO_EXCLUSIVE off|on OUTPUT_JSON [PACE_MS_OR_SOURCE] [PERSISTENCE_THRESHOLD] [NUM_STATE_MASKING_BLOCKS]
...
use reth_ethereum::{
    ...
    storage::{
        BlockHashReader, BlockNumReader, BlockReader, DatabaseProviderFactory, ReceiptProvider,
...
async fn main() -> eyre::Result<()> {
    ...
    let mut engine_args = reth_node_core::args::EngineArgs::default();
    if let Some(value) = args.get(8) {
        engine_args.persistence_threshold = value.parse()?;
    }
    if let Some(value) = args.get(9) {
        engine_args.num_state_masking_blocks = value.parse()?;
    }
    engine_args.validate()?;
    ...
    config.persistence_threshold = Some(engine_args.persistence_threshold);
    config.num_state_masking_blocks = engine_args.num_state_masking_blocks;
    ...
    for (index, (block, expected_receipts)) in inputs.into_iter().enumerate() {
        ...
        let checkpoint = node
            ...
            .ok_or_eyre("missing receiver Finish checkpoint")?
            .1;
        ...
        rows.push(serde_json::json!({"number": number, "database_finish":checkpoint.block_number,"partial_state_trie":checkpoint.finish_stage_checkpoint().and_then(|finish| finish.partial_state_trie()), "schedule_lag_us": schedule_lag_us, "hash": hash, "state_root": root, "receipts_root": receipt_root, "transactions": tx_count, "successful_transactions": successful_transactions, "failed_transactions": failed_transactions, "timestamp_millis": timestamp_millis, "gas_used": gas_used, "new_payload_us": new_payload_us, "fcu_us": fcu_us, "exact_receipts": true}));
        eprintln!(
    ...
    std::fs::write(
        ...
        serde_json::to_vec_pretty(
            &serde_json::json!({"source":source_path,"receiver":receiver_path,"from":from,"to_exclusive":to,"parent":parent,"incoming_replay":enabled,"pace":pace,"persistence_threshold":engine_args.persistence_threshold,"num_state_masking_blocks":engine_args.num_state_masking_blocks,"memory_block_buffer_target":engine_args.memory_block_buffer_target(),"entrypoint":"BeaconConsensusEngineHandle::new_payload -> EngineApiTreeHandler::on_new_payload -> BasicEngineValidator::execute_transactions", "allocator":"jemalloc", "asm_keccak":true, "keccak_cache_global":true, "prometheus_recorder":true, "available_parallelism":std::thread::available_parallelism()?.get(),"blocks":rows}),
        )?,
    )?;
    Ok(())
}
