...
fn can_restart_after_joining_from_snapshot() {
    ...
    let cfg = deterministic::Config::default().with_seed(setup.seed);
    let executor = Runner::from(cfg);
    executor.start(move |ctx| _can_restart_after_joining_from_snapshot(ctx, setup))
}

async fn _can_restart_after_joining_from_snapshot(mut context: Context, setup: Setup) {
    let (mut validators, execution_runtime) = setup_validators(&mut context, setup.clone()).await;
