        ...
        let rt = std::thread::spawn(move || {
            ...
            rt.block_on(async move {
                while let Some(msg) = from_handle.recv().await {
                    // create a new task runtime for the new node instance
                    let task_manager = TasksRuntime::test();
                    match msg {
...
pub struct ExecutionNode {
    ...
    /// The [`TasksRuntime`] that drives the node's services.
    pub task_manager: TasksRuntime,
