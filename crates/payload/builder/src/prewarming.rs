...
use tempo_precompiles::{
    ...
    storage::{StorageKey as _, domains, raw_address_slot},
...
fn add_tip20_call_touches(
    ...
    match call {
        ITIP20::ITIP20Calls::transfer(call) => {
            ...
            add_tip403_receive_policy_config_touch(touches, call.to);
...
fn add_tip20_reward_touches(touches: &mut Vec<StorageTouch>, token: Address, account: Address) {
    let base_slot = raw_address_slot::<{ domains::TIP20_USER_REWARD_INFO }>(account);
...
fn add_tip403_receive_policy_config_touch(touches: &mut Vec<StorageTouch>, account: Address) {
    add_storage_touch(
        touches,
        TIP403_REGISTRY_ADDRESS,
        raw_address_slot::<{ domains::TIP403_RECEIVE_POLICIES }>(account),
    );
}
...
mod tests {
    ...
    fn tip20_touch_collection_dedups_overlapping_fee_and_call_slots() {
        ...
        assert!(touches.contains(&StorageTouch::Storage {
            address: token,
            slot: raw_address_slot::<{ domains::TIP20_BALANCES }>(sender)
        }));
        assert!(touches.contains(&StorageTouch::Storage {
            address: token,
            slot: raw_address_slot::<{ domains::TIP20_BALANCES }>(recipient)
        }));
        assert!(touches.contains(&StorageTouch::Storage {
            address: token,
            slot: raw_address_slot::<{ domains::TIP20_USER_REWARD_INFO }>(sender)
        }));
        assert!(touches.contains(&StorageTouch::Storage {
            address: TIP403_REGISTRY_ADDRESS,
            slot: raw_address_slot::<{ domains::TIP403_RECEIVE_POLICIES }>(recipient)
        }));
    }
