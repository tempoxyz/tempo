        ...
        self.executor
            .spawn_blocking_named("builder-roots-task", move || {
                ...
                while let Ok((tx, receipt)) = transactions_rx.recv() {
                    if transactions.len() == transactions.capacity() {
                        let queued = transactions_rx.len();
                        transactions.reserve(queued.saturating_add(1));
                        senders.reserve(queued.saturating_add(1));
                    }
