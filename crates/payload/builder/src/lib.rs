        ...
        StateProviderFactory + ChainSpecProvider<ChainSpec = TempoChainSpec> + Clone + 'static,
{
            ...
            + 'static,
    {
        ...
        let state_root_task_hook = state_root_handle
            .as_mut()
            .map(|handle| handle.take_state_hook());
        let bal_task_handle = if self.enable_bal {
            let bal_task_handle = self.spawn_bal_task(state_root_task_hook);
            executor
                .evm_mut()
                .db_mut()
            ...
            if let Some(state_root_task_hook) = state_root_task_hook {
                executor
                    .evm_mut()
                    .db_mut()
                    .set_state_hook(Some(Box::new(state_root_task_hook)));
            }
            None
        };
