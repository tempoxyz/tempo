...
pub fn calc_gas_balance_spending(gas_limit: u64, gas_price: u128) -> U256 {
    if let Some(spending) = (gas_limit as u128).checked_mul(gas_price) {
        let scaled = spending / TEMPO_GAS_PRICE_SCALING_FACTOR_U128
            + u128::from(spending % TEMPO_GAS_PRICE_SCALING_FACTOR_U128 != 0);
        return U256::from(scaled);
    }
