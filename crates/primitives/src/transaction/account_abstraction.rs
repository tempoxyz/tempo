...
mod tests {
    ...
    fn test_fee_payer_commits_to_fee_token() {
        ...
        let tx_token1 = TxAA { fee_token: Some(token1), ..tx_no_token.clone() };

        // Transaction with fee_token = token2
        let tx_token2 = TxAA { fee_token: Some(token2), ..tx_no_token.clone() };
    ...
    fn test_user_signature_without_fee_payer() {
        ...
        let tx_no_payer_token1 = TxAA { fee_token: Some(token1), ..tx_no_payer_no_token.clone() };

        // Transaction WITHOUT fee_payer, fee_token = token2
        let tx_no_payer_token2 = TxAA { fee_token: Some(token2), ..tx_no_payer_no_token.clone() };
    ...
    fn test_rlp_encoding_includes_fee_token() {
        ...
        let tx_without_token = TxAA { fee_token: None, ..tx_with_token.clone() };
    ...
    fn test_signature_hash_behavior_with_and_without_fee_payer() {
        ...
        let tx_no_payer_with_token =
            TxAA { fee_token: Some(token), ..tx_no_payer_no_token.clone() };
    ...
    fn test_backwards_compatibility_key_authorization() {
        ...
        let tx_with = TxAA { key_authorization: Some(key_auth.clone()), ..tx_without.clone() };
