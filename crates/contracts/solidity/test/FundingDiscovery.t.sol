...
import {FundingDiscovery, IDiscoverySource} from "../src/FundingDiscovery.sol";
...
contract FundingDiscoveryTest {
    ...
    function setUp() public {
        ...
        helper = new FundingDiscovery(IFundingPolicy(address(store)));
    }
    ...
    function source(uint256 amount, uint256 budget, uint256 count) internal returns (address) {
        IDiscoverySource.Candidate[] memory candidates = new IDiscoverySource.Candidate[](count);
        for (uint256 i; i < count; ++i) {
            candidates[i] = IDiscoverySource.Candidate(abi.encode(i), amount);
        }
        return address(new Source(ACCOUNT, address(token), amount, budget, candidates));
    }
    ...
    function testSourceFailuresPropagate() public {
        ...
        require(bytes4(failure(1, address(token), 50)) == Source.SourceFailure.selector);
    }

    function testRejectsInvalidCandidates() public {
        for (uint256 mode; mode < 3; ++mode) {
            IDiscoverySource.Candidate[] memory candidates = new IDiscoverySource.Candidate[](1);
            candidates[0] = IDiscoverySource.Candidate(
                mode == 0 ? bytes("") : bytes(hex"01"), mode == 1 ? 0 : mode == 2 ? 51 : 50
            );
            address target = address(new Source(ACCOUNT, address(token), 50, 50, candidates));
