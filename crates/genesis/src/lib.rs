...
pub struct GenesisArgs {
    ...
    #[arg(long)]
    pub pathusd_admin: Option<Address>,

    #[arg(long, default_value_t = u64::MAX)]
    pub pathusd_amount: u64,

    /// Custom admin address for validator config.
    /// If not set, uses the first generated account.
    #[arg(long)]
    pub validator_admin: Option<Address>,
    ...
    #[arg(long)]
    pub no_extra_tokens: bool,

    /// Enable creating deployment gas token.
    #[arg(long)]
    pub deployment_gas_token: bool,

    /// Custom admin address for deployment gas token.
    #[arg(long)]
    pub deployment_gas_token_admin: Option<Address>,

    /// Disable minting pairwise FeeAMM liquidity.
    #[arg(long)]
    pub no_pairwise_liquidity: bool,
...
impl ConsensusConfig {
    pub fn to_genesis_dkg_outcome(&self) -> OnchainDkgOutcome {
        OnchainDkgOutcome {
            epoch: Epoch::zero(),
...
impl Validator {
    ...
    pub fn dst_dir(&self, path: impl AsRef<Path>) -> PathBuf {
        path.as_ref().join(self.addr.to_string())
    }
    pub fn dst_signing_key(&self, path: impl AsRef<Path>) -> PathBuf {
        self.dst_dir(path).join("signing.key")
    }

    pub fn dst_signing_share(&self, path: impl AsRef<Path>) -> PathBuf {
...
impl GenesisArgs {
    ...
    pub async fn generate_genesis(self) -> eyre::Result<(Genesis, Option<ConsensusConfig>)> {
...
/// Writes the genesis and consensus config to disk at the given output path.
pub fn write_genesis(
    output: &Path,
    genesis: &Genesis,
    consensus_config: Option<&ConsensusConfig>,
) -> eyre::Result<()> {
    let json = serde_json::to_string_pretty(genesis).wrap_err("failed encoding genesis as JSON")?;
    ...
    std::fs::write(&genesis_dst, json)
        .wrap_err_with(|| format!("failed writing genesis to file `{}`", genesis_dst.display()))?;
...
pub fn generate_consensus_config(
