    ...
    for TempoNextBlockEnvAttributes
{
    fn build_pending_env(parent: &crate::SealedHeader<tempo_primitives::TempoHeader>) -> Self {
        ...
        // Use parent's general_gas_limit directly since pending block building is disabled
        // for Tempo (PendingBlockKind::None) - blocks require consensus data that RPC doesn't have.
        Self {
            inner: NextBlockEnvAttributes::build_pending_env(parent),
            general_gas_limit: parent.general_gas_limit,
            shared_gas_limit: parent.gas_limit() / tempo_consensus::TEMPO_SHARED_GAS_DIVISOR,
...
mod tests {
    ...
    fn test_build_pending_env_uses_parent_values() {
        // Pending env uses parent's general_gas_limit directly since pending blocks are disabled
        ...
        let parent_header = TempoHeader {
            ...
            shared_gas_limit: 0, // not used - calculated from gas_limit
        };
        let parent = SealedHeader::seal_slow(parent_header);
        let pending_env = TempoNextBlockEnvAttributes::build_pending_env(&parent);

        // general_gas_limit copied from parent
        assert_eq!(pending_env.general_gas_limit, general_gas_limit);

        // shared_gas_limit calculated from gas_limit
        let expected_shared = gas_limit / tempo_consensus::TEMPO_SHARED_GAS_DIVISOR;
        assert_eq!(pending_env.shared_gas_limit, expected_shared);
