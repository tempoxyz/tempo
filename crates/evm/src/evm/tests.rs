...
use evm2::{
    ...
    bytecode::Bytecode,
    evm::{
        AccountInfo, InMemoryDB, PendingState, StateChangeSink, StateChangeSource, StorageChange,
        SystemTx,
    },
...
use tempo_precompiles::{
    ...
    nonce::NonceManager,
    storage::{
        ContractStorage, FromWord, Handler, StorageAction, StorageActions, StorageCtx, StorageKey,
...
impl P256KeyPair {
    ...
    fn sign_tx_keychain(&self, tx: TempoTransaction) -> eyre::Result<tempo_primitives::AASigned> {
    ...
    for action in actions {
        match *action {
            ...
            StorageAction::Sinc(address, slot, sload_value, delta) => {
                let key = (address, slot);
                let current = storage_state.apply_sload_value(key, sload_value, "SINC", hardfork);
            ...
            StorageAction::Sdec(address, slot, sload_value, delta) => {
                let key = (address, slot);
                let current = storage_state.apply_sload_value(key, sload_value, "SDEC", hardfork);
...
fn test_tip20_full_evm_storage_actions() {
    for hardfork in TempoHardfork::VARIANTS {
        ...
        let validator_token_slot = beneficiary.mapping_slot(fee_manager_slots::VALIDATOR_TOKENS);
        ...
        let sender_nonce_key_slot =
            nonce_key.mapping_slot(sender.mapping_slot(tempo_precompiles::nonce::slots::NONCES));
...
fn test_tempo_tx() -> eyre::Result<()> {
    ...
    let signed_key_auth = key_auth.into_signed(PrimitiveSignature::WebAuthn(key_auth_webauthn_sig));
...
#[test]
fn test_t3_key_authorization_accepts_empty_recipient_allowlist_as_unconstrained() -> eyre::Result<()>
{
    let key_pair = P256KeyPair::random();
    let caller = key_pair.address;
...
fn test_tip1060_spec_transition_classes_credit_accounting_table() -> eyre::Result<()> {
    ...
    let transition_classes = [
        ...
        TransitionClass {
            ...
            new_value: 0,
            expected_slot: 0,
            credit_cases: no_initial_credits(expectations((35_968, 0), (284_133, 1), (284_133, 1))),
        },
        TransitionClass {
...
fn test_tip1060_refund_settlement_min_pending_balance() -> eyre::Result<()> {
    ...
    for (creates, starting_balance, expected_balance) in cases {
        ...
        for slot in 0..creates {
            bytecode.extend_from_slice(&[opcode::PUSH1, 0x01, opcode::PUSH1, slot, opcode::SSTORE]);
        }
        bytecode.push(opcode::STOP);
...
fn zone_portal_runtime_commits_subsequent_token_enablements() {
    ...
    let spec = evm.config_spec_id();
    let mut storage = EvmPrecompileStorageProvider::new_max_gas(&mut evm, spec);
    let second_token = StorageCtx::enter(&mut storage, || -> Result<_, TempoPrecompileError> {
    ...
    assert!(TestZonePortal::isSequencerCall::abi_decode_returns(&sequencer_status.output).unwrap());
    ...
    let actual =
        TestZonePortal::tokenEnablementHashCall::abi_decode_returns(&commitment.output).unwrap();
    let initial =
        keccak256((B256::ZERO, PATH_USD_ADDRESS, "pathUSD", "pathUSD", "USD").abi_encode_params());
    let expected =
...
#[test]
fn rejected_pool_transaction_does_not_leak_key_expiry() -> eyre::Result<()> {
    const BLOCK_TIMESTAMP: u64 = 10;

    let root = P256KeyPair::random();
    let access_key = P256KeyPair::random();
    let authorization =
        KeyAuthorization::unrestricted(1, SignatureType::WebAuthn, access_key.address)
            .with_expiry(BLOCK_TIMESTAMP);
    let authorization_signature = root.sign_webauthn(authorization.signature_hash().as_slice())?;

    let mut evm = configured_evm(
        TempoHardfork::T10,
        BLOCK_TIMESTAMP,
        false,
        InMemoryDB::default(),
    );
    crate::TempoPoolValidationEvm::configure_for_pool(&mut evm);

    let rejected = TxBuilder::new()
        .call(Address::repeat_byte(0x44), &[])
        .key_authorization(
            authorization.into_signed(PrimitiveSignature::WebAuthn(authorization_signature)),
        )
        .gas_limit(1_000_000)
        .build();
    let rejected: TempoTxEnv =
        Recovered::new_unchecked(TempoTxEnvelope::AA(root.sign_tx(rejected)?), root.address).into();
    let rejected = Recovered::new_unchecked(rejected, root.address);
    assert!(
        crate::TempoPoolValidationEvm::validate_pool_transaction(&mut evm, &rejected).is_err(),
        "the first transaction must be rejected"
    );

    let valid = legacy_tx_env(
        root.address,
        0,
    ...
    let (_, key_expiry) =
        crate::TempoPoolValidationEvm::validate_pool_transaction(&mut evm, &valid)
