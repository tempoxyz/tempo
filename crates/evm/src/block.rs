    ...
    I: Inspector<TempoContext<DB>>,
{
    ...
    pub(crate) fn validate_tx(
        ...
        self.validate_tx_with_subblock_proposer(tx, gas_used, tx.subblock_proposer())
    }

    fn validate_tx_with_subblock_proposer(
        &self,
        tx: &TempoTxEnvelope,
        gas_used: u64,
        subblock_proposer: Option<PartialValidatorKey>,
    ) -> Result<BlockSection, BlockValidationError> {
        // Start with processing of transaction kinds that require specific sections.
        if tx.is_system_tx() {
            self.validate_system_tx(tx)
        } else if let Some(tx_proposer) = subblock_proposer {
            match self.section {
    ...
    I: Inspector<TempoContext<DB>>,
{
    ...
    fn execute_transaction_without_commit(
        ...
        let subblock_proposer = if tx_env.is_subblock_transaction() {
            recovered.tx().subblock_proposer()
        } else {
            None
        };
        ...
        if let Some(validator) = subblock_proposer {
        ...
        let next_section = if let Some(next_section) = next_section {
            ...
            self.validate_tx_with_subblock_proposer(
                recovered.tx(),
                block_gas_used,
                subblock_proposer,
            )?
