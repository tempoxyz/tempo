...
use alloy_primitives::{
    Address, B256, BlockHash, BlockNumber, Bytes, StorageKey, StorageValue, U256, keccak256,
};
use commonware_cryptography::{Sha256, sha256};
use commonware_runtime::{BufferPooler, Runner as _, ThreadPooler, buffer, tokio};
use commonware_storage::{
    journal::contiguous::fixed::Config as JournalConfig,
    merkle::Location,
    mmr::{self, journaled::Config as MmrConfig},
    qmdb::current::{FixedConfig, unordered::fixed::Db as UnorderedFixedDb},
    translator::EightCap,
};
...
impl QmdbConfig {
    ...
    pub fn new(path: impl Into<PathBuf>) -> Self {
        Self {
            path: path.into(),
            partition_prefix: "reth-qmdb".to_string(),
            worker_threads: 2,
        }
    }
    ...
    fn journal_path(&self) -> PathBuf {
        self.path
            .join(format!("{}-commit-journal.bin", self.partition_prefix))
    }
}
...
impl QmdbState {
    ...
    pub fn overlay_root(&self, hashed_state: HashedPostState) -> Result<QmdbCommit, QmdbError> {
        self.request(|response| Command::Overlay {
            hashed_state,
            response,
        })
    }
    ...
    pub fn overlay_mutations(
        ...
        self.request(|response| Command::OverlayMutations {
            mutations,
            response,
        })
    }
    ...
    pub fn commit_hashed_state(
        ...
        self.request(|response| Command::Commit {
            hashed_state,
            response,
        })
    }
    ...
    pub fn commit_block_mutations(
        ...
        self.commit_blocks_mutations(vec![QmdbBlockMutations {
            block,
            expected_root,
            mutations,
        }])?
        ...
        Provider: BlockNumReader + HeaderProvider,
    {
        ...
        let Some(mut qmdb_head) = self.head()? else {
            return if canonical_head == 0 {
                ...
                Err(QmdbError::CanonicalAheadOfQmdb {
                    canonical_head,
                    qmdb_head: 0,
                })
            };
    ...
    pub fn account(&self, hashed_address: B256) -> Result<Option<Account>, QmdbError> {
        self.request(|response| Command::Account {
            hashed_address,
            response,
        })
    }
    ...
    pub fn storage(
        ...
        self.request(|response| Command::Storage {
            hashed_address,
            hashed_slot,
            response,
        })
...
impl QmdbStage {
    ...
    pub const fn new(qmdb: QmdbState) -> Self {
        Self {
            qmdb,
            batch_blocks: QMDB_STAGE_BATCH_BLOCKS,
        }
    }
            ...
            + StorageChangeSetReader,
    {
        ...
        for number in (from..=target).rev() {
            let account_changes = provider
                .account_changesets_range(number..=number)
                .map_err(QmdbError::provider)?;
            let storage_changes = provider
                .storage_changeset(number)
                .map_err(QmdbError::provider)?;
            ...
            if number <= end {
                ...
                for (block_address, storage_change) in &storage_changes {
                    ...
                    storage
                        .storage
                        .insert(keccak256(storage_change.key), current);
                }
            ...
            for (block_address, storage_change) in storage_changes {
                storage_overrides.insert(
                    (block_address.address(), storage_change.key),
        ...
        + Send,
{
    ...
    fn execute(&mut self, provider: &Provider, input: ExecInput) -> Result<ExecOutput, StageError> {
        let Some((from, to)) = self.next_range(input).map_err(qmdb_stage_error)? else {
            return Ok(ExecOutput::done(
                input.checkpoint().with_block_number(input.target()),
            ));
        };

        let blocks = self
            .collect_blocks(provider, from, to, input.target())
        ...
        Ok(ExecOutput {
            checkpoint: StageCheckpoint::new(to),
            done: to == input.target(),
        })
    }
    ...
    fn unwind(
        ...
        self.qmdb
            .rewind_to_block(input.unwind_to)
            .map_err(qmdb_stage_error)?;
        Ok(UnwindOutput {
...
fn current_account<Provider: AccountReader>(
    ...
    overrides.get(&address).copied().map(Ok).unwrap_or_else(|| {
        provider
...
impl<S> StorageRootProvider for QmdbStateRootProvider<S> {
    fn storage_root(
        ...
        Err(qmdb_mpt_unsupported(
            "QMDb storage roots are not MPT-compatible",
        ))
    }
    ...
    fn storage_proof(
        ...
        Err(qmdb_mpt_unsupported(
            "QMDb storage proofs are not MPT-compatible",
        ))
    }
    ...
    fn storage_multiproof(
        ...
        Err(qmdb_mpt_unsupported(
            "QMDb storage multiproofs are not MPT-compatible",
        ))
    }
}
...
impl<S> StateProofProvider for QmdbStateRootProvider<S> {
    fn proof(
        ...
        Err(qmdb_mpt_unsupported(
            "QMDb state proofs are not MPT-compatible",
        ))
    }
    ...
    fn multiproof(
        ...
        Err(qmdb_mpt_unsupported(
            "QMDb state multiproofs are not MPT-compatible",
        ))
    }
    ...
    fn witness(
        ...
        Err(qmdb_mpt_unsupported(
            "QMDb execution witnesses are not MPT-compatible",
        ))
    }
}
...
impl<P: StateProviderFactory> StateProviderFactory for QmdbStateProviderFactory<P> {
    ...
    fn state_by_block_number_or_tag(
        ...
        self.inner
            ...
            .map(|provider| self.wrap(provider))
    }
...
impl QmdbCommitJournal {
    fn open(path: PathBuf) -> Result<Self, QmdbError> {
        if let Some(parent) = path.parent() {
            fs::create_dir_all(parent).map_err(|source| QmdbError::JournalIo {
        ...
        let mut file = OpenOptions::new()
            ...
            .truncate(false)
            .open(&path)
            .map_err(|source| QmdbError::JournalIo {
                path: path.clone(),
                source,
            })?;
        ...
        if bytes.len() < JOURNAL_MAGIC.len() || &bytes[..JOURNAL_MAGIC.len()] != JOURNAL_MAGIC {
            return Err(QmdbError::InvalidJournal {
                path,
                message: "invalid journal header",
            });
        }
    ...
    fn index_by_number(&self, number: u64) -> Option<usize> {
        self.records
            .binary_search_by_key(&number, |commit| commit.block.number)
            .ok()
    }
...
fn decode_journal_record(path: &Path, encoded: &[u8]) -> Result<QmdbBlockCommit, QmdbError> {
    ...
    let log_size = u64::from_be_bytes(
        body[104..112]
            .try_into()
            .expect("journal log size is fixed size"),
    );
    let entries = u64::from_be_bytes(
        body[112..120]
            .try_into()
            .expect("journal entries is fixed size"),
    );
...
fn run_actor(
    ...
    tokio::Runner::new(runtime_config).start(|context| async move {
        ...
        let mut db = QmdbDb::init(context, qmdb_config)
            .await
    ...
    E: commonware_storage::Context,
{
    match command {
        ...
        Command::GetMany { keys, response } => {
            let values = try_join_all(keys.iter().map(|key| db.get(key)))
                .await
                .map_err(commonware_error);
    ...
    E: commonware_storage::Context,
{
    ...
    if mutations.is_empty() {
        ...
        return Ok(QmdbCommit {
            root: digest_to_b256(db.root()),
            entries: 0,
        });
    }
    ...
    E: commonware_storage::Context,
{
    ...
    let rollback_size = journal
        .head_record()
        .map_or(INITIAL_LOG_SIZE, |commit| commit.log_size);
    ...
    for (block, hashed_state) in blocks {
        if let Some(current) = parent {
            ...
            return Err(QmdbError::UnknownBlock {
        ...
        let block_commit = QmdbBlockCommit {
            block,
            root: commit.root,
            log_size,
            entries: commit.entries as u64,
        };
    ...
    E: commonware_storage::Context,
{
    ...
    while journal
        .head_record()
        .is_some_and(|head| head.log_size > db_size)
    {
...
where
    E: commonware_storage::Context,
{
    db.rewind(QmdbLocation::new(log_size))
        .await
        .map_err(commonware_error)?;
    db.sync().await.map_err(commonware_error)
}
...
where
    E: commonware_storage::Context,
{
    let existing = try_join_all(desired.iter().map(|(key, _)| db.get(key)))
        .await
        .map_err(commonware_error)?;
    Ok(desired
        .into_iter()
...
fn decode_account(value: QmdbValue) -> Result<Account, QmdbError> {
    ...
    let nonce = u64::from_be_bytes(
        bytes[1..9]
            .try_into()
            .expect("account nonce has fixed size"),
    );
    let balance_bytes: [u8; 32] = bytes[9..41]
        .try_into()
        .expect("account balance has fixed size");
...
mod tests {
    ...
    fn commits_hashed_state_and_treats_wipes_as_sparse_updates() {
        ...
        assert_eq!(
            qmdb.storage(hashed_address, slot_a).unwrap(),
            Some(U256::from(13))
        );
        ...
        assert_eq!(
            qmdb.storage(hashed_address, slot_b).unwrap(),
            Some(U256::from(17))
        );
    ...
    fn commits_cached_block_mutations_and_rewinds_on_root_mismatch() {
        ...
        let mutations = vec![
            ...
            (
                ...
                Some(encode_storage(U256::from(200))),
            ),
        ];
        ...
        let head = qmdb
            .commit_block_mutations(block, overlay.root, mutations)
            .unwrap();
        ...
        assert_eq!(qmdb.account(hashed_address).unwrap(), Some(account));
        assert_eq!(
            qmdb.storage(hashed_address, hashed_slot).unwrap(),
            Some(U256::from(200))
        );
    ...
    fn transaction_shaped_state(number: u64) -> HashedPostState {
        ...
        let sender_account = Account {
            nonce: number,
            balance: U256::from(number * 3),
            bytecode_hash: None,
        };
        let receiver_account = Account {
            nonce: 0,
            balance: U256::from(number * 7),
            bytecode_hash: None,
        };

        HashedPostState::default()
            .with_accounts([
                (sender, Some(sender_account)),
