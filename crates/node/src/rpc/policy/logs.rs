...
use alloy::{consensus::BlockHeader, sol_types::SolEvent};
        ...
        + 'static,
{
    ...
    pub async fn filter_logs(&self, filter: Filter) -> RpcResult<Vec<Log>> {
        ...
        self.eth_api
            .spawn_blocking_io_fut(move |this| {
                Box::pin(async move {
                    Self::new(this)
                        .get_logs_in_block_range_inner(&filter, 0, to_block, limits)
                        .await
                        .map_err(EthApi::Error::from_eth_err)
                })
            })
