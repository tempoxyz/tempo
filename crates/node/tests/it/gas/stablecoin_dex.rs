...
async fn test_stablecoin_dex_revert_gas_snapshots(hardfork: TempoHardfork) -> eyre::Result<()> {
    ...
    gas.insert("swap_exact_out_accumulation_overflow", receipt.gas_used);
    ...
    assert!(
        !receipt.status(),
        "overflow placement unexpectedly succeeded"
    );
    gas.insert("place_aggregate_overflow", receipt.gas_used);
