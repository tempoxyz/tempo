...
async fn test_bids() -> eyre::Result<()> {
    ...
    for order_id in 1..=num_orders {
        let order = exchange.getOrder(order_id).gas(300_000).call().await?;
    ...
    assert_eq!(
        exchange.nextOrderId().gas(300_000).call().await?,
        num_orders + 1
    );
    ...
    let order = exchange.getOrder(num_orders).gas(300_000).call().await?;
...
async fn test_multi_hop_swap() -> eyre::Result<()> {
    ...
    let bob_usdc_before = bob_usdc.balanceOf(bob).gas(300_000).call().await?;
    let bob_eurc = ITIP20::new(*eurc.address(), bob_provider.clone());
    let bob_eurc_before = bob_eurc.balanceOf(bob).gas(300_000).call().await?;
    let bob_linking_usd = ITIP20::new(*linking_usd.address(), bob_provider);
    let bob_linking_usd_wallet_before = bob_linking_usd.balanceOf(bob).gas(300_000).call().await?;
    ...
    let bob_usdc_after = bob_usdc.balanceOf(bob).gas(300_000).call().await?;
    let bob_eurc_after = bob_eurc.balanceOf(bob).gas(300_000).call().await?;
    let bob_linking_usd_wallet_after = bob_linking_usd.balanceOf(bob).gas(300_000).call().await?;
