...
#[tokio::test(flavor = "multi_thread")]
async fn test_tip1060_poc_cross_token_keychain_fee_recreation_redeems_stale_credit()
-> eyre::Result<()> {
    reth_tracing::init_test_tracing();
    ...
    // Tx1 pays fees in the default token but clears token B's keychain limit under enabled
    // execution accounting. NonCreditableSlots only covers the fee-token limit, so token B mints
    // an AccountKeychain-owned storage credit.
    ...
    assert_eq!(
        credit_after_clear,
        credit_before + 1,
        "clearing token B's keychain spending-limit slot must mint one AccountKeychain credit"
    );
    ...
    *validator.lock().unwrap() = root_addr;
    ...
    // TIP-1060 disabled, so the same keychain limit slot is recreated without consuming the credit.
    ...
    assert_eq!(
        ...
        "disabled fee pre-collection recreated the token B limit without burning the stale credit"
    );

    // Tx3 creates ordinary AccountKeychain storage in Refund mode. The stale credit is economically
    // usable: end-of-transaction settlement consumes it for the AccountKeychain storage creation.
