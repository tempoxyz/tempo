...
async fn test_keychain_fee_refund_retains_unbacked_storage_credit() -> eyre::Result<()> {
    ...
    assert_eq!(
        credit_before, 0,
        "keychain storage-credit balance must start at zero after setup"
    );
    ...
    assert!(
        ...
        "post-tx fee refund must recreate the keychain spending-limit slot"
    );
    assert_eq!(
        credit_after - credit_before,
        1,
        "keychain must retain exactly one storage credit from the fee-precharge clear"
    );
