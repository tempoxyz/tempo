...
async fn test_bids() -> eyre::Result<()> {
    ...
    let mut pending = vec![];
    pending.push(base.approve(STABLECOIN_EXCHANGE_ADDRESS, U256::MAX).send().await?);
    ...
    for (_, signer) in &account_data {
        let account_provider =
            ProviderBuilder::new().wallet(signer.clone()).connect_http(http_url.clone());
        let quote = ITIP20::new(*quote.address(), account_provider);
        pending.push(quote.approve(STABLECOIN_EXCHANGE_ADDRESS, U256::MAX).send().await?);
    }
    ...
    let tx =
        exchange.swapExactAmountIn(*base.address(), *quote.address(), amount_in, 0).send().await?;
    tx.get_receipt().await?;

    for order_id in 1..num_orders {
        let err = exchange.getOrder(order_id).call().await.expect_err("Expected error");
    ...
    let level = exchange.getTickLevel(*base.address(), tick, true).call().await?;
    ...
    let balance = exchange.balanceOf(*last_account, *base.address()).call().await?;
...
async fn test_asks() -> eyre::Result<()> {
    ...
    let pending = quote.approve(STABLECOIN_EXCHANGE_ADDRESS, U256::MAX).send().await?;
    pending.get_receipt().await?;
    ...
    // Assert the last order is partially filled
    let level = exchange.getTickLevel(*base.address(), tick, false).call().await?;
    ...
    for (account, _) in account_data.iter().take(account_data.len() - 1) {
        let balance = exchange.balanceOf(*account, *quote.address()).call().await?;
        assert_eq!(balance, expected_quote_per_order);
    }

    let (last_account, _) = account_data.last().unwrap();
    let balance = exchange.balanceOf(*last_account, *quote.address()).call().await?;
...
async fn test_multi_hop_swap() -> eyre::Result<()> {
    ...
    let alice_provider =
        ProviderBuilder::new().wallet(alice_signer.clone()).connect_http(http_url.clone());
    ...
    let mut pending = vec![];
    pending.push(alice_usdc.approve(STABLECOIN_EXCHANGE_ADDRESS, U256::MAX).send().await?);
    pending.push(alice_eurc.approve(STABLECOIN_EXCHANGE_ADDRESS, U256::MAX).send().await?);
    pending.push(alice_linking_usd.approve(STABLECOIN_EXCHANGE_ADDRESS, U256::MAX).send().await?);
    await_receipts(&mut pending).await?;
    ...
    let tx = alice_exchange.place(*usdc.address(), liquidity_amount, true, 0).send().await?;
    tx.get_receipt().await?;

    // For PathUSD -> EURC: need ask on EURC (selling EURC for PathUSD)
    let tx = alice_exchange.place(*eurc.address(), liquidity_amount, false, 0).send().await?;
    tx.get_receipt().await?;

    // Bob approves exchange to spend his USDC
    let bob_provider = ProviderBuilder::new().wallet(bob_signer).connect_http(http_url.clone());
    let bob_usdc = ITIP20::new(*usdc.address(), bob_provider.clone());
    let tx = bob_usdc.approve(STABLECOIN_EXCHANGE_ADDRESS, U256::MAX).send().await?;
    tx.get_receipt().await?;
    ...
    let bob_linking_usd_exchange_before =
        bob_exchange.balanceOf(bob, *linking_usd.address()).call().await?;
    ...
    let bob_linking_usd_exchange_after =
        bob_exchange.balanceOf(bob, *linking_usd.address()).call().await?;
...
async fn test_place_rejects_order_below_dust_limit() -> eyre::Result<()> {
    ...
    let expected_selector =
        format!("0x{}", alloy::hex::encode(IStablecoinExchange::BelowMinimumOrderSize::SELECTOR));
    ...
    let result = exchange.place(*base.address(), below_dust_amount, true, 0).call().await;

    assert!(result.is_err(), "Expected bid order below dust limit to fail");
    let err = result.unwrap_err();
    assert!(err.to_string().contains(&expected_selector));

    // Try to place an ask order below dust limit (should also fail)
    let result = exchange.place(*base.address(), below_dust_amount, false, 0).call().await;

    assert!(result.is_err(), "Expected ask order below dust limit to fail");
    let err = result.unwrap_err();
    assert!(err.to_string().contains(&expected_selector));

    // Place an order at exactly the dust limit (should succeed)
    let tx = exchange.place(*base.address(), min_order_amount, true, 0).send().await?;
    tx.get_receipt().await?;

    // Place an order above the dust limit (should succeed)
    let above_dust_amount = min_order_amount + 1;
    let tx = exchange.place(*base.address(), above_dust_amount, false, 0).send().await?;
    tx.get_receipt().await?;
...
async fn test_place_flip_rejects_order_below_dust_limit() -> eyre::Result<()> {
    ...
    let min_order_amount = MIN_ORDER_AMOUNT;
    let below_dust_amount = min_order_amount - 1;
    let result = exchange.placeFlip(*base.address(), below_dust_amount, true, 0, 10).call().await;

    assert!(result.is_err(), "Expected flip bid order below dust limit to fail");
    let err = result.unwrap_err();
    assert!(err.to_string().contains(&expected_selector));

    // Try to place a flip ask order below dust limit (should also fail)
    let result = exchange.placeFlip(*base.address(), below_dust_amount, false, 10, 0).call().await;

    assert!(result.is_err(), "Expected flip ask order below dust limit to fail");
    let err = result.unwrap_err();
    assert!(err.to_string().contains(&expected_selector));

    // Place a flip order at exactly the dust limit (should succeed)
    let tx = exchange.placeFlip(*base.address(), min_order_amount, true, 0, 10).send().await?;
    tx.get_receipt().await?;

    // Place a flip order above the dust limit (should succeed)
    let above_dust_amount = min_order_amount + 1;
    let tx = exchange.placeFlip(*base.address(), above_dust_amount, false, 10, 0).send().await?;
    tx.get_receipt().await?;
