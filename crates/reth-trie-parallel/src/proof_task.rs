...
impl AvailabilitySheet {
    ...
    fn new(count: usize) -> Self {
        let flags = (0..count)
            .map(|_| crossbeam_utils::CachePadded::new(AtomicBool::new(false)))
            .collect();
        Self { flags }
    }
...
impl ProofWorkerHandle {
    ...
    pub fn dispatch_storage_proof(
        ...
        self.storage_work_tx
            ...
            .map_err(|err| {
                let StorageWorkerJob::StorageProof {
                    proof_result_sender,
                    ..
                } = err.0;
    ...
    Factory: DatabaseProviderROFactory<Provider: TrieCursorFactory + HashedCursorFactory>,
{
    ...
    fn run(mut self) -> ProviderResult<()> {
        ...
        {
            self.metrics
                .record_storage_worker_idle_time(total_idle_time);
        ...
        HC: HashedStorageCursor<Value = U256>,
    {
        ...
        *storage_proofs_processed += 1;
        ...
        if proof_result_sender
            .send(StorageProofResultMessage {
                hashed_address,
                result,
            })
            .is_err()
    ...
    Factory: DatabaseProviderROFactory<Provider: TrieCursorFactory + HashedCursorFactory>,
{
    ...
    fn run(mut self) -> ProviderResult<()> {
        ...
        {
            self.metrics
                .record_account_worker_idle_time(total_idle_time);
            self.cursor_metrics.record(&mut cursor_metrics_cache);
            self.metrics
                .record_value_encoder_stats(&value_encoder_stats_cache);
        }
