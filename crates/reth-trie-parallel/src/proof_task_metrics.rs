...
impl ProofTaskTrieMetrics {
    ...
    pub(crate) fn record_value_encoder_stats(&self, stats: &ValueEncoderStats) {
        self.deferred_encoder_dispatched
            .record(stats.dispatched_count as f64);
        self.deferred_encoder_from_cache
            .record(stats.from_cache_count as f64);
        ...
        self.account_worker_storage_wait_seconds
            .record(stats.storage_wait_time.as_secs_f64());
    }
}
...
impl ProofTaskCursorMetrics {
    ...
    pub fn record(&mut self, cache: &mut ProofTaskCursorMetricsCache) {
        self.account_trie_cursor
            .record(&mut cache.account_trie_cursor);
        self.account_hashed_cursor
            .record(&mut cache.account_hashed_cursor);
        self.storage_trie_cursor
            .record(&mut cache.storage_trie_cursor);
        self.storage_hashed_cursor
            .record(&mut cache.storage_hashed_cursor);
        cache.reset();
    }
}
...
impl ProofTaskCursorMetricsCache {
    ...
    pub fn extend(&mut self, other: &Self) {
        ...
        self.account_hashed_cursor
            .extend(&other.account_hashed_cursor);
        self.storage_trie_cursor.extend(&other.storage_trie_cursor);
        self.storage_hashed_cursor
            .extend(&other.storage_hashed_cursor);
    }
    ...
    pub fn record_spans(&self) {
        ...
        self.account_hashed_cursor
            .record_span("account_hashed_cursor");
        self.storage_trie_cursor.record_span("storage_trie_cursor");
        self.storage_hashed_cursor
            .record_span("storage_hashed_cursor");
    }
}
