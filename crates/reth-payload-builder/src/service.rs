...
use reth_trie_parallel::state_root_task::SharedStateRootHandle;
use std::{
    future::Future,
    pin::Pin,
    sync::Arc,
...
impl<T: PayloadTypes> PayloadBuilderHandle<T> {
    ...
    pub async fn payload_timestamp(
        ...
        let (tx, rx) = oneshot::channel();
        self.to_service
            .send(PayloadServiceCommand::PayloadTimestamp(id, tx))
            .ok()?;
        rx.await.ok()?
    }
}
                        ...
                        if this.contains_payload(id) {
                            ...
                            match job_result {
                                Ok(job) => {
                                    ...
                                    this.payload_jobs.push((job, id, job_span));
                                    this.payload_events
                                        .send(Events::Attributes(attributes))
                                        .ok();
...
pub enum PayloadServiceCommand<T: PayloadTypes> {
    ...
    PayloadTimestamp(
        PayloadId,
        oneshot::Sender<Option<Result<u64, PayloadBuilderError>>>,
    ),
...
pub struct BuildNewPayload<T> {
    ...
    /// Optional handle to a background shared state-root task.
    ///
    /// This field keeps the historical `trie_handle` name for compatibility with existing
    /// payload-builder call sites.
    pub trie_handle: Option<SharedStateRootHandle>,
}
