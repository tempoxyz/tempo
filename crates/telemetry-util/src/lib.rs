...
/// Formats an [`Option`] using [`std::fmt::Display`], showing either the value
/// or a user provided message.
///
/// On `Some`, displays the value using its [`std::fmt::Display`] implementation.
/// On `None`, displays `msg`.
///
/// # Example
///
/// ```
...
///
/// tracing::warn!(
///     some_field = %display_option_or(&some_value, "not set"),
///     none_field = %display_option_or(&none_value, "not set"),
///     "example log",
/// );
/// ```
pub fn display_option_or<'a, T>(
    option: &'a Option<T>,
    msg: &'static str,
) -> DisplayOptionOr<'a, T> {
    DisplayOptionOr(option, msg)
}

pub struct DisplayOptionOr<'a, T>(&'a Option<T>, &'static str);

impl<T: std::fmt::Display> std::fmt::Display for DisplayOptionOr<'_, T> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self.0 {
            Some(value) => write!(f, "{value}"),
