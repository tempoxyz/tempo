    ...
    TContext: commonware_runtime::Storage + Metrics,
{
    ...
    share_secret: SigningShareSecret,
}
...
pub(super) struct Builder {
    ...
    share_secret: Option<SigningShareSecret>,
}
...
impl Builder {
    ...
    pub(super) fn share_secret(self, share_secret: SigningShareSecret) -> Self {
        Self {
...
impl State {
    ...
    fn encrypt(&self, secret: &SigningShareSecret, rng: &mut impl CryptoRngCore) -> AtRest {
        let Self {
            epoch,
            seed,
            output,
...
impl AtRest {
    fn decrypt(&self, secret: &SigningShareSecret) -> eyre::Result<State> {
        let Self {
            epoch,
            seed,
            output,
...
impl Events {
    fn insert(&mut self, event: Event, secret: &SigningShareSecret) -> eyre::Result<()> {
        match event {
            Event::Dealing {
                dealer: public_key,
                public_msg,
...
impl<T: Write + EncodeSize> Encrypted<T> {
    fn encrypt(value: &T, secret: &SigningShareSecret, rng: &mut impl CryptoRngCore) -> Self {
...
impl<T: Read<Cfg = ()>> Encrypted<T> {
    pub(super) fn decrypt_decode(&self, secret: &SigningShareSecret) -> eyre::Result<T> {
        secret
