    ...
    TContext: commonware_runtime::Storage + Metrics,
{
    ...
    /// Appends a finalized log to the journal
    ///
    /// Returns the dealer of the log if there was one.
    pub(super) async fn append_log_from_block(
        &mut self,
        round: &Round,
        block: Block,
        ...
        if let Some(log) = self
            .cache
            .get(&round.epoch())
            .and_then(|events| events.finalized_logs.get(&block.height()))
        {
            info!(
                height = block.height(),
                "finalized log for height already found in cache; dropping"
            );
            return Ok(log.log.as_ref().map(|(dealer, _)| dealer).cloned());
        }

        let reduced = ReducedBlock::from_block_for_round(&block, round);
        let dealer = reduced.dealer().cloned();

        let section = round.epoch().get();

        if let Some((dealer, log)) = reduced.log.clone() {
            self.events
                .append(
                    section,
                    Event::Log {
        ...
        cache
            .hash_to_finalized_height
            .insert(reduced.digest, reduced.height);
        cache.finalized_logs.insert(reduced.height, reduced);
    ...
    pub(super) fn cache_notarized_block(&mut self, round: &Round, block: Block) {
        let cache = self.cache.entry(round.epoch).or_default();
        if !cache.finalized_logs.contains_key(&block.height()) {
            let log = ReducedBlock::from_block_for_round(&block, round);
            cache.notarized_logs.insert(log.digest, log);
        }
    }
    ...
    pub(super) fn create_dealer_for_round(
        ...
        tracing::error!(
            ?pub_msg,
            me = %me.public_key(),
            "generated a dealer for this round",
        );
    ...
    pub(super) fn produce_finalized_logs(
        &self,
        epoch: &Epoch,
    ) -> BTreeMap<PublicKey, dkg::DealerLog<MinSig, PublicKey>> {
        self.cache.get(epoch).map_or_else(BTreeMap::new, |events| {
            events
                .finalized_logs
                .iter()
                .filter_map(|(_, block)| block.log.clone())
                .collect()
        })
    }

    /// Produces a map of dealer -> logs for all observed blocks up to `tip`.
    ///
    /// If closed chain can be stablished from `tip` to the last observed block,
    /// then a [`Hole`] will be returned.
    pub(super) fn produce_notarized_logs(
        &mut self,
        epoch_length: u64,
        epoch: Epoch,
        tip: Digest,
    ) -> Result<BTreeMap<PublicKey, dkg::DealerLog<MinSig, PublicKey>>, Hole> {
        let events = self.cache.get_mut(&epoch).ok_or(Hole { digest: tip })?;

        if let Some(logs) = events.produced_logs.get(&tip).cloned() {
            return Ok(logs);
        }
        ...
        let mut next = tip;
        'walk: loop {
            if let Some(height) = events.hash_to_finalized_height.get(&next) {
                logs.extend(
                    events
                        .finalized_logs
                        .range(..=*height)
                        .filter_map(|(_, log)| log.log.clone()),
                );
                break 'walk;
            }
            let log = events
                .notarized_logs
                .get(&next)
                .ok_or(Hole { digest: next })?;
            if let Some((dealer, log)) = log.log.clone() {
                logs.insert(dealer, log);
            }
            if log.height == first_block_in_epoch(epoch_length, epoch) {
                break 'walk;
            }
            next = log.parent;
        }
        events.produced_logs.insert(tip, logs.clone());
        Ok(logs)
    }
...
struct Events {
    ...
    // All logs extracted from blocks and stored as "reduced blocks".
    //
    // Note that this map is dense and contains all heights for an epoch.
    // This is an optimization used when constructing speculative DKG outcomes
    // so that one does not have to walk back the entire ancestry chain until
    // the block that has a log.
    finalized_logs: BTreeMap<u64, ReducedBlock>,
    hash_to_finalized_height: HashMap<Digest, u64>,

    notarized_logs: HashMap<Digest, ReducedBlock>,
    produced_logs: HashMap<Digest, BTreeMap<PublicKey, dkg::DealerLog<MinSig, PublicKey>>>,
    ...
    fn insert(&mut self, event: Event) {
        match event {
            ...
            Event::Log {
                ...
                self.hash_to_finalized_height.insert(digest, height);
            }
        }
    }
}
...
pub(super) struct ReducedBlock {
    ...
    log: Option<(PublicKey, dkg::DealerLog<MinSig, PublicKey>)>,
}
...
impl ReducedBlock {
    ...
    pub(super) fn dealer(&self) -> Option<&PublicKey> {
        self.log.as_ref().map(|(dealer, _)| dealer)
    }
}

/// A hole is a block missing from the events cache to walk a chain of blocks
/// to the beginning of an epoch.
