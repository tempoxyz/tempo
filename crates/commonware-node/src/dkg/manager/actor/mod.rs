use std::{net::SocketAddr, num::NonZeroU32, task::Poll, time::Duration};
...
use commonware_consensus::{
    ...
    types::Epoch,
    utils::{self, is_last_block_in_epoch},
};
use commonware_cryptography::{
    Digest as _, Signer as _,
    bls12381::{
        dkg::{DealerPrivMsg, DealerPubMsg, PlayerAck, SignedDealerLog, observe},
...
use crate::{
    ...
    epoch::{self, EpochTransition, RelativePosition, is_first_block_in_epoch},
};
        ...
        + Sync,
{
            ...
            select_biased!(
                ...
                msg = self.mailbox.next() => {
                    ...
                    match msg.command {
                        ...
                        Command::GetDkgOutcome(request) => {
                            ...
                            {
                                let stream = match self.config.marshal.ancestry((None, hole.digest)).await {
                                    Some(stream) => stream,
                ...
                notarized_block = ancestry_stream.next() => {
                    if let Some(block) = notarized_block {
                        ...
                        if let Some((hole, request)) = self
                            .handle_speculative_dkg_outcome(&cause, storage, &player_state, &round, &state, request)
                        {
                            let stream = match self.config.marshal.ancestry((None, hole.digest)).await {
                                Some(stream) => stream,
                                None => break Err(eyre!("marshal mailbox is closed")),
        ...
        TSender: Sender<PublicKey = PublicKey>,
    {
        ...
        if is_last_block_in_epoch(self.config.epoch_length, block.height()).is_none() {
            if let Some(dealer) = storage
                .append_log_from_block(round, block.clone())
                .await
                .wrap_err("failed to append and cache log")?
                && let Some(dealer_state) = dealer_state
                && self.config.me.public_key() == dealer
            {
                info!(
                    "found own dealing in finalized block; deleting it from state \
                    to not write it again"
                );
            ...
            storage.get_dkg_outcome(&state.epoch, &block.parent_digest())
        {
            ...
            let logs = storage.produce_finalized_logs(&round.epoch());
    ...
    #[instrument(
        ...
        fields(
            ...
            tip = %get_dkg_outcome.parent.1,
        ),
    )]
    fn handle_speculative_dkg_outcome<TStorageContext>(
        &mut self,
        cause: &Span,
        storage: &mut state::Storage<TStorageContext>,
    ...
    ) -> Option<(state::Hole, GetDkgOutcome)>
    where
        TStorageContext: commonware_runtime::Metrics + commonware_runtime::Storage,
    {
        let output = if let Some((output, _)) = storage
            .get_dkg_outcome(&state.epoch, &get_dkg_outcome.parent.1)
            .cloned()
        {
            output
        } else {
            let logs = match storage.produce_notarized_logs(
                self.config.epoch_length,
                get_dkg_outcome.round.epoch(),
                get_dkg_outcome.parent.1,
            ) {
                Err(hole) => {
                    info!(
                        "could not produce notarized log to generate speculative \
                        DKG outcome because there was a hole in the chain of logs; \
                        need to fill it first",
                    );
                    return Some((hole, get_dkg_outcome));
                }
                Ok(logs) => logs,
            };

            // Need to create an ad-hoc player just for the speculative DKG outcome
            // because the Player object is not cloneable.
            let (output, share) = if player_state.is_some() {
                let adhoc_player = storage
                    .create_player_for_round(self.config.me.clone(), round)
                ...
                tracing::debug!(?logs, "all dealer logs");
                match adhoc_player.finalize(logs, 1) {
                    Ok((new_output, share)) => {
                        info!("DKG ceremony was a success");
                        (new_output, Some(share))
                    }
                    Err(error) => {
                        warn!(error = %eyre::Report::new(error), "DKG ceremony was a failure");
                        (state.output.clone(), state.share.clone())
                    }
                }
            } else {
            ...
            storage.cache_dkg_outcome(state.epoch, get_dkg_outcome.parent.1, output.clone(), share);
            output
        };
...
impl AncestorStream {
    ...
    fn tip(&self) -> Option<Digest> {
        self.pending_request.as_ref().map(|(_, req)| req.parent.1)
    }
