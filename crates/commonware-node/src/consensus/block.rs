...
use crate::{
    consensus::Digest,
    dkg::{LocalOutcome, PublicOutcome},
};
...
impl Block {
    pub(crate) fn insert_ceremony_deal_outcome(self, deal: LocalOutcome) -> Self {
    ...
    pub(crate) fn try_read_ceremony_deal_outcome(&self) -> Option<LocalOutcome> {
        LocalOutcome::decode(&mut self.header().extra_data().as_ref())
