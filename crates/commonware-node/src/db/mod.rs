    ...
    TContext: Clock + Metrics + Storage,
{
    ...
    async fn get<K, V>(&self, key: K) -> Result<Option<V>, eyre::Error>
    where
        K: AsRef<[u8]>,
    ...
    async fn get_raw<K>(&self, key: K) -> Option<Bytes>
    where
        K: AsRef<[u8]>,
    {
        let key_hash = key_to_u64(key.as_ref());
...
trait CachedValue: Any + Send + Sync {
