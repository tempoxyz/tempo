...
pub struct Builder<TBlocker, TPeerManager> {
    ...
    /// Require marshal durability at certification instead of before notarization.
    ...
    TPeerManager: AddressableManager<PublicKey = PublicKey> + Sync,
{
    ...
    async fn run(
        ...
        let application_reporter = self.application.mailbox().clone();
        ...
        let marshal = self.marshal.start(
            Reporters::from((
                Reporters::from((self.epoch_manager_mailbox, application_reporter)),
                Reporters::from((
