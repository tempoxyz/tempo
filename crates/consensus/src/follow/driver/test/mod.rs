...
/// Repeated fallback failures request each local epoch boundary only once. An
/// authenticated boundary advances the epoch and permits one request for the
/// next boundary.
#[test_traced]
fn fallback_boundary_hint_is_deduplicated_until_epoch_advances() {
    deterministic::Runner::default().start(|mut context| async move {
        let rig = start_rig(&mut context);
        let rotated = dkg_fixture(&mut context, Epoch::new(1));
        let strategy = FixedEpocher::new(EPOCH_LENGTH);
        let first_boundary = strategy
            .last(Epoch::zero())
            .expect("epoch zero has a boundary");

        for height in [EPOCH_LENGTH.get() + 1, EPOCH_LENGTH.get() + 2] {
            let block = make_block(height, None);
            let certificate = make_finalization(&block, Epoch::new(1), &rotated.schemes);
            let result = rig
                .mailbox
                .process_certificate(certificate)
                .await
        ...
        assert_eq!(rig.marshal.hints(), vec![first_boundary]);

        let boundary_block = make_block(first_boundary.get(), Some(&rotated.outcome));
        let (ack, installed) = Exact::handle();
        let _ = rig
            .mailbox
            .to_marshal_reporter()
            .report(Update::Block(boundary_block.into(), ack));
        installed
            .await
            .expect("boundary update should be acknowledged");

        let future = dkg_fixture(&mut context, Epoch::new(2));
        let block = make_block(EPOCH_LENGTH.get() * 2 + 1, None);
        let certificate = make_finalization(&block, Epoch::new(2), &future.schemes);
        let result = rig
            .mailbox
            .process_certificate(certificate)
            .await
