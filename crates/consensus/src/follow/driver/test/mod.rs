...
use utils::{
    EPOCH_LENGTH, StubExecutionProvider, StubExecutor, StubMarshal, dkg_fixture, make_block,
...
fn gossiped_certificate_is_admitted_and_nudges_the_execution_layer() {
    deterministic::Runner::default().start(|mut context| async move {
        ...
        assert_eq!(executor.certified_tips(), vec![(round, digest)]);
...
fn unverifiable_gossiped_certificate_is_not_blamed_on_the_sender() {
    deterministic::Runner::default().start(|mut context| async move {
        ...
        assert!(executor.certified_tips().is_empty());
    });
}
