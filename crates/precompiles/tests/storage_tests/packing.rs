...
fn test_packed_two_slot_contents() {
    ...
    value.store(&mut storage, base_slot, LayoutCtx::FULL).unwrap();
...
fn test_partial_update_preserves_adjacent_fields() {
    ...
    initial.store(&mut storage, base_slot, LayoutCtx::FULL).unwrap();
    ...
    updated.store(&mut storage, base_slot, LayoutCtx::FULL).unwrap();
