...
pub enum OrderError {
    ...
    #[error(
        "invalid flip tick for bid order: flip_tick ({flip_tick}) must be greater than or equal to tick ({tick})"
    )]
    InvalidBidFlipTick {
        /// The order's tick
        tick: i16,
    ...
    #[error(
        "invalid flip tick for ask order: flip_tick ({flip_tick}) must be less than or equal to tick ({tick})"
    )]
    InvalidAskFlipTick {
        /// The order's tick
        tick: i16,
...
mod tests {
    ...
    fn test_invalid_flip_tick_bid() {
        ...
        assert!(msg.contains("greater than or equal to"));
    ...
    fn test_invalid_flip_tick_ask() {
        ...
        assert!(msg.contains("less than or equal to"));
        assert!(msg.contains("5"));
