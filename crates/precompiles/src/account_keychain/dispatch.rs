...
impl Precompile for AccountKeychain {
    fn call(&mut self, calldata: &[u8], msg_sender: Address) -> PrecompileResult {
        ...
        // T11's strict ABI decoder rejected enum value 3 before `SignatureType::Multisig`
        // existed, and multisig access keys are not supported yet, so keep failing the same way.
...
mod tests {
    ...
    /// Multisig access keys are not supported yet: value 3 must fail exactly like an unknown
    /// signature type on every hardfork, including gas.
    #[test]
    fn multisig_key_type_fails_like_an_unknown_type() -> eyre::Result<()> {
            ...
            for key_id in [Address::ZERO, Address::repeat_byte(2)] {
                for expiry in [0, u64::MAX] {
                    ...
                    for mut calldata in calls {
                        ...
                        assert_eq!(actual, unknown, "{spec}: {calldata:?}");
                    }
                }
            }
        }
