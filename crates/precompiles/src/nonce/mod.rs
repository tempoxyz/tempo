...
impl NonceManager {
    ...
    pub fn check_and_mark_expiring_nonce(
        ...
        let mut seen_slot = self.expiring_nonce_seen[expiring_nonce_hash].clone();
        ...
        let idx = ptr;
        let mut ring_slot = self.expiring_nonce_ring[idx].clone();
        ...
        if old_hash != B256::ZERO {
            let mut old_seen_slot = self.expiring_nonce_seen[old_hash].clone();
