...
impl NonceManager {
    ...
    /// Validates and records an expiring nonce transaction using a caller-provided block timestamp.
    ///
    /// This is equivalent to [`Self::check_and_mark_expiring_nonce`] but avoids re-reading the
    /// timestamp when the execution handler has already loaded it for error reporting.
