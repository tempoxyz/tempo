...
impl NonceManager {
    #[inline]
    fn expiring_nonce_seen_slot(&self, hash: B256) -> Slot<u64> {
        Slot::new(
            hash.mapping_slot(self.expiring_nonce_seen.slot()),
            NONCE_PRECOMPILE_ADDRESS,
        )
    }

    #[inline]
    fn expiring_nonce_ring_slot(&self, index: u32) -> Slot<B256> {
        Slot::new(
            index.mapping_slot(self.expiring_nonce_ring.slot()),
            NONCE_PRECOMPILE_ADDRESS,
        )
    }
    ...
    pub fn check_and_mark_expiring_nonce(
        ...
        let mut seen_slot = self.expiring_nonce_seen_slot(expiring_nonce_hash);
        ...
        let idx = ptr;
        let mut ring_slot = self.expiring_nonce_ring_slot(idx);
        ...
        if old_hash != B256::ZERO {
            let mut old_seen_slot = self.expiring_nonce_seen_slot(old_hash);
