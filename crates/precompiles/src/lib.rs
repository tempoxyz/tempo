...
/// Dispatches a state-mutating call that returns an ABI boolean.
///
/// Successful TIP-20 transfer calls return `true`; use static bytes for that hot success path.
#[inline]
fn mutate_true<T: SolCall<Return = bool>>(
    call: T,
    sender: Address,
    ...
    f(sender, call).into_precompile_result(0, 0, |ret| {
        if ret {
            Bytes::from_static(&ABI_TRUE)
        } else {
            T::abi_encode_returns(&ret).into()
        }
    })
}
