...
use crate::{
    ...
    storage::{StorageCtx, evm::EvmActions},
...
/// Returns the full Tempo precompiles for the given config and shared container for [`storage::evm::EvmAction`]s.
...
impl TipFeeManager {
    ...
    pub fn create_precompile(cfg: &CfgEnv<TempoHardfork>, actions: EvmActions) -> DynPrecompile {
...
mod tests {
    ...
    fn test_precompile_delegatecall() {
        ...
        let precompile = tempo_precompile!("TIP20Token", &cfg, EvmActions::default(), |input| {
    ...
    fn test_invalid_calldata_hardfork_behavior() {
        let call_with_spec = |calldata: Bytes, spec: TempoHardfork| {
            ...
            let tx = TxEnv::default();
            let precompile =
                tempo_precompile!("TIP20Token", &cfg, EvmActions::default(), |input| {
                    TIP20Token::from_address(PATH_USD_ADDRESS).expect("PATH_USD_ADDRESS is valid")
                });
