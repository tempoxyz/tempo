...
pub use crate::contracts::tip403_registry::{ITIP403Registry, ITIP403Registry::traits::*};

use crate::{
    TIP403_REGISTRY_ADDRESS,
    contracts::tip403_registry::ITIP403Registry::PolicyType,
...
impl ITIP403Registry::Interface for TIP403Registry {
...
mod tests {
    use super::*;
    use crate::{
        contracts::TIP403_REGISTRY_ADDRESS,
    ...
    fn test_create_policy() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            // Check policy data
            let (policy_type, policy_admin) = registry.policy_data(2)?;
    ...
    fn test_policy_data_reverts_for_non_existent_policy() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let result = registry.policy_data(100);
            assert_eq!(
                result.unwrap_err(),
    ...
    fn test_policy_exists() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            assert!(registry.policy_exists(0)?);
            assert!(registry.policy_exists(1)?);

            // Test 100 random policy IDs > 1 should not exist initially
            let mut rng = rand_08::thread_rng();
            for _ in 0..100 {
                let random_policy_id = rng.gen_range(2..u64::MAX);
                assert!(!registry.policy_exists(random_policy_id)?);
            }
    ...
    fn test_create_compound_policy() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            assert!(registry.policy_exists(compound_id)?);

            // Verify policy type is COMPOUND
            let (policy_type, policy_admin) = registry.policy_data(compound_id)?;
            ...
            let (sender_id, recipient_id, mint_recipient_id) =
                registry.compound_policy_data(compound_id)?;
