...
impl<'evm> StorageCtx {
    ...
    /// Enters storage for expiring nonce bookkeeping.
    ///
    /// Expiring nonce bookkeeping is charged manually by transaction intrinsic gas. It must not
    /// mint, consume, or settle TIP-1060 storage credits while updating the nonce ring and seen set.
    pub fn enter_expiring_nonce_bookkeeping<J, R>(
        journal: &'evm mut J,
        block_env: &'evm dyn Block,
        cfg: &CfgEnv<TempoHardfork>,
        tx_env: &'evm impl Transaction,
