...
use crate::{
    ...
    storage::{
        ...
        types::{Persistent, StorageMode, Transient},
    },
};
...
#[derive(Debug, Clone)]
pub struct Slot<T, M: StorageMode = Persistent> {
    slot: U256,
    ctx: LayoutCtx,
    address: Address,
    _ty: PhantomData<(T, M)>,
...
impl<T> StorageOps for Slot<T, Persistent> {
...
impl<T> StorageOps for Slot<T, Transient> {
    fn load(&self, slot: U256) -> Result<U256> {
        let storage = StorageCtx;
...
impl<T: Storable, M: StorageMode> Handler<T> for Slot<T, M>
where
    Self: StorageOps,
{
    #[inline]
...
mod tests {
    ...
    use crate::{
        storage::{
            Handler, PrecompileStorageProvider, StorageKey,
            types::{Persistent, Transient},
        },
    ...
    fn test_transient_ops() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let mut u256_slot = Slot::<U256, Transient>::new(U256::from(1), address);
            assert_eq!(u256_slot.read()?, U256::ZERO);

            let num1 = U256::random();
            let num2 = U256::random();
            u256_slot.write(num1)?;
            assert_eq!(u256_slot.read()?, num1);
            u256_slot.write(num2)?;
            assert_eq!(u256_slot.read()?, num2);
            u256_slot.delete()?;
            assert_eq!(u256_slot.read()?, U256::ZERO);

            // Address: default, roundtrip, overwrite, delete
            let mut addr_slot = Slot::<Address, Transient>::new(U256::from(2), address);
            assert_eq!(addr_slot.read()?, Address::ZERO);

            let addr1 = Address::random();
            let addr2 = Address::random();
            addr_slot.write(addr1)?;
            assert_eq!(addr_slot.read()?, addr1);
            addr_slot.write(addr2)?;
            assert_eq!(addr_slot.read()?, addr2);
            addr_slot.delete()?;
            assert_eq!(addr_slot.read()?, Address::ZERO);

            // bool: default, roundtrip, overwrite, delete
            let mut bool_slot = Slot::<bool, Transient>::new(U256::from(3), address);
            assert!(!bool_slot.read()?);
    ...
    fn test_transient_persistence_isolation() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || -> eyre::Result<()> {
            let mut p_slot = Slot::<U256, Persistent>::new(slot_num, address);
            let mut t_slot = Slot::<U256, Transient>::new(slot_num, address);

            // Write different values to each storage type
            p_slot.write(s_value)?;
            t_slot.write(t_value)?;
            assert_eq!(p_slot.read()?, s_value);
            assert_eq!(t_slot.read()?, t_value);

            // Delete transient, persistent remains
            t_slot.delete()?;
            assert_eq!(p_slot.read()?, s_value);
            assert_eq!(t_slot.read()?, U256::ZERO);
        ...
        StorageCtx::enter(&mut storage, || {
            let p_slot = Slot::<U256, Persistent>::new(slot_num, address);
            let t_slot = Slot::<U256, Transient>::new(slot_num, address);
            assert_eq!(p_slot.read()?, s_value);
            assert_eq!(t_slot.read()?, U256::ZERO);
            Ok(())
        })
