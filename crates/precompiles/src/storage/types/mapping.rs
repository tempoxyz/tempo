...
use crate::storage::{
    Layout, LayoutCtx, StorableType, StorageKey,
    types::{HandleFor, HandlerCache, Persistent, StorageMode, Transient},
};
...
/// - `M`: Storage mode (defaults to `Persistent`)
///
...
#[derive(Clone)]
pub struct Mapping<K, V: StorableType, M: StorageMode = Persistent>
where
    V: HandleFor<M>,
{
    base_slot: U256,
    address: Address,
    cache: HandlerCache<K, <V as HandleFor<M>>::HandlerMode>,
}

impl<K, V: StorableType, M: StorageMode> std::fmt::Debug for Mapping<K, V, M>
where
    V: HandleFor<M>,
{
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("Mapping")
            ...
            .field("cache", &"HandlerCache")
            .finish()
    }
}

impl<K, V: StorableType, M: StorageMode> Mapping<K, V, M>
where
    V: HandleFor<M>,
{
    ...
    pub fn at(&self, key: &K) -> &<V as HandleFor<M>>::HandlerMode
    where
        K: StorageKey + Hash + Eq + Clone,
    {
        let (base_slot, address) = (self.base_slot, self.address);
        self.cache.get_or_insert(key, || {
            <V as HandleFor<M>>::handle_for(key.mapping_slot(base_slot), LayoutCtx::FULL, address)
        })
    }
    ...
    /// The handler is computed on first access and cached for subsequent accesses.
    /// Takes a reference to avoid cloning on cache hits.
    pub fn at_mut(&mut self, key: &K) -> &mut <V as HandleFor<M>>::HandlerMode
    where
        K: StorageKey + Hash + Eq + Clone,
    {
        let (base_slot, address) = (self.base_slot, self.address);
...
impl<K, V: StorableType, M: StorageMode> Default for Mapping<K, V, M>
where
    V: HandleFor<M>,
{
    fn default() -> Self {
...
impl<K, V: StorableType, M: StorageMode> Index<K> for Mapping<K, V, M>
where
    V: HandleFor<M>,
    K: StorageKey + Hash + Eq + Clone,
{
    type Output = <V as HandleFor<M>>::HandlerMode;
...
impl<K, V: StorableType, M: StorageMode> IndexMut<K> for Mapping<K, V, M>
where
    V: HandleFor<M>,
    K: StorageKey + Hash + Eq + Clone,
{
...
impl<K, V> StorableType for Mapping<K, V, Persistent>
where
    V: StorableType,
{
...
impl<K, V> StorableType for Mapping<K, V, Transient>
where
    V: StorableType + HandleFor<Transient>,
{
    const LAYOUT: Layout = Layout::Slots(1);
    type Handler = Self;
