...
use crate::storage::{
    Layout, LayoutCtx, PrecomputedMappingSlots, StorableType, StorageKey, types::HandlerCache,
};
...
/// - `PRECOMPUTED_MIN`, `PRECOMPUTED_MAX`: optional top-level contract-field key range where
///   mapping slots are precomputed by the `#[contract]` macro. `MIN == MAX` disables the fast path.
///   Non-empty ranges use Rust range semantics: `MIN..MAX`.
///
/// `Mapping<K, V>` is essentially a slot computation helper. The `[key]` method uses a
/// precomputed slot table when available, otherwise performs the keccak256 hash to compute
/// the actual storage slot and returns a `Handler` that can be used for read/write operations.
///
/// # Storage Layout
///
...
pub struct Mapping<
    K,
    V: StorableType,
    const PRECOMPUTED_MIN: u32 = 0,
    const PRECOMPUTED_MAX: u32 = 0,
> {
    base_slot: U256,
    address: Address,
    cache: HandlerCache<K, V::Handler>,
    precomputed_slots: Option<PrecomputedMappingSlots<K>>,
}

impl<K, V: StorableType, const PRECOMPUTED_MIN: u32, const PRECOMPUTED_MAX: u32>
    Mapping<K, V, PRECOMPUTED_MIN, PRECOMPUTED_MAX>
{
    ...
    /// Creates a new `Mapping` with a precomputed storage slot table.
    ///
    /// This is used by the `#[contract]` macro for mapping fields with a non-empty
    /// `PRECOMPUTED_MIN..PRECOMPUTED_MAX` range.
    #[inline]
    pub fn new_with_precomputed_slots(
        base_slot: U256,
        address: Address,
        precomputed_slots: PrecomputedMappingSlots<K>,
    ) -> Self {
        Self {
            base_slot,
            address,
            cache: HandlerCache::new(),
            precomputed_slots: Some(precomputed_slots),
        }
    }
        ...
        K: StorageKey + Hash + Eq + Clone,
    {
        ...
        let precomputed_slots = self.precomputed_slots;
        self.cache.get_or_insert(key, || {
            let slot = precomputed_slots
                .and_then(|slots| slots.slot(key))
                .unwrap_or_else(|| key.mapping_slot(base_slot));
            V::handle(slot, LayoutCtx::FULL, address)
        })
    }
...
impl<K, V: StorableType, const PRECOMPUTED_MIN: u32, const PRECOMPUTED_MAX: u32> Default
    for Mapping<K, V, PRECOMPUTED_MIN, PRECOMPUTED_MAX>
{
    fn default() -> Self {
...
impl<K, V: StorableType, const PRECOMPUTED_MIN: u32, const PRECOMPUTED_MAX: u32> Index<K>
    for Mapping<K, V, PRECOMPUTED_MIN, PRECOMPUTED_MAX>
where
    K: StorageKey + Hash + Eq + Clone,
{
...
impl<K, V: StorableType, const PRECOMPUTED_MIN: u32, const PRECOMPUTED_MAX: u32> IndexMut<K>
    for Mapping<K, V, PRECOMPUTED_MIN, PRECOMPUTED_MAX>
where
    K: StorageKey + Hash + Eq + Clone,
{
...
impl<K, V, const PRECOMPUTED_MIN: u32, const PRECOMPUTED_MAX: u32> StorableType
    for Mapping<K, V, PRECOMPUTED_MIN, PRECOMPUTED_MAX>
where
    V: StorableType,
{
