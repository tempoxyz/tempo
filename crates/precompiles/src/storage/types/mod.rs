...
use std::{cell::UnsafeCell, collections::HashMap, fmt, hash::Hash};
...
///
/// Uses `UnsafeCell` for interior mutability. The cache is append-only and handlers are boxed, so
/// returned handler references remain stable across insertions.
...
impl<K, H> HandlerCache<K, H> {
    ...
    #[inline]
    fn inner_mut(&self) -> &mut HashMap<K, Box<H>> {
        // SAFETY: storage handler caches are owned by a single contract handle during precompile
        // execution. Existing callers may hold handler references returned from earlier lookups,
        // but the cache mutates only by inserting additional boxed handlers; existing boxes are not
        // moved or mutated through this shared path.
        unsafe { &mut *self.inner.get() }
    }
}

impl<K, H> Default for HandlerCache<K, H> {
    fn default() -> Self {
        Self::new()
    }
}

impl<K, H> fmt::Debug for HandlerCache<K, H> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("HandlerCache").finish_non_exhaustive()
    }
}
