...
use crate::{
    Precompile, SelectorSchedule, charge_input_cost, dispatch_call, metadata, mutate, mutate_bool,
    mutate_void,
        ...
        dispatch_call(
            ...
            |call| match call {
                ...
                TIP20Call::TIP20(ITIP20Calls::transferFrom(call)) => {
                    mutate_bool(call, msg_sender, |s, c| self.transfer_from(s, c))
                }
                TIP20Call::TIP20(ITIP20Calls::transfer(call)) => {
                    mutate_bool(call, msg_sender, |s, c| self.transfer(s, c))
                }
                TIP20Call::TIP20(ITIP20Calls::approve(call)) => {
                    mutate_bool(call, msg_sender, |s, c| self.approve(s, c))
                }
                ...
                TIP20Call::TIP20(ITIP20Calls::transferFromWithMemo(call)) => {
                    mutate_bool(call, msg_sender, |sender, c| {
