...
impl TIP20Call {
    fn is_roles_auth_selector_error(err: &alloy::sol_types::Error) -> bool {
        matches!(
            err,
            alloy::sol_types::Error::UnknownSelector { selector, .. }
    ...
    fn decode(calldata: &[u8]) -> Result<Self, alloy::sol_types::Error> {
        match ITIP20Calls::abi_decode(calldata) {
            Ok(call) => Ok(Self::TIP20(call)),
            Err(err) if Self::is_roles_auth_selector_error(&err) => {
