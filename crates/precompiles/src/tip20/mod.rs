...
impl TIP20Token {
    ...
    pub(crate) fn _transfer(&mut self, from: Address, to: &Recipient, amount: U256) -> Result<()> {
        ...
        if to.target != Address::ZERO {
            self.credit_recipient(to.target, amount)?;
        }
    ...
    fn _transfer_to_nonzero_recipient(
        ...
        debug_assert!(!to.target.is_zero(), "recipient must be validated");
    ...
    fn credit_recipient(&mut self, to: Address, amount: U256) -> Result<()> {
    ...
    pub(crate) fn validate_inbound_or_block(
        ...
        let kind = if let Some(total_supply) = mint_total_supply {
            ...
            self._transfer_to_nonzero_recipient(originator, &guard, amount)?;
    ...
    pub(crate) fn release_blocked_funds(
        ...
        self._transfer_to_nonzero_recipient(RECEIVE_POLICY_GUARD_ADDRESS, &destination, amount)?;
