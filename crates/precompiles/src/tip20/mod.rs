...
use crate::{
    ...
    tip1028_escrow::{ITIP1028Escrow, TIP1028Escrow},
};
use alloy::{
...
impl TIP20Token {
    ...
    pub fn mint(&mut self, msg_sender: Address, call: ITIP20::mintCall) -> Result<()> {
        ...
        let blocked = self._mint(msg_sender, &to, call.amount, B256::ZERO)?;
        ...
        if let Some(blocked) = blocked {
            self.emit_mint_blocked(msg_sender, &to, call.amount, B256::ZERO, blocked)?;
    ...
    pub fn mint_with_memo(
        ...
        let to = Recipient::resolve(call.to)?;
        let blocked = self._mint(msg_sender, &to, call.amount, call.memo)?;
        ...
        if let Some(blocked) = blocked {
            self.emit_mint_blocked(msg_sender, &to, call.amount, call.memo, blocked)?;
        } else if let Some(hop) = to.build_virtual_transfer_event(call.amount) {
            self.emit_event(hop)?;
        }
        Ok(())
    ...
    pub fn transfer(&mut self, msg_sender: Address, call: ITIP20::transferCall) -> Result<bool> {
        ...
        let blocked = self.transfer_or_blocked(msg_sender, &to, call.amount, B256::ZERO)?;
    ...
    pub fn transfer_from(
        ...
        let to = Recipient::resolve(call.to)?;
        let blocked = self._transfer_from(msg_sender, call.from, &to, call.amount, B256::ZERO)?;
        if !blocked && let Some(hop) = to.build_virtual_transfer_event(call.amount) {
            self.emit_event(hop)?;
        }
        Ok(true)
    ...
    pub fn transfer_from_with_memo(
        ...
        let to = Recipient::resolve(call.to)?;
        let blocked = self._transfer_from(msg_sender, call.from, &to, call.amount, call.memo)?;
    ...
    pub fn system_transfer_from(
        ...
        let blocked = self.transfer_or_blocked(from, &to, amount, B256::ZERO)?;
    ...
    fn _transfer_from(
        ...
        self.transfer_or_blocked(from, recipient, amount, memo)
    }
    ...
    pub fn transfer_with_memo(
        ...
        let blocked = self.transfer_or_blocked(msg_sender, &to, call.amount, call.memo)?;
        if !blocked {
            self.emit_event(TIP20Event::TransferWithMemo(ITIP20::TransferWithMemo {
                from: msg_sender,
                to: call.to,
...
impl TIP20Token {
    ...
    fn blocked_inbound(
        ...
        let (authorized, blocked_reason) =
            registry.validate_receive_policy(self.address, sender, receiver)?;
        if authorized {
            return Ok(None);
        }
    ...
    fn transfer_or_blocked(
        ...
        self._transfer(from, &Recipient::direct(TIP1028_ESCROW_ADDRESS), amount)?;
        ...
        self.emit_transfer_blocked(from, recipient, amount, memo, stored)?;
        Ok(true)
    }
    ...
    fn emit_transfer_blocked(
        ...
        self.emit_event(TIP20Event::TransferBlocked(ITIP20::TransferBlocked {
            ...
            recipient: recipient.requested(),
            amount,
            blockedReason: blocked.blocked_reason.into(),
...
///
/// `target` is always the effective address where the balance is credited. For virtual recipients,
/// `virtual_addr` carries the original virtual address for event emission and blocked receipts.
///
/// [TIP-1022]: <https://docs.tempo.xyz/protocol/tip1022>
