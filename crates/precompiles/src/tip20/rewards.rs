...
use crate::{
    ...
    storage::Handler,
    tip20::{Recipient, TIP20Token, TIP20UserState},
};
...
impl TIP20Token {
    ...
    pub fn set_reward_recipient(
        ...
        if hardfork.is_t6() {
            let new_balance = TIP20UserState {
    ...
    pub fn claim_rewards(&mut self, msg_sender: Address) -> Result<U256> {
        ...
        if max_amount > U256::ZERO {
            let new_contract_balance = TIP20UserState::new(
            ...
            let new_recipient_balance =
                TIP20UserState::new(recipient_balance.checked_add(max_amount)?, flag, hardfork)?;
    ...
    pub(crate) fn handle_rewards_on_transfer(
        &mut self,
        from: Address,
        _from_balance: TIP20UserState,
        to: Address,
        _to_balance: Option<TIP20UserState>,
        amount: U256,
