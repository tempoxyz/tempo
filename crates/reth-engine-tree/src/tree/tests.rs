...
use reth_chain_state::{
    BlockState, ComputedTrieData, StateTrieOverlayManager, test_utils::TestBlockBuilder,
};
...
impl TestBlockFactory {
    fn new(chain_spec: ChainSpec) -> Self {
        Self {
            builder: TestBlockBuilder::eth().with_chain_spec(chain_spec),
        }
    }
    ...
    fn create_invalid_consensus_block(&mut self, parent_hash: B256) -> SealedBlock<Block> {
        let mut block = self
            .builder
            .generate_random_block(1, parent_hash)
            .into_block();
...
fn test_backpressure_waits_for_persistence_before_reading_incoming() {
    ...
    test_harness
        .tree
        .persistence_state
        .start_save(persisted, persist_rx);
    ...
    assert!(matches!(
        event,
        super::LoopEvent::PersistenceComplete { .. }
    ));
    assert_eq!(test_harness.tree.incoming.len(), 2);
...
fn test_tree_state_on_new_head_deep_fork() {
    ...
    for block in &chain_a {
        test_harness
            ...
            .state
            .tree_state
            .insert_executed(ExecutedBlock::new(
                Arc::new(block.clone()),
...
async fn test_get_canonical_blocks_to_persist() {
    ...
    let blocks_to_persist = test_harness
        .tree
        .get_canonical_blocks_to_persist(PersistTarget::Threshold)
        .unwrap();
    ...
    assert!(
        test_harness
            ...
            .state
            .tree_state
            .sealed_header_by_hash(&fork_block_hash)
            .is_some()
    );
...
async fn test_fcu_with_canonical_ancestor_updates_latest_block() {
    ...
    test_harness
        ...
        .state
        .tree_state
        .set_canonical_head(current_head.num_hash());
    test_harness
        .tree
        .canonical_in_memory_state
        .set_canonical_head(current_head_sealed);
...
fn test_on_new_payload_canonical_insertion() {
    ...
    assert!(
        ...
        "Disconnected payload should be syncing"
    );

    // Verify no canonicalization event
    assert!(
        outcome1.event.is_none(),
        "Should not trigger canonicalization when syncing"
    );
...
fn test_on_new_payload_invalid_ancestor() {
    ...
    test_harness
        ...
        .state
        .invalid_headers
        .insert(BlockWithParent {
            block: sealed1.num_hash(),
            parent: parent1,
        });
    ...
    assert_eq!(
        sealed2.parent_hash(),
        hash1,
        "Block 2 should have block 1 as parent"
    );
...
fn test_on_new_payload_backfill_buffering() {
    ...
    assert!(
        outcome.outcome.is_syncing(),
        "Payload should be syncing during backfill"
    );
...
fn test_validate_block_multiple_scenarios() {
    ...
    assert!(
        result2.is_err(),
        "Consensus-invalid block (invalid state root) should be rejected"
    );
    ...
    assert!(
        result3.is_err(),
        "Execution-invalid block (gas limit exceeded) should be rejected"
    );
...
mod check_invalid_ancestors_tests {
    ...
    fn test_find_invalid_ancestor_no_invalid() {
        ...
        assert!(
            result.is_none(),
            "Should return None when no invalid ancestors exist"
        );
    }
    ...
    fn test_find_invalid_ancestor_with_invalid_parent() {
        ...
        let status = test_harness
            .tree
            .handle_invalid_ancestor_payload(payload2, invalid)
            .unwrap();
        assert!(
            status.is_invalid(),
            "Status should be invalid when parent is invalid"
        );
    }
    ...
    fn test_genesis_block_handling() {
        ...
        assert!(
            result.is_none(),
            "Genesis block should have no invalid ancestors"
        );
    }
    ...
    fn test_find_invalid_ancestor_detects_block_itself() {
        ...
        assert!(
            result.is_some(),
            "Should detect block itself in invalid headers cache"
        );
    }
...
mod forkchoice_updated_tests {
    ...
    async fn test_validate_forkchoice_state() {
        ...
        let result = test_harness
            .tree
            .validate_forkchoice_state(zero_state)
            .unwrap();
        ...
        let result = test_harness
            .tree
            .validate_forkchoice_state(valid_state)
            .unwrap();
    ...
    async fn test_handle_canonical_head() {
        ...
        // Create test blocks
        let blocks: Vec<_> = test_harness
            .block_builder
            .get_executed_blocks(0..3)
            .collect();
        test_harness = test_harness.with_blocks(blocks);
    ...
    async fn test_on_forkchoice_updated_integration() {
        ...
        assert!(
            ...
            "Should trigger download event for missing block"
        );
        ...
        let fcu_result = result.outcome.await.unwrap();
        assert!(
            fcu_result.payload_status.is_syncing(),
            "Should return syncing during backfill"
        );
    }
    ...
    async fn test_fcu_with_invalid_ancestor() {
        ...
        let result = test_harness
            .tree
            .check_invalid_ancestor(invalid_block_hash)
            .unwrap();
    ...
    fn test_update_reorg_metrics() {
        ...
        test_harness
            .tree
            .canonical_in_memory_state
            .set_finalized(seal_header(30));
        test_harness
            .tree
            .canonical_in_memory_state
            .set_safe(seal_header(50));

        // Reorg at block 20 (below finalized=30) -> finalized reorg
        test_harness
            .tree
            .update_reorg_metrics(5, Some(NumHash::new(20, B256::random())));

        // Reorg at block 40 (below safe=50, above finalized=30) -> safe reorg
        test_harness
            .tree
            .update_reorg_metrics(3, Some(NumHash::new(40, B256::random())));

        // Reorg at block 60 (above safe=50) -> head reorg
        test_harness
            .tree
            .update_reorg_metrics(2, Some(NumHash::new(60, B256::random())));
    }
...
fn test_on_valid_downloaded_non_head_sync_target_continues_to_head() {
    ...
    test_harness
        ...
        .state
        .tree_state
        .set_canonical_head(genesis.recovered_block().num_hash());
    ...
    let result = test_harness
        .tree
        .on_valid_downloaded_block(safe_num_hash)
        .unwrap();
...
fn test_backfill_sync_target_without_sync_state_returns_none() {
    ...
    assert_eq!(
        test_harness
            .tree
            .backfill_sync_target(0, MIN_BLOCKS_FOR_PIPELINE_RUN + 100, None),
        None
    );
}
...
async fn assert_post_backfill_recheck_retriggers_to_buffered_target(engine_kind: EngineApiKind) {
    ...
    let main_chain = test_harness.block_builder.create_fork(
        base_chain[0].recovered_block(),
        MIN_BLOCKS_FOR_PIPELINE_RUN + 50,
    );
    ...
    test_harness
        ...
        .state
        .buffer
        .insert_block(target_block.clone_sealed_block());
