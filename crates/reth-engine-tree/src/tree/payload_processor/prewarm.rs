...
use alloy_primitives::{B256, StorageKey, keccak256};
        ...
        Tx: ExecutableTxFor<Evm> + Send + 'static,
    {
        ...
        self.executor.spawn_blocking_named("prewarm-txs", move || {
            ...
            let _ = actions_tx.send(PrewarmTaskEvent::FinishedTxExecution {
                executed_transactions: tx_count,
            });
        });
    }
    ...
    fn save_cache(
        ...
        if let Some(saved_cache) = saved_cache {
            ...
            execution_cache.update_with_guard(|cached| {
                ...
                if new_cache
                    .cache()
                    .insert_state(&execution_outcome.state)
                    .is_err()
                {
            ...
            match event {
                ...
                PrewarmTaskEvent::FinishedTxExecution {
                    ...
                    self.ctx
                        .metrics
                        .transactions
                        .set(executed_transactions as f64);
                    self.ctx
                        .metrics
                        .transactions_histogram
                        .record(executed_transactions as f64);
        ...
        let state_provider = match provider {
            ...
            slot @ None => {
                ...
                let saved_cache = self
                    .saved_cache
                    .as_ref()
                    .expect("BAL prewarm should only run with cache");
                let caches = saved_cache.cache().clone();
...
fn multiproof_targets_from_state(state: EvmState) -> (MultiProofTargetsV2, usize) {
    ...
    for (addr, account) in state {
        ...
        if !storage_slots.is_empty() {
            targets
                .storage_targets
                .insert(hashed_address, storage_slots);
        }
    }
