...
use crate::tree::{
    multiproof::{
        DEFAULT_MAX_TARGETS_FOR_CHUNKING, StateRootComputeOutcome, StateRootMessage,
    ...
    S: SparseTrie + Default + Clone,
{
    ...
    fn run_hashing_task(
        ...
        while let Ok(message) = updates.recv() {
            ...
            let msg = match message {
                ...
                StateRootMessage::QmdbRawUpdate(_) => {
                    idle_start = Instant::now();
                    continue;
                }
            };
        ...
        metrics
            .hashing_task_idle_time_seconds
            .record(total_idle_time.as_secs_f64());
    }
    ...
    pub(super) fn run(&mut self) -> Result<StateRootComputeOutcome, ParallelStateRootError> {
        ...
        self.metrics
            ...
            .record(end.duration_since(start));
        self.metrics
            .sparse_trie_total_duration_histogram
            .record(end.duration_since(now));

        self.metrics
            .sparse_trie_account_cache_hits
            .record(self.account_cache_hits as f64);
        self.metrics
            .sparse_trie_account_cache_misses
            .record(self.account_cache_misses as f64);
        self.metrics
            .sparse_trie_storage_cache_hits
            .record(self.storage_cache_hits as f64);
        self.metrics
            .sparse_trie_storage_cache_misses
            .record(self.storage_cache_misses as f64);
    ...
    fn on_hashed_state_update(&mut self, hashed_state_update: HashedPostState) {
        ...
        for (&address, &account) in &hashed_state_update.accounts {
            ...
            self.new_account_updates
                .insert(address, LeafUpdate::Touched);
    ...
    fn process_leaf_updates(&mut self, new: bool) -> SparseTrieResult<()> {
        let storage_updates = if new {
            &mut self.new_storage_updates
        } else {
            &mut self.storage_updates
        };
        ...
        for (address, updates) in storage_updates {
            ...
            if !targets.is_empty() {
                self.pending_targets
                    .extend_storage_targets(address, targets);
            }
        }
    ...
    fn process_account_leaf_updates(&mut self, new: bool) -> SparseTrieResult<bool> {
        let account_updates = if new {
            &mut self.new_account_updates
        } else {
        ...
        self.trie
            .trie_mut()
            .update_leaves(account_updates, |target, min_len| {
                match self.fetched_account_targets.entry(target) {
                    Entry::Occupied(mut entry) => {
                        if min_len < *entry.get() {
                            entry.insert(min_len);
                            self.pending_targets.push_account_target(
                                ProofV2Target::new(target).with_min_len(min_len),
                            );
                        }
                    }
                    Entry::Vacant(entry) => {
    ...
    fn compute_drained_storage_roots(&mut self) {
        ...
        let parent_span = debug_span!(
            "compute_drained_storage_roots",
            n = tries_to_compute_roots.len()
        );
        tries_to_compute_roots
            .into_par_iter()
            .for_each(|(address, SendStorageTriePtr(trie))| {
...
mod tests {
    ...
    fn test_run_hashing_task_hashed_state_update_forwards() {
        ...
        updates_tx
            ...
            .unwrap();
        updates_tx
            .send(StateRootMessage::FinishedStateUpdates)
            .unwrap();
        drop(updates_tx);
        ...
        assert_eq!(
            account.balance,
            expected_state.accounts[&address].unwrap().balance
        );
        assert_eq!(
            account.nonce,
            expected_state.accounts[&address].unwrap().nonce
        );
    ...
    fn run_returns_parent_root_without_revealing_blind_trie_when_no_state_updates() {
        ...
        assert!(
            task.trie.state_trie_ref().is_none(),
            "blind trie should not be revealed"
        );
    }
}
