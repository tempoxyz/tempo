...
impl BlockGasTracker {
    ...
    fn validate_tx_limit(&self, tx_gas_limit: u64) -> Result<(), BlockExecutionError> {
        ...
        let tx_min_gas_limit = self
            .tx_gas_limit_cap
            .map_or(tx_gas_limit, |cap| tx_gas_limit.min(cap));
    ...
    const fn record_result<H>(&mut self, result: &ResultAndState<H>) {
        ...
        self.cumulative_tx_gas_used = self
            .cumulative_tx_gas_used
            .saturating_add(gas.tx_gas_used());
        self.block_regular_gas_used = self
            .block_regular_gas_used
            .saturating_add(gas.block_regular_gas_used());
    }
}
...
mod tests {
    ...
    fn reference_bal_for_empty_block(evm_config: &EthEvmConfig) -> BlockAccessList {
        ...
        state
            .take_built_alloy_bal()
            .expect("with_bal_builder was set")
    }
    ...
    fn empty_block_happy_path_round_trip() {
        ...
        assert!(
            !input_bal.is_empty(),
            "empty BAL means system calls didn't record state"
        );
    ...
    fn multi_tx_happy_path_round_trip() {
        ...
        assert!(
            !reference_bal.is_empty(),
            "expected BAL entries from pre-exec + txs"
        );
        ...
        match result {
            Ok(output) => {
                ...
                assert!(
                    output.gas_used >= 2 * 21_000,
                    "expected at least 42k gas used"
                );
            }
            Err(e) => panic!("expected success, got {e:?}"),
        }
    ...
    fn assert_shadow_equal(
        ...
        let (serial, reference_bal) = run_serial_path(
            &evm_config,
            canonical_db_template.clone(),
            &block_header_only,
            &txs,
        );
    ...
    fn returns_built_bal_for_final_hash_mismatch() {
        ...
        match result {
            Ok((_, built_bal)) => {
                ...
                assert_ne!(
                    rebuilt, tampered_hash,
                    "rebuilt and header hashes must differ"
                );
            }
    ...
    fn worker_tx_recovery_error_becomes_other_error() {
        ...
        tx_tx
            .send((0, Err(std::io::Error::other("sig fail"))))
            .unwrap();
        drop(tx_tx);
    ...
    fn gas_tracker_non_amsterdam_uses_cumulative_gas() {
        ...
        assert!(
            non_amsterdam
                .validate_tx_limit(second_tx_gas_limit)
                .is_err(),
