...
use reth_tasks::{ForEachOrdered, Runtime, utils::increase_thread_priority};
use reth_trie::{
    ...
    Evm: ConfigureEvm,
{
    fn wait_for_caches(&self) -> CacheWaitDurations {
        ...
        self.executor
            .spawn_blocking_named("wait-exec-cache", move || {
        ...
        let execution_cache_duration = execution_rx
            .recv()
            .expect("execution cache wait task failed to send result");
        let sparse_trie_duration = sparse_trie_rx
            .recv()
    ...
    Evm: ConfigureEvm<Primitives = N> + 'static,
{
    ...
    fn spawn_tx_iterator<I: ExecutableTxIterator<Evm>>(
        ...
        IteratorPrewarmTxReceiver<Evm, I>,
        IteratorExecuteTxReceiver<Evm, I>,
    ) {
        ...
        P: BlockReader + StateProviderFactory + StateReader + Clone + 'static,
    {
        let mode = if parallel_bal_execution {
            PrewarmMode::BlockAccessList(
                env.decoded_bal
                    .clone()
                    .expect("BAL dispatch implies decoded BAL"),
            )
        ...
        let saved_cache = self
            .disable_state_cache
            .not()
            .then(|| self.cache_for(env.parent_hash));
...
impl<Tx, Err, R: Send + Sync + 'static> PayloadHandle<Tx, Err, R> {
    ...
    pub fn state_root(&mut self) -> Result<StateRootComputeOutcome, ParallelStateRootError> {
        self.state_root_handle
            .as_mut()
            .expect("state_root_handle is None")
            .state_root()
    }
...
mod tests {
    use crate::tree::{
        ...
        payload_processor::{ExecutionEnv, PayloadProcessor, evm_state_to_hashed_post_state},
    ...
    fn execution_cache_allows_single_checkout() {
        ...
        assert!(
            second.is_none(),
            "second checkout should be blocked while guard is active"
        );
        ...
        assert!(
            third.is_some(),
            "third checkout should succeed after guard is dropped"
        );
    }
    ...
    fn execution_cache_mismatch_parent_clears_and_returns() {
        ...
        assert!(
            cache.is_some(),
            "cache should be returned for reuse after clearing"
        );
        ...
        assert!(
            original.is_some(),
            "canonical chain gets cache back via mismatch+clear"
        );
    }
    ...
    fn execution_cache_update_after_release_succeeds() {
        ...
        let guard = execution_cache
            .get_cache_for(initial)
            .expect("expected initial checkout to succeed");
        ...
        assert!(
            new_checkout.is_some(),
            "new checkout should succeed after release and update"
        );
    }
    ...
    fn on_inserted_executed_block_skips_on_parent_mismatch() {
        ...
        assert!(
            cached3.is_none(),
            "New block cache should not be created on mismatch"
        );
    }
    ...
    fn fork_prewarm_dropped_without_save_does_not_corrupt_cache() {
        ...
        assert!(
            prewarm_cache.is_some(),
            "prewarm should obtain cache for fork block"
        );
        ...
        prewarm_cache
            .cache()
            .insert_storage(fork_addr, fork_key, Some(U256::from(999)));
