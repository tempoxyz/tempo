...
use crate::{
    ...
    tree::{
        ...
        multiproof::{SharedStateRootHandle, StateRootComputeOutcome, StateRootHandle},
...
use reth_trie::{HashedPostState, trie_cursor::TrieCursorFactory, updates::TrieUpdates};
    ...
    Evm: ConfigureEvm<Primitives = N> + 'static,
{
        ...
        Evm: ConfigureEngineEvm<T::ExecutionData, Primitives = N>,
    {
        ...
        if let Some(decoded_bal) = decoded_bal.as_deref() {
            ensure_ok!(Self::validate_received_bal_gas(
                decoded_bal,
                input.gas_limit()
            ));
        }
        ...
        let mut hashed_state: LazyHashedPostState = self
            .payload_processor
            .executor()
        ...
        Evm: ConfigureEngineEvm<T::ExecutionData, Primitives = N>,
    {
        ...
        let executor = executor.with_state_hook(
            handle
                .state_hook()
                .map(|hook| Box::new(hook) as Box<dyn OnStateHook>),
        );
    ...
    > {
        ...
        match task_rx.recv_timeout(timeout) {
            ...
            Err(RecvTimeoutError::Disconnected) => Ok(Err(ParallelStateRootError::Other(
                "sparse trie task dropped".to_string(),
            ))),
            Err(RecvTimeoutError::Timeout) => {
                ...
                self.payload_processor
                    .executor()
                    .spawn_blocking_named("serial-root", move || {
    ...
    > {
        match strategy {
            ...
            | StateRootStrategy::Custom(_) => {
                ...
                let handle =
                    self.payload_processor
                        .spawn_cache_exclusive(env, txs, provider_builder);
...
>: Send + Sync + 'static
{
    ...
    /// Spawns a shared state-root pipeline and returns a handle for the payload builder.
    fn shared_state_root_handle_for(
        ...
        self.sparse_trie_handle_for(parent_hash, parent_state_root, state)
    }
