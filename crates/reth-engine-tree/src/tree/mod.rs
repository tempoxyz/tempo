...
use reth_tasks::{WorkerPool, spawn_os_thread, utils::increase_thread_priority};
    ...
    V: EngineValidator<T> + WaitForCaches,
{
    ...
    pub fn run(mut self) {
        loop {
            ...
            let event = if self.should_backpressure() {
                ...
                self.metrics
                    .engine
                    .backpressure_stall_duration
                    .record(stall_start.elapsed());
                event
            } else {
    ...
    fn on_backfill_sync_finished(
        ...
        if let Some(backfill_target) =
            ctrl.block_number()
                .zip(newest_target)
                .and_then(|(progress, target_number)| {
        ...
        if let Some(lowest_buffered) = self
            .state
            .buffer
            .lowest_ancestor(&sync_target_state.head_block_hash)
        {
    ...
    fn on_canonicalized_sync_target(&mut self, target: B256) {
        ...
        self.state
            .forkchoice_state_tracker
            .promote_sync_target_to_valid(sync_target_state);
    }
    ...
    fn check_invalid_ancestor(&mut self, head: B256) -> ProviderResult<Option<PayloadStatus>> {
        ...
        match self.prepare_invalid_response(header.parent) {
            ...
            Err(err) => {
                ...
                Ok(Some(PayloadStatus::from_status(
    ...
    fn on_valid_downloaded_block(
            ...
            && sync_target.contains(block_num_hash.hash)
        {
            ...
            if self.state.tree_state.canonical_block_hash() != sync_target.head_block_hash {
                ...
                return Ok(Some(TreeEvent::Download(DownloadRequest::single_block(
                    target,
                ))));
            }
    ...
    fn on_downloaded_block(
        ...
        if self
            .check_invalid_ancestor_with_head(lowest_buffered_ancestor, &block)?
            .is_some()
        {
            return Ok(None);
        }
        ...
        Err: From<InsertBlockError<N::Block>>,
    {
        ...
        if let Some(stats) = timing_stats {
            ...
            self.execution_timing_stats
                .insert(executed.recovered_block().hash(), stats);
        }
    ...
    fn on_insert_block_error(
        ...
        self.emit_event(EngineApiEvent::BeaconConsensus(
            ConsensusEngineEvent::InvalidBlock {
                block: Box::new(block),
        ...
        match self.find_canonical_header(finalized_block_hash) {
            ...
            Ok(Some(finalized)) => {
                    ...
                    != self.canonical_in_memory_state.get_finalized_num_hash()
                {
                    ...
                    self.canonical_in_memory_state
                        .set_finalized(finalized.clone());
                    // Update finalized block height metric
                    self.metrics
                        .tree
                        .finalized_block_height
                        .set(finalized.number() as f64);
                }
            }
            Err(err) => {
    ...
    fn process_payload_attributes(
        ...
        if let Err(err) = self
            .payload_validator
            .validate_payload_attributes_against_header(&attributes, head)
        {
        ...
        let trie_handle = if self.config.share_sparse_trie_with_payload_builder() {
            self.payload_validator.shared_state_root_handle_for(
                state.head_block_hash,
                head.state_root(),
                &self.state,
            )
