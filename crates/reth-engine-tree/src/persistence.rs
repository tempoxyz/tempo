...
mod tests {
    ...
    use alloy_primitives::{B256, BlockHash, BlockNumber, Bytes, Sealed, U256, keccak256};
    ...
    fn test_save_blocks_then_prune_preserves_new_history() {
        ...
        batch1
            .append_account_history_shard(tracked_addr, 20..25u64)
            .unwrap();
    ...
    fn test_read_only_consistency_across_reorg() {
...

        {
            ...
            provider_rw
                .save_blocks(blocks_a, SaveBlocksMode::Full)
                .unwrap();
        ...
        let reorg_handle = std::thread::spawn(move || {
            ...
            let provider_rw = pf.database_provider_rw().unwrap();
            provider_rw
                .save_blocks(vec![block_b2], SaveBlocksMode::Full)
                .unwrap();
            provider_rw.commit().unwrap();
        });
        ...
        assert!(
            account_at_1.is_some(),
            "signer account must exist at block 1 after reorg"
        );
        let account_at_1 = account_at_1.unwrap();
        assert_eq!(
            account_at_1.balance, balance_after_block1,
        ...
        assert!(
            ...
            "signer account must exist at block 2 after reorg"
        );
