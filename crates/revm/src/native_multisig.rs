...
pub(super) fn validate_state<DB: Database + Debug>(
    ...
    let early_multisig_balance = if !tx.is_subblock_transaction()
    ...
    let caller_account_info = if validates_caller_multisig || outer_keychain_signature {
        Some(journal.load_account(tx.caller())?.data.info.clone())
    } else {
        None
    };
    let keychain_caller_has_code = outer_keychain_signature
        && !caller_account_info
            .as_ref()
