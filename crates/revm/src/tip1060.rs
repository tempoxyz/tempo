...
pub fn apply_refund<DB: Database, I>(evm: &mut TempoEvm<DB, I>) -> Result<(), DB::Error> {
    ...
    // Snapshot the transient (key, credit) pairs at the storage credits contract written during this
    ...
    for (key, credit) in credits {
        ...
        let current = journal.sload(STORAGE_CREDITS_ADDRESS, key)?.data;

        // SSTORE the accumulated total back into the contract's persistent storage.
        journal.sstore(STORAGE_CREDITS_ADDRESS, key, current.saturating_add(credit))?;
    }
