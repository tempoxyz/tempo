...
use tempo_precompiles::{ECRECOVER_GAS, native_multisig::MULTISIG_ACCOUNT_CREATE2_GAS};
...
pub(crate) const NATIVE_MULTISIG_COMMITMENT_READ_GAS: u64 = COLD_SLOAD_COST;

/// Gas for checking that each nested multisig owner account has no code or delegation.
const NATIVE_MULTISIG_NESTED_ACCOUNT_GAS: u64 = COLD_ACCOUNT_ACCESS_COST;
...
fn native_multisig_node_gas(signature: &MultisigSignature, depth: usize) -> u64 {
    ...
    let config_proof_gas = if config.version == 0 {
        keccak_gas(config.account_salt_preimage_len()).saturating_add(MULTISIG_ACCOUNT_CREATE2_GAS)
    } else {
