...
def run-local-e2e-phase [run: record, ctx: record] {
    ...
    if $ctx.lifecycle {
        ...
        # The wrapper emits only validated progress on stderr. Redirect that channel around
        # `complete` so markers arrive live while the exact reporter exit status is retained.
        let report = (^python3 contrib/bench/lifecycle/progress.py ...$scheduler_report_args --prune --expected-detail $capture_detail --out $lifecycle_report_dir --warmup $ctx.summary_warmup_blocks --window $"($lifecycle_dir)/window.json" $"($lifecycle_dir)/a.jsonl" $"($lifecycle_dir)/b.jsonl" err> /dev/stderr | complete)
