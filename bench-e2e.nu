...
const E2E_SEED = 42
const E2E_A_CPUS = "0-6,16-23"
const E2E_B_CPUS = "8-14,24-31"
const E2E_TRACY_CAPTURE_CPUS = "7,15"
...
def run-local-e2e-phase [run: record, ctx: record] {
    ...
    if $phase_exit == 0 and $ctx.tracy != "off" {
        ...
        if $ctx.tracy_offset > 0 {
            print $"  Tracy-capture will start on CPU(s) ($ctx.tracy_capture_cpus) in ($ctx.tracy_offset)s($limit_msg)..."
