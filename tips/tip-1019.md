...
- **Nonce remainder** (`nonce_remainder`): The low 11 bytes (`uint88`) of the nonce key. For sequential nonces, this is the nonce value within an independent `(account, nonceKey)` sequence in the NonceManager. For expiring nonces, this is set to max uint88.
...
- **Validator in the set**: An address whose corresponding validator entry in `ValidatorConfig` has `deactivatedAtHeight == 0`.
...
- This is the same mechanism validators use; builders MUST set a fee token preference before their subblocks can route fees
- If `FeeManager.validatorTokens[builder]` is unset (zero), fees are routed in PathUSD (the default)
...
2. For each 0x5b transaction:
   ...
   g. Restore block.beneficiary after tx
