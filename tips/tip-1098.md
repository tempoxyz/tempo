...
`tempo-nitro-attestation` crate. Serde derives payload field selection and duplicate/missing-field
checks, and pinned `ciborium` decodes text, bytes, and the timestamp and encodes `Sig_structure`.
A pinned `minicbor` pull decoder retains COSE framing, strict wire-type checks, and explicit
size, collection, and nesting
bounds before allocating decoded strings. Unknown values are skipped without interpreting their
tags or requiring them to fit Serde's data model. The Zone verifier supplies AWS-LC SHA-384 and
P-384 operations.
