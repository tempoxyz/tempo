...
During rollout, a temporary fallback mode is an explicit exception to the Nitro-only requirements;
it retains the existing sequencer-quorum trust assumption until a later hardfork removes it.

This amendment proposes the following supported modes, encoded as a single-byte `verifierConfig`:

- `0x01`: **Nitro** — existing mode requiring a valid Nitro attestation.
- `0x02`: **Fallback** — new, temporary mode permitting quorum-authorized settlement without a
  Nitro proof. Support requires verifier and sequencer implementation plus hardfork activation.

1. **Automatic fallback:** T13 permits fallback alongside Nitro verification. Sequencers attempt
   normal proving first, with a local flag initially set to `true` allowing each signer to approve
   fallback batches when proving fails. Fallback use is monitored and alerted.
2. **Operator-enabled fallback:** after two weeks of successful proving, set the flag's default
   and deployed configuration to `false`. Operators must explicitly re-enable it on sufficient
   sequencers to authorize recovery, then disable it when normal proving resumes. This changes
   operational policy without a hardfork.
3. **Proof-required settlement:** a later hardfork disables fallback regardless of sequencer
   configuration. Every batch must then satisfy the Nitro verification rules below.

While permitted, fallback requires an empty proof.
It retains the common calldata, ABI, and canonical-portal checks, but skips Nitro validation and
pays only the common input gas charge. The portal's existing quorum signatures bind the mode and
settlement inputs; all other portal checks remain in force. Nitro mode (`0x01`) stays strict:
a failed proof never implicitly becomes fallback. No new admin key or on-chain configuration is
introduced, and only hardfork rules can enable or disable protocol acceptance of fallback.

Eligible failures, including whether execution mismatches permit automatic fallback, remain a
runbook review question before activation. Rollout readiness is tracked in the
...
> **Rollout exception:** Empty proofs are permitted only in the temporary fallback mode described
