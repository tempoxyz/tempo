...
Nitro verification checks. Operators SHOULD use `NoProof` only after a previous Nitro verification
failed with `InvalidProof`. A later hardfork MUST disable `NoProof`, after which every batch must
satisfy the Nitro verification rules below.
...
settlement inputs; all other portal checks remain in force. `Nitro` stays strict: a failed proof
never implicitly becomes `NoProof`. No new admin key or on-chain configuration is introduced, and
only hardfork rules can enable or disable protocol acceptance of `NoProof`.
