...
The Nitro verification rules in this TIP remain the target for proof-required settlement.
During rollout, the temporary `NoProof` fallback mode is an explicit exception to the Nitro-only
requirements; it retains the existing sequencer-quorum trust assumption until a later hardfork
removes it.

This amendment proposes the following supported modes, encoded as a single-byte `verifierConfig`:

- **Nitro** (`0x01`) — existing mode requiring a valid Nitro attestation.
- **NoProof** (`0x02`) — new, temporary mode permitting quorum-authorized settlement without a
  Nitro proof. Support requires verifier and sequencer implementation plus hardfork activation.

During the rollout phase, a sequencer operator MAY submit settlements using `NoProof` to bypass
Nitro verification checks. Operators SHOULD use `NoProof` only as a fallback when necessary,
including either of these cases:

- **Prover unavailable:** a Nitro proof cannot be obtained because the prover is unavailable.
  Proof validation failures are not prover-unavailability failures. No protocol-level prover
  deadline is defined.
- **Verifier rejection:** a Nitro settlement is rejected with `InvalidProof`.

This fallback policy is not an additional on-chain validity condition: while enabled by the active
hardfork, either mode is valid regardless of which mode was used for a previous settlement. A later
hardfork MUST disable `NoProof`, after which every batch must satisfy the Nitro verification rules
below.

While permitted, `NoProof` requires an empty proof. It retains the common calldata, ABI,
canonical-portal, continuity, pause, and settlement checks, but skips Nitro validation and pays only
the common input gas charge. The portal's existing quorum remains required, and quorum signatures
bind the mode and settlement inputs. Followers validate those inputs before signing, but accept the
trusted leader's choice between the two hardfork-enabled modes. No new admin key or on-chain
configuration is introduced, and only hardfork rules can enable or disable protocol acceptance of
`NoProof`.
...
`verifierConfig` MUST be one of the modes enabled by the active hardfork. For `Nitro` (`0x01`),
`proof` MUST be the raw COSE/CBOR Nitro attestation document. For `NoProof` (`0x02`), `proof` MUST
be empty and the verifier skips Nitro document validation.
...
arguments revert. Successfully decoded calls return `false` for a noncanonical caller, a mode not
enabled by the active hardfork, a proof whose presence does not match its mode, Nitro policy
failure, or commitment mismatch. Out-of-gas remains an exceptional halt.
...
configuration, an empty Nitro proof, a nonempty `NoProof` proof, and proof length above 24,576
bytes return `false` without Nitro base or signature gas.
...
or NSM failure. Production sequencers MUST NOT submit an empty proof using `NoProof`.

> **Rollout exception:** Empty proofs are permitted only in the temporary `NoProof` mode described
> in the [Verifier Rollout Plan](#verifier-rollout-plan), while enabled by the active hardfork.
> Nitro mode continues to reject empty proofs.
...
During the [rollout phase](#verifier-rollout-plan), operators SHOULD alert when settlement falls
back to `NoProof`.
