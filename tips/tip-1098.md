...
The verifier extends the proof-agnostic TIP-1096 interface with the executed Zone height:
...
`nextZoneHeight` MUST be the same height that the portal records for the accepted batch.
The measured application MUST derive it from the final executed Zone header and include it in
the attestation commitment; it MUST NOT attest an unchecked caller-supplied height. The T13
portal runtime and measured application MUST use this interface and commitment together.
...
| ZV3 | Complete binding | Changing any verifier input, chain, or verifier changes `user_data`; only the canonical portal for the committed `zoneId` can obtain `true`. |
