...
- `0x02`: **NoProof** — new, temporary mode permitting quorum-authorized settlement without a
  Nitro proof. Support requires verifier and sequencer implementation plus hardfork activation.

During the rollout phase, a sequencer operator MAY submit settlements using verifier mode `0x02`
(`NoProof`) to bypass Nitro verification checks. Operators SHOULD use `NoProof` only after a
previous Nitro verification failed with `InvalidProof`. A later hardfork MUST disable `NoProof`,
after which every batch must satisfy the Nitro verification rules below.

While permitted, `NoProof` requires an empty proof.
...
a failed proof never implicitly becomes `NoProof`. No new admin key or on-chain configuration is
introduced, and only hardfork rules can enable or disable protocol acceptance of `NoProof`.

Rollout readiness is tracked in the
