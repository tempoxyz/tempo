...
All validation is internal to `verify`. After the caller and configuration checks, Nitro
validation MUST complete before the PCR, timestamp, and commitment policy is applied.
...
3. The payload is a CBOR map containing non-null, non-duplicate `module_id`, `digest`, `timestamp`,
   ...
   MAY be absent, CBOR null, or CBOR undefined, which is represented internally as empty bytes.
4. PCR indices are unique integers from 0 through 31, with 1–32 total entries and values of 32,
   ...
   payload keys and values are ignored after successful Ciborium decoding. Definite and indefinite
   CBOR are accepted with Ciborium's recursion limit set to 16.
...
CBOR is decoded using the pinned Ciborium 0.2.2 Serde value model. This normalizes undefined to
null and representable tag-2/tag-3 bignums encoded with definite byte strings of at most 16 bytes
to integers before the above checks; other tags remain tagged values. Unsupported simple values
are rejected even in ignored fields. Field limits
are checked on decoded values; the document and payload byte limits bound decoding input, and
the decoder limits recursion. The original protected-header and payload contents are retained
unchanged for signature verification.
...
The caller MUST equal `portalAddress(zoneId)` using the canonical TIP-1091 derivation: the
12-byte prefix `0x5ad000000000000000000000` followed by the zone ID as an 8-byte big-endian
integer. A noncanonical caller MUST return `false` before attestation validation. The portal
is therefore bound by the zone ID and is not a separate field in the attestation digest.
...
entire calldata, including the function selector and ABI padding. Noncanonical callers, unsupported
...
`tempo-nitro-attestation` crate. Ciborium's Serde decoder handles the COSE envelope, protected
header, and payload; its Serde encoder constructs `Sig_structure`. A derived `AttestationDocument`
handles payload fields, required fields, and duplicate field rejection. Small adapters preserve
strict field types and PCR map entries for duplicate-index validation. Schema errors are reported
as `InvalidPayload`; Nitro bounds are checked after deserialization. `minicbor` is used
only to generate independent test inputs. The Zone verifier supplies AWS-LC SHA-384 and P-384
operations.
...
| ZV3 | Complete binding | Changing any committed verifier input, chain, or verifier changes `user_data`; the caller must be the canonical portal for the committed zone ID. |
