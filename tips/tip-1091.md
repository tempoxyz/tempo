...
owner and MUST revert when `source` has no runtime bytecode. A successful call
MUST copy the complete runtime bytecode at `source` to
`ZONE_PORTAL_IMPL_ADDRESS`, equivalent to `EXTCODECOPY`, and emit
`PortalImplementationUpdated(source, codeHash)`.
...
2. build the canonical zone genesis from the zone ID, portal, initial token,
   admin, sequencer set, settlement threshold, and parent Tempo header; and
...
sequencer set, threshold, genesis block hash, and Tempo block number to the
portal initializer.
...
6. calls the portal's factory-only initializer with the sequencer set,
   threshold, computed genesis commitment, and the protocol-managed verifier
