...
MUST install that runtime from its verified source deployment using
`setPortalImplementation`. The launch configuration MUST record the revision and
...
The verified activation source deployment addresses remain to be assigned:
...
`PortalImplementationUpdated(source, codeHash)`. Validators MUST NOT embed the
full runtime bytecode in their binaries.

This function installs the initial portal implementation and upgrades every
existing portal, because every portal proxy delegates to the fixed
`ZONE_PORTAL_IMPL_ADDRESS`. Replacement implementations MUST preserve the
existing storage layout and MUST embed `ZONE_FACTORY_ADDRESS` as their
initializer authority.

Transferring ownership to the zero address disables ordinary account-controlled
upgrades. A future hardfork MAY submit `setPortalImplementation` as a system
transaction from the zero address at a hardfork boundary, allowing upgrades to
be hardfork-gated without changing the factory ABI.
...
At activation, T9 MUST copy the `Verifier` and shared `ZoneMessenger` runtime
bytecode from their verified source deployments to `ZONE_VERIFIER_ADDRESS` and
...
binaries. The shared messenger MUST use `ZONE_FACTORY_ADDRESS` as its factory
authority.
