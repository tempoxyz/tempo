...
The central portal logic bytecode is stored at:
...
The chain specification for the activation hardfork MUST define the following
consensus-critical values:

- `ZONE_FACTORY_OWNER`, a nonzero account authorized to create zones during the
  initial permissioned launch;
- `INITIAL_ZONE_VERIFIER_ADDRESS`, a nonzero account containing the canonical
  verifier runtime; and
- `ZONE_MESSENGER_ADDRESS`, a nonzero account containing the canonical shared
  messenger runtime.

The activation transition MUST install and initialize these components in the
following order before calls to `ZONE_FACTORY_ADDRESS` are enabled:

1. install the verifier runtime at `INITIAL_ZONE_VERIFIER_ADDRESS`;
2. install the shared messenger runtime at `ZONE_MESSENGER_ADDRESS`, constructed
   with its immutable factory reference equal to `ZONE_FACTORY_ADDRESS`;
3. etch the initial `ZonePortal` runtime, constructed with
   `ZONE_FACTORY_ADDRESS`, at `ZONE_PORTAL_LOGIC_ADDRESS`; and
4. initialize the native factory state so that its next zone ID is `1`, its
   owner is `ZONE_FACTORY_OWNER`, its default verifier is
   `INITIAL_ZONE_VERIFIER_ADDRESS`, that verifier is valid, and its messenger is
   `ZONE_MESSENGER_ADDRESS`.

These writes are consensus state-transition rules; the constructor in the
Solidity factory artifact is only an executable model of them. The transition
MUST fail if any configured address is zero, if the messenger's embedded
factory is not `ZONE_FACTORY_ADDRESS`, or if installed runtime does not match
the runtime committed by the activation chain specification. In particular, a
messenger deployed for the legacy EVM factory MUST NOT be reused.
...
The activation hardfork MUST etch the initial `ZonePortal` runtime, constructed
with `ZONE_FACTORY_ADDRESS`, at `ZONE_PORTAL_LOGIC_ADDRESS`. A later hardfork MAY
replace the runtime at that same address, but the replacement runtime MUST embed
the same initializer authority. Because every portal proxy delegates to the
fixed logic address, the replacement upgrades all existing portals without
changing their proxy bytecode or per-portal storage. Replacement implementations
MUST preserve the existing storage layout or the hardfork MUST migrate affected
portal state.
