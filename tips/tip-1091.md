...
The native factory MUST start with zone ID `1` and hardcoded nonzero values for
its initial owner, default verifier, and shared messenger. The default verifier
MUST be valid for zone creation. The shared messenger MUST authorize
`ZONE_FACTORY_ADDRESS`; a messenger configured for the legacy EVM factory MUST
NOT be used.
