...
block of epoch `E`. The last block of epoch `E` is the block at height `h` where
...
Omitting a call to a deployed contract can change the state root. Removing Ethereum
validation, rewards, or DAO processing can change block validity or balances on
custom chains. These differences require a fork boundary even if the
excluded contracts are absent from a network's current state.

# Tooling

N/A. Existing block-building, validation, and replay tools use the executor's
