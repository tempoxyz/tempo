...
Tempo stores protocol feature activation state in an onchain feature registry. The registry stores the active feature head, one scheduled feature head, a schedule nonce, and the latest schedule readiness recorded for each proposer public key.
...
- `schedule_nonce`: a monotonically increasing identifier for successful scheduling attempts
- `validator_confirmed_schedule_nonce`: the latest schedule nonce reported ready by each proposer public key
...
contract FeatureRegistry {
  ...
  uint64 schedule_nonce;

  mapping(bytes32 public_key => uint64 schedule_nonce) validator_confirmed_schedule_nonce;
...
`validatorConfirmedScheduledFeatureReadiness(publicKey)` MUST return `true` only when a feature head is scheduled and the public key's latest reported-ready nonce equals the current `schedule_nonce`. `scheduledFeatureSupport()` MUST return `(0, 0)` when no feature head is scheduled, and `hasScheduledFeatureQuorum()` MUST return `false` when the required count is zero.
...
Scheduling MUST increment `schedule_nonce`, write `scheduled_feature_head` and `scheduled_activation_epoch`, then emit an event to notify network participants. The schedule nonce identifies the current schedule instance, so readiness from canceled, failed, or superseded schedules cannot count for a later schedule, even when the same feature head and activation epoch are scheduled again. The scheduled activation epoch is the consensus epoch whose first block executes with the activated feature head if quorum was reached. Operators should upgrade before that epoch if they are running a release that does not support the scheduled feature head.
...
`scheduledFeatureSupport()` MUST evaluate readiness for the current schedule by iterating the current committee public keys and counting each public key whose `validator_confirmed_schedule_nonce[public_key] == schedule_nonce`. A scheduled feature head has quorum when at least 80% of current committee public keys have reported readiness for that schedule instance. The required validator count is `ceil(4 * current_committee_size / 5)`.
...
When a validator reports `ready = true`, the registry writes `validator_confirmed_schedule_nonce[public_key] = schedule_nonce` and emits a readiness event for the scheduled feature head. A later ready report for a different schedule overwrites the public key's previous report.

When a validator reports `ready = false`, the registry clears the public key's stored readiness if and only if `validator_confirmed_schedule_nonce[public_key] == schedule_nonce`, then emits a readiness event for the scheduled feature head. This lets a validator that rolled back or no longer supports the scheduled feature head cancel its current report without clearing readiness for another schedule instance.
...
A current-committee public key contributes to `scheduledFeatureSupport()` when `validator_confirmed_schedule_nonce[public_key] == schedule_nonce`. A validator running a newer Tempo release can still report readiness for a scheduled feature head below its highest supported head if that head is present in its local hash chain.

## Cancellation and Supersession

Before activation, the registry owner can cancel a scheduled feature head. Cancellation MUST clear `scheduled_feature_head` and `scheduled_activation_epoch`, and MUST NOT reset `schedule_nonce`. Stale readiness remains stored but cannot count for future schedules because each successful schedule uses a new nonce. Cancellation does not remove feature code from released binaries; it only removes the scheduled onchain activation. A new target feature head can be scheduled later.
...
Mitigation: Readiness reports use the current block proposer public key, and quorum checks use the TIP-1070 current committee public keys at the end of the reporting epoch. Readiness is bound to the current schedule nonce, which is bumped on each successful schedule, so stale reports from canceled or failed schedules do not count. The registry activation path gates activation on quorum before the scheduled activation epoch begins. If quorum is missing, the scheduled feature head MUST NOT activate and the active feature head MUST remain unchanged.
...
1. `bytes32(0)` means no active or scheduled feature head.
...
8. `schedule_nonce` increments on every successful schedule and is not reset on cancellation or activation.
9. Only the registry owner can schedule or cancel feature activation.
10. Only the system caller can record validator readiness reports or activate a scheduled feature head during block processing.
11. A validator can only report readiness for the currently scheduled feature head.
12. A readiness-report system transaction, if present, must be the first transaction in the block transaction list and must contain canonical `reportFeatureReadiness(bool)` ABI calldata before its block-number suffix.
13. Quorum for the current schedule counts a current-committee public key if and only if its latest reported-ready nonce equals the current `schedule_nonce`.
14. Stale readiness from an earlier schedule nonce must not count for a later schedule.
15. A scheduled feature head activates only if the TIP-1070 current committee has quorum readiness for that schedule at the end of the epoch before activation.
16. A node can import pre-activation blocks that schedule an unknown feature head, but it must not report readiness for an unsupported scheduled feature head.
17. `activateScheduledFeatureHead()` returns `bytes32(0)` when no feature head activates and a nonzero feature head only when it activates that head.
18. A block executor can commit activation-call state only after the call succeeds, its return value decodes to `bytes32`, and any nonzero returned feature head is supported locally.
19. A node must not validate, build, simulate, or serve post-activation protocol behavior for a chain whose active feature head is not present in the node's supported feature cache.
