...
A feature has quorum when enough validators in the active validator set support it. The threshold is the registry's activation quorum, expressed as `n/m`, where `m` is the active validator set size and `n` is the required number of active validators that support the feature. Quorum does not activate the feature by itself. It is rechecked at the activation boundary against the then-current active validator set.
