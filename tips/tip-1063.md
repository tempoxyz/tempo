...
A designated multisig schedules a feature by setting a future activation epoch in an onchain feature registry. Scheduling announces the intended activation boundary and makes the activation epoch the upgrade deadline. When the chain crosses into the scheduled epoch, Tempo rechecks the current active validator set for quorum and validates the feature's dependency bitmap against the active feature bitmap. If both checks pass, the feature becomes active. Validators that have not upgraded must stop before processing blocks in epochs that require the active feature.
...
A feature has quorum when at least 80% of the active validator set supports it. The required validator count is `ceil(4 * active_validator_count / 5)`. This activation quorum is intentionally higher than Tempo's consensus quorum so a small number of misconfigured validators cannot cause the chain to activate a feature without broad operator support. Quorum does not activate the feature by itself. It is rechecked at the activation boundary against the then-current active validator set.
...
interface IFeatureRegistry {
    ...
    /// @notice Returns the global activation quorum threshold; currently 4/5, or 80%.
...
/// @notice Required feature dependencies are not active at the activation epoch.
error FeatureDependenciesNotSatisfied();
```
