...
When building a proposed block, a node should submit a readiness report only when its local support for the scheduled feature head differs from the scheduled readiness recorded for its proposer public key. If submitted, the node MUST place the readiness-report system transaction first in the block transaction list.

For a scheduled feature head `H`, a current-committee public key contributes to quorum when `validator_confirmed_feature_head[public_key] == H`. A validator running a newer Tempo release can still report readiness for an older scheduled feature head if that head is present in its local hash chain.
...
11. A readiness-report system transaction, if present, must be the first transaction in the block transaction list.
