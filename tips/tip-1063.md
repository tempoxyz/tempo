...
When building a proposed block, a node should submit a readiness report only when its local support for the scheduled feature head differs from the scheduled readiness recorded for its validator address. If submitted, the node MUST place the readiness-report system transaction first in the block transaction list.
...
12. Quorum for scheduled feature head `H` counts a validator if and only if its latest reported-ready head is exactly `H`.
13. A scheduled feature head activates only if the TIP-1070 current committee has quorum readiness for that head.
