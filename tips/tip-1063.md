...
A scheduled feature becomes active at the boundary of its activation epoch. Activation is applied during the pre-execution hooks of the first block of that epoch. If a feature's `activation_epoch` is less than or equal to the current epoch, the registry validates that the feature's dependency bitmap is a subset of the active feature bitmap and that the current active validator set has quorum for the feature's `minimum_supported_version_key`.

Dependencies activating in the same epoch do not count as active dependencies. A dependent feature must activate in a later epoch than the features it depends on.
