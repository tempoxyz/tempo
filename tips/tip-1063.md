...
A designated multisig schedules a feature by setting a future activation epoch in an onchain feature registry. Scheduling announces the intended activation boundary and makes the activation epoch the upgrade deadline. On activation, two requirements must be true: 1) The feature has reached quorum, 2) If a feature has configured dependencies, those features MUST be activated before. If both checks pass, the feature becomes active. Validators that have not upgraded must stop before processing blocks in epochs that require the active feature.
