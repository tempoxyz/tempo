...
The registry owner schedules a feature by setting a future activation epoch in an onchain feature registry. Scheduling announces the intended activation boundary and makes the activation epoch the upgrade deadline. At or after the scheduled epoch, `activateFeature` evaluates two checks: the feature has reached activation quorum, and every feature listed in the required feature bitmap is already active. If both checks pass, the feature becomes active. Validators that have not upgraded must stop before processing blocks in epochs that require the active feature.

Activation eligibility is bound to epoch boundaries rather than wall-clock timestamps for two reasons. First, epoch transitions in Tempo's consensus are not reorged, so once `activateFeature` succeeds the feature switch is recorded in a specific epoch and every node observes the same activation boundary. Second, anchoring activation eligibility to epoch boundaries lets the same feature flag mechanism gate consensus-layer features as well, since the CL can read the registry at epoch boundaries and apply activation in lockstep with the EL.

## Feature Registry

Tempo stores protocol feature data in an onchain feature registry. Each feature is keyed by feature ID, and the registry records the minimum supported version key, required feature bitmap, status, and activation epoch for that feature. Registry state is the source of truth for feature activation on Tempo.
...
If the activation checks pass, the registry marks the feature `active`, sets `activation_epoch` to the current epoch, sets its bit in `active_feature_bitmap`, appends its feature ID to `active_feature_order`, and emits the activation event. Every block from that point on is processed with the feature active. Because epoch boundaries are finalized in Tempo's consensus, the activation cannot be reorged once observed.
...
The feature registry exposes a `minimum_required_version()` function for registering minimum version checkpoints. This function is only callable by the registry owner. In practice, this is the Tempo version the network is being asked to confirm support for.
