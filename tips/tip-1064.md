...
The `dex_storage_credits` mapping is StablecoinDEX precompile state. Creating or updating a user's `dex_storage_credits` counter, including the first creation of that counter's backing storage slot, is subject to normal TIP-1000 state gas and TIP-1060 rules. The StablecoinDEX MUST consume a TIP-1060 storage token to change the `dex_storage_credits[U]` array from `0` to `1`. Inversely it will obtain a TIP-1060 storage credit when `dex_storage_credits[U]` returns to its zero value. One of the user `U`'s storage credit is effectively stored in the slot occupied by the `dex_storage_credits[U]` value.
