...
This TIP introduces receiver-side controls while preserving liveness. Instead of reverting when a receiver blocks an inbound, the transfer or mint succeeds at the protocol level and the funds are escrowed. This ensures that senders and integrations are not disrupted, while still giving receivers control over acceptance.
...
If `to` is a TIP-1022 virtual address, TIP-1022 recipient resolution MUST occur before TIP-1028 receiver-side authorization. If virtual-address resolution fails, the operation MUST revert.
...
Reward flows are not subject to escrow. Reward payouts MUST still respect address-level receive controls and MUST revert if the recipient would reject the inbound.
...
Below is a simplified sequence diagram showing the TIP-20 inbound flow, including escrow of blocked inbounds and how funds are claimed.
...
sequenceDiagram
    ...
    Sender->>TIP20: transfer / mint to Receiver
    TIP20->>TIP20: resolve receiver and evaluate inbound rules
    TIP20->>Escrow: record blocked inbound receipt
    TIP20->>TIP20: credit ESCROW_ADDRESS
    TIP20-->>Sender: success

    Receiver->>Escrow: claim blocked receipt
