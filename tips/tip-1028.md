...
- MUST still require the receiver to be currently authorized under the token's TIP-403 policy for the original inbound kind: the transfer-recipient role for blocked transfers and the mint-recipient role for blocked mints.

A blocked mint, for example, has already passed the token's original TIP-403 mint-recipient policy check on the inbound path, but the issuer's current TIP-403 mint-recipient policy for the receiver still applies before release. A nonzero recovery authority may use this resume path because the receiver selected that authority.
...
- require the resolved destination to be authorized under the token's TIP-403 policy for the original inbound kind: the transfer-recipient role for blocked transfers and the mint-recipient role for blocked mints; and
