...
`TransferBlocked`, `ReceiptClaimed`, and `ReceiptBurned` MUST emit the receipt's encoded `recoveryAuthority` field. For originator recovery, this field is `address(0)`; the actual authorized caller is `originator`.
