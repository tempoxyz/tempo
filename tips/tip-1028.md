...
3. `recoveryContract` - an optional address authorized to claim blocked receipts on behalf of the receiver, one receipt at a time; `address(0)` means the receiver claims directly
...
The precompile does not enumerate receiver-owned receipts onchain. Claimers MUST supply the blocked receipt they want to consume, typically using logs or offchain indexing. The protocol claim interface is intentionally single-receipt; recovery contracts or callers that want batching MAY loop or use multicall outside the protocol surface.
...
- consumes only the explicitly supplied receipt bucket
- releases only to `to`
...
- MUST interpret `receipt` as the tuple `(token, receiver, receipt.originator, receipt.requestedRecipient, recoveryContract, receipt.blockedReason, receipt.kind, receipt.memo, receipt.blockedAt, receipt.blockedNonce)`
- MUST require `blockedReceiptAmount[receiptKey] > 0`
- MUST consume the entire stored amount for that receipt or revert; partial claims are not allowed
...
- the recovery contract is the only direct caller of `claimBlockedReceipt(...)` for receipts keyed to that contract
