...
Base token policy checks are unchanged and continue to revert on failure. TIP-1028 policies only apply to TIP-20 precompile flows. Ordinary contracts and other precompiles are unaffected.
...
- TIP-20 transfer and mint flows are updated to check receive policies before crediting a receiver. The existing TIP-20 checks (pause, balance, allowance) and base token policy checks still run first and continue to revert on failure. Only a receive policy failure redirects delivery to `ReceivePolicyGuard`, where a proof is recorded.
...
sequenceDiagram
    ...
    TIP20->>TIP403: check token policy(sender, receiver)
...
The release path inside the TIP-20 token debits `balances[RECEIVE_POLICY_GUARD_ADDRESS]`, credits the destination, emits `Transfer(RECEIVE_POLICY_GUARD_ADDRESS, to, amount)` followed by `ProofClaimed(...)`, and treats `ReceivePolicyGuard` as a reward-exempt always-opted-out source. `RECEIVE_POLICY_GUARD_ADDRESS` is not the real token-policy subject for a claim; the claim-specific checks below apply the token's base policy to the receiver, originator, or destination as appropriate. Releasing a previously blocked mint does not emit a fresh `Mint` event because the claim is a transfer out of `ReceivePolicyGuard`, not a new mint.
...
Claim authority and token-policy status are separate. The authorized claimer is determined by `recoveryAuthority`. The policy subject for claim-time sender authorization and proof burns is `proof.originator` for originator recovery, and `receiver` for receiver recovery or third-party recovery. A third-party recovery authority acts as the receiver's delegate for this purpose; its own token-policy status does not control whether the proof can be rerouted or burned.
