...
Each blocked inbound gets its own fine-grained escrow bucket. The escrow precompile stores only one keyed amount for each open blocked receipt onchain. The rest of the receipt identity — the literal requested recipient, block reason, transfer-vs-mint kind, memo, blocked timestamp, and blocked nonce — is authenticated by the receipt witness and emitted in the blocked event. This preserves TIP-1022 attribution and memo-bearing semantics without requiring the escrow precompile to store a large receipt struct in persistent state.
