...
A claim consumes one full proof and releases the funds to one destination, partial claims are not supported. 
...
Once consumed, a proof is permanently retired: its `proofKey` slot is empty and any later claim against the same witness MUST revert with `InvalidProof()`. 
...
For transfer-like proofs, `originator` is `from`, including for `transferFrom` where `from` may differ from `msg.sender`. For mint proofs, `originator` is the mint caller. 
...
This path exists because `burnBlocked(RECEIVE_POLICY_GUARD_ADDRESS, amount)` cannot safely burn the aggregate guard balance. The caller MUST hold `BURN_BLOCKED_ROLE` for `proof.token`. A proof is burnable only when its policy subject, as defined above, is currently unauthorized as a sender under the token's TIP-403 policy.
