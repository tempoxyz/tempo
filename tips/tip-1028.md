...
TIP-1028 preserves liveness by escrowing receiver-blocked inbounds instead of reverting them. Senders and integrations keep a successful delivery path, while receivers keep control over acceptance.

Blocked inbounds must remain recoverable and attributable, allowing the escrow model to preserve per-transfer context such as originator, requested recipient, and metadata, enabling flexible recovery and offchain handling.
...
If `to` is a TIP-1022 virtual address, TIP-1022 recipient resolution MUST occur before TIP-1028 receiver-side authorization. If virtual-address resolution fails, the operation MUST revert. If TIP-1028 activates before TIP-1022 recipient resolution, these virtual-address rules are inactive until TIP-1022 is active.

TIP-1028 does **not** alter `approve`, `permit`, `burn`, fee accounting via `transfer_fee_pre_tx` or `transfer_fee_post_tx`, or system-credit paths that do not identify a concrete originator.

The TIP-403 registry precompile owns token sets and address-level receive configuration. A dedicated escrow precompile owns blocked-receipt storage and claim authorization. TIP-20 precompiles call the registry to verify receiver-side authorization and call escrow only when a receiver-side check fails.

High-level flow:

1. If `to` is a TIP-1022 virtual address, resolve it first to `effectiveReceiver`; otherwise `effectiveReceiver = to`.
2. Run existing TIP-20 sender/issuer checks, including TIP-403 and TIP-1015, using `effectiveReceiver` where applicable.
...
The next two sections define the registry state used in step 3. Section 4 defines the escrow precompile used in step 5.

```mermaid
sequenceDiagram
    actor Originator
    participant TIP20 as TIP-20 precompile
    participant Registry as TIP-403 registry
    participant Escrow as Escrow precompile
    actor Receiver

    Originator->>TIP20: transfer or mint to Receiver
    TIP20->>TIP20: run existing token-level checks
    TIP20->>Registry: verifyAddressInbound(token, originator, receiver)
    alt authorized
        TIP20->>Receiver: credit funds
    else blocked by receiver
        TIP20->>TIP20: credit ESCROW_ADDRESS
        TIP20->>Escrow: recordBlockedInbound(...)
    end
    Receiver->>Escrow: claim receipt
    Escrow->>TIP20: release from ESCROW_ADDRESS
    TIP20->>Receiver: credit claimed funds
```

## 2. Token Sets

Token sets are a TIP-403 registry primitive for TIP-20 token addresses. They answer a different question from address policies:

- address policy: "is this originator authorized?"
- token set: "is this token authorized?"

Token sets use a separate ID space from policy IDs. They are not aliases for ordinary TIP-403 policy lists and do not reuse the compound-policy surface, but they mirror the ordinary TIP-403 list ergonomics, including create-with-members and batched membership updates. The TIP-403 registry precompile implements `ITIP403TokenSets`.
...
The TIP-403 registry precompile stores address-level receive controls and exposes `setAddressReceivePolicy(...)`. Any address MAY configure:

- `receivePolicyId`: which originators may credit the address;
- `tokenSetId`: which TIP-20 token addresses may credit the address; and
- `recoveryContract`: an optional claimer for blocked receipts, with `address(0)` meaning the receiver claims directly.
...
`recoveryContract` MAY be `address(0)`. If nonzero, it designates the sole direct claimer for future blocked receipts for this receiver and MUST be able to call the escrow precompile. It MUST NOT equal `ESCROW_ADDRESS`, be a TIP-1022 virtual address, or be a system precompile address that cannot initiate calls.
...
The receipt amount MUST be deleted before the escrow-release path credits `to`.
...
- enforce token-level sender authorization for `receiver`
- enforce token-level transfer-recipient authorization for `to`
- enforce `to`'s address-level receive controls against `receiver`
- revert if that receipt fails the destination's address-level checks
- if `recoveryContract == address(0)` and the reroute is initiated through an access key, require AccountKeychain to treat the claim as an ordinary TIP-20 spend by `receiver`, including spending-limit metering and recipient scoping for `to`; otherwise reject the reroute
...
The protocol does not mandate any particular recovery-contract design. The standard pattern is a receiver-owned instance, proxy, or smart-wallet module set as `recoveryContract`. That contract stores one canonical `receiver`, uses `claimToReceiver(...)` as the default unwind path, and may permit reroutes to `to != receiver`.

If originator self-claim is supported, the recovery contract should restrict it to receipts whose supplied `originator` equals the caller and only to the caller itself. If the receiver rotates to a new recovery contract, the old contract SHOULD remain callable until receipts keyed to it are drained.
...
flowchart TD
    ...
    Record -->  AttrEvent[["emit TransferBlocked event<br/>with digest information"]]:::event
...
Reward flows are never escrowed, but they MUST respect recipient consent. `distributeReward` MUST remain token-internal accounting and MUST NOT route through receiver-side escrow logic.

For `setRewardRecipient(holder, recipient)`, `recipient == address(0)` remains the opt-out path and bypasses receive-policy checks. `recipient == ESCROW_ADDRESS` and TIP-1022 virtual recipients MUST be rejected. Any other recipient MUST pass `verifyAddressInbound(token, holder, recipient)`.

For `claimRewards(...)`, the token MUST determine the actual payout recipient under its existing reward rules. `ESCROW_ADDRESS`, TIP-1022 virtual payout recipients, and recipients that fail `verifyAddressInbound(token, address(token), recipient)` MUST be rejected rather than escrowed.
...
TIP-20 delivery helpers MUST return whether the inbound credited the intended receiver or escrowed it. Outer wrappers for memo-bearing transfers, virtual-address forwarding, mints, and protocol payouts MUST use that outcome to avoid emitting direct-credit events when the balance moved to `ESCROW_ADDRESS`.
...
- **Fee deposits and refunds are exempt.** `transfer_fee_pre_tx` and `transfer_fee_post_tx` are fee-accounting operations, not new third-party inbounds. They MUST bypass address-level receive controls and MUST NOT be escrowed.
...
- **Receipt balances are not holder balances.** `burnBlocked`-like paths MUST NOT debit `ESCROW_ADDRESS` or consume blocked receipts unless they are explicitly receipt-aware.
...
- **Claim-to-receiver is an unwind, while reroutes are new transfers.** Claim-to-receiver bypasses the receiver's token-level and address-level receive checks. A reroute to `to != receiver` must satisfy token-level sender authorization for `receiver`, token-level recipient authorization for `to`, the destination's address-level receive controls against `receiver`, and, for direct receiver reroutes through an access key, ordinary AccountKeychain spending-limit and recipient-scoping checks.
...
9. A rerouted claim to `to != receiver` MUST enforce token-level sender authorization for `receiver` and MUST revert with `ClaimSenderUnauthorized()` if it fails.

10. A rerouted claim to `to != receiver` MUST enforce token-level recipient authorization for `to` and MUST revert with `ClaimDestinationUnauthorized()` if it fails.

11. A rerouted claim to `to != receiver` MUST enforce the destination's address-level receive controls against `receiver`.

12. If `recoveryContract == address(0)`, `to != receiver`, and the claim is initiated through an access key, AccountKeychain MUST treat the claim as an ordinary TIP-20 spend by the receiver, including spending-limit metering and recipient scoping for `to`; otherwise the reroute MUST be rejected.

13. Fee deposits and refunds via `transfer_fee_pre_tx` and `transfer_fee_post_tx` MUST bypass address-level receive controls and MUST NOT be escrowed.

14. `ESCROW_ADDRESS` MUST be treated as a protected system address by system-balance-sensitive TIP-20 logic, including any escrow-release and `burnBlocked`-like paths.

15. Changing `addressRecoveryContract[receiver]` MUST affect only future blocked receipts. Existing receipt buckets remain governed by the recovery contract captured in their key.

16. Escrow-related raw TIP-20 events MUST be truthful:
   - blocked transfer: `Transfer(from, ESCROW_ADDRESS, amount)`
   - blocked mint: `Transfer(address(0), ESCROW_ADDRESS, amount)` and `Mint(ESCROW_ADDRESS, amount)`
   - claim release: `Transfer(ESCROW_ADDRESS, beneficiary, amount)`

17. Every blocked inbound MUST emit exactly one blocked-receipt attribution event naming the receiver, the requested recipient, the reason, the governing `recoveryContract`, the receipt's version, the receipt's `blockedAt`, and the receipt's blocked nonce. That event's `blockedReason` MUST NOT be `NONE`.

18. Claims MUST consume whole blocked receipts. Once a receipt is claimed, its keyed amount MUST be deleted before the release credits `to`.

19. Reward accounting for blocked transfers, blocked mints, and claim releases MUST treat `ESCROW_ADDRESS` as a reward-exempt always-opted-out synthetic sink/source, preserve the same opted-in-supply effects as a movement into or out of an always-opted-out address, and MUST NOT create, update, or consult per-user reward state for `ESCROW_ADDRESS`.

20. `setRewardRecipient` MUST reject `ESCROW_ADDRESS`, TIP-1022 virtual recipients, and any nonzero recipient whose address-level receive controls would block the holder as originator.

21. `claimRewards` MUST reject `ESCROW_ADDRESS`, TIP-1022 virtual payout recipients, and any payout recipient whose address-level receive controls would block the token contract as originator.

22. `verifyAddressInbound(...)` MUST return `blockedReason == NONE` exactly when `authorized == true`.
