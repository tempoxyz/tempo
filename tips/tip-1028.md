...
When a user's receive policy blocks an inbound TIP-20 transfer, delivery is redirected to `ReceivePolicyGuard`, a proof is recorded, and the amount becomes claimable by the recovery authority designated by the user.
...
The token's TIP-403 policy checks are unchanged and continue to revert on failure. TIP-1028 policies only apply to TIP-20 precompile flows. Ordinary contracts and other precompiles are unaffected.
...
- A new precompile for blocked transfers is introduced at `RECEIVE_POLICY_GUARD_ADDRESS` and records a proof for each blocked transfer or mint, which can be claimed later by the receiver, the originator, or a recovery authority.
...
Token-level checks controlled by the issuer are unchanged and continue to revert on failure.
