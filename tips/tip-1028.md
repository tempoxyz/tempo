...
For claim-time sender authorization and proof burns, the proof controller is the address that can claim the proof under these rules: `proof.originator` for originator recovery, `receiver` for receiver recovery, and `recoveryAuthority` for third-party recovery.
...
All other claims are reroutes. This includes every originator-authorized claim, even if `to == receiver`. A reroute MUST reject `to == RECEIVE_POLICY_GUARD_ADDRESS`. If `to` is a TIP-1022 virtual address, it is resolved to its master address before destination checks and final credit; if resolution fails, the call MUST revert with `InvalidClaimAddress()`. A reroute is treated as a new movement by the proof controller: it enforces token-level sender authorization for the proof controller, token-level transfer-recipient authorization for the resolved destination, and receive policy checks against the resolved destination using the proof controller as the sender. If any of these checks fails, the claim MUST revert. The `ProofClaimed` event preserves the literal `to` supplied by the caller for attribution. If a direct receiver-authorized reroute is initiated through an access key, the claim MUST meter the claimed amount against the receiver's AccountKeychain spending limit as an ordinary TIP-20 spend by the receiver. If an originator-authorized reroute is initiated through an access key, it MUST meter against the originator. If the receiver uses a third-party recovery authority, any equivalent delegation, timelock, multisig, or key-policy enforcement is the responsibility of that authority.

`burnBlockedProof(...)` consumes one full proof and burns the stored amount from `proof.token` at `RECEIVE_POLICY_GUARD_ADDRESS`. It exists because `burnBlocked(RECEIVE_POLICY_GUARD_ADDRESS, amount)` cannot safely burn the aggregate guard balance. The caller MUST hold `BURN_BLOCKED_ROLE` for `proof.token`. A proof is burnable only when its proof controller is currently unauthorized as a sender under the token's transfer policy. A later block of `proof.originator` alone MUST NOT make a receiver- or third-party-controlled proof burnable, and MUST NOT prevent that controller from resuming the original inbound.
