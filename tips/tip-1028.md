...
The receiver or a designated recovery contract can later claim these funds. Claims back to the receiver resume the original transfer while claims to other addresses are treated as new transfers.
...
The following tables enumerate the events emitted for each TIP-1028 path, in order. `virtual` is a TIP-1022 virtual address and `master` is its resolved master. Mint to a virtual address mirrors the virtual transfer row, with `from = address(0)` and an additional `Mint` event paired with each `Transfer`.

| Scenario | Allowed | Blocked |
|----------|---------|---------|
| Transfer | `Transfer(from, to, amount)` | `Transfer(from, ESCROW_ADDRESS, amount)` <br> `TransferBlocked(...)` |
| Virtual transfer | `Transfer(from, virtual, amount)` <br> `Transfer(virtual, master, amount)` | `Transfer(from, ESCROW_ADDRESS, amount)` <br> `TransferBlocked(...)` (receipt keyed to `master`, `recipient = virtual`) |
| Mint | `Transfer(address(0), to, amount)` <br> `Mint(to, amount)` | `Transfer(address(0), ESCROW_ADDRESS, amount)` <br> `Mint(ESCROW_ADDRESS, amount)` <br> `MintBlocked(...)` |
...
`recoveryContract` is optional. If not set, only the receiver can claim blocked funds directly.
...
The persistent escrow key for a blocked transfer or mint is versioned. `receiptVersion` is an outer discriminator, and the version-specific receipt body is encoded separately. For version `1`, the receipt body is:
...
The version `1` persistent escrow key is:
...
Regardless of what was originally blocked or where the funds are sent, every claim emits the same two events in order:

```text
Transfer(ESCROW_ADDRESS, dest, amount)
BlockedReceiptClaimed(...)
```

`dest` is `receiver` on resume (`to == receiver`) and the supplied `to` on reroute (`to != receiver`), where `receiver` is the canonical receiver captured in the receipt. Note that for previously blocked virtual transfers, `receiver` is the resolved `master`, so resume releases directly to `master` without a forwarding leg; the original virtual target is preserved in the `BlockedReceiptClaimed.recipient` field for attribution.

Releasing a previously blocked mint does not emit a fresh `Mint` event — the claim is a transfer out of escrow, not a new mint.
...
- Unlike token-level (TIP-403 and TIP-1015) policies, address-level inbound policies CANNOT make transactions revert.
- Only the allowed claimer —address owner (when `recoveryContract = address(0)`) or `recoveryContract`— can claim the funds from the ESCROW precompile.
