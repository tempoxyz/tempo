...
Funding must finish before the transaction can proceed. Delegated funding also depends on the owner trusting the selected policy and its funding sources.
...
The owner chooses whom to trust; the protocol limits what approved funding sources can spend. The responsibilities below define that boundary.
...
`requireFunds` states which token balances the account needs before its application calls run. Existing funds count, so sources only fill what is missing.
...
Funding requirements are signed transaction data. Transactions without funding keep their existing encoding.
...
An owner gives an access key funding permission by attaching a policy. That permission does not authorize the later payment or increase the key's spending limits.
...
An owner can reuse a policy or create one while installing a key. The installed key keeps only the policy ID.
...
A funding policy is a shared set of rules for where funds may come from and how much conversion loss is allowed. Multiple keys can use the same policy.

**Rules**

`FundingPolicy` assigns global policy IDs. A policy contains admins, aggregate slippage, and a `sources` map from output token to ordered funding source rules. Access key funding selects `policy.sources[requireFunds.token]`. Multiple accounts and keys may reference one policy. Funding sources may further restrict supported outputs.
...
`FundingPolicy` is the built-in contract for creating, updating, and reading shared policies. It also helps find funding options allowed by a policy.
...
Anyone can create a policy; its admins control later changes. An owner who selects that policy also trusts those future changes.

**Rules**

Anyone may create a policy, but creation grants no account authority. IDs are global `uint64` values, never reused; zero is reserved. `policyIdCounter` starts at 1 and increments on creation; overflow reverts. `policyExists` returns false for unknown IDs; other ID-based methods revert with `PolicyNotFound`.

Admins MUST be nonempty, unique, and nonzero. Output tokens MUST be unique and nonzero; source addresses MUST be nonzero and unique per token, but may recur across tokens with different rules. Invalid admins, tokens, slippage, or sources revert with `InvalidPolicy`. An empty map allows no requirements; an empty source list for a token allows balance checks only.
...
The owner's signature covers the complete `fundingPolicy` value. Referenced IDs must exist; inline policies are created atomically with key installation. Selecting a policy trusts future admin changes, which affect all referencing accounts and keys without changing key validity, limits, consumed budgets, or funding credit.

Snapshot the policy at access key funding start for the whole operation. Before any balance check, including zero requirements, reject missing tokens with `TokenNotAllowed` and validate source membership and nondecreasing policy position. Sources may be omitted or repeated; repeats execute in transaction order. Token-map insertion order has no effect.
...
Policy discovery lists approved ways to fill a missing balance. It estimates options without spending funds or promising that they can all be used together.

**Rules**

Anyone may call `FundingPolicy.discover`. It returns the requested token and target amount, policy slippage, and ordered candidates without granting authority or checking access key limits. Resolve the policy and token before checking balances; missing entries revert with `PolicyNotFound` or `TokenNotAllowed`. A sufficient balance returns no sources.
...
Preserve policy source order and candidate order. Return each source's address, `requestData` as `data`, and `availableAmount`. Estimates are independent: do not sum them, reduce later budgets, or stop once estimates cover the shortfall. Shared inputs, liquidity, slippage, and key authorization require sequential simulation and execution.
...
`TIP20Funder` is Tempo's built-in funding component. Before application calls run, it checks the account's balance and asks funding sources to supply any missing tokens within the authorized limits. Funding and application calls succeed or revert together.
...
The transaction handler runs funding; contracts cannot start it themselves.
...
The protocol checks a funding request before letting its source execute it. Only the execution step can spend funds.
...
`supportsToken` answers whether a source has an allowed path to the requested token. It does not check whether funds or liquidity are available.
...
Source discovery lists concrete funding options, such as swapping a token or redeeming a vault. Each option includes the request data needed to quote and execute it.
...
A quote estimates how much one funding request can deliver and sets its input bounds. Execution obtains a fresh quote before allowing the source to spend.
...
Policy data says what is allowed; request data says what to do this time. Each funding source checks that the request stays within its approved rules.
...
Input valuation expresses consumed tokens or shares in the requested token's units, so all sources can share one cost limit. It includes fees and conversion losses.
...
A source can supply tokens without taking inputs from the account, for example by releasing a funded claim. That grants no permission to debit the account and does not imply the funding is free.
...
A source gets temporary permission to spend only its quoted input token and amount. The permission ends with that source call and leaves no allowance behind.
...
The funder tries sources in order until the required balance is available. If they cannot supply enough, the transaction reverts.
...
The native DEX source swaps an approved token for the token the account needs.
...
The Earn source redeems vault shares and, when needed, swaps the proceeds into the requested token.
...
Conversions between differently valued tokens, such as USD and EUR tokens, need a separate reference valuation. They are future work.
...
Owners can authorize funding directly by signing the transaction, without selecting a shared policy. Input limits and token protections still apply.
...
Charge the access key once for the missing output tokens. Temporary credit prevents charging it again when those funded tokens are spent in the same transaction.
...
Slippage limits total conversion cost across all sources for one requirement. For a 50 USDC shortfall and 1% tolerance, the normalized input cost may be at most 50.50 USDC.
...
Wallets and relays can construct funding requests before signing. The protocol still validates those requests when the transaction executes.
...
A relay helps turn a payment request into a complete unsigned transaction, including where its funds should come from.
...
Consumers can supply exact funding requests, just the required balances, or ask the relay to infer them from application calls.
...
Events show policy changes and the funds actually delivered. A funding event alone does not prove that a later payment completed.
...
These guarantees must hold for every funding operation, regardless of the source or caller.
