...
Funding policies constrain access key funding and guide route discovery. Owner-authorized transactions may discover using supplied rules without a stored policy, or use a policy without binding execution to it. Discovery grants no spending authority.
...
Before signing, consumers call `FundingDiscovery.discover` with rules to find candidates for a target balance. The rules-only overload supports owner-authorized transactions without a stored policy. The overload with `policyId` also verifies the rules against that policy’s commitment. Both apply source rules and slippage limits without moving funds or granting authority.
...
sequenceDiagram
    ...
    C->>P: Discover with rules and optional policy ID
    opt Policy ID supplied
        P->>R: Read policy commitment
        R-->>P: Rules hash
        P->>P: Verify rules commitment
    end
    P->>P: Validate supplied rules and calculate shortfall
    opt Funds are needed
        loop Sources in supplied rule order
...
interface IFundingDiscovery {
    ...
    function discover(address account, address token, uint256 amount, bytes calldata policyRules)
        external
        view
...
Anyone may call either overload of `discover`; neither authenticates the caller as the account owner or checks access key limits. Both return the token, target amount, supplied rules’ `maxSlippageBps` as `slippageBps`, and ordered candidates without granting authority.

Both overloads require canonical `policyRules = abi.encode(rules)` using `IFundingPolicy.Rules`. The rules-only overload does not read a stored policy or verify a commitment. The overload with `policyId` MUST reject missing policies or rules that do not exactly match the stored commitment, including when the balance already suffices.

1. Both overloads MUST reject invalid or noncanonical rules with `InvalidPolicyData`, `maxSlippageBps > 10_000` with `InvalidSlippage`, and absent tokens with `TokenNotAllowed` before checking balances.
2. Return no sources if the balance suffices; otherwise calculate the shortfall and cost budget.
3. Call every entry’s `discover` in the requested token’s source list via `STATICCALL`, including repeated targets, using identical bounds and that entry’s data. Propagate failures; require nonempty request data and `0 < availableAmount <= shortfall`. Do not re-quote candidates.
...
Discover candidates for an owner-authorized transaction using rules without a stored policy.
...
To verify the rules against a stored funding policy, include its ID.
