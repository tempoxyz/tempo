...
The input token's hook determines the output available within the account's spendable balance, DEX liquidity, and `maxInput`, capped at `amount`. It swaps for that contribution, or returns without debiting if none is available. The native DEX debits the authenticated account within its access key permission and input limits and delivers the requested token to that account.
