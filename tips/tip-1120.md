...
The SDK encodes the map as `Route[] sources` for the registry ABI and inline key authorization. Entries MUST be sorted by ascending token address bytes, with no duplicate addresses, including differently cased spellings. Each source list retains its order. SDK registry reads convert entries back to the map.
...
Validate every supplied funding source's membership and nondecreasing position within the requested token's list before checking the existing balance. Funding sources MAY be omitted or repeated; repeated calls execute in transaction order. An empty list permits balance checks for that token but no hooks.

Policy data defines approved rules; transaction data defines a particular call. They are not compared for byte equality. Each funding source's quote hook MUST validate its decoded call arguments against the requested token's source policy data. Unsupported inputs, outputs, venues, and attempts to expand policy authority MUST revert.
...
interface IFundingPolicyRegistry {
    ...
    function modifyPolicy(uint64 policyId, uint16 slippageBps, Route[] calldata sources) external;
...
interface IFundingSource {
    ...
    /// @param policyData Requested token's source rules; empty for owner-authorized funding.
...
For access keys, consumers look up the requested token in `policy.sources` and pass the selected source's rules as `policyData` with `ownerAuthorized = false`. Without a policy, consumers pass their selected source data, empty `policyData`, and `ownerAuthorized = true`. These caller-supplied quote arguments grant no execution authority.
    ...
    internal returns (uint256 fundedAmount)
{
    ...
    if (ctx.isAccessKey) {
        ctx.sourceRules = requireTokenSources(ctx.policy.sources, token);
