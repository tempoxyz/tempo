...
The temporary permission replaces the allowance requirement for supported funding debits. Token pause and transfer policy checks still apply. Permission MUST end when `fund` returns or reverts, and debit accounting MUST roll back with reverted asset movements. No standing allowance is created.
...
check token pause and transfer policies
