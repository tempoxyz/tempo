...
type RequireFunds = {
  ...
  sources: {
     ...
     * Required for root-key execution; must be less than uint256 maximum.
     * For access keys, omission adds no caller cap; protocol limits still apply.
     */
    maxInput?: bigint
...
`Source.maxInput` caps input consumed per source call. For access keys, omission encodes as `type(uint256).max`; root execution rejects that sentinel. Zero permits no debit. `TIP20Funder` passes the smaller of this cap and the limit allowed by the remaining shared budget and any access-key restrictions to `fund`.
...
function requireFunds(
    ...
    // ... for access keys, validate factory membership and order across the entire source list.
    // ... for root execution, reject omitted or unbounded source input caps.
...
1. Validate the asset, account, and key. Check factory membership/order for access keys and explicit input caps for root execution. Reject nested, delegated, or reentrant calls and access keys without `requireFunds`.
...
A root-signed transaction authorizes its factories, targets, input caps, and data directly through the signed `requireFunds` array or explicit precompile call. No prior factory allowlist or token allowance is required for integrated funding paths. Factories still validate targets, and the protocol enforces input caps, total slippage, and delivery.

Each supplied source MUST specify `maxInput` below `type(uint256).max`; omission or the unbounded sentinel MUST revert before checking the existing balance. The cap uses the input token's or vault share's base units. Root signatures authorize the selected factory and its supported conversion rules for that invocation only.

```typescript
// Source entry in a root-signed funding requirement.
{
...
9. **Single budget charge.** For access-key execution, the initial shortfall MUST be charged once before source input access. Individual contributions MUST NOT add another output-budget charge. Credit MUST cover only verified delivery and MUST NOT be consumed more than once. Unspent funded tokens MUST remain charged.
...
17. **Root authorization.** Root funding MUST bind temporary debit authority to the signed factory, target, and bounded input cap. It MUST reject omitted or unbounded caps, preserve token policies, and leave no standing authority. Root funding MUST NOT charge access-key budgets or create access-key funding credit.
...
Root cases include funding without token allowances, missing or unbounded input caps, attempted debits outside the signed target, and reuse of temporary input access after the funding call.
