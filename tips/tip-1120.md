...
const policyId = await fundingPolicy.createPolicy({
  ...
  rules: {
    ...
    sources: {
      [EURC]: [
        { data: encode({ tokenIn: OTHER_EUR_TOKEN }), target: DEX_SOURCE },
        // Future non-parity support; the pool determines the input token.
        { data: encode({ pool: OUSD_EURC_POOL }), target: PROPAMM_SOURCE },
      ],
      [OUSD]: [
        { data: encode({ tokenIn: USDC_E }), target: DEX_SOURCE },
        { data: encode({ tokenIn: USDC }), target: DEX_SOURCE },
...
function verify(bytes calldata requestData, bytes calldata policyData) external pure returns (bool) {
    DexRequest memory request = decodeDexCall(requestData);
    DexPolicy memory policy = decodeDexPolicy(policyData);
