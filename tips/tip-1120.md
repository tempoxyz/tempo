...
```typescript
{
  ...
  requireFunds: [{
    ...
    sources: [
      { data: encode({ maxAmountIn: 30_000_000n, tokenIn: USDC_E }), target: NATIVE_DEX_SOURCE },
      { data: encode({ tokenIn: OUSD }), target: NATIVE_DEX_SOURCE },
    ],
  }],
}
```
...
Funding policies constrain access key funding and guide route discovery. Owner-authorized transactions may use them for discovery without binding execution to them. Discovery grants no spending authority.
...
Within a funding policy, `Source.data` defines approved rules. In `requireFunds`, source `data` describes a concrete funding request.
...
const policyId = await fundingPolicy.createPolicy({
  ...
  rules: {
    ...
    sources: {
      [EURC]: [
        { data: encode({ tokenIn: OTHER_EUR_TOKEN }), target: NATIVE_DEX_SOURCE },
        // Future non-parity support; the pool determines the input token.
        { data: encode({ pool: OUSD_EURC_POOL }), target: PROPAMM_SOURCE },
      ],
      [OUSD]: [
        { data: encode({ tokenIn: USDC_E }), target: NATIVE_DEX_SOURCE },
        { data: encode({ tokenIn: USDC }), target: NATIVE_DEX_SOURCE },
        { data: encode({ vault: MY_VAULT }), target: EARN_SOURCE },
      ],
    },
  },
})
...
Consumers retrieve rules from creation or update events, an indexer, or another offchain source. Supplied rules must match the funding policy’s current `rulesHash`.
...
2. Select the first entry at or after that position whose target matches and whose `verify(request.data, entry.data)` returns true. Otherwise, revert with `FundingNotAuthorized`.
...
Immediately before funding, `TIP20Funder` calls `quote` with the authenticated account and funding policy rules, then passes `quote.requestData` unchanged to `fund`.
...
`supportsToken` reports whether `policyData` permits a supported path to the requested output token, regardless of balances or liquidity. No supported path returns false; malformed configurations MUST revert.
...
`discover` returns ordered candidates with reusable request data and independent output estimates, limited to paths permitted by `policyData`.

- Each candidate MUST include nonempty `requestData` and `availableAmount` in output-token base units.
...
`Quote.requestData` MUST be nonempty and accepted by both `quote` and `fund`. Consumers copy it into `requireFunds.sources[].data`. Re-quoting MUST preserve the selected input, path, and proofs, and preserve or tighten caller restrictions.

#### Quote authorization

The protocol derives quote authorization from authenticated context:

| Execution | `policyData` | `ownerAuthorized` |
| --- | --- | --- |
...
- Before funding, re-quote each request against the remaining shortfall and cost budget, validate its bounds, and pass its `requestData` to `fund`.
...
`quote.maxAmountIn` MUST be the minimum of the caller cap, funding policy caps, and input capacity allowed by `maxCost`. `quote.requestData` carries this bound and validated execution arguments.
    ...
    internal returns (uint256 fundedAmount)
{
    ...
    bytes[] memory matchedRules = new bytes[](sources.length);
    if (ctx.isAccessKey) {
        ...
        for (uint256 i; i < sources.length; ++i) {
            ...
            for (uint256 j = position; j < entries.length; ++j) {
                ...
                if (!IFundingSource(request.target).verify(request.data, entries[j].data)) continue;
                matchedRules[i] = entries[j].data;
                position = j;
                matched = true;
                break;
            }
    ...
    for (uint256 i; i < sources.length && balance < amount; ++i) {
        Source calldata request = sources[i];
        bytes memory policyData = matchedRules[i];
...
Before signing, consumers call `FundingDiscovery.discover` with a funding policy and its rules to find candidates for a target balance. Discovery applies source rules and slippage limits without moving funds or granting spending authority.
...
`FundingDiscovery` is a protocol-deployed Solidity contract at `0x1120000000000000000000000000000000000003`, executed by the ordinary EVM. It reads commitments from the separate Funding Policy precompile.
...
The contract reads Funding Policy at `0x1120000000000000000000000000000000000002`. Source discovery uses ordinary EVM static calls with Funding Discovery as `msg.sender`, including normal gas and rollback rules.

#### Discovery behavior

Anyone may call `discover`. It returns the token, target amount, funding policy’s `maxSlippageBps` as `slippageBps`, and ordered candidates. It grants no authority and does not check access key limits.

1. Verify `policyRules` against the funding policy commitment. Reject missing policies, invalid rules, or absent tokens before checking balances.
2. Return no sources if the balance suffices; otherwise calculate the shortfall and cost budget.
3. Call every funding policy entry’s `discover` via `STATICCALL`, including repeated targets, using identical bounds and that entry’s data. Propagate failures; require nonempty request data and `0 < availableAmount <= shortfall`. Do not re-quote candidates.

#### Discovery results

- Preserve funding policy entry and candidate order; do not deduplicate targets. Each request MUST pass its entry’s `verify`, or revert with `InvalidCandidate`.
- Return `target`, `requestData` as `data`, and `availableAmount`. Estimates are independent: do not sum them, reduce later budgets, or stop when estimates cover the shortfall.
...
Discover candidates for a target balance of 50 USDC using a funding policy.
...
Construct each requirement with the requested token, target amount, returned slippage, and selected candidates. Discovery reserves no funds and guarantees no delivery. Access key execution still validates the funding policy, source order, spending limits, and fresh quotes.
...
Funding policy data approves one `tokenIn` and optional `maxAmountIn`. Requests select that input within the cap. The source validates parity support and swaps with the authenticated account as both input owner and output recipient.

```solidity
function verify(bytes calldata requestData, bytes calldata policyData) external pure returns (bool) {
    NativeDexRequest memory request = decodeNativeDexCall(requestData);
    NativeDexPolicy memory policy = decodeNativeDexPolicy(policyData);
    return request.tokenIn == policy.tokenIn && request.maxAmountIn <= policy.maxAmountIn;
}
...
function discover(
    ...
    NativeDexPolicy memory policy = decodeNativeDexPolicy(policyData);
    if (policy.tokenIn == assetOut || TIP20(policy.tokenIn).balanceOf(account) == 0) return new Candidate[](0);
    Quote memory result = this.quote(
        account, assetOut, amountOut, maxCost,
        abi.encode(policy.tokenIn, policy.maxAmountIn), policyData, false
    );
    if (result.amountOut == 0) return new Candidate[](0);
    candidates = new Candidate[](1);
    candidates[0] = Candidate(result.requestData, result.amountOut);
}
...
function quote(
    ...
    NativeDexRequest memory request = decodeNativeDexCall(requestData);
    if (!ownerAuthorized) {
        require(this.verify(requestData, policyData));
    }
...
function fund(address account, address assetOut, uint256 amountOut, bytes calldata requestData) external {
    ...
    (address tokenIn, uint256 cap) = abi.decode(requestData, (address, uint256));
...
The Earn source redeems an approved vault’s shares, optionally swapping into the requested token. Funding policy data specifies one `vault` and optional share-input cap. Requests MUST respect that cap; quoting authenticates the vault and supported engine configuration.
...
function verify(bytes calldata requestData, bytes calldata policyData) external pure returns (bool) {
    ...
    EarnPolicy memory policy = decodeEarnPolicy(policyData);
    return request.vault == policy.vault && request.maxAmountIn <= policy.maxAmountIn;
}
...
function discover(
    ...
    EarnPolicy memory policy = decodeEarnPolicy(policyData);
    if (TIP20(earnShare(policy.vault)).balanceOf(account) == 0) return new Candidate[](0);
    Quote memory result = this.quote(
        account, assetOut, amountOut, maxCost,
        abi.encode(policy.vault, policy.maxAmountIn), policyData, false
    );
    if (result.amountOut == 0) return new Candidate[](0);
    candidates = new Candidate[](1);
    candidates[0] = Candidate(result.requestData, result.amountOut);
...
A root-signed transaction authorizes its selected sources and request data without a funding policy. The protocol derives `ownerAuthorized = true` from authenticated context and supplies empty `policyData` to `quote`.
