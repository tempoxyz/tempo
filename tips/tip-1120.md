  ...
  requireFunds: [
    {
      ...
      slippageBps: 100,
      sources: [
        { factory: TIP20_FACTORY, source: USDC.e, maxInput: 30 },
        { factory: TIP20_FACTORY, source: OUSD },
      ],
    },
  ],
  calls: [
...
interface TIP20Funder {
    struct Source {
        ...
        address source;   // Token or vault authenticated by the factory.
...
interface ITIP20FunderSource {
    ...
    /// @param source Token or vault whose provenance and configuration the factory must validate.
...
function requireFunds(
    ...
    for (uint256 i = 0; i < sources.length && balance < amount; ++i) {
        ...
        ITIP20FunderSource(source.factory).fund(asset, shortfall, source.source, maxInput, source.data);
        ...
        if (received > shortfall) {
            revert UnexpectedFundingAmount(source.source, shortfall, received);
        }
        if (received == 0) {
        ...
        emit SourceFunded(account, asset, source.source, source.factory, inputAsset, inputAmount, received);
...
The TIP-20 factory precompile implements the following `fund`. It authenticates `source` as a supported TIP-20 and uses the native DEX as its default venue:
...
function fund(
    ...
    // ... verify source is a supported TIP-20; validate data and the selected route.
    ...
    dex.swapExactAmountOut(account, source, asset, contribution, maxInput);
}
```
...
function fund(
    ...
    // ... authenticate TIP20Funder and validate source against the factory registry and supported vault configuration.
    // ... load account, source vault underlying, and access key permission; validate data.
...
- **Input-token exclusions.** An explicit zero access-key limit could disable a funding input. Its interaction with output-only charging, share tokens, and nonzero input limits still needs definition; this draft does not claim those exclusions are enforced.
- **Witness-bound rules.** Factory data could carry proofs against a TIP-1053 witness. Native funding context still needs a way to authenticate that witness for registered keys; TIP-1053's existing key-info getters do not expose it. Witness checks and replay protection need a separate design.
- **Credit-backed funding.** A source could deliver tokens without an input debit, but borrowing authority, debt limits, and repayment obligations are outside this TIP. A funding permission alone does not authorize debt.
- **Native integration.** Factory source valuation, bounded-debit access, permission encoding, and gas costs still need concrete native interfaces. Funding remains synchronous; arbitrary data does not add cross-chain or asynchronous settlement support.
