...
USDC and PathUSD share native DEX and Earn rules. EURC permits a native DEX conversion from an approved EUR parity token before a future USD-to-EUR propAMM conversion. Matching currency metadata alone does not establish parity. The propAMM conversion requires the reference valuation described under [Non-Parity Tokens](#non-parity-tokens).

The SDK encodes the map as `Route[] sources` for the registry ABI and inline key authorization. Entries MUST be sorted by ascending token address bytes, with no duplicate addresses, including differently cased spellings. Each source list retains its order. Registry reads convert entries back to the map; reused JavaScript arrays encode as independent token entries.
