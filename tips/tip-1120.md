...
Both signatures MUST cover the full array. Funding and calls share gas, rollback, and credit accounting. Omitted or empty arrays skip funding. Only the transaction handler initiates funding; Solidity cannot call `requireFunds`. Existing validity bounds and fees apply. Funding cannot pay transaction fees.
...
Each requirement encodes as `[asset, amount, sources, slippage]`, with each source encoded as `[address, data]`. Slippage is `[]` when omitted or `[slippageBps]` when supplied, preserving explicit zero. Values above 10,000, empty funding extensions, and placeholders without funding MUST be rejected.

Owner-authorized execution activates at T13. Access key funding requires a later activation and MUST remain disabled until its policy and accounting support is complete.
...
interface ITIP20Funder {
    ...
    /// @notice Satisfy the account's required balance. Protocol-only; not callable by Solidity.
...
6. **Prepared authority.** Preparation runs under STATICCALL with no input permission. Only its validated result may establish a bounded funding invocation. Bind the plan and permission to the account, output asset, funding source, and call frame. Nested, delegated, and reentrant requireFunds calls MUST revert.
