...
interface IFundingDiscovery {
    ...
    function discover(uint64 policyId, address account, address token, uint256 amount, bytes calldata rules)
        external
        view
        returns (Discovery memory);
}
