...
`createPolicy`, `modifyPolicy`, and `setAdmins` emit `PolicyCreated`, `PolicyUpdated`, and `PolicyAdminsUpdated` from the FundingPolicy address. State, counter changes, and events follow transaction rollback. Inline creation during key installation uses this same precompile and emits `PolicyCreated` there with the authenticated owner as `updater`.

#### Funding Discovery

Anyone may call `FundingPolicy.discover` to construct a funding requirement from a policy. The result preserves the requested `token` and target `amount`, includes policy `slippageBps`, and returns ordered source requests with quote-only `availableAmount` metadata. Calling this method neither selects a policy for execution nor grants authority.
...
For each approved source, call `discover(account, token, shortfall, costBudget, policyData)` through `STATICCALL`. Pass the same initial shortfall and budget to every source. Require nonempty candidate request data and `0 < availableAmount <= shortfall`. Discovery failures propagate. The precompile does not separately quote the returned candidates.

Each discovery candidate includes the source address, source `Candidate.requestData` as `data`, and its `availableAmount`. Preserve policy source order and each source's candidate order. Estimates are independent and capped by the requested shortfall and budget. Do not sum them, subtract them from later budgets, or stop because their sum covers the shortfall.

The precompile does not execute funding or check an installed access key's remaining limits. Shared inputs, liquidity, aggregate slippage, and key authorization require sequential transaction simulation and execution. Owners may use a registered policy to construct requests, then authorize them directly by signing. Consumers strip `availableAmount` when constructing transaction sources; the discovery result's `target` maps to SDK `address`.
