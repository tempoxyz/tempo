...
Source `discover` returns `Candidate[]` in configured order. Each candidate contains a nonempty concrete `requestData` and an independently estimated `availableAmount` in output-token base units. Inspect only candidates in `config`, using the source's policy schema. DEX discovery expands `inputTokens`; Earn discovery expands `vaults`.

Discovery MUST apply the same validation, valuation, liquidity checks, and input limits as `quote`, treating `config` as the applicable rules. Return the reusable request data and output estimate produced by that logic. Each estimate is capped by `amountOut` and `maxCost`. Omit zero-output candidates; return an empty array when none remain.
...
function discover(
    ...
    address[] memory inputs = decodeNativeDexPolicy(config).inputTokens;
    ...
    for (uint256 i; i < inputs.length; ++i) {
        ...
        Quote memory result = this.quote(
            ...
            abi.encode(inputs[i], type(uint256).max), config, false
        );
...
function discover(
    ...
    address[] memory vaults = decodeEarnPolicy(config).vaults;
    ...
    for (uint256 i; i < vaults.length; ++i) {
        ...
        Quote memory result = this.quote(
            account, assetOut, amountOut, maxCost,
            abi.encode(vaults[i], type(uint256).max), config, false
        );
        if (result.amountOut == 0) continue;
        candidates[count++] = Candidate(result.requestData, result.amountOut);
    }
