...
Introduces a new precompile that lets a consumer specify tokens and amounts they require and automatically satisfy that requirement using source assets (e.g. other TIP-20 tokens, or earn vault shares). Funding is sourced in order from user assets and subsequent actions execute atomically, with access key spending limits enforced on the requested token.
...
- When using access keys, the owner approves source asset factories (e.g. TIP20Factory or EarnFactory - an "asset category"). Each source asset factory validates its underlying assets and conversion rules, including deployment configuration and upgrades. Factory approval covers current and future supported deployments without separate source asset approvals. Offchain preferences supply no spending authority.
...
| Source asset factory | Authenticates deployed source assets and enforces supported routes and valuation rules. Caller-supplied source asset addresses and data cannot expand its funding authority. |
...
The consumer specifies a token, a required balance, and an ordered list of source assets (e.g. token or vault share). The precompile counts existing funds and obtains any shortfall from those source assets within the account's authorization and cost limits. Once funded, the transaction continues with its remaining calls. Funding and subsequent actions execute atomically.
...
Tempo transactions MAY include a signed `requireFunds` array to obtain any missing tokens from the specified source assets before executing `calls`. Entries are processed in order.
...
`TIP20Funder` coordinates funding.
...
interface TIP20Funder {
    ...
    error InvalidInputValuation(address factory);
    ...
    /// @param sources Ordered source assets.
...
## Source Asset Factory

`ITIP20FunderFactory` is implemented by source asset factories, such as `TIP20Factory` and `EarnFactory`. Individual deployed tokens and vaults need not implement `fund`.

```solidity
interface ITIP20FunderFactory {
    /// @notice Resolve the original input asset and its value in the requested token.
    /// @param asset Requested TIP-20 token.
    /// @param target Source asset whose provenance and configuration must be validated.
    /// @return inputAsset Token or share token consumed from the funding account.
    /// @return rate Requested token base units per input base unit, scaled by 1e18 and rounded up.
    /// @dev Uses approved onchain accounting. Reverts for unsupported assets, configurations, or valuation.
    function getInputValuation(address asset, address target)
        external view returns (address inputAsset, uint256 rate);
...
`TIP20Funder` resolves `account` from authenticated transaction context and passes it to `fund` as the input owner and output recipient. Factories MUST accept `fund` calls only from `TIP20Funder`. The account argument grants no spending authority by itself; all input debits remain subject to the active funding call’s bounded input authority.
...
After authorizing the factory and before granting input access, `TIP20Funder` MUST call `getInputValuation(asset, target)` using `STATICCALL`. The factory MUST validate the source asset and supported parity conversion. A zero input address or zero rate MUST revert with `InvalidInputValuation`; query failures propagate.

Factories MUST return `rate` as requested-token base units per input base unit, scaled by `1e18` and rounded up. `TIP20Factory` returns `(target, 1e18)`. `EarnFactory` uses canonical backing per share after pending fee dilution, before redemption fees or execution losses.

Valuation MUST come from approved onchain accounting and MUST NOT depend on caller-supplied quotes or route data. Vaults without a supported valuation MUST revert. `previewWithdraw` and `previewRedeem` may plan execution, but net redemption quotes MUST NOT replace the reference value or hide redemption fees from slippage accounting.

The funder MUST snapshot the input asset and rate for each `fund` invocation. That snapshot remains fixed until the invocation ends. Cost budgets use requested-token base units. Caps round down; cumulative gross input costs round up:

```text
authorizedCap = floor(remainingCostBudget * 1e18 / rate)
maxInput = min(source.maxInput, authorizedCap)
inputCost = ceil(inputAmount * rate / 1e18)
```

Multiply and divide with full precision. Clamp `authorizedCap` to `uint256.max`; other unrepresentable results MUST revert. Each debit charges the increase in cumulative `inputCost`, using protocol-recorded consumption. Refunds do not reduce consumption, and intermediate conversions do not add a second charge for the same input.

For example, with equal token decimals, `rate = 2e18` means 2 USDC per Earn share. A 10 USDC budget permits 5 shares. Consuming 4 shares charges 8 USDC, leaving 2 USDC of budget for later sources.

### Funding input authority

Before each `fund` call, `TIP20Funder` MUST create a journaled transient record containing `account`, `factory`, `inputAsset`, and `remainingInput`. The record uses the authenticated account, authorized factory, validated input asset, and effective input cap. Only protocol code may create or modify this record.

The protocol MUST bind the record to that factory's `fund` invocation. Supported debit paths within the invocation may consume it, and unrelated calls MUST NOT. This association is internal protocol state and adds no arguments to `fund` or separate debit-operator permission.

Each original input debit MUST verify the account and asset, reduce `remainingInput`, and charge the shared cost budget before moving or burning assets. Exceeding either limit MUST revert. This includes wallet transfers, internal DEX debits, and Earn share burns. Refunds MUST NOT replenish the cap; intermediate conversions MUST NOT double count costs.

The temporary permission replaces allowance checks for supported funding debits. Those debits MUST NOT separately check or deduct the input token's access-key spending limit. Key validity, token pause, and transfer policy checks still apply.

Permission MUST end when `fund` returns or reverts, and debit accounting MUST roll back with reverted asset movements. No standing allowance is created.

The following pseudocode describes node-internal operations, not new public APIs. Permission updates, cost accounting, and asset movements share the EVM revert journal.

```text
// Inside TIP20Funder, after authorization and input valuation.
assert activeFunding == none
// Retain the valuation snapshot and cumulative counters in journaled invocation state.
inputAmount = 0
inputCost = 0
activeFunding = {
    account,
...
try:
    // Bind this invocation to the factory call frame and its descendants.
    in funding frame for source.factory:
        call source.factory.fund(
            account, asset, shortfall, source.target, maxInput, source.data
        ) as TIP20_FUNDER
finally:
    activeFunding = none

// Before each original-input debit in a supported token, DEX, or vault path.
assert activeFunding != none
assert current frame belongs to the active factory funding invocation
assert owner == activeFunding.account
assert token == activeFunding.inputAsset
assert amount <= activeFunding.remainingInput
check key validity, token pause, and transfer policies
newInputAmount = inputAmount + amount
newInputCost = ceil(newInputAmount * rate / 1e18)
costIncrease = newInputCost - inputCost
assert costIncrease <= remainingCostBudget
activeFunding.remainingInput -= amount
remainingCostBudget -= costIncrease
inputAmount = newInputAmount
inputCost = newInputCost
debit or burn amount from owner
```
...
function requireFunds(
    ...
    for (uint256 i = 0; i < sources.length && balance < amount; ++i) {
        ...
        (address inputAsset, uint256 rate) =
            ITIP20FunderFactory(source.factory).getInputValuation(asset, source.target);
        // ... reject invalid valuation; snapshot it for this invocation.
        // Full-precision floor division, saturated at uint256 maximum.
        uint256 authorizedCap = mulDivDownSaturating(remainingCostBudget, 1e18, rate);
        ...
        // ... create the transient input permission and bind it to this factory invocation.
        ITIP20FunderFactory(source.factory).fund(account, asset, shortfall, source.target, maxInput, source.data);

        // 5. Clear the transient input permission and measure actual delivery.
...
The `TIP20Factory` precompile implements `getInputValuation` and `fund`:

```solidity
function getInputValuation(address asset, address target)
    external view returns (address inputAsset, uint256 rate)
{
    // ... verify asset and target are supported TIP-20 tokens with 1:1 parity.
    // Both tokens use six decimals.
    return (target, 1e18);
}
...
The protocol derives share caps using the [input valuation](#input-valuation) snapshot and remaining shared cost budget. Redemption fees and execution losses consume that budget. `maxInput` may further limit shares consumed. The factory MUST also bound any composed swap to the underlying released by this redemption.

The `EarnFactory` contract implements `getInputValuation` and `fund`. In this pseudocode, `_previewBackingAndSupply` is an internal helper that returns managed backing before redemption fees in underlying base units and share supply after pending fee dilution. It rejects unsupported or unavailable valuation.
    ...
    external view returns (address inputAsset, uint256 rate)
{
    ...
    IEarnVault vault = IEarnVault(target);
    // ... verify vault.asset() and asset are supported TIP-20 tokens with 1:1 parity.
    (uint256 backingAssets, uint256 shareSupply) = _previewBackingAndSupply(target);
    rate = Math.mulDiv(backingAssets, 1e18, shareSupply, Math.Rounding.Ceil);
    return (vault.earnShare(), rate);
}
...
function fund(
    ...
    // ... maxInput already includes the caller cap and remaining shared cost budget.

    // ... quote contribution <= amount within spendable shares, withdrawal liquidity, and maxInput.
...
Root execution uses the same [funding input authority](#funding-input-authority) as access-key execution.
...
SDKs need interface bindings and encoding/signing support for source asset factory permissions and each source asset's factory, address, cap, and data.

## Relay RPC API

The Relay RPC API sits between the user and the Node. The Relay discovers supported deployments, apply offchain preferences, and construct and simulate funding requirements. `eth_fillTransaction` accepts complete requirements, target balances without sources, or `true` to infer target balances from the transaction calls.
...
type FillRequireFunds =
  /**
   * Preserves the supplied assets, amounts, source order, caps, data, and slippage settings.
   */
  | RequireFunds

  /**
   * Preserves each `asset` and `amount`, and automatically fills ordered sources
  ...
  /**
   * Infers required tokens and target balances from the calls, then select sources.
...
The relay returns an unsigned transaction with a complete `requireFunds` array for signing. Shorthand entries and `true` are relay inputs only and MUST NOT appear in the signed transaction. The filled request MUST respect factory order, input limits, and authorized slippage. Relay preferences grant no authority, and simulation does not replace execution-time checks.
...
15. **Cost measurement.** Input costs MUST include conversion fees and use consistent token decimals. Input caps and costs MUST use the invocation's fixed valuation snapshot and prescribed rounding, with redemption fees and execution losses included. Intermediate assets MUST NOT be counted twice, and rounding MUST NOT enlarge authorized input limits.
