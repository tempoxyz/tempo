...
Source entries consistently use `{ target, data }`. In funding policies and discovery inputs, `data` is source configuration passed as `configData`. In funding requirements and discovery results, `data` contains execution arguments passed as `executionData`.
...
interface IFundingSource {
    ...
    /// @param configData Source-specific rules or owner-selected configuration.
    ...
    /// @param configData Source-specific input rules or owner-selected candidate configuration.
    function discover(
        address account,
        address assetOut,
        uint256 amountOut,
    ...
    /// @param executionData Nonempty concrete request with caller caps.
    /// @param configData Approved source rules; empty for owner-authorized execution.
    ...
    /// @param executionData Prepared execution payload returned by this source's quote hook.
...
Immediately before funding, `TIP20Funder` calls `quote` with the authenticated account and funding policy rules, then passes `quote.executionData` unchanged to `fund`.
...
`supportsToken` reports whether `configData` permits a supported path to the requested output token, regardless of balances or liquidity. No supported path returns false; malformed configurations MUST revert.
...
`discover` returns ordered candidates with reusable request data and independent output estimates, limited to paths permitted by `configData`.

- Each candidate MUST include nonempty `executionData` and `availableAmount` in output-token base units.
...
`Quote.executionData` MUST be nonempty and accepted by both `quote` and `fund`. Consumers copy it into `requireFunds.sources[].data`. Re-quoting MUST preserve the selected input, path, and proofs, and preserve or tighten caller restrictions.

#### Quote authorization

The protocol derives quote authorization from authenticated context:

| Execution | `configData` | `ownerAuthorized` |
| --- | --- | --- |
...
- Before funding, re-quote each request against the remaining shortfall and cost budget, validate its bounds, and pass its `executionData` to `fund`.
...
`quote.maxAmountIn` MUST be the minimum of the caller cap, funding policy caps, and input capacity allowed by `maxCost`. `quote.executionData` carries this bound and validated execution arguments.
...
IFundingSource(request.target).fund(account, assetOut, amountOut, quote.executionData);
    ...
    internal returns (uint256 fundedAmount)
{
    ...
    if (ctx.isAccessKey) {
        ...
        for (uint256 i; i < sources.length; ++i) {
            ...
            for (uint256 j = position; j < entries.length; ++j) {
                ...
                matchedConfigs[i] = entries[j].data;
                position = j;
                matched = true;
                break;
            }
    ...
    for (uint256 i; i < sources.length && balance < amount; ++i) {
        Source calldata request = sources[i];
        bytes memory configData = matchedConfigs[i];
        IFundingSource.Quote memory quote = IFundingSource(request.target).quote(
            ctx.account, token, amount - balance, costBudget - totalCost, request.data, configData, !ctx.isAccessKey
        );
        ...
        IFundingSource(request.target).fund(ctx.account, token, amount - balance, quote.executionData);
        (uint256 amountIn, uint256 inputCost) = closeInputPermission();
        totalCost += inputCost;
...
sequenceDiagram
    ...
    opt Funds are needed
        loop Sources in configured order
            P->>F: Discover with configData and same cost budget
...
The policy-free overload accepts ordered `{ target, data }` sources for the requested token and `slippageBps`. It requires no token-keyed routes, stored policy, or commitment. An empty source list returns no candidates.
...
3. Call every selected entry’s `discover` via `STATICCALL`, including repeated targets, using identical bounds and that entry’s `data` as `configData`. Propagate failures; require nonempty `executionData` and `0 < availableAmount <= shortfall`. Do not re-quote candidates.
...
- Return `target`, candidate `executionData` as `data`, and `availableAmount`. Estimates are independent: do not sum them, reduce later budgets, or stop when estimates cover the shortfall.
...
sources[0] = IFundingPolicy.Source({
    target: DEX_SOURCE,
    data: abi.encode(USDC_E, type(uint256).max)
});
...
Source `configData` approves one `tokenIn` and optional `maxAmountIn`. Requests select that input within the cap. The source validates parity support and swaps with the authenticated account as both input owner and output recipient.

```solidity
function verify(bytes calldata executionData, bytes calldata configData) external pure returns (bool) {
    DexRequest memory request = decodeDexCall(executionData);
    DexConfig memory settings = decodeDexConfig(configData);
...
function discover(
    ...
    Quote memory result = this.quote(
        ...
        abi.encode(settings.tokenIn, settings.maxAmountIn), configData, false
    );
    if (result.amountOut == 0) return new Candidate[](0);
    candidates = new Candidate[](1);
    candidates[0] = Candidate(result.executionData, result.amountOut);
}
...
function quote(
    ...
    DexRequest memory request = decodeDexCall(executionData);
    if (!ownerAuthorized) {
        require(this.verify(executionData, configData));
    }
...
function fund(address account, address assetOut, uint256 amountOut, bytes calldata executionData) external {
    ...
    (address tokenIn, uint256 cap) = abi.decode(executionData, (address, uint256));
...
The Earn source redeems an approved vault’s shares, optionally swapping into the requested token. Source `configData` specifies one `vault` and optional share-input cap. Requests MUST respect that cap; quoting authenticates the vault and supported engine configuration.
...
```solidity
function verify(bytes calldata executionData, bytes calldata configData) external pure returns (bool) {
    EarnRequest memory request = decodeEarnCall(executionData);
    EarnConfig memory settings = decodeEarnConfig(configData);
...
function discover(
    ...
    Quote memory result = this.quote(
        account, assetOut, amountOut, maxCost,
        abi.encode(settings.vault, settings.maxAmountIn), configData, false
    );
    if (result.amountOut == 0) return new Candidate[](0);
    candidates = new Candidate[](1);
    candidates[0] = Candidate(result.executionData, result.amountOut);
...
function fund(address account, address assetOut, uint256 amountOut, bytes calldata executionData) external {
    require(msg.sender == TIP20_FUNDER);
    (address vault, uint256 shareCap) = abi.decode(executionData, (address, uint256));
...
A root-signed transaction authorizes its selected sources and request data without a funding policy. The protocol derives `ownerAuthorized = true` from authenticated context and supplies empty `configData` to `quote`.
