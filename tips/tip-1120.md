...
Admins MUST be nonempty, unique, and nonzero. Output tokens MUST be unique and nonzero; source addresses MUST be nonzero and unique per token, but may recur across tokens with different rules. Invalid admins, tokens, slippage, or sources revert with `InvalidPolicy`. An empty map allows no requirements; an empty token list allows balance checks only.
