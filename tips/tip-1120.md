...
Inline policies contain no `id`. During owner-authorized key installation, the protocol creates the policy in `FundingPolicy` using the same validation as `createPolicy`, allocates an ID, emits `PolicyCreated` with the owner as `updater`, and stores the ID on the key. Policy creation and key installation MUST succeed or revert together.
...
The SDK encodes the map as `Route[] routes` for the policy ABI and inline key authorization. Entries MUST be sorted by ascending token address bytes, with no duplicate addresses, including differently cased spellings. Each source list retains its order. SDK policy reads convert entries back to the map.
...
- Admins MUST be nonempty, unique, and nonzero.
- Output tokens MUST be unique and nonzero; source addresses MUST be nonzero and unique per token, but may recur across tokens with different rules.
- Invalid admins, tokens, slippage, or sources revert with `InvalidPolicy`.
- An empty map allows no requirements; an empty source list for a token allows balance checks only.
- Any current admin may replace routes and slippage with `modifyPolicy`, or replace all admins with `setAdmins`, including removing themselves. Authenticate before replacement; `modifyPolicy` preserves admins.
- Access key execution and funding callbacks MUST NOT create or mutate policies.
...
- Snapshot the policy at access key funding start for the whole operation.
- Before any balance check, including zero requirements, reject missing tokens with `TokenNotAllowed` and validate source membership and nondecreasing policy position.
...
1. Resolve the policy and token before checking balances; missing entries revert with `PolicyNotFound` or `TokenNotAllowed`. A sufficient balance returns no sources.
2. Otherwise, compute the shortfall and slippage budget. `STATICCALL` every approved source's `discover(account, token, shortfall, costBudget, policyData)` with those same bounds. Propagate failures; require nonempty request data and `0 < availableAmount <= shortfall`. Do not re-quote candidates.
3. Preserve policy source order and candidate order. Return each source's address, `requestData` as `data`, and `availableAmount`.

Estimates are independent: do not sum them, reduce later budgets, or stop once estimates cover the shortfall. Shared inputs, liquidity, slippage, and key authorization require sequential simulation and execution.
...
- Only the transaction handler may invoke `TIP20Funder.requireFunds`, when processing the transaction's funding requirements.
- EVM calls cannot invoke this operation.
- The handler executes source hooks through ordinary EVM frames: static calls for `quote`, then calls for `fund`.
- Funding and application calls share gas and rollback.
- Source hooks receive `TIP20_FUNDER` as `msg.sender` and retain the transaction sender as `tx.origin`.
- `TIP20_FUNDER` is the funding precompile address used for source callbacks and funding events.
- Transactions, deployed code, and delegated code MUST NOT originate calls from it.
- Calling it cannot initiate funding.
- Source hooks MUST reject other callers.
- Callbacks charge the active fork's CALL or STATICCALL base cost, account access, and input memory expansion.
- They receive remaining transaction gas, without an extra EIP-150 reduction at entry.
...
- Each `Candidate` MUST contain nonempty concrete `requestData` and an `availableAmount` in output-token base units.
- Inspect only candidates in `policyData`, using the source's policy schema.
- DEX discovery expands `inputTokens`; Earn discovery expands `vaults`.
- Discovery MUST apply the same validation, valuation, liquidity checks, and input limits as `quote`, treating `policyData` as the applicable rules. Return the reusable request data and output estimate produced by that logic.
- Each estimate is capped by `amountOut` and `maxCost`. Omit zero-output candidates; return an empty array when none remain.
- Sources that fund without account inputs may return candidates for eligible claims.
- Unsupported configurations or required external proofs that cannot be obtained onchain MUST revert rather than silently omit candidates. Independent estimates do not establish aggregate availability.

Anyone may call discovery. Owners may supply their own configuration; policy discovery uses stored policy rules. Neither grants authority. Discovery MUST NOT modify state, reserve liquidity, consume proofs, or create input permissions. It is called before signing, not during `requireFunds` execution.
...
- `Quote.requestData` MUST be a nonempty concrete request accepted by both `quote` and `fund`. Consumers place it directly in `requireFunds.sources[].data`.
- Re-quoting MUST preserve the selected input and path, retain proofs, and preserve or tighten caller restrictions. The source uses one encoding for concrete requests and prepared funding payloads.

#### Authorization and execution

- For access keys, `policyData` contains the authenticated rules for the source address and `ownerAuthorized = false`.
- Owner execution uses signed request data, empty policy data, and `ownerAuthorized = true`. Read-only quote arguments grant no authority. The protocol derives execution arguments from authenticated context.
- Quotes MUST NOT modify state, reserve liquidity, consume proofs, or open input permissions.
- Every transaction source MUST contain nonempty concrete data, checked before the balance shortcut.
...
- For access keys, `quote` MUST validate the original call data against the selected policy rule.
- For owner-authorized funding, the signed call data directly authorizes the requested route; only authenticated `ownerAuthorized` permits omitting policy validation.
- Both modes MUST validate supported assets, venues, and configurations during preparation.
- Checks requiring the authenticated account or contribution amount occur in `fund` before spending.
- A funding source MUST derive `quote.maxAmountIn` as the minimum of the decoded caller cap, its approved policy caps, and the input capacity allowed by `maxCost`.
- `quote.requestData` carries the effective bound and any validated execution arguments. It MUST preserve any account- or amount-bound authorization in the original data and MUST NOT expand caller or policy authority. The output asset remains fixed by the protocol.
...
- Signed quotes and proofs used to authorize funding MUST be bound to the approved route and applicable account, asset, amount, and authorization domain.
- Funding sources MUST enforce their required signatures, expiry, and replay protection.
- `fund` MUST check account- and amount-bound proofs against its authenticated arguments before any input debit or external funding action.
- Stateful proof consumption happens during `fund` and follows transaction rollback.
- Runtime data MUST NOT replace approved inputs, venues, or signers.
...
- Credit follows nested reverts and expires at transaction end.
- Incoming transfers, refunds, and approval decreases MUST NOT create credit or restore budget. Unspent funds from `requireFunds` remain charged.
...
`FundingPolicy` emits `PolicyCreated`, `PolicyUpdated`, and `PolicyAdminsUpdated` for policy creation, rule updates, and admin changes. `TIP20Funder` emits the funding events. `SourceFunded` records each positive verified contribution with its funding source, original request hash, actual input asset, gross input amount, and delivered output. Zero contributions emit no funding event; no-input contributions record zero input asset and amount.
