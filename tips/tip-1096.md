...
A header-only batch exposes identity deposit and token-enablement transitions.
Its public withdrawal queue hash is zero, and the verifier checks that
...
Activation leaves the portal's new token-enablement cursor uninitialized. The
portal MUST reject new token enablements while the cursor is uninitialized.
...
one batch, the verifier treats `N` as the previous settled count instead of
requiring an initialized portal cursor. After accepting the proof, `submitBatch`
sets `lastProcessedEnabledTokenCount` to the proven final Zone count and marks
the cursor initialized.

The first post-activation batch MUST use operational settlement, though it MAY
contain header-only recovery blocks before its final full block. This prevents
replaying already initialized tokens or treating post-checkpoint enablements as
processed.
...
`lastProcessedEnabledTokenCount()` are added. Portal storage also records whether
the token-enablement cursor has completed post-activation initialization.
