...
### Header-only production coordination

Header-only block validity does not bind the block beneficiary to a Tempo leader.
This permits a single recovery block to cross leadership activation boundaries.

Live sequencer implementations MUST nevertheless designate a unique producer
for each header-only block. The designated producer is the leader governing
the first imported Tempo header. Leadership changes after that first header
do not invalidate the range.

Peers MUST accept a live header-only block announcement only from that
designated producer. This sender rule does not apply when validating
an already-settled or otherwise canonical historical block that carries
no live producer claim.

After importing the block, sequencers recompute leadership for the next Tempo
anchor before producing another block.
