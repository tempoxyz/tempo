...
The deposit cap changes from a per-Tempo-block limit to a global outstanding
limit, with capacity replenished when deposits are settled.
...
split point, and with current Tempo/Reth limitations fetching proofs for blocks
arbitrarily far in the past is not feasible.
...
state. Crossing leadership changes is permitted for robustness; the reference
producer splits header-only blocks at leadership boundaries.
...
be produced by its catch-up leader or by the sequencer authorized by a forced
recovery override. A leadership change crossed later in the same imported header
range does not change the catch-up leader for that block. A later header-only
block derives its catch-up leader independently from its own first imported header.

Motivation for binding catch-up to a specific leader is to prevent sequencers
from building conflicting catch-up chains. Due to proof fetching limitations,
verifying the active leader at historical block against the state root might
not be feasible but this is accepted under current safety model where any
whitelisted sequencer peer is expected to not be malicious.
...
If a historical leader at the time of last produced zone block happens to be offline,
the expectation is for the zone operator to apply forced recovery procedure and hand
off the block production to a different available sequencer.
...
MUST validate the complete block sequence and the corresponding `ZoneInbox`
state transitions before signing. The header-only chain is not independently
...
header-only. A batch consisting of a header-only prefix followed by a suffix
of full blocks is valid.
...
token initialization and deposits processing is verified against the same root.
...
The first post-activation full block extends the `processedTokenEnablementHash`
authenticated by the parent Zone state with the supplied token-enablement suffix.
The resulting hash MUST equal `ZonePortal.tokenEnablementHash` at the full block's
final imported Tempo root. Let `C` be `ZonePortal.enabledTokenCount` authenticated
at that same root and `k` be the number of token enablements supplied to this
full block. The block MUST process the complete pending suffix and initialize
`processedEnabledTokenCount` to `C`.

For the activation batch, `TokenEnablementTransition.prevProcessedTokenCount`
MUST remain `0`, the pre-state value of the newly introduced storage field.
Previously processed tokens are authenticated by the existing
`processedTokenEnablementHash`. The verifier treats an uninitialized portal
...
k <= MAX_UNPROCESSED_TOKEN_ENABLEMENTS
```

While `tokenEnablementCursorInitialized` is false and
`TokenEnablementTransition.nextProcessedTokenCount` is nonzero, `submitBatch`
MUST also require `enabledTokenCount - nextProcessedTokenCount <=
MAX_UNPROCESSED_TOKEN_ENABLEMENTS`, using the portal's enabled-token count at
submission time.
...
value also includes tokens processed before activation, authenticated by the
existing token-enablement hash.
