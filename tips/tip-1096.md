...
T12 extends `ZoneInbox.TempoAdvanced` with the cumulative token-enablement cursor
...
Before the coordinated T12 activation, `TempoAdvanced` and `BatchSubmitted`
...
This TIP activates through the coordinated T12 Tempo upgrade and Zone hardfork.
T12 installs the new `ZonePortal` runtime and activates the new Zone
