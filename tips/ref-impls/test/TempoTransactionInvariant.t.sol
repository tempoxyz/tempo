...
contract TempoTransactionInvariantTest is InvariantChecker {
        ...
        external
    {
        ...
        try vmExec.executeTransaction(signedTx1) {
            ...
            assertEq(
                nonce.getNonce(actor, key2),
                nonce2,
                "NON6: key2 nonce changed after key1 increment"
            );
        } catch {
        ...
        try vmExec.executeTransaction(signedTx2) {
            ...
            assertEq(
                nonce1After,
                nonce1 + 1,
                "NON6: key1 nonce changed after key2 increment"
            );
        } catch {
    ...
    function handler_nonceOverflow(uint256 actorSeed, uint256 nonceKeySeed) external {
        ...
        bytes32 slot = keccak256(
            abi.encode(uint256(nonceKey), keccak256(abi.encode(actor, uint256(0))))
        );
        ...
        calls[0] = TempoCall({
            to: address(feeToken),
            value: 0,
            data: abi.encodeCall(ITIP20.transfer, (recipient, 1e6))
        });
        ...
        try vmExec.executeTransaction(signedTx) {
            ...
            assertEq(
                actualNonce,
                type(uint64).max,
                "NON9: Nonce changed despite overflow rejection"
            );
        }
    }
