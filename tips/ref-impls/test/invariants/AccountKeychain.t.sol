        ...
        internal
    {
        if (scenario == AuthorizeKeyRevertScenario.ZeroPublicKey) {
            ...
            if (_loggingEnabled) {
                _log(
                    string.concat(
                        "TRY_AUTHORIZE_REVERT[ZeroPublicKey]: account=", _getActorIndex(account)
                    )
                );
            }
            ...
            if (_loggingEnabled) {
                _log(
                    string.concat(
                        "TRY_AUTHORIZE_REVERT[KeyAlreadyExists]: account=",
                        _getActorIndex(account),
                        " keyId=",
                        vm.toString(keyId)
                    )
            ...
            if (_loggingEnabled) {
                _log(
                    string.concat(
                        "TRY_AUTHORIZE_REVERT[ExpiryInPast]: account=",
                        _getActorIndex(account),
                        " keyId=",
                        vm.toString(keyId),
                        " expiry=",
                        vm.toString(badExpiry)
                    )
                );
            }
        } else {
            ...
            if (_loggingEnabled) {
                _log(
                    string.concat(
                        "TRY_AUTHORIZE_REVERT[KeyAlreadyRevoked]: account=",
                        _getActorIndex(account),
                        " keyId=",
                        vm.toString(keyId)
                    )
        ...
        if (_loggingEnabled) {
            _log(
                string.concat(
                    ...
                    " keyId=",
                    vm.toString(keyId),
                    hadNonZeroLimit ? " hadLimit=true" : " hadLimit=false"
                )
            );
        }
    }
        ...
        if (_loggingEnabled) {
            _log(
                string.concat(
                    ...
                    vm.toString(token),
                    " old=",
                    vm.toString(selectedLimitBefore),
                    " new=",
                    vm.toString(newLimit),
                    infoBefore.enforceLimits ? " ratchet=true" : " ratchet=enabled"
                )
            );
        }
    }
        ...
        internal
    {
        if (scenario == UpdateLimitRevertScenario.NonExistent) {
            ...
            if (_loggingEnabled) {
                _log(
                    string.concat(
                        "TRY_UPDATE_LIMIT_REVERT[NonExistent]: account=",
                        _getActorIndex(account),
                        " keyId=",
                        vm.toString(keyId)
                    )
            ...
            if (_loggingEnabled) {
                _log(
                    string.concat(
                        "TRY_UPDATE_LIMIT_REVERT[Revoked]: account=",
                        _getActorIndex(account),
                        " keyId=",
                        vm.toString(keyId)
                    )
            ...
            if (_loggingEnabled) {
                _log(
                    string.concat(
                        "TRY_UPDATE_LIMIT_REVERT[OtherAccount]: owner=",
                        _getActorIndex(owner),
                        " caller=",
                        _getActorIndex(caller),
                        " keyId=",
                        vm.toString(keyId)
                    )
                );
        ...
        internal
    {
        if (scenario == RevokeKeyRevertScenario.NeverAuthorized) {
            ...
            if (_loggingEnabled) {
                _log(
                    string.concat(
                        "TRY_REVOKE_REVERT[NeverAuthorized]: account=",
                        _getActorIndex(account),
                        " keyId=",
                        vm.toString(keyId)
                    )
            ...
            if (_loggingEnabled) {
                _log(
                    string.concat(
                        "TRY_REVOKE_REVERT[AlreadyRevoked]: account=",
                        _getActorIndex(account),
                        " keyId=",
                        vm.toString(keyId)
                    )
            ...
            if (_loggingEnabled) {
                _log(
                    string.concat(
                        "TRY_REVOKE_REVERT[OtherAccount]: owner=",
                        _getActorIndex(owner),
                        " caller=",
                        _getActorIndex(caller),
                        " keyId=",
        ...
        if (_loggingEnabled) {
            _log(
                string.concat(
                    ...
                    " token=",
                    vm.toString(token),
                    " correctly rejected at timestamp==expiry"
                )
            );
        }
    }
        ...
        returns (address keyId, bool skip)
    {
        ...
        for (uint256 i = 0; i < _potentialKeyIds.length; i++) {
            ...
            if (
                ...
                _createKeyInternal(owner, candidate, false);
                return (candidate, false);
            }
        }
