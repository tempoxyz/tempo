...
contract TIP20InvariantTest is InvariantBaseTest {
        ...
        external
    {
        ...
        try token.transfer(recipient, 0) returns (bool success) {
            ...
            _logHandler(
                "TRANSFER_ZERO",
                string.concat(
                    _getActorIndex(actor), " -> ", _getActorIndex(recipient), " 0 ", token.symbol()
                )
            );
        } catch (bytes memory reason) {
            vm.stopPrank();
            _logRevert("TRANSFER_ZERO", reason);
            assertTrue(_isKnownTIP20Error(bytes4(reason)), "Unknown error encountered");
        }
    }
        ...
        external
    {
        ...
        try token.transferFrom(owner, recipient, amount) returns (bool success) {
            ...
            vm.stopPrank();
            _logRevert("TRANSFER_FROM", reason);
            assertTrue(_isKnownTIP20Error(bytes4(reason)), "Unknown error encountered");
        }
    }
    ...
    function mint(uint256 tokenSeed, uint256 recipientSeed, uint256 amount) external {
        ...
        try token.mint(recipient, amount) {
            ...
            _logHandler(
                "MINT",
                string.concat(
                    vm.toString(amount), " ", token.symbol(), " to ", _getActorIndex(recipient)
                )
            );
        } catch (bytes memory reason) {
            vm.stopPrank();
    ...
    function burn(uint256 tokenSeed, uint256 amount) external {
        ...
        try token.burn(amount) {
            ...
            _logHandler("BURN", string.concat(vm.toString(amount), " ", token.symbol()));
        } catch (bytes memory reason) {
            vm.stopPrank();
        ...
        external
    {
        ...
        try token.transferWithMemo(recipient, amount, memo) {
            ...
            vm.stopPrank();
            _logRevert("TRANSFER_WITH_MEMO", reason);
            assertTrue(_isKnownTIP20Error(bytes4(reason)), "Unknown error encountered");
        }
    }
        ...
        external
    {
        ...
        try token.transferFromWithMemo(owner, recipient, amount, memo) returns (bool success) {
            ...
            vm.stopPrank();
            _logRevert("TRANSFER_FROM_MEMO", reason);
            assertTrue(_isKnownTIP20Error(bytes4(reason)), "Unknown error encountered");
        }
    }
        ...
        external
    {
        ...
        try token.setRewardRecipient(newRecipient) {
            ...
            vm.stopPrank();
            _logRevert("SET_REWARD_RECIPIENT", reason);
            assertTrue(_isKnownTIP20Error(bytes4(reason)), "Unknown error encountered");
        }
    }
    ...
    function distributeReward(uint256 actorSeed, uint256 tokenSeed, uint256 amount) external {
        ...
        try token.distributeReward(amount) {
            ...
            _logHandler(
                "DISTRIBUTE_REWARD",
                string.concat(
                    _getActorIndex(actor), " distributed ", vm.toString(amount), " ", token.symbol()
                )
            );
        } catch (bytes memory reason) {
            vm.stopPrank();
            _logRevert("DISTRIBUTE_REWARD", reason);
            assertTrue(_isKnownTIP20Error(bytes4(reason)), "Unknown error encountered");
        }
    }
    ...
    function distributeRewardTiny(uint256 actorSeed, uint256 tokenSeed) external {
        ...
        try token.distributeReward(amount) {
            ...
            _logHandler(
                ...
                string.concat(
                    _getActorIndex(actor), " distributed 1 ", token.symbol(), " (delta=0)"
                )
            );
        } catch (bytes memory reason) {
            vm.stopPrank();
            _logRevert("DISTRIBUTE_REWARD_TINY", reason);
            assertTrue(_isKnownTIP20Error(bytes4(reason)), "Unknown error encountered");
        }
    }
    ...
    function distributeRewardZeroOptedIn(uint256 actorSeed, uint256 tokenSeed) external {
        ...
        _logHandler(
            ...
            string.concat(_getActorIndex(actor), " correctly rejected on ", token.symbol())
        );
    }
    ...
    function claimRewards(uint256 actorSeed, uint256 tokenSeed) external {
        ...
        try token.claimRewards() returns (uint256 claimed) {
            ...
            vm.stopPrank();
            _logRevert("CLAIM_REWARDS", reason);
            assertTrue(_isKnownTIP20Error(bytes4(reason)), "Unknown error encountered");
        }
    }
    ...
    function claimRewardsVerified(uint256 actorSeed, uint256 tokenSeed) external {
        ...
        try token.claimRewards() returns (uint256 claimed) {
            ...
            vm.stopPrank();
            _logRevert("CLAIM_VERIFIED", reason);
            assertTrue(_isKnownTIP20Error(bytes4(reason)), "Unknown error encountered");
        }
    }
    ...
    function burnBlocked(uint256 tokenSeed, uint256 targetSeed, uint256 amount) external {
        ...
        try token.burnBlocked(target, amount) {
            ...
            _logHandler(
                "BURN_BLOCKED",
                string.concat(
                    vm.toString(amount), " ", token.symbol(), " from ", _getActorIndex(target)
                )
            );
        } catch (bytes memory reason) {
            vm.stopPrank();
            _logRevert("BURN_BLOCKED", reason);
            assertTrue(_isKnownTIP20Error(bytes4(reason)), "Unknown error encountered");
        }
    }
    ...
    function changeTransferPolicyId(uint256 tokenSeed, uint256 policySeed) external {
        ...
        try token.changeTransferPolicyId(newPolicyId) {
            ...
            _logRevert("CHANGE_POLICY", reason);
            assertTrue(_isKnownTIP20Error(bytes4(reason)), "Unknown error encountered");
        }
    }
    ...
    function updateQuoteToken(uint256 tokenSeed, uint256 quoteTokenSeed) external {
        ...
        try token.setNextQuoteToken(ITIP20(address(newQuoteToken))) {
            ...
            try token.completeQuoteTokenUpdate() {
                ...
                _logHandler("UPDATE_QUOTE_TOKEN", string.concat(token.symbol(), " quote changed"));
            } catch (bytes memory reason) {
                vm.stopPrank();
    ...
    function setSupplyCap(uint256 tokenSeed, uint256 newCap) external {
        ...
        try token.setSupplyCap(newCap) {
            ...
            _logHandler(
                "SET_SUPPLY_CAP",
                string.concat(
                    token.symbol(), " cap ", vm.toString(currentCap), " -> ", vm.toString(newCap)
                )
            );
        } catch (bytes memory reason) {
            vm.stopPrank();
            _logRevert("SET_SUPPLY_CAP", reason);
            assertTrue(_isKnownTIP20Error(bytes4(reason)), "Unknown error encountered");
        }
    }
    ...
    function toggleBlacklist(uint256 actorSeed, uint256 tokenSeed, bool blacklist) external {
        ...
        try registry.modifyPolicyBlacklist(policyId, actor, blacklist) {
            ...
            vm.stopPrank();
            _logRevert("TOGGLE_BLACKLIST", reason);
            assertTrue(_isKnownTIP20Error(bytes4(reason)), "Unknown error encountered");
        }
    }
    ...
    function togglePause(uint256 tokenSeed, bool pause) external {
        ...
        _logHandler(
            "TOGGLE_PAUSE", string.concat(token.symbol(), " ", pause ? "PAUSED" : "UNPAUSED")
        );
    }
        ...
        external
    {
        ...
        _logHandler(
            "TRY_TRANSFER_PAUSED",
            string.concat(_getActorIndex(actor), " blocked on paused ", token.symbol())
        );
    }

    function afterInvariant() public {
        string[] memory lines = new string[](4);
        lines[0] = string.concat("Rewards distributed: ", vm.toString(_totalRewardsDistributed));
        lines[1] = string.concat("Rewards claimed: ", vm.toString(_totalRewardsClaimed));
        lines[2] = string.concat("Ghost reward input sum: ", vm.toString(_ghostRewardInputSum));
        lines[3] = string.concat("Ghost reward claim sum: ", vm.toString(_ghostRewardClaimSum));
        _logSummary("TIP20 Final Summary", lines);
    }
