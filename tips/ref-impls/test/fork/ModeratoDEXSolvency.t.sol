...
/// @title Moderato DEX Solvency Fork Test
/// @notice Forks moderato testnet, walks the orderbook tick-level linked lists to discover
...
///      Run with --fork-url pointed at moderato RPC.
contract ModeratoDEXSolvencyTest is Test {
    ...
    function setUp() public {
        baseTokens.push(ALPHA_USD);
        baseTokens.push(BETA_USD);
    }
    ...
    function _walkBackward(uint128 tailId) internal {
        ...
        uint256 guard = 0;
        while (cur != 0 && guard < 500_000) {
            if (seenOrder[cur]) break; // Already discovered via forward walk
    ...
    function test_forkExitAllAndVerifySolvency() public {
        ...
        require(orderIds.length > 0, "No active orders");
