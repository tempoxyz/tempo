...
The discount is intentionally narrow. It applies only to payment-lane transactions that call a small set of TIP-20 transfer or burn functions and actually use no more than 250,000 gas. Approvals, mints, arbitrary contract execution, mixed batches, and higher-gas transactions continue to pay the normal base fee.
...
| Maximum discounted payment gas used | 250,000 gas | Per-transaction execution cap for the discount |
...
3. The transaction uses no more than 250,000 gas.
...
A transaction that satisfies the eligibility rules is charged the discounted payment gas price. A transaction that does not satisfy the eligibility rules is charged the normal base fee.
...
A transaction that receives the discount is charged the discounted payment gas price. A transaction that does not receive the discount is charged the normal base fee. If a transaction receives the discount, the unused authorized amount is refunded through the existing gas refund path.
...
This TIP is layered on top of the payment lane introduced in TIP-1045. If a transaction is not eligible for the payment lane, it is not eligible for this discount.
...
- The discount never applies to a transaction that is not eligible for the payment lane.
- A transaction that authorizes the normal base fee remains valid whether or not the discount applies.
