...
- If `is_admin` is omitted, the authorization creates a non-admin key.
- If `is_admin` is `true`, the authorization creates an admin key and `account` MUST be present and equal the target account being modified.
- If the authorization is signed by an admin access key, `account` MUST be present and equal the target account being modified, even when `is_admin` is omitted.
