...
As the number of TIP-20 tokens grows, maintaining deep FeeAMM liquidity for every possible `(userToken, validatorToken)` pair becomes impractical. Most tokens have a `quoteToken` that points to a liquid hub token. By routing through this intermediate token, we can support fee swaps for any pair `(userToken, validatorToken)` where pools `(userToken, userToken.quoteToken())` and `(userToken.quoteToken(), validatorToken)` both exist with sufficient liquidity, without requiring direct pool liquidity.
