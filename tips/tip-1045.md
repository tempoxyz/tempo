...
The constraints depend on whether the entry's parameters contain any dynamic ABI types (`bytes`, `string`, dynamic arrays, or tuples/arrays transitively containing them):

- **Static-only entries**: calldata length MUST equal `4 + num_params * 32` bytes.
- **Entries with any dynamic type**: calldata length MUST be `<= 2048` bytes.
