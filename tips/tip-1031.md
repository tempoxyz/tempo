...
Embed consensus metadata into the `TempoHeader` to opt into Commonware's `Deferred` [verification](https://github.com/commonwarexyz/monorepo/pull/2604) which allows optimistic notarize votes while execution verification runs in parallel to reduce finalization latency.
...
* The migration to `Deferred` implies that the notarization certified will not longer carry any gaurantees on the validity of the block. Only ensuring the consensus view is valid.

* The migration to `Deferred` is also not stricly coupled with this TIP. This TIP only asserts the inclusion and validity of the consensus context in the header as a prerequisite to using the `Deferred` implementation. Usage to `Deferred` is not a consensus breaking change, explained in [Block Verification](#block-verification).
