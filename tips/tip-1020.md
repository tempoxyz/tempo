...
The `verify` function MUST support only stateless signature types (secp256k1, P256, WebAuthn). If a Keychain signature (type prefix `0x03`) is used, it MUST revert.

#### `verifyStateful`

For stateless signature types (secp256k1, P256, WebAuthn), `verifyStateful` MUST behave identically to `verify`.

For Keychain signatures (type prefix `0x03`), `verifyStateful` performs the following steps:
