...
Keychain signatures are not verified by this precompile. Contracts SHOULD verify keychain signatures by:

1. Verifying the inner signature with this signature verification precompile.
2. Checking the key is authorized on the root account and not expired via the AccountKeychain precompile.
