...
The TIP replaces the narrower "zkTLS" framing with a broader attestation system. Authenticated HTTPS claims are one source of attestations, not the product category. The architecture is: the verifier proves facts, the attested signature exercises authority, and the registry stores reusable attestations.
...
The current "zkTLS" name is too narrow and misleading. The system is not only about TLS, and the current deployment model is not a zero-knowledge protocol. The useful primitive is an attestation: an approved program in a verifiable execution environment signs a bounded statement under a schema. The protocol brand should reflect that.
