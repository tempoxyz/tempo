...
This TIP introduces **Tempo Attestations**: a protocol-native way to verify claims produced by approved trusted execution environments and to persist those claims in an EAS-style registry. It replaces the narrower "zkTLS" framing with a broader attestation system that covers identity, authenticated HTTPS responses, account recovery, login with Gmail, selective disclosure, and future claim sources.

The TIP also introduces an **attested signature** as a native Tempo signature type. An attested signature is verified by checking a TEE-backed attestation against protocol state, then treating the attestation subject as the recovered signer.

## Motivation

Tempo needs a general primitive for statements that are true because approved code ran in an approved environment and signed the result. Gmail login is one use case: a user proves control of a Google account to a TEE, the TEE issues a privacy-preserving statement, and Tempo can use that statement to authorize a locally generated key or perform account recovery. The same primitive should also support authenticated web claims, KYC-style statements, app-specific credentials, selective disclosure, and offchain-to-onchain bridges.
...
Ethereum Attestation Service (EAS) gives the right shape for the persistent registry: schemas get stable UIDs, attestations reference schemas, attestations can reference other attestations, and revocation is schema-controlled. Tempo should keep that model, but make TEE verification and native Tempo signatures first-class.
...
## Native Attested Signature Type
...
7. The user submits the attestation on chain to authorize the local key or to satisfy a recovery policy.
...
## Integration With Account Recovery
