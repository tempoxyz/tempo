...
This TIP introduces **Tempo Attestations**: a set of protocol primitives for building products that need trusted offchain data, private identity, account recovery, delegated account actions, verifiable AI, and reusable attestations. The core primitive verifies claims produced by approved trusted execution environments. Higher-level primitives turn verified claims into native account authority and optional on-chain state.
...
- **Programmable account delegation**: users should be able to delegate narrow capabilities from web accounts, APIs, or app credentials without giving an operator raw standing authority. A TEE can hold the credential, enforce a one-time, count-limited, time-limited, or policy-gated action, and emit an attestation proving what happened.
- **Attested agents**: autonomous software can control accounts, keys, or external credentials only if users can verify the runtime, policy, and action log. A TEE-backed agent can prove which code is running, what authority it holds, which outputs it produced, and which actions it took without exposing private state.
- **Private data rooms and verifiable AI**: users, companies, or protocols can contribute private data to an approved computation and receive only the allowed output. This supports model routing, inference, training, scoring, matching, and decision support where the raw data, model weights, or routing policy should remain private but the computation should be auditable.
- **Content provenance**: media, documents, and generated outputs increasingly need verifiable origin and transformation history. A TEE can attest to capture metadata, source accounts, model/runtime identity, and editing pipelines while revealing only the public provenance needed by the consumer.
...
- **Layer 2: Tempo attestation** is the typed statement being verified: subject, schema, provider hash, claim hash, nonce, expiry, source hash, and TEE identity. It can describe an observed fact, an authorized action, or the output of an approved computation. It carries commitments, not raw private data.
...
- **Programmable account delegation**

  Product goal: let a user delegate a narrow account, API, or app capability without giving the operator raw standing authority.

  Primitive path: the user grants an external credential or session to an approved TEE app, the TEE enforces an action policy, and the TEE signs a `TempoAttestation` over the action commitment, policy hash, result hash, and expiry. `TempoAttestationRegistry` can store public action receipts, revocations, and policy references when later consumers need them.

  Authority rule: delegated capabilities SHOULD be count-limited, time-limited, audience-limited, and action-specific. The TEE MUST NOT emit raw credentials, session cookies, OAuth tokens, or reusable secrets on chain.

- **Attested agents**

  Product goal: let autonomous software hold keys, app credentials, budgets, or account capabilities while users can verify the code, policy, and action trail.

  Primitive path: the agent runs in an approved TEE app, signs actions with `AttestedSignature`, and emits registry attestations for durable state such as policy commitments, model/runtime identity, spending limits, and action receipts.

  Control rule: applications should separate agent operating authority from user recovery authority. A compromised or revoked agent should lose the ability to act without preventing the user from rotating to a new local key or agent policy.
...
- **Private data rooms and verifiable AI**

  Product goal: let multiple parties contribute private data to an approved computation and receive an auditable output without exposing raw inputs to each other or to the operator.

  Primitive path: the TEE receives encrypted records, model inputs, or API credentials, verifies participant consent and source policy, runs an approved program or model router, and signs a `TempoAttestation` over the input commitment, program or model hash, output commitment, disclosure scope, and expiry.

  Disclosure rule: raw data, model weights, prompts, and routing policy stay offchain unless the schema explicitly discloses them. Registry entries SHOULD store commitments, aggregate outputs, audit metadata, or revocation state.

- **Content provenance**

  Product goal: let consumers verify where media, documents, or generated content came from and which approved transformations were applied.

  Primitive path: the TEE attests to source account, capture context, model/runtime identity, transformation pipeline hash, input commitment, and final content hash. The registry stores the provenance record or references a chain of prior provenance attestations.

  Provenance rule: provenance attestations do not make content true. They prove source, process, and continuity under a schema, so applications must display the exact claim being made.
