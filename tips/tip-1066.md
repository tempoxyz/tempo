...
- **Reusable public state**: some attestations are ephemeral and used only inside a signature. Others should be discoverable, referenceable, revocable, and consumed later by applications. Ethereum Attestation Service (EAS) gives the right model for that persistent layer: stable schema UIDs, attestations that reference schemas, attestations that reference other attestations, and schema-controlled revocation.
