...
players(E+1) = validators.filter(v =>
    v.addedAtHeight <= boundary(E-1) &&
    (v.deactivatedAtHeight == 0 || v.deactivatedAtHeight > boundary(E))
)
```

Note the asymmetry: addition has a one-epoch warmup (`boundary(E-1)`), while deactivation takes effect immediately (`boundary(E)`).
...
**Validator Addition (Warmup Period)**: New validators have a one-epoch warmup period before participating in DKG. A validator added during epoch `E` (i.e., `addedAtHeight <= boundary(E)`) becomes a DKG player in epoch `E+2`.

```
Epoch E                    Epoch E+1                  Epoch E+2
│                          │                          │
│  addValidator() called   │  Validator syncs,        │  Validator participates
│                          │  prepares for DKG        │  as DKG player
│                          │                          │
├──────────────────────────┼──────────────────────────┼──────────────────────────►
           boundary(E)              boundary(E+1)
```

**Validator Deactivation (Immediate)**: When a validator is deactivated via `deactivateValidator`, it is removed from the validator set at the next epoch boundary. There is no cooldown period—a validator deactivated during epoch `E` is excluded from the DKG for epoch `E+1`.
...
Note the asymmetry: addition uses `boundary(E-1)` (one-epoch warmup) while deactivation uses `boundary(E)` (immediate removal).
...
10. **Owner authorization**: Only the owner can call `addValidator`,
    `deactivateValidator`, `transferOwner`, and `setNextFullDkgCeremony`.

11. **Dual authorization**: `rotateValidator`, `transferValidatorOwnership`, and
    `setIpAddresses` can be called by either the owner or the validator itself.

12. **Initialized once**: The `initialized` bit (bit 255 of slot 0) can only
...
44. **Writes blocked before init**: `addValidator`, `deactivateValidator`, `rotateValidator`
