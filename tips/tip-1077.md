...
- Account-authority descriptors SHOULD prefer provider schemas that expose a
  stable, non-reassigned account identifier, such as OIDC account providers.
...
### Recommended Account Provider Shape

`ClaimSignature` is provider-generic. Any TIP-1076 provider schema MAY define a
`ClaimAccountType` if it can safely derive a stable, private signer namespace.

For root accounts, admin recovery keys, and native multisig owners, wallets and
apps SHOULD prefer OIDC-style account providers when they are available. They
are a good fit because the schema can bind issuer, client, callback, token
validation, nonce, expiry, and a stable provider account id before deriving
`signerId`.

Other provider schemas can still be useful, but they need more scrutiny before
being used for account authority. In particular, an account-authority provider
schema MUST define:

- a stable account identifier or equivalent private identity source;
- rules for non-reassignment and account deletion, when the provider exposes
  them;
- re-authentication and user-intent requirements for each fresh signature;
- signer derivation that does not depend on mutable display fields;
- a source and extraction policy that is unlikely to change silently; and
- a recovery story if the provider changes the relevant API or page.

Authenticated HTTPS receipt providers are often better suited to stateless app
claims or registry predicates than root account authority. They MAY be used as
account descriptors only when the schema can meet the stability and signer
derivation requirements above.
