...
`operator` is an immutable channel authority selected by the payer in `open`. `address(0)` means
only the payee can submit payee-side operations. A nonzero `operator` MAY submit `settle` and
`close` immediately after the channel is opened, without `requestClose` and without waiting for the
close grace period, but all settlement and close-capture payouts still transfer to `payee`.

Because the payer supplies `operator`, payees and integrators MUST NOT treat a nonzero operator as
payee-side protection unless they have independently accepted that operator. Before relying on a
channel, they SHOULD verify that `operator` is either `address(0)` or an address inside their trust
model, and reject channels whose operator is unknown.
...
6. `settle` and `close` MUST be callable only by `payee`, or by the payer-selected `operator` when `operator != address(0)`.
...
5. Only payee, or a nonzero payer-selected operator, can `settle` and `close`.
