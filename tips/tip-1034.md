...
3. **Approve-less Deposit UX**: `open` and `topUp` can deposit TIP-20 funds into the channel reserve through native system transfer (`systemTransferFrom` semantics), removing the extra `approve` transaction from the user flow and working uniformly for passkey accounts.
...
1. `open` deposits `deposit` from `payer` into channel reserve state without requiring a prior user `approve` transaction.
2. `topUp` deposits `additionalDeposit` the same way.
3. `settle`, `close`, and `withdraw` payout paths continue to transfer TIP-20 value using protocol-native token movement.

Although escrow custody moves tokens through the precompile address, the channel represents a logical payment from `payer` to `payee`. Implementations MUST enforce the token's TIP-403 policy for that logical path for two reasons:
