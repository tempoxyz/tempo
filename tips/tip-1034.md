...
Before relying on a channel, payees and integrators SHOULD verify that `operator` is either
`address(0)` or an address inside their trust model, and reject channels whose operator is unknown
or untrusted.
...
4. If `topUp.additionalDeposit == 0` and `closeRequestedAt == 0`, `topUp` MUST return successfully without changing state and without emitting `TopUp`.
5. If `closeRequestedAt != 0`, a successful `topUp` MUST clear it back to `0` and emit `CloseRequestCancelled`.
6. `requestClose` MUST set `closeRequestedAt = uint32(block.timestamp)` on the first successful call and leave it unchanged on later successful calls.
7. `settle` and `close` MUST be callable only by `payee`, or by `operator` when `operator != address(0)`.
...
12. A `close` voucher with `cumulativeAmount > deposit` remains valid for signature verification; `captureAmount` is the reserve-bounded amount that may actually be paid out.
...
Payment-lane classification for channel reserve operations is defined by TIP-1045. Implementations
MUST include channel reserve calls in the TIP-1045 payment call allow-list iff:
...
All auxiliary-payload restrictions, transaction-type rules, and authorization-field handling follow
TIP-1045. In particular, `authorization_list` and `tempo_authorization_list` MUST be empty, and a
present `key_authorization` MAY remain payment-lane eligible only when its RLP encoding is bounded
as specified by TIP-1045.

### Required Integration Points

1. The T5 consensus payment classifier MUST follow TIP-1045 and include the channel reserve allow-list entries above.
2. The strict builder/pool classifier (`is_payment_v2`) MUST follow TIP-1045 and include the channel reserve allow-list entries above.
