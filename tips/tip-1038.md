...
## 4. Verify paused state before mint and burn

**PR**: [#3411](https://github.com/tempoxyz/tempo/pull/3411) · **Author**: @0xrusowsky

Enforces `check_not_paused()` on `mint`, `mintWithMemo`, `burn`, `burnWithMemo`, and `burnBlocked` when on T3+. Previously only transfers checked the pause flag; mints and burns could still execute on a paused token. A new `validate_mint` helper consolidates the pause check, recipient validation, and TIP-403 policy check into a single call.
