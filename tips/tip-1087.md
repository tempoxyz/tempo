...
Orderbooks keep their existing logical shape and gain a persisted book index in unused bytes:
...
slot 1:
  ...
  offset 20  idFlag       bool      1 byte
  offset 21  id           uint32    4 bytes
...
`id` is the `book_keys` vector index for this orderbook. `idFlag` indicates whether `id` is active, distinguishing an active index `0` from the pre-activation default value. These fields are packed into unused bytes in existing `Orderbook` slots, so this TIP does not increase the orderbook slot count.

Starting at activation, newly created orderbooks MUST store `id = book_keys.length` and `idFlag = true` on the orderbook record before appending the key to `book_keys`. Orderbooks created before activation have `idFlag = false` and no active stored index.
...
function bookIndexForKey(bytes32 bookKey) external view returns (uint32 index, bool indexed);
function bookKeyForIndex(uint32 index) external view returns (bytes32 bookKey);
```

`setIndexForKey` MUST verify that `book_keys[index] == bookKey`, that `bookKey` is initialized, and that the corresponding orderbook exists. If verification succeeds, it MUST store `id = index` and `idFlag = true` on the orderbook record. If the orderbook already has `idFlag = true`, `setIndexForKey` MUST revert with `IndexAlreadySet` and MUST NOT rewrite `id`, even when the supplied `index` matches the existing value.

`bookIndexForKey` is a view helper and MUST NOT scan `book_keys`. It returns the persisted `id` and whether `idFlag` is true. `bookKeyForIndex` MUST fail if the requested index is not present in `book_keys`.
...
Tools that decode raw DEX storage MUST add support for the orderbook `id` / `idFlag` fields, version `2` orders, and resolving `bookIndex` through `book_keys`. Existing tools that consume order events or call `getOrder(uint128)` can continue using the same interface.
