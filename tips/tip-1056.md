...
If the attempt to flip a fully filled flip order fails with a non-system error that is swallowed instead of reverting the fill, the Stablecoin DEX MUST emit `FlipFailed`. `orderId` is the filled flip order's ID, `maker` is the order maker, and `reason` is the ABI selector of the swallowed error.

`FlipFailed` is not emitted when the flip succeeds. `FlipFailed` is also not emitted for system errors that revert the transaction.
...
Indexers that treat a terminal `OrderFilled(..., false)` for a flip order as followed by an automatic flip MUST also process `FlipFailed`. If `FlipFailed` is emitted after the fill, the order did not rest on the opposite side and there is no active flipped order for that `orderId`.
...
Systems that alert on failed automatic flip transitions can consume `FlipFailed(orderId, maker, reason)` to identify the failed logical order and the ABI selector for the swallowed error.
...
- `FlipFailed` means a fully filled flip order did not rest on the opposite side because the flip attempt failed with a swallowed non-system error
...
- `FlipFailed` is emitted exactly once for each swallowed failed flip transition.
