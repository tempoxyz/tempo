...
12. Enforced bounds reject too many scopes/selectors.
13. `setAllowedCalls` create-or-replace semantics are enforced.
14. `setAllowedCalls(..., allowAnySelector = true, selectors = [])` allows any selector on that target.
15. `setAllowedCalls(..., allowAnySelector = false, selectors = [])` blocks all calls to that target (unless unrestricted mode is active).
16. Access-key transactions with CREATE as first call are rejected.
17. Access-key transactions with any CREATE in a batch are rejected.
