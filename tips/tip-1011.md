...
# TIP-xxxx: Subblocks v2 - Builder Delegation and Fallback Inclusion
...
1. Resolve the user's fee token using standard Tempo fee token resolution (tx preference → user preference → inference → PathUSD)
2. Extract `feeRecipient` from nonce key
3. Look up `builder = SubblockRegistry.builderOf(validatorFor(feeRecipient))`
4. If registered and `builder.feeToken` differs, attempt to swap via Fee AMM if liquidity available
...
- No validator token preference applies—fees are paid directly in the user's resolved fee token
...
Fee token resolution for 0x5b transactions follows the standard Tempo resolution order:

1. `tx.fee_token` if explicitly set in the transaction
2. `TipFeeManager.user_tokens[fee_payer]` (stored user preference)
3. Inference from the TIP-20 token being called (for simple transfers)
4. PathUSD as the final fallback

Once the user's fee token is resolved:

1. Extract `fee_recipient` from the transaction's nonce key
2. Look up `builder_fee_token = SubblockRegistry.feeTokenFor(fee_recipient)`

**AMM Swap Behavior:**

If `user_fee_token != builder_fee_token` and the builder has a token preference (non-zero):
- If sufficient AMM liquidity exists, swap to `builder_fee_token`
- If insufficient liquidity, the fee recipient receives `user_fee_token` directly (no swap)
...
- No builder token preference applies
- Fees are paid to `fee_recipient` directly in the user's resolved fee token (no swap attempted)
...
8. **Unregistered fee recipient**: Fallback tx pays fees directly in user's resolved fee token to fee_recipient
...
17. **Fee token swap without liquidity**: If no AMM liquidity, fee recipient receives user's fee token directly
18. **Unregistered recipient fee**: Fallback 0x5b tx to unregistered recipient pays in user's resolved fee token (no swap)
