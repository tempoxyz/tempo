...
1. `allowAnySelector = true` sets `selectors = None` semantics (any selector allowed on `target`) and `selectors` input MUST be ignored.
2. `allowAnySelector = false` with `selectors = []` disables that target scope.
