...
1. `selectorRules = []` sets allow-all-selector semantics for `target`.
...
5. If any rule has a non-empty `recipients` list, `target` MUST be a TIP-20 token address.
6. If any rule has a non-empty `recipients` list, its `selector` MUST be one of:
   1. `0xa9059cbb` (`transfer(address,uint256)`)
   2. `0x095ea7b3` (`approve(address,uint256)`)
   3. `0x95777d59` (`transferWithMemo(address,uint256,bytes32)`)
7. If any rule has a non-empty `recipients` list, each recipient in that list MUST be non-zero.
8. If any rule has a non-empty `recipients` list, recipients in that list MUST be unique.
...
4. Implementations MUST reject any selector rule with a non-empty `recipients` list whose `target` is not a TIP-20 token address.
5. Implementations MUST reject any selector rule with a non-empty `recipients` list and selector outside the fixed constrained-selector set.
...
4. Nested scope-list fields (`selector_rules` and `recipients`) canonically omit empty trailing lists on wire. Implementations MUST accept omitted, explicit `0x80`, and explicit empty-list (`0xc0`) encodings as equivalent allow-all inputs.
...
Call-scope storage is account-scoped and represented as nested scope membership plus per-scope scoping flags:
...
| `key_scopes[account_key].target_scopes[target].is_scoped` | `bool` | Whether the target allows any selector or uses explicit selector membership |
| `key_scopes[account_key].target_scopes[target].selectors` | `Set<u32>` | Explicit selector membership |
| `key_scopes[account_key].target_scopes[target].selector_scopes[selector].is_scoped` | `bool` | Whether the selector allows any recipient or uses constrained recipient membership |
...
Absent target or selector entries represent disabled inner scopes; implementations do not need a separate inner "deny-all" state.
