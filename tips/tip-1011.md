...
If a transaction is signed with an access key (`key != Address::ZERO`), contract creation MUST be rejected in all configurations during scoped-call pre-execution validation.
...
1. If the transaction uses an access key and the call is contract creation, implementations MUST fail execution before the first user call begins.
