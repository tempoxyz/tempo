...
authors: Mallesh Pai
status: Draft
related: TIP-1009, TIP-1010, Sub-block Specification
protocolVersion: TBD
---
...
struct SubblockBuilder {
    ...
    address feeToken;        // TIP-20 token for fee payment
}
```
...
interface ISubblockRegistry {
    ...
    /// @notice Returns the fee token for a given fee recipient.
    /// @param feeRecipient The fee recipient address embedded in nonce keys.
    /// @return The TIP-20 token address for fee payment.
    /// @dev Returns the default fee token if feeRecipient is not registered.
...
**Registration:**
- Only addresses in the active validator set can call `setSubblockBuilder()`
- Each `feeRecipient` must be unique across all validators
...
**Default fee token:**
- If `feeTokenFor()` is called with an unregistered `feeRecipient`, return the chain's default fee token
...
1. Look up `signingKey` from registry using `fee_recipient`
2. Reconstruct the `SubBlock` from the referenced transaction range
3. Compute `signature_hash = keccak256(0x78 || RLP(subblock))`
4. Verify `ed25519_verify(signingKey, signature_hash, signature)`
5. If verification fails, the block is invalid
...
### 5.5 Fee Token Resolution

The fee token is resolved as follows:

1. Extract `fee_recipient` from the transaction's nonce key
2. Call `SubblockRegistry.feeTokenFor(fee_recipient)`
3. If registered, use the registered `feeToken`
4. If not registered, use the chain's default fee token
...
- The `signingKey` is registered for the `fee_recipient` in the Subblock Registry
...
| ID | Invariant |
|----|-----------|
| **S1** | Fee recipient uniqueness: Each `feeRecipient` is registered by at most one validator |
| **S2** | Nonce key format: All 0x5b transactions have fee recipient in bytes 11-30 |
| **S3** | Signature validity: Signed subblock signatures verify against the registered `signingKey` |
| **S4** | Range validity: Metadata tx ranges are non-overlapping, ordered, and within bounds |
| **S5** | Fee routing: Fees are paid to the fee recipient extracted from the nonce key |
| **S6** | Root-only signing: 0x5b transactions reject keychain signatures |
| **S7** | Gas budget: Each validator's subblock does not exceed `per_validator_gas` |
| **S8** | Immediate effect: Builder configuration changes take effect in the next block |
...
8. **Unregistered fee recipient**: Fallback tx uses default fee token
