...
2. `Some([r1, r2, ...])`: allow exactly the listed selector rules.
3. `Some([])` is invalid and MUST be rejected.
...
3. `{ target: 0x123, selector_rules: Some([]) }`: invalid and rejected.
...
8. Authorizations with `CallScope.selector_rules = Some([])` MUST be rejected.
9. Authorizations with `SelectorRule.recipients = Some([])` MUST be rejected.
