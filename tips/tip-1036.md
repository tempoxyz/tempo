...
## 14. Reject zero-address ecrecover in permit
...
1. **Admin-op isolation**: AccountKeychain admin operations can only be invoked by the account owner via a direct transaction (not via contract call). If `tx_origin` is not seeded, admin ops are rejected (failed-closed).
...
12. **Permit signature integrity**: `permit()` always rejects signatures where `ecrecover` returns `address(0)`, even if `owner` is also `address(0)`.
