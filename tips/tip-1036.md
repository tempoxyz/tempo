...
This is a meta TIP that collects all audit-driven bug fixes, security hardening, and minor correctness improvements gated behind the T2 hardfork activation. Each change was identified through external and internal audits or code review and is individually small, but together they form the complete set of state-function-breaking fixes shipping in T2.

## Motivation

During the internal and external audits, several correctness and security issues were identified across precompiles, the transaction pool, fee handling, and the account keychain. Since these fixes change state functions, they require hardfork gating and are collected here as a single meta TIP for T2.
...
9. **DEX order correctness**: `place_flip` order behaviour is consistent with T2 semantics and gated to preserve pre-T2 re-execution compatibility.
