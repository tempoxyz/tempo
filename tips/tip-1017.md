...
- Append-only growth and permissionless rotation require query paths that remain safe as history grows.
...
2. **Validator activity consistency**: Filtering the reconstructed validator set by `deactivatedAtHeight == 0` must produce exactly `getActiveValidators()` (same members, same order).
