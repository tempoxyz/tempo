...
It adds explicit `feeRecipient` configuration so accumulated fees can be held in a dedicated treasury wallet while a separate treasury-ops multisig manages validator operations, reducing blast radius if operational keys are compromised.
...
**Validator addition and deactivation**: there is no warmup or cooldown in V2. Added validators join on the next epoch; deactivated validators leave on the next epoch.
