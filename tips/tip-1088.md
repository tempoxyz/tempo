...
This TIP routes `quote()` through the same per-order fill logic as `swap()`, executed in a read-only simulation. Quote becomes "swap without state writes," so a quote returns exactly what the matching swap executes. The change is gated at T11; below T11 the existing per-tick quote is unchanged.

Since T11 quotes no longer use the stored per-tick `totalLiquidity` aggregate, T11 also stops maintaining that aggregate and instead derives `getTickLevel.totalLiquidity` on demand from active orders.
...
4. In `Execute` mode, the engine behavior remains functionally equivalent to current swap semantics, except that at T11 write paths MUST NOT update the stored per-tick `totalLiquidity` field. This applies to order placement, partial and full fills, cancellation, and flip-order placement.
...
5. Tick-level liquidity correctness:
   - At T11, `getTickLevel.totalLiquidity` MUST equal the checked sum of `remaining` across all active orders in the requested tick.
