...
5. Fork gating:
   - The change applies at T8. `quote()` output is deterministic protocol state reproduced during historical re-execution, so it MUST NOT change retroactively. Before T8, both quote entrypoints keep the existing per-tick behavior, so this TIP is a no-op below T8.
...
5. Pre-fork no-op:
   - Below T9, both quote entrypoints MUST return exactly their legacy per-tick results.
