...
5. Fork gating:
   - The change applies at T9. `quote()` output is deterministic protocol state reproduced during historical re-execution, so it MUST NOT change retroactively. Before T9, both quote entrypoints keep the existing per-tick behavior, so this TIP is a no-op below T9.
