...
The current DEX order layout uses 6 storage slots per order. A packed layout can store the same active order state in 4 slots by using the mapping key as the canonical `orderId` instead of storing `orderId` inside the value:

```solidity
mapping(uint128 orderId => Order order) orders;
```

Adding a version number makes that layout change backwards-compatible and gives future order layouts a clear upgrade path. The DEX also has several hot paths that read and update individual order fields (`remaining`, `prev`, and `next`) directly, so order storage should be accessed through a dedicated abstraction rather than scattering version checks through matching, cancellation, and flip-order logic.
...
- a version `0` flip order that flips after T6 activates is rewritten under the same `orderId` as version `1`
