...
- **All validators imported**: Both active and inactive V1 validators are
  imported; active ones have `addedAtHeght.block.height`, inactive ones have
  `addedAtHeight == deactivatedAtHeight == block.height`.
...
37. **All validators imported on migration**: Both V1 active and inactive validators
    are imported; active ones have `addedAtHeight = block.height`, inactive ones have
    `addedAtHeight == deactivatedAtHeight == block.height`.
38. **addedAtHeight set correctly**: Active validators have `addedAtHeight > 0`
    at migration time.
38. **deletedAtHeight set correctly**: Deleted validators have
    `addedAtHeight == deletedAtHeight` at migration time.
