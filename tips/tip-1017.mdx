...
interface IValidatorConfigV2 {
    ...
    ///      Inactive V1 validators get addedAtHeight=deactivatedAtHeight=block.height at migration time.
...
3. **Address update**: `transferValidatorOwnership` updates the address of an
    ...
    by the contract owner (not the owner of the entry) and is required to for
...
12. **Dual authorization**: `rotateValidator`, `transferValidatorOwnership`, and
...
- **All validators imported**: Both active and inactive V1 validators are
  imported; inactive ones have `addedAtHeight == deactivatedAtHeight == block.height`,
...
37. **All validators imported on migration**: Both V1 active and inactive validators
    are imported; inactive ones have `addedAtHeight == deactivatedAtHeight == block.height`,
    active ones have `addedAtHeight == 0`
38. **addedAtHeight set correctly**: Active validators have `addedAtHeight == 0`,
    inactive validators have `addedAtHeight == block.height` at migration time
