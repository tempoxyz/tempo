...
interface IValidatorConfigV2 {
    ...
    ///         value only makes sense in conjuction with isInitialized()
    function getInitializedAtHeight() external view turns (uint64);
...
11. **Owner authorization**: Only the owner can call `addValidator`,
    `transferOwner`, `migrateValidator`, `initiatializeIfMigraated` and
...
13. **Initialized once**: The `initializedAtHeight` bit (bit 255 of slot 0) can only
