...
`regular_gas_remaining` is a **global counter**: always propagated to subcalls and always returned to parent. It is never reverted.

- **Success**: child's remaining gas returned to parent, refunds propagated.
- **Revert**: child's remaining gas returned to parent, state changes rolled back. Gas consumed by reverted storage creation operations is permanently lost (user pays) but does not reduce `regular_gas_remaining` (doesn't count against protocol limits).
- **Halt** (OOG, invalid opcode): child's gas fully consumed, nothing returned.
