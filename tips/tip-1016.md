...
- On child **revert** or **exceptional halt**, all state gas consumed by the child, both from the reservoir and any that spilled into `gas_left`, is restored to the parent's reservoir. State gas is fully preserved on failure because state changes are reverted, so no state was actually grown. **Exception**: state gas charged for account creation (`CREATE`, `CALL` to new account, and EOA delegation) is consumed even if the frame reverts — this is consistent with pre-EIP-8037 behavior where `GAS_NEW_ACCOUNT` was consumed on revert.
