...
Opcode-level `CREATE`/`CREATE2` follows the deployment flow above, including `HASH_COST(L)` for deployed bytecode. Precompile initialization instead writes a fixed `0xEF` placeholder only to mark the account as deployed, so it charges the CREATE base and code-deposit split for new accounts but does not model the opcode-level `HASH_COST(L)` term.
