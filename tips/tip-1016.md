...
2. **New account throughput penalty**: TIP-20 transfer to new address costs 271,000 gas total (26k regular + 245k state) vs 24,000 gas to existing. At 500M payment lane gas limit:
   ...
   - With reservoir + bottleneck model: state gas is the bottleneck at 245k/tx, yielding ~2,040 transfers/block = ~4,080 TPS
   - Existing account transfers unaffected: ~21,000 transfers/block = ~42,000 TPS
   - The bottleneck model ensures state-heavy blocks don't crowd out regular execution capacity
...
At the transaction level, the user pays for both. At the block level, only the bottleneck dimension determines whether the block is full.
...
- `calldata_floor_gas_cost`: The calldata floor gas cost (if applicable)
...
**Note**: Tempo has two block limits — general gas limit (~25M) for contracts and payment lane limit (500M) for simple transfers. The bottleneck model applies to each lane independently.
...
- State gas: 245,000 (the state creation charge)
- Regular gas: `GAS_STORAGE_UPDATE - GAS_COLD_SLOAD - GAS_WARM_ACCESS` (2,800)

The net cost after refund is `GAS_WARM_ACCESS` (100), consistent with pre-TIP-1016 `SSTORE` restoration behavior. Refunds use `refund_counter` rather than direct gas accounting decrements, so that reverted frames do not benefit from the refund.

## Revert Behavior for State Gas

State gas charged for account creation (`CREATE`, `CALL` to new account) is consumed even if the frame reverts — state changes are rolled back but gas is not refunded. This is consistent with existing behavior where `GAS_NEW_ACCOUNT` was consumed on revert.
...
**When opcode execution starts:** Always charge `GAS_CREATE` (5,000 regular + 245,000 state for account creation, plus 5,000 regular + 495,000 state for metadata)
...
- Account creation state gas is still consumed (not refunded on revert)
...
- Block accounting: adds 26,000 to `block_regular_gas_used`, adds 245,000 to `block_state_gas_used`
...
### Block Throughput
At 500M payment lane gas limit (applying bottleneck model `gas_used = max(block_regular_gas, block_state_gas)`):

**New account transfers only** (state-heavy):
- Per tx: 26k regular gas + 245k state gas
- Bottleneck: state gas (245k > 26k), so 500M / 245k ≈ **2,040 transfers/block ≈ 4,080 TPS**
- Still better than TIP-1000 single-dimension (500M / 271k ≈ 1,845/block ≈ 3,690 TPS) because regular gas capacity isn't wasted

**Existing account transfers only** (no state gas):
- Per tx: ~24k regular gas, 0 state gas
- Bottleneck: regular gas, so 500M / 24k ≈ **21,000 transfers/block ≈ 42,000 TPS**

**Mixed workload** (the common case):
- State gas from new accounts and regular gas from existing accounts fill independent dimensions
- A block with 1,000 new account transfers (245M state gas) can simultaneously fit ~10,500 existing account transfers (252M regular gas) — both dimensions under 500M
- **Total: ~11,500 transfers/block in a mixed block** vs ~1,845 in TIP-1000 where all gas competed in one dimension
...
3. **Protocol Block Limits**: Block gas (`max(block_regular_gas, block_state_gas)`) MUST NOT exceed applicable limit:
   - General transactions: `general_gas_limit` (25M target, currently 30M)
   - Payment lane transactions: `payment_lane_limit` (500M)
4. **Reservoir Model**: Gas accounting MUST use the reservoir model — `gas_left` and `state_gas_reservoir` initialized from `tx.gas`, with state gas drawing from reservoir first
5. **GAS Opcode**: The `GAS` opcode MUST return `gas_left` only (excluding `state_gas_reservoir`)
6. **Reservoir Passing**: The `state_gas_reservoir` MUST be passed in full to child frames (no 63/64 rule). Unused reservoir MUST be returned to parent on child completion
7. **Exceptional Halt**: On exceptional halt, both `gas_left` and `state_gas_reservoir` MUST be set to zero
8. **Regular Gas Component**: Storage creation operations MUST charge regular gas for computational overhead (writing, hashing)
9. **Total Cost**: Transaction cost MUST equal `(regular_gas + state_gas) × (base_fee_per_gas + priority_fee)`
10. **Gas Split**: Storage creation operations MUST split cost into regular gas (computational) and state gas (permanent burden)
11. **Hot vs Cold**: Hot SSTORE (non-zero → non-zero) has NO state gas component; cold SSTORE (zero → non-zero) has both
12. **Refund via Counter**: SSTORE slot restoration refunds MUST use `refund_counter`, not direct gas decrements
13. **Revert Behavior**: Account creation state gas MUST be consumed even if the frame reverts
...
| Storage creation gas counts toward protocol limits | Yes | No (via reservoir + bottleneck model) |
...
| Block gas accounting | Single counter | Bottleneck: `max(block_regular_gas, block_state_gas)` |

---

# Alignment with EIP-8037

This TIP adopts the reservoir model and multidimensional gas metering from [EIP-8037](https://eips.ethereum.org/EIPS/eip-8037), with the following Tempo-specific differences:
...
| Quantization | Top-4 significant bits with offset for `cost_per_state_byte` | Not applicable (fixed costs) |

The core EVM mechanism — reservoir model, `GAS` opcode semantics, block-level bottleneck accounting, SSTORE refund/revert behavior, and receipt semantics — is identical, minimizing implementation divergence from upstream.
...
3. **Better throughput for mixed workloads**: ~11,500 transfers/block in mixed new+existing vs ~1,845 in TIP-1000 (~6x improvement); existing-only transfers reach ~21,000/block
