...
- On child **exceptional halt**, only `gas_left` is consumed (zeroed); the `state_gas_reservoir` is not consumed, it is returned to the parent frame or preserved at the top level for refund.
