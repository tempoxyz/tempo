...
| Event | `gas_left` | `state_gas_reservoir` |
|-------|------------|-----------------------|
| Regular gas charge | Deducted | Unchanged |
| State gas charge | Deducted only if reservoir exhausted | Deducted first |
| `GAS` opcode | Returned | Not included |
| Child frame entry | Passed via 63/64 rule | Passed in full (no 63/64) |
| Child success | Remaining returned to parent | Remaining returned to parent |
| Child revert | State gas restored to parent reservoir | State gas restored to parent reservoir |
| Child exceptional halt | Consumed (zeroed) | Restored to parent reservoir |
| Top-level exceptional halt | Attributed to `execution_regular_gas_used`, zeroed | Preserved for refund |
