...
1. **`GAS` opcode inaccuracy**: The `GAS` opcode would return remaining gas from `tx.gas` minus all gas consumed (execution + storage), which doesn't reflect the actual execution budget. A transaction with a high gas limit that has used 15.9M execution gas with a 16M EIP-7825 per-transaction gas limit would see `GAS` report millions of gas remaining, but OOG after just ~100k more execution gas.
...
On child **revert** or **exceptional halt**, all state gas consumed by the child is restored to the parent's `state_gas_reservoir`. State gas represents the cost of permanent state growth; since state changes are rolled back on failure, no state was actually grown, so state gas is not spent. This is consistent with EIP-8037's principle that state gas pays only for state that persists.
...
15. **Execution Gas Floor**: The execution gas component of each storage creation operation MUST be at least the pre-TIP-1000 (standard EVM) cost for that operation (SSTORE: 20,000, account creation: 25,000, CREATE base: 32,000, code deposit: 200/byte)
16. **EIP-7702 Delegation**: Each EIP-7702 authorization MUST charge 25,000 execution gas + 225,000 storage creation gas (250,000 total). Authorizations with `auth.nonce == 0` MUST additionally charge the account creation cost (25,000 exec + 225,000 storage)
17. **Precompile Consistency**: All precompile storage operations MUST use the same gas accounting path as standard EVM SSTORE, inheriting the execution/storage gas split automatically
18. **Keychain Authorization**: Keychain `authorize_key` intrinsic gas MUST split SSTORE costs using the same exec/storage ratio as standard EVM SSTOREs (20,000 exec + 230,000 storage per new slot)
...
| Cold SSTORE pricing | 250,000 gas | 250,000 gas (20,000 regular + 230,000 state) |
| Account creation pricing | 250,000 gas | 250,000 gas (25,000 regular + 225,000 state) |
