...
# Subcall Gas Propagation

Two runtime counters track gas during execution:

- **`gas_remaining`**: total gas (regular + storage creation), initialized to `tx.gas_limit`. Ensures the user is never charged more than authorized.
- **`regular_gas_remaining`**: regular gas only, initialized to `max(tx.gas_limit, max_transaction_gas_limit)`. Enforces protocol limits. The `max` ensures regular gas is not artificially constrained below the protocol limit when the user sets a high `gas_limit` for storage-heavy transactions.

Both counters are **global**: always propagated to subcalls and always returned to parent. They are never reverted.

- **Success**: child's remaining gas returned to parent on both counters, refunds propagated.
- **Revert**: child's remaining gas returned to parent on both counters, state changes rolled back. Gas consumed by reverted storage creation operations is permanently lost (user pays via `gas_remaining`) but does not reduce `regular_gas_remaining` (doesn't count against protocol limits).
- **Halt (OOG on `regular_gas_remaining`)**: protocol execution limit exhausted. Subcall halts and all of the subcall's `gas_remaining` is also consumed — both counters lose the subcall's gas.
- **Halt (OOG on `gas_remaining`)**: user's total gas budget exhausted. Subcall halts but `regular_gas_remaining` is NOT fully consumed — regular gas capacity remains.
- **Halt** (invalid opcode, stack error): child's gas fully consumed on both counters, nothing returned.

---

# Intrinsic Gas

Intrinsic gas is split between regular and storage creation:

- Base cost (21,000), calldata costs, access list warming: **regular gas** (counts against protocol limits)
- CREATE transaction account creation: 5,000 regular + 245,000 storage creation gas
- Value transfer to new account: 5,000 regular + 245,000 storage creation gas

Only the regular portion of intrinsic gas is deducted from `regular_gas_remaining`.
