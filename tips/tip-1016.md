...
2. **New account throughput penalty**: TIP-20 transfer to new address costs ~300,000 gas total (~55k regular + 245k state) vs ~50,000 gas to existing. At 500M payment lane gas limit:
   - Without exemption (single dimension): only ~1,700 new account transfers/block = ~3,400 TPS
   - With reservoir model (block limits apply to regular gas only): ~9,090 new account transfers/block = ~18,200 TPS
...
(100) plus `sstore_set_without_load_cost` (4,900), which together equal the TIP-1060 residual
(`SSTORE_SET_COST = 5,000`). When the slot is cold, the existing Berlin cold-slot access charge
(`GAS_COLD_SLOAD = 2,100`) is retained on top of that write component, for a total of 7,100
...
| Key slot write (SSTORE) | 4,900 | 245,000 | Zero-to-non-zero write component only; cold-slot access charged separately |
| Per spending limit (SSTORE × N) | 4,900 × N | 245,000 × N | Zero-to-non-zero write component only per token limit; cold-slot access charged separately |
| Buffer (TSTORE, keccak, event) | 2,000 | 0 | Computational overhead |

**Total per authorization:** ~12,000 + 4,900 × N regular gas, 245,000 × (1 + N) state gas.
...
- State gas: 245,000 (the full creditable portion; EIP-8037 equivalent: `32 × cost_per_state_byte`)
- Regular gas: the write component `sstore_set_without_load_cost` (EIP-8037 equivalent: 2,800; Tempo: 4,900)

The refund mechanism is identical to EIP-8037. The numeric values differ because Tempo uses fixed pricing (see Storage Gas Operations table) rather than EIP-8037's dynamic `cost_per_state_byte`. The full write component is refunded; the net regular cost after refund is the access charges that are not returned (2,100 cold-slot access + 2 × 100 warm access = 2,300). Refunds use `refund_counter` rather than direct gas accounting decrements, so that reverted frames do not benefit from the refund.
...
- **Total**: ~55,000 regular gas + 245,000 state gas = ~300,000 gas
- User must authorize: `gas_limit >= 300,000`
- Counts toward block limit: ~55,000 regular gas
...
- Block accounting: adds ~55,000 to `block_regular_gas_used` (state gas is exempt from block limits)
...
- **New account transfers**: ~55k regular gas each → ~9,090 transfers/block ≈ 18,200 TPS
...
15. **Regular Gas Floor**: The regular gas component of each storage creation operation MUST be at least the pre-TIP-1000 (standard EVM) cost for that operation (account creation: 25,000, CREATE base: 32,000, code deposit: 200/byte), except SSTORE, whose regular component is the TIP-1060 residual (`SSTORE_SET_COST = 5,000`, i.e. 4,900 write component + 100 warm access) so that clean creations are priced with the same numbers as TIP-1060
...
18. **Keychain Authorization**: Keychain `authorize_key` intrinsic gas MUST split SSTORE costs using the same regular/state ratio as standard EVM SSTOREs (4,900 regular + 245,000 state per new slot, access charges separate)
