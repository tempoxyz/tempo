...
- The reservoir is passed **in full** to child frames (no 63/64 rule). On child completion, reservoir return depends on outcome — see [Subcall Gas Return on Completion](#subcall-gas-return-on-completion).
- On **top-level exceptional halt** (outermost frame), both `gas_left` and `state_gas_reservoir` are set to zero (all gas consumed), consistent with existing EVM out-of-gas semantics. This is not applied to system transactions.
- On **subcall revert or halt**, the child's `state_gas_spent` is refilled into the parent's reservoir (state gas is preserved when state changes are rolled back). See [Subcall Gas Return on Completion](#subcall-gas-return-on-completion).
...
## Subcall Gas Return on Completion

When a child frame (CALL, CREATE, etc.) completes, gas is returned to the parent frame depending on the outcome. The key principle is: **state gas must be preserved when state changes are rolled back**, while regular gas follows standard EVM behavior.

### Success

On successful completion, state changes persist, so state gas remains consumed:

```python
parent.gas_left += child.gas_left
parent.state_gas_reservoir += child.state_gas_reservoir
# child.state_gas_spent is consumed (state persists)
```

### Revert

On REVERT, state changes are rolled back. Regular gas behavior is unchanged (remaining `gas_left` is returned to parent). Additionally, the reservoir is refilled with the child's `state_gas_spent` to preserve state gas that is no longer needed:

```python
parent.gas_left += child.gas_left
parent.state_gas_reservoir += child.state_gas_reservoir + child.state_gas_spent
```

### Exceptional Halt (OOG, invalid opcode, etc.)

On exceptional halt, state changes are rolled back. Regular gas behavior is unchanged (all `gas_left` is consumed, nothing returned). The reservoir is refilled with the child's `state_gas_spent` to preserve state gas:

```python
# child.gas_left is consumed (not returned) — standard halt behavior
parent.state_gas_reservoir += child.state_gas_reservoir + child.state_gas_spent
```

### Rationale

State gas pays for permanent storage burden. When a child frame reverts or halts, its state changes are rolled back — no permanent storage was created. If state gas were not refilled, it would be "fake spent": consumed without any corresponding state growth. Refilling the reservoir ensures state gas is only permanently consumed when state actually persists.

The expression `child.state_gas_reservoir + child.state_gas_spent` reconstructs the full state gas budget that was available to the child. When state gas charges exceed the reservoir, the excess "spills" into `gas_left`. On revert/halt, this spill is recovered back into the parent's reservoir rather than being lost as regular gas.

### Example

Parent frame before call: `gas_left = 2M`, `state_gas_reservoir = 2M`

Parent issues subcall with `gas_left = 1M` (via 63/64 rule), `state_gas_reservoir = 2M` (passed in full).

Parent retains: `gas_left = 1M`, `state_gas_reservoir = 0`.

During subcall execution:
- 0.1M regular gas consumed → `gas_left = 0.9M`
- 2.3M state gas consumed → reservoir first (2M → 0), then 0.3M spills from `gas_left` → `gas_left = 0.6M`
- Child state: `gas_left = 0.6M`, `state_gas_reservoir = 0`, `state_gas_spent = 2.3M`

**On REVERT:**

```
returned gas_left     = 0.6M  (standard revert: remaining gas returned)
returned reservoir    = 0 + 2.3M = 2.3M  (reservoir + state_gas_spent)

parent.gas_left       = 1M + 0.6M = 1.6M
parent.reservoir      = 0 + 2.3M  = 2.3M
```

Net effect: 0.1M regular gas consumed (CPU work). All 2.3M state gas preserved in reservoir. The 0.3M that spilled from `gas_left` into state gas is recovered back into the reservoir.

**On exceptional halt:**

```
returned gas_left     = 0    (standard halt: all gas consumed)
returned reservoir    = 0 + 2.3M = 2.3M  (reservoir + state_gas_spent)

parent.gas_left       = 1M + 0 = 1M
parent.reservoir      = 0 + 2.3M = 2.3M
```

Net effect: 1M regular gas consumed (all gas_left given to child is lost). All 2.3M state gas preserved in reservoir.
...
7. **Reservoir Passing**: The `state_gas_reservoir` MUST be passed in full to child frames (no 63/64 rule)
8. **Exceptional Halt (top-level)**: On top-level exceptional halt, both `gas_left` and `state_gas_reservoir` MUST be set to zero
9. **Subcall Revert/Halt Reservoir Refill**: On subcall revert or exceptional halt, the child's `state_gas_spent` MUST be refilled into the parent's `state_gas_reservoir` (preserving state gas when state changes are rolled back)
10. **Regular Gas Component**: Storage creation operations MUST charge regular gas for computational overhead (writing, hashing)
11. **Total Cost**: Transaction cost MUST equal `(regular_gas + state_gas) × (base_fee_per_gas + priority_fee)`
12. **Gas Split**: Storage creation operations MUST split cost into regular gas (computational) and state gas (permanent burden)
13. **Hot vs Cold**: Hot SSTORE (non-zero → non-zero) has NO state gas component; cold SSTORE (zero → non-zero) has both
14. **Refund via Counter**: SSTORE slot restoration refunds MUST use `refund_counter`, not direct gas decrements
15. **Revert Behavior**: Account creation state gas MUST be consumed even if the parent frame reverts
