...
### Precompile Storage Operations

All Tempo precompile storage operations (TIP-20, AccountKeychain, NonceManager, StablecoinDex, TipFeeManager, TIP403Registry, ValidatorConfig) route through the same EVM `SSTORE` gas path via `EvmPrecompileStorageProvider`. They inherit the same execution/storage gas split as standard EVM operations:

| Precompile Operation | Underlying EVM Op | Execution Gas | Storage Gas |
|---------------------|-------------------|---------------|-------------|
| TIP-20 balance write (new account) | Cold SSTORE | 20,000 | 230,000 |
| TIP-20 balance write (existing) | Hot SSTORE | 2,900 | 0 |
| TIP-20 allowance write (new) | Cold SSTORE | 20,000 | 230,000 |
| TIP-20 token initialization (`set_code`) | Code deposit (1 byte) | 200 | 2,300 |
| TIP-20 token init storage (name, symbol, etc.) | Cold SSTORE × N | 20,000 × N | 230,000 × N |
| TIP-20 rewards state (per new slot) | Cold SSTORE | 20,000 | 230,000 |
| Keychain `authorize_key` (key slot) | Cold SSTORE | 20,000 | 230,000 |
| Keychain spending limit (per token) | Cold SSTORE | 20,000 | 230,000 |
| 2D nonce increment (new key) | Cold SSTORE | 20,000 | 230,000 |
| Expiring nonce writes | Hot SSTORE | 2,900 | 0 |
| StablecoinDex order creation (per slot) | Cold SSTORE | 20,000 | 230,000 |
| TipFeeManager preferences (new) | Cold SSTORE | 20,000 | 230,000 |
| TipFeeManager AMM pool state (new) | Cold SSTORE | 20,000 | 230,000 |
| TIP-403 policy creation (per slot) | Cold SSTORE | 20,000 | 230,000 |

**Note on expiring nonces (TIP-1009):** Expiring nonce writes use `WARM_SSTORE_RESET` (2,900 gas) with zero storage creation gas because they are ephemeral — entries are evicted from a fixed-size circular buffer within 30 seconds. They do not contribute to permanent state growth.
...
- Precompile operations inherit gas splits automatically because they use the same `sstore_dynamic_gas` code path as standard EVM SSTORE
