...
9. **Uncapped `tx.gas_limit`**: EIP-7825 `max_transaction_gas_limit` MUST NOT apply to `tx.gas_limit` — users may set `gas_limit` above 16M for storage-heavy transactions
...
- **Success/Revert**: remaining gas returned to parent on both counters.
- **Halt (OOG on `regular_gas_remaining`)**: both counters lose the subcall's gas.
- **Halt (OOG on `gas_remaining`)**: `regular_gas_remaining` is NOT consumed.
...
| Max transaction gas limit (EIP-7825) | 30M | Can reduce to 16M; only limits regular gas, not `tx.gas_limit` |
