...
**Gas incentive**: When validators do not fully utilize their subblock allocation, the proposer may reclaim the unused gas via a **gas incentive section**. Transactions placed in this section — including non-payment transactions — are not bound by `general_gas_limit` and instead draw from the reclaimed subblock budget. This means the 30M general gas limit acts as a **soft cap** in the non-shared section; total non-payment gas in a block may exceed 30M when subblock capacity goes unused.
:::
...
| General gas limit | 30,000,000 gas/block | Soft cap for non-payment transactions in non-shared section |
...
5. **Block Gas Validity**: A block MUST be invalid if:
   - Payment lane transactions exceed the payment lane gas limit
   - General transactions in the **non-shared section** exceed the `general_gas_limit`

6. **Gas Incentive Budget**: Transactions in the gas incentive section (including non-payment) draw from unused subblock gas. A block MUST be invalid if gas incentive usage exceeds the reclaimable subblock budget.
...
7. **Gas incentive overflow**: Verify that non-payment transactions can exceed `general_gas_limit` via the gas incentive section when subblock gas is unused
8. **Gas incentive cap**: Verify that gas incentive usage cannot exceed the reclaimable subblock budget
