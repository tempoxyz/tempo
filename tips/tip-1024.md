...
When multiple orders exist at the same tick, `quote()` rounds once against the aggregate while `swap()` rounds per-order, causing deterministic price divergence even with no intervening state changes. This is surprising for integrators and makes tasks like swap routing more complicated, since quoted prices cannot be trusted as exact execution prices.
...
3. In `Simulate` mode, the engine MUST:
   ...
   - Apply the same rounding direction at every conversion step (see [Rounding rules](#rounding-rules)).
   ...
   - Enforce isolation in the engine itself, not only at the dispatcher: state-mutating operations MUST be unreachable in `Simulate` mode (e.g. via a read-only state handle, or a mode guard at every write site). A dispatcher-level mode flag alone is insufficient.
...
5. API compatibility:
   ...
   - Returned values from the orderbook fill engine are semantically identical, except quote precision now matches swap precision exactly for the same state snapshot. Parity is scoped to the fill math (tick/order traversal, rounding, and fill amounts); caller-level and token-specific constraints (e.g., TIP-20 transfer fees, callback validation) are outside this guarantee.
...
8. Performance and complexity:
   ...
   - `quote()` traversal is bounded by the swap size and number of ticks crossed, the same bound that applies to `swap()`. This is an acceptable cost since `quote()` now mirrors `swap()` exactly.

### Rounding rules

`Simulate` and `Execute` MUST produce bit-identical results, applying these directions at every order boundary:

| Path | Conversion | Direction |
| --- | --- | --- |
| Bid fill | `base → quote`, taker output | **Down** (taker receives less) |
| Ask fill | `base → quote`, maker settlement | **Up** (maker receives more) |
| Exact-out, bid leg | `quote → base`, taker input | **Up** (taker pays more) |
| Exact-in cross-leg | `quote → base`, taker input | **Down** (conservative budget) |

Multi-tick exact-out paths MUST round each per-order conversion independently. Aggregating quote amounts across orders or ticks before converting is forbidden: it collapses per-order rounding and reintroduces the drift this TIP eliminates.
...
2. Quote/swap deterministic parity (fill math only):
   - For any fixed state snapshot and identical inputs, `quote()` MUST produce the same fill amounts and price progression as the orderbook fill engine of `swap()` before state writes are applied. This means identical tick/order traversal sequence, identical rounding direction at every conversion step (per the [Rounding rules](#rounding-rules)), and identical terminal fill amounts. Caller-level and token-specific constraints (e.g., TIP-20 transfer fees, callback validation) are outside this guarantee.
