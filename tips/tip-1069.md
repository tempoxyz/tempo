...
`msg.sender` of `transferStealth` is public. Senders who do not want
their known identity linked to a payment SHOULD pay from a stealth
address they themselves control rather than from a labelled wallet.
This is the symmetric form of the recipient sweep — same primitive,
applied to outgoing payments.
...
code) SHOULD route outgoing `transferStealth` calls through one of
those balances. The on-chain `msg.sender` is then a fresh stealth
...
balance SHOULD first call `transferStealth` to themselves (using their
own payment code and a fresh ephemeral key), then issue the outbound
payment from the resulting stealth address. The cost is one extra
transaction; the benefit is that the outbound `msg.sender` is no
longer the sender's labelled wallet.
