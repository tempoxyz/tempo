...
// Derive the account address using the canonical Account Identity formula.
...
- An initialized configurable account registered as an admin key MAY authorize another key for its parent account. The authorization's `account` MUST equal the parent address, and the configurable signature's `account` MUST equal the admin key address.
