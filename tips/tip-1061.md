...
- **Bootstrap.** Used for a configurable account's first transaction, before its configuration exists in state.
...
The bootstrap form is used only for a configurable account's first transaction. It carries the initial configuration so validation can derive the account and establish its owner state.
...
The initialized form is used once the account configuration exists. It identifies the account so validation can authorize approvals against the current stored configuration.
...
A bootstrap transaction is the account's first transaction. It derives the account from its initial owner configuration and establishes that configuration before its calls execute.
