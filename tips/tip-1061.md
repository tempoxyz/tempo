...
/// Domain prefix for configurable account address derivation.
pub const CONFIGURABLE_ACCOUNT_ADDRESS_DOMAIN: &[u8] = b"tempo:configurable:account";

/// Domain prefix for configurable account owner signatures.
pub const CONFIGURABLE_ACCOUNT_SIGNATURE_DOMAIN: &[u8] = b"tempo:configurable:signature";

/// Maximum owner count that leaves room for fresh nonce creation and transaction overhead.
pub const MAX_CONFIGURABLE_ACCOUNT_OWNERS: usize = 50;
...
The account binding prevents an owner signature from being reused for another configurable account or as an ordinary primitive signature. The configuration version prevents an owner signature from becoming valid again after later owner updates.
...
- Bootstrap MUST store configuration version `0`. Every successful `updateAccountConfig` MUST increment the version by one and MUST revert with `InvalidConfig` on `uint64` overflow.
...
- `getAccountConfig` MUST return the current version, threshold, and owners or revert with `NotConfigurableAccount` or `InvalidConfig`.
...
```text
configurable_account_digest(key_authorization.signature_hash(), account, config_version)
```
...
- A configurable account MUST NOT be registered as an access key for another account.
...
direct_configurable_account_surcharge =
...
The direct subtraction accounts for the secp256k1 transaction signature already included in ordinary intrinsic gas. A registered 1-of-1 secp256k1 configurable account therefore adds 8,400 gas; a registered 2-of-2 adds 17,700 gas.
...
- Every registered configurable account node MUST add `registered_config_validation` for its complete stored configuration, including nested nodes and RPC simulation.
- The 3,000 gas subtraction MUST apply once at the outer account node, never to nested nodes or key authorization sidecars.
- Direct configurable authorization MUST NOT add the account keychain's 900 gas processing buffer.
...
// Ref: Authorization formula
digest = configurable_account_digest(tx.signature_hash(), account, config.version)
...
// Ref: Authorization formula
current_version = getAccountConfig(account).version
...
// Ref: Authorization formula
next_config = getAccountConfig(account)
...
- **Bootstrap exclusivity.** When a configurable account signature authorizes a caller without a header, the transaction MUST carry exactly one bootstrap signature as its outer or key authorization signature. Transactions that do not use configurable account authorization do not require bootstrap. After a header exists for that account, bootstrap signatures MUST be rejected.
...
- **Signature contexts.** Configurable signatures MUST be outer, nested owner, or key authorization signatures. Bootstrap MUST be outer or authorize the outer access key.
...
- **Account keychain access.** Configurable accounts MAY own access keys but MUST NOT be registered as access keys for another account. Access keys MUST NOT replace parent owner sets.
- **Fee payer.** When `fee_payer_signature` is present, it MUST be secp256k1 and its recovered fee payer MUST NOT be a configurable account.
