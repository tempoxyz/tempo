...
- A packed account header with `threshold == 0` and `owner_count == 0` means the account is not a native multisig account.
...
interface INativeMultisig {
    ...
    /// @return isMultisig True when account has a complete native multisig config header.
...
- If the packed account header is empty, the function MUST return `false`.
- If the packed account header is complete, the function MUST return `true`.
- If the packed account header is partially initialized, the function MUST revert with `InvalidConfig()`.
...
5. require the packed account header is complete (`InvalidConfig`)
