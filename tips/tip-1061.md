...
The Tempo transaction RLP field list adds `multisig_init` as a fixed positional slot immediately after `key_authorization`:
...
Multisig field encoding:

- `InitMultisig` is encoded as `rlp([salt, threshold, owners])`.
...
- `tx.signature_hash()` is computed over the canonical trailing-canonical RLP encoding defined above.
...
- Transactions with both `key_authorization` and `multisig_init` absent produce the same signing payload as the pre-activation format.
- Transactions with `key_authorization` present and `multisig_init` absent produce the same signing payload as the pre-activation format.
...
- At and after activation, `multisig_init` is a positional slot and follows the trailing-canonical rule above.
...
29. require every recovered owner address and primitive signature type is in `multisig_init.owners`
30. sum the configured weights for the recovered owners
31. require the recovered owner weight sum is at least `multisig_init.threshold`
32. store `multisig_init` as the current config for `(signature.account, signature.config_id)`
...
4. require `configId != bytes32(0)` (`InvalidConfigId`)
5. require `multisig_accounts[msg.sender] == true` (`NotMultisigAccount`)
6. require `multisig_config_ids[msg.sender] == configId` (`InvalidConfigId`)
7. require `msg.sender == derive_multisig_account(configId)` (`InvalidAccount`)
8. require a config exists for `(msg.sender, configId)` (`ConfigNotFound`)
9. validate `owners` is non-empty (`InvalidOwner`)
10. validate `owners.length <= MAX_MULTISIG_OWNERS` (`TooManyOwners`)
11. validate every owner signature type is valid (`InvalidSignatureType`)
...
- Existing Tempo transaction encodings without `multisig_init` remain canonical: trailing-canonical encoding preserves the existing wire format for transactions where `multisig_init` is absent, whether `key_authorization` is present or not.
...
- Accept a transaction with both `key_authorization` and `multisig_init` absent (both slots stripped under trailing-canonical).
- Accept a transaction with `key_authorization` present and `multisig_init` absent (`multisig_init` slot stripped).
- Accept a transaction with `key_authorization` absent and `multisig_init` present (`key_authorization` encoded as `0x80` placeholder, `multisig_init` follows).
- Accept a transaction with both `key_authorization` and `multisig_init` present at decoding, then reject it during validation.
- Reject a transaction with a trailing `0x80` after the last non-absent slot (non-canonical).
- Reject `TempoSignature::Multisig` before this TIP activates.
- Reject any encoded `multisig_init` before this TIP activates.
- Reject a slot whose RLP shape does not match its positional type (e.g. a `SignedKeyAuthorization` in the `multisig_init` slot, or an `InitMultisig` in the `key_authorization` slot).
- Reject a primitive or keychain transaction whose `multisig_init` slot is present.
- Confirm `tx.signature_hash()` is unchanged for transactions without `multisig_init`.
- Confirm `tx.fee_payer_signature_hash(sender)` is unchanged for transactions without `multisig_init`.
- Confirm `tx.signature_hash()` changes when `multisig_init` changes from absent to present or vice versa.
- Confirm `tx.fee_payer_signature_hash(sender)` changes when `multisig_init` changes from absent to present or vice versa.
...
- Confirm `tx.signature_hash()` changes when only `multisig_init.salt` changes.
