...
- Before this TIP activates, Tempo transactions MUST use the pre-activation transaction RLP field list.
- At and after activation, Tempo transactions MUST use the explicit nullable `key_authorization` and `multisig_init` fields.
...
Bootstrap claims unused derived account state. The derived account MAY have a nonzero balance before bootstrap, but it MUST NOT have code, delegation code, or consumed nonce state.
...
14. require `expected_account` has no code or EIP-7702 delegation code
15. require `expected_account` has no consumed transaction nonce state
16. require `signature.account == expected_account`
17. compute `multisig_digest` using `tx.signature_hash()`, `signature.account`, and `signature.config_id`
18. verify each primitive owner signature over that digest
19. require recovered owner addresses are strictly ascending
20. require every recovered owner is in `multisig_init.owners`
21. sum the configured weights for the recovered owners
22. require the recovered owner weight sum is at least `multisig_init.threshold`
23. store `multisig_init` as the current config for `(signature.account, signature.config_id)`
24. set `multisig_config_ids[signature.account] = signature.config_id`
25. set `multisig_accounts[signature.account] = true`
26. emit `MultisigConfigInitialized`
27. commit the initial config, canonical config ID, account marker, and initialization event as transaction-level state
28. execute the transaction from `signature.account`
...
5. require `multisig_config_ids[signature.account] == signature.config_id`
6. load the stored config for `(signature.account, signature.config_id)`
7. compute `multisig_digest` using `tx.signature_hash()`, `signature.account`, and `signature.config_id`
8. verify each primitive owner signature over that digest and recover the owner address
9. require recovered owner addresses are strictly ascending
10. require every recovered owner is in the current owner set
11. sum the current configured weights for the recovered owners
12. require the recovered owner weight sum is at least the current threshold
13. authorize execution from `signature.account`
...
- Future native multisig support for `tempo_authorization_list` MUST define stateful config validation.
...
Future access key support requires a separate design where a single primitive key cannot impersonate the multisig account's `msg.sender`.
...
- Preserve the stored initial config, canonical config ID, native multisig account marker, nonce consumption, and initialization event when bootstrap validation succeeds but the subsequent EVM call reverts.
- Do not write the initial config, canonical config ID, native multisig account marker, or initialization event when bootstrap validation fails.
...
- Reject the post-activation transaction RLP field list before this TIP activates.
- Reject the pre-activation transaction RLP field list at and after this TIP activates.
...
3. **Unused bootstrap account.** Bootstrap MUST be accepted only when the derived account has no code, delegation code, or consumed nonce state. A nonzero balance alone MUST NOT prevent bootstrap.

4. **Config validity.** Every stored config MUST have `1 <= owners.len() <= MAX_MULTISIG_OWNERS`, strictly ascending unique nonzero owners, nonzero owner weights, and `1 <= threshold <= sum(owners.weight)`.

5. **Owner signature validity.** Every owner approval MUST be a `PrimitiveSignature` over `multisig_digest`. Encoded `KeychainSignature` and nested `MultisigSignature` owner approvals MUST be rejected.

6. **Owner set membership.** Recovered owner addresses MUST be strictly ascending, and every recovered owner MUST be present in the current stored owner set.

7. **Threshold enforcement.** A multisig transaction MUST be rejected unless the current configured weights of valid recovered owners sum to at least the current threshold.

8. **Current-state authorization.** A multisig transaction MUST be authorized against the stored config in effect at its block position.

9. **Top-level multisig signatures.** `TempoSignature::Multisig` MUST be accepted only as the outer transaction signature.

10. **No access keys.** A transaction with `TempoSignature::Multisig` MUST be rejected when `key_authorization` is present. AccountKeychain mutating calls MUST reject when `multisig_accounts[msg.sender] == true`.

11. **Config update authority.** `updateMultisigConfig` MUST reject unless `msg.sender == derive_multisig_account(configId)`. Config updates MUST NOT accept an additional signature parameter.
