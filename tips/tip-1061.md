...
- Pools MAY index by stateless recovery but MUST revalidate affected authorizations after initialization or owner updates, including transactions that name a newly initialized account in a multisig-restricted role.
...
Registration also affects primitive-signed transactions that name a multisig account as an explicit fee payer, newly authorized access key, or authorization-list authority. Those roles gain state-dependent intrinsic gas and are rejected where this TIP forbids multisig accounts. Generic `SignedKeyAuthorization` decoders also accept the new multisig signature form, while consensus rejects it before T11 and primitive encodings remain unchanged.
