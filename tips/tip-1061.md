...
- Keychain authorization-list signatures authenticate an access key for a claimed root account. The root MUST be checked against the multisig registry and rejected if registered.
...
registry_gating_surcharge =
  ...
  - each keychain authorization-list authority
