...
/// Maximum encoded byte length of one primitive owner approval.
...
An account may store up to 58 owners, but a transaction may submit at most 8 approvals. Primitive approvals have a byte limit. Configurable account approvals may nest once: the outer account is depth 1 and its initialized owner account is depth 2.
...
- Primitive approvals MUST NOT exceed `MAX_CONFIGURABLE_ACCOUNT_PRIMITIVE_SIGNATURE_BYTES`; nested approvals are bounded recursively by the approval-count and nesting-depth limits.
...
- Nested owner approvals and account keychain signatures MUST use the initialized encoding.
- Key authorization signatures MUST use initialized encoding unless the authorization supplies `init` for a top-level bootstrap.
...
- When `key_authorization.signature` carries `init`, the outer V2 account keychain signature MUST use the key authorized by that same authorization.
...
At T11, `SignedKeyAuthorization.signature` expands from `PrimitiveSignature` to `TempoSignature`. Its position and RLP representation remain unchanged, so existing primitive authorizations remain byte-identical.
...
- The authorization MUST name the target account. Its signature MUST name or derive the same account and use the digest above.
...
Wallets, SDKs, RPCs, and indexers need to expose configurable accounts across address derivation, signing, transaction decoding, and configuration history.

Rules:

- Tooling MUST support the T11 signature encodings, signed key authorization shape, precompile ABI and events, and account keychain type `3`; address derivation SHOULD use `deriveAccount`.
...
- **Stable identity.** Bootstrap MUST satisfy `tx.from == derive_configurable_account(init)`. `updateAccountConfig` MUST NOT change the address.
