...
- Configs with `threshold == 0` or `owner_count == 0` are invalid and MUST NOT be created.
...
interface INativeMultisig {
    ...
    /// @dev Reverts if account is multisig and its stored config is missing.
...
- If `multisig_accounts[account] == true` and no config exists for `account`, the function MUST revert with `ConfigNotFound()`.
...
5. require a config exists for `msg.sender` (`ConfigNotFound`)
...
- Confirm `getMultisigConfig` reverts with `ConfigNotFound()` when a marked native multisig account's config is missing.
