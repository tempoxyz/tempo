...
31. store `signature.init` as the current config for `(signature.account, signature.config_id)`
32. set `multisig_config_ids[signature.account] = signature.config_id`
33. set `multisig_accounts[signature.account] = true`
34. emit `MultisigInitialized(signature.account, signature.config_id)`
35. consume the protocol nonce for `signature.account`
36. commit the initial config, canonical config ID, account marker, and protocol nonce consumption as transaction-level state
