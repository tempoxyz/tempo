...
/// Maximum owner count whose bootstrap storage charge stays below the transaction gas cap.
pub const MAX_CONFIGURABLE_ACCOUNT_OWNERS: usize = 59;
...
An account may store up to 59 owners, but a transaction may submit at most 8 approvals. Each approval has a byte limit. Configurable account approvals may nest once: the outer account is depth 1 and its initialized owner account is depth 2.

Rules:

- Configurations MUST contain 1 to 59 unique, nonzero, address-sorted owners. Weights MUST be nonzero and total at most 255.
...
- `signatures` MUST contain 1 to `MAX_CONFIGURABLE_ACCOUNT_SIGNATURES` approvals, each using the existing `TempoSignature` byte encoding. Decoders MUST reject an empty list before intrinsic gas is computed.
...
Initialized transactions are authorized against the account's configuration at their block position. The account is the sender for the full call batch. Configuration updates are visible to later calls in the batch but do not change the transaction's authorization.
...
- Reconstructing the initial configuration MUST decode `init` from the bootstrap transaction signature.
