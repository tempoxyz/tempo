...
- RLP integer fields use canonical RLP integer encoding. This applies to `threshold`, `signature_type`, and `weight`.
...
- After sender recovery for any transaction signature type, if `multisig_accounts[recovered_sender] == true`, the outer signature MUST be `TempoSignature::Multisig` or `TempoSignature::Keychain`.
...
16. compute `expected_config_id` from `signature.init`
...
24. require `signature.account == expected_account`
25. require no config exists for `(signature.account, signature.config_id)`
26. compute `multisig_digest` using `tx.signature_hash()`, `signature.account`, and `signature.config_id`
27. decode each owner signature byte string as `PrimitiveSignature`, verify it over that digest, and recover the owner address and primitive signature type
28. require recovered owner addresses are strictly ascending
29. require every recovered owner address and primitive signature type is in `signature.init.owners`
30. sum the configured weights for the recovered owners
31. require the recovered owner weight sum is at least `signature.init.threshold`
32. store `signature.init` as the current config for `(signature.account, signature.config_id)`
33. set `multisig_config_ids[signature.account] = signature.config_id`
34. set `multisig_accounts[signature.account] = true`
35. emit `MultisigInitialized(signature.account, signature.config_id)`
...
6. `INativeMultisig` precompile calls
7. storage-creating state gas under the active Tempo gas schedule

### Access Keys

Native multisig accounts support AccountKeychain access keys through the AccountKeychain precompile. The multisig quorum fills the root key role: anything the existing protocol restricts to the root key or a TIP-1049 admin access key requires a multisig-signed transaction for a native multisig account.

Native multisig accounts do not support TIP-1049 admin access keys. An admin key is single-key authority over the keychain, which would bypass the quorum for key management. The TIP-1049 root-or-admin caller rule therefore collapses to quorum-only for native multisig accounts.
...
Access keys for native multisig accounts are registered exclusively through `authorizeKey` precompile calls in `tx.calls` of a multisig-signed transaction. There is no same-transaction authorize-and-use flow for native multisig accounts.

#### Keychain Transactions

- `TempoSignature::Keychain` with `user_address` equal to a native multisig account is valid once `multisig_accounts[user_address] == true`.
...
- AccountKeychain mutating calls (`authorizeKey` overloads, `revokeKey`, `updateSpendingLimit`, `setAllowedCalls`, `removeAllowedCalls`, `burnKeyAuthorizationWitness`) are valid when `msg.sender` is a native multisig account and the outer transaction signature is `TempoSignature::Multisig`.
- AccountKeychain mutating calls from a native multisig account MUST be rejected when the transaction executes under an access key. This follows from the existing root-or-admin caller rule: a non-admin transaction key is rejected, and native multisig accounts have no admin keys.
- `authorizeAdminKey` MUST be rejected when `msg.sender` is a native multisig account.
...
- Witness burns from a native multisig account submitted through an access key MUST be rejected. The TIP-1049 admin-key burn path never applies because native multisig accounts have no admin keys.
...
6. require `multisig_accounts[msg.sender] == true` (`NotMultisigAccount`)
...
12. validate every owner signature type is valid (`InvalidSignatureType`)
13. validate every owner address is nonzero (`InvalidOwner`)
14. validate every owner weight is nonzero (`InvalidWeight`)
15. compute `total_weight = sum(owners.weight)`
16. validate `total_weight <= u32::MAX` (`InvalidWeight`)
17. validate `1 <= threshold <= total_weight` (`InvalidThreshold`)
18. validate `owners` is strictly ascending by `owner` (`DuplicateOwner` / `InvalidOwnerOrder`)
19. replace the stored current config under the same `(msg.sender, configId)`
20. emit `MultisigConfigUpdated`
...
- Every access key registration requires a quorum-signed `authorizeKey` call. No single owner can mint an access key.
...
- The quorum retains exclusive control of the trust anchors: access keys cannot authorize other keys, mutate keychain state, burn witnesses, or call `updateMultisigConfig`. Compromising an access key never compromises the owner set or threshold.
- TIP-1049 admin access keys are excluded for the same reason: an admin key is standing single-key authority over the keychain, which would let one key mint unrestricted keys and bypass the quorum for key management.
- The quorum can revoke any key at any time via `revokeKey`.

The transaction-level `key_authorization` field stays rejected because its signature is a single primitive signature that must recover to the transaction sender, which is impossible for a derived multisig address. Registration through `authorizeKey` calls needs no new signature form: authorization comes from the outer multisig transaction signature. A multisig-shaped key authorization signature (threshold owner approvals over the key authorization digest) would enable same-transaction authorize-and-use and is left to a future TIP.
...
- AccountKeychain semantics for existing accounts are unchanged. This TIP extends eligibility to accounts where `multisig_accounts[account] == true`, with the quorum filling the root key role through `authorizeKey` precompile calls.
...
- All multisig-signed transactions reject `key_authorization`; access keys are registered via `authorizeKey` calls from the first post-bootstrap transaction onward.
...
Quorum-only mutation:

- Accept AccountKeychain mutators (`authorizeKey`, `revokeKey`, `updateSpendingLimit`, `setAllowedCalls`, `removeAllowedCalls`) called from `tx.calls` of a multisig-signed transaction.
- Reject AccountKeychain mutators for a native multisig account when the transaction executes under an access key.
- Reject `authorizeAdminKey` when `msg.sender` is a native multisig account.
...
- Reject a TIP-1053 witness burn submitted through an access key.
...
4. **Config validity.** Every stored config MUST have a nonzero `config_id` derived from `(salt, threshold, owners)` per the rules in Multisig Identity, `1 <= owners.len() <= MAX_MULTISIG_OWNERS`, valid owner signature types, strictly ascending unique nonzero owners, nonzero owner weights, `sum(owners.weight) <= u32::MAX`, and `1 <= threshold <= sum(owners.weight)`. `salt` MAY be any 32-byte value.
...
14. **Quorum-gated keychain mutation.** AccountKeychain mutating calls and TIP-1053 witness burns for a native multisig account MUST be authorized by the multisig quorum: the outer transaction signature MUST be `TempoSignature::Multisig`. A native multisig account MUST NOT have a TIP-1049 admin access key: `authorizeAdminKey` MUST be rejected for native multisig accounts. Access keys for a native multisig account MUST NOT authorize other keys, mutate keychain state, or burn witnesses.
