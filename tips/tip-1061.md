...
- For transaction authorization, the inner digest is `tx.signature_hash()`, which includes the explicit nullable `init_multisig` field and excludes the outer transaction signature as usual.
...
`SIGNATURE_TYPE_MULTISIG` is a new Tempo signature type byte. `0x03` and `0x04` are already used by Keychain signature variants, so multisig uses the next available value in the `TempoSignature` byte-encoding namespace. This namespace is separate from EIP-7702 authorization digest magic bytes.
...
`TempoSignature::Multisig` is valid only as the outer transaction signature in this TIP. A `TempoSignedAuthorization` entry in `tempo_authorization_list` MUST reject `TempoSignature::Multisig`.
...
`init_multisig` is also part of the fee payer signing payload. `tx.fee_payer_signature_hash(sender)` MUST include the explicit `init_multisig` field, whether absent or present.

`init_multisig` MUST be absent unless the outer transaction signature is `TempoSignature::Multisig` and bootstrap validation applies.
...
```text
multisig_digest(inner_digest, account, config_id) = keccak256(
...
Each `PrimitiveSignature` in `MultisigSignature.signatures` MUST verify against `multisig_digest(tx.signature_hash(), signature.account, signature.config_id)`. Verification derives the owner address from the primitive signature using the existing Secp256k1, P256, or WebAuthn recovery rules.
...
Sender recovery is stateless. For `TempoSignature::Multisig`, sender recovery MUST return `signature.account` as the recovered transaction sender after:
...
4. computing `multisig_digest(tx.signature_hash(), signature.account, signature.config_id)`
5. verifying each primitive owner signature over that digest
6. requiring recovered owner addresses are strictly ascending

Sender recovery MUST fail if any of these checks fail.
...
14. compute `multisig_digest(tx.signature_hash(), signature.account, signature.config_id)`
15. verify each primitive owner signature over that digest
16. require recovered owner addresses are strictly ascending
17. require every recovered owner is in `init_multisig.owners`
18. sum the configured weights for the recovered owners
19. require the recovered owner weight sum is at least `init_multisig.threshold`
20. store `init_multisig` as the current config for `(signature.account, signature.config_id)`
21. set `multisig_accounts[signature.account] = true`
22. emit `MultisigConfigInitialized`
23. commit the initial config, account marker, and initialization event as transaction-level state
24. execute the transaction from `signature.account`
...
5. compute `multisig_digest(tx.signature_hash(), signature.account, signature.config_id)`
6. verify each primitive owner signature over that digest and recover the owner address
...
Replacing the stored config MUST remove any previous owner entry that is not present in the new `owners` list. Multisig transactions after the successful update MUST be authorized only against the new stored owner set, weights, and threshold.
...
Tempo transaction RLP encoding changes at this TIP's protocol activation because `key_authorization` and `init_multisig` become explicit nullable fields. This change is part of the transaction and fee payer signing payloads and therefore changes `tx.signature_hash()` and `tx.fee_payer_signature_hash(sender)` for post-activation transactions.

Existing non-multisig accounts remain eligible for AccountKeychain access keys. This TIP changes AccountKeychain behavior only for accounts where `multisig_accounts[account] == true`.
...
4. **Owner signature validity.** Every owner approval MUST be a `PrimitiveSignature` over `multisig_digest(tx.signature_hash(), signature.account, signature.config_id)`. Encoded `KeychainSignature` and nested `MultisigSignature` owner approvals MUST be rejected.
...
- Reject a primitive or keychain transaction that includes `init_multisig`.
- Confirm `tx.signature_hash()` changes when `init_multisig` changes.
- Confirm `tx.fee_payer_signature_hash(sender)` changes when `init_multisig` changes.
