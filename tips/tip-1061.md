...
pub const MAX_MULTISIG_SIGNATURES: usize =
    MAX_MULTISIG_THRESHOLD as usize;
...
- `updateMultisigConfig` MUST reject same-transaction bootstrap accounts.
...
- Bootstrap MUST store configuration version `0`. Every successful `updateMultisigConfig` MUST increment the version by one and MUST revert with `InvalidConfig` on `uint64` overflow.
...
- `getMultisigConfig` MUST return the current version, threshold, and owners or revert with `NotMultisigAccount` or `InvalidConfig`.
- `updateMultisigConfig` MUST be a direct top-level call with `msg.sender == tx.origin` and a zero keychain transaction key.
- It MUST require a complete header and valid configuration, and reject same-transaction bootstrap accounts.
- `updateMultisigConfig` MUST NOT accept a separate authorization signature.
- Configuration validation MUST return the first applicable error in this order:
  ...
  - `InvalidThreshold` when the threshold is zero or above `MAX_MULTISIG_THRESHOLD`.
...
- `getMultisigConfig` and `updateMultisigConfig` MUST return `NotMultisigAccount` for an absent header. `isMultisigAccount`, `getMultisigConfig`, and `updateMultisigConfig` MUST return `InvalidConfig` for a partial header.
- `updateMultisigConfig` MUST return `UnauthorizedCaller` for an indirect call, `msg.sender != tx.origin`, or a nonzero keychain transaction key, and `SameTransactionUpdateNotAllowed` for an account bootstrapped in the same transaction.
...
- Access keys MUST NOT call `updateMultisigConfig` for their parent.
...
// Ref: Authorization formula
current_version = getMultisigConfig(account).version
...
// Ref: Authorization formula
next_config = getMultisigConfig(account)
...
- **Stable identity.** Bootstrap MUST satisfy `tx.from == multisig_address`. `updateMultisigConfig` MUST NOT change the address.
...
- **Config update frame.** `updateMultisigConfig` MUST run in a protocol-created top-level frame for one `tx.calls` entry.
- **Config update identity.** `updateMultisigConfig` MUST require a complete multisig account header for `msg.sender`.
- **Config update bootstrap exclusion.** `updateMultisigConfig` MUST reject accounts initialized earlier in the same transaction.
- **No config update signature.** `updateMultisigConfig` MUST NOT accept an additional signature parameter.
