...
| `0x4D50500000000000000000000000000000000000` | TIP20ChannelReserve (MPP) | Removes redundant approvals for channel `open` and `topUp` reserve funding |

With this TIP activation, `StablecoinDEX` and `TIP20ChannelReserve` MUST switch from using `transfer_from` to `system_transfer_from`.
