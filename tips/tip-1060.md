...
This baseline represents the work the block performs and is left intact, with two exceptions: `SSTORE_CLEARS_SCHEDULE` is set to `0` (the clearing case is now handled by storage credits), and the TIP-1000 net-new storage creation cost is layered on top. TIP-1000 adds a 250,000-gas long-term-storage charge to a creation; this TIP lowers the always-charged portion of that charge to a 20,000-gas residual and lets a storage credit (or 230,000 additional gas) cover the rest. The baseline schedule above is "work for this block"; the TIP-1000 component layered on creations represents long-term storage.
...
When account `A` changes one of `A`'s storage slots from zero to nonzero (the *present* value before the write is zero and the *new* value is nonzero), the write is a storage creation for TIP-1060 purposes. The slot's transaction-original value is irrelevant: credit accounting keys only off the present-to-new transition. Tracking the transaction-original value would be incorrect because a creation of slot A can be funded by a credit minted by clearing an unrelated slot B; the two are not paired by slot. Every zero-to-nonzero transition consumes a credit (subject to mode), and every nonzero-to-zero transition mints one, so within-transaction recreations such as `X -> 0 -> Y` correctly mint on the `X -> 0` step and consume on the `0 -> Y` step. On each storage creation, the protocol selects exactly one of the following branches based on `storage_creation_mode[A]`:
...
Notation: `0` is the zero value; `X` is the slot's transaction-original nonzero value; `Y`, `Z` are other distinct nonzero values. Columns: **O** = original (start-of-transaction) value, **P** = present value before the write, **N** = new value. Warm access is assumed; add `COLD_SLOAD_COST` (2,100 gas) when the slot is cold. The **EVM refund** column already reflects `SSTORE_CLEARS_SCHEDULE = 0`; values that remain nonzero (19,900 / 2,800) are the preserved restore-to-original refunds. The TIP-1060 credit classification keys only on the `P -> N` transition (`nonzero -> 0` mints, `0 -> nonzero` is a creation), never on `O`.
...
| 0 | Y | 0 | clear of same-tx creation | 0      | 19,900 | **mint +1** |
| X | X | 0 | clean clear               | 2,800  | 0      | **mint +1** |
| X | Y | 0 | dirty clear               | 0      | 0      | **mint +1** |
| 0 | 0 | Y | clean creation            | 19,900 | 0      | **creation** |
| X | 0 | X | recreation to original    | 0      | 2,800  | **creation** |
| X | 0 | Y | recreation to other value | 0      | 0      | **creation** |

A `nonzero -> 0` transition mints one credit irrespective of `O` and of mode. A `0 -> nonzero` transition is a creation handled per mode in the next table; every other transition neither mints nor consumes a credit.

### Creation handling by mode (`P = 0 -> N = nonzero`)

The TIP-1000 storage-creation component is `250,000` gas, split into an always-charged `20,000`-gas residual and a `230,000`-gas creditable portion (`230,000` = one storage credit value). This is layered on top of the baseline EVM dyn gas / refund from the table above.

| Mode | Condition | Credit balance | `pending_refund_eligible_creations` | TIP-1000 component charged | Net TIP-1000 cost |
|------|-----------|----------------|--------------------------------------|----------------------------|-------------------|
| `Refund`   | always (default) | unchanged at write time | +1 | 250,000 upfront | 20,000 if a credit is available at settlement (230,000 refunded), else 250,000 |
| `Preserve` | always | unchanged | — | 250,000 | 250,000 |
| `Direct`   | `balance > 0` and `budget > 0` | −1 synchronously | — | 20,000 residual only | 20,000; budget −1, auto-switch to `Preserve` if budget reaches 0 |
| `Direct`   | `balance == 0` or `budget == 0` | unchanged | — | 250,000 | 250,000; if `budget == 0`, switch to `Preserve` |

Notes:

- A `Direct`-mode creation that consumed a credit (paying only the 20,000-gas residual) MUST NOT later receive any additional refund for the offset 230,000 gas. `Refund`-mode creations pay the full 250,000 upfront, so the 230,000-gas settlement refund is for a cost actually paid earlier in the same transaction.
...
- Because credits key only on `P -> N`, a within-transaction `X -> 0 -> Y` sequence mints on `X -> 0` and consumes (per mode) on `0 -> Y`, so the credit balance nets out while the 20,000-gas residual is charged on the creation step.
