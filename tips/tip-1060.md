...
This TIP applies to every account on Tempo. Storage credit minting, consumption, and mode selection follow the same rules whether the account is an ordinary smart contract, a precompile, or an EOA. The effective SSTORE cost for accounts that never delete and re-create storage slots is unchanged, because their `storage_credit_balance` stays at zero and `Refund` mode degenerates to charging the full TIP-1000 cost with no refund.
...
Existing contracts don't incur any negative effects unless they choose to interact with the Storage Credits precompile, and keep the default `Refund` mode and a zero balance until they delete one of their own existing storage slots. Two protocol-wide changes do, however, alter storage gas economics for every account: the pre-existing storage-clearing gas refund (nonzero-to-zero) is removed and replaced by storage credit minting, and the standard EVM refund cap is removed. A contract that previously received the immediate clearing refund now instead earns a storage credit on deletion and redeems it (in the default `Refund` mode) for a 230,000-gas refund at end-of-transaction settlement when it later creates a slot; a deletion that is not matched by a later creation in the same account yields a carried-forward credit rather than an immediate refund.
...
13. `setMode(newMode)` from account `A` MUST set `storage_creation_mode[A]` to `newMode` if `newMode ∈ {Refund, Preserve, Direct}` and MUST revert with `InvalidMode` otherwise. It MUST NOT change `storage_creation_mode` for any account other than `A`.
