...
This TIP preserves the existing SSTORE gas mechanism (EIP-2200 as amended by EIP-3529) and modifies it in only two ways. First, the storage-clearing refund is set to zero — `SSTORE_CLEARS_SCHEDULE` from EIP-3529 is set to `0` — because storage credits now cover the nonzero-to-zero case. Second, the TIP-1000 storage-creation cost is lowered from 250,000 gas to a fixed 20,000-gas residual (`GAS_STORAGE_SET = 20,000`); the remaining 230,000 gas of long-term-storage cost is paid either by consuming one storage credit or by charging an additional 230,000 gas. All other SSTORE refunds — for example the refund for setting a slot back to its original value — are preserved unchanged. Storage credit settlement is the new mechanism by which the nonzero-to-zero case affects the transaction's gas refund counter; the standard EVM refund cap (one fifth of gas used, EIP-3529) continues to apply to the ordinary gas refund counter but does not limit the storage-credit settlement refund.
...
This TIP builds on the existing SSTORE gas schedule (EIP-2200 as amended by EIP-2929 and EIP-3529, with the OSAKA fork enabled). That schedule is a function of three values for the slot being written: the *original* value (the slot's value at the start of the transaction), the *present* value (its value immediately before this write), and the *new* value being assigned. By keying off all three, it charges only for the actual work the write imposes on this block — a no-op (`present == new`) costs a warm read, a clean creation (`original == present == 0`) costs the full set cost, a dirty rewrite of an already-touched slot costs only a warm read, and restoring a slot to its original value awards a matching refund so that within-transaction churn nets out to the work actually performed.

For reference, the OSAKA-fork baseline dynamic gas and refund (with the EIP-3529 constants `SSTORE_SET = 20,000`, `WARM_SSTORE_RESET = 2,900`, `WARM_STORAGE_READ_COST = 100`, `COLD_SLOAD_COST = 2,100`, and `SSTORE_CLEARS_SCHEDULE = 4,800`) is:

```text
# dynamic gas (cold access cost added separately when the slot is cold)
sstore_dynamic_gas(original, present, new):
    gas = 0
    if new != present and present == original:        # first touch of a clean slot
        if original == 0:
            gas += SSTORE_SET  - WARM_STORAGE_READ_COST   # 19,900  (creation)
        else:
            gas += WARM_SSTORE_RESET - WARM_STORAGE_READ_COST  # 2,800  (overwrite)
    return gas   # (+ COLD_SLOAD_COST if the slot is cold; warm read otherwise)

# refund delta added to the transaction refund counter
sstore_refund(original, present, new):
    if new == present: return 0
    if present == original and new == 0:               # clean clear
        return SSTORE_CLEARS_SCHEDULE                   # 4,800
    refund = 0
    if original != 0:
        if present == 0:   refund -= SSTORE_CLEARS_SCHEDULE   # un-clear
        elif new == 0:     refund += SSTORE_CLEARS_SCHEDULE   # re-clear
    if original == new:                                 # restored to original
        refund += (SSTORE_SET - WARM_STORAGE_READ_COST)        if original == 0  # 19,900
        refund += (WARM_SSTORE_RESET - WARM_STORAGE_READ_COST) if original != 0  # 2,800
    return refund
```

This baseline represents the work the block performs and is left intact, with two exceptions: `SSTORE_CLEARS_SCHEDULE` is set to `0` (the clearing case is now handled by storage credits), and the TIP-1000 net-new storage creation cost is layered on top. TIP-1000 adds a 250,000-gas long-term-storage charge to a creation; this TIP lowers the SSTORE-gas-function portion of that charge to a 20,000-gas residual (`GAS_STORAGE_SET`, charged on clean creations only) and lets a storage credit (or 230,000 additional gas) cover the rest. The baseline schedule above is "work for this block"; the TIP-1000 component layered on creations represents long-term storage.
...
This TIP sets the pre-existing storage-clearing gas refund to zero for storage operations governed by this TIP. A storage operation MUST NOT receive both a legacy storage-clearing refund and a TIP-1060 settlement refund. After this TIP, the end-of-transaction settlement defined below is the new mechanism by which the nonzero-to-zero case affects the transaction's gas refund counter.
...
The standard EVM refund cap (the limit of one fifth, i.e. 20%, of the transaction's gas used per EIP-3529) still applies to the transaction's ordinary gas refund counter, but the `Refund`-mode storage credit settlement is exempt from it. The full `settled * 230_000` gas is credited on top of the capped ordinary refund, regardless of the transaction's total gas used; the cap continues to bound the preserved baseline SSTORE refunds (such as the restore-to-original refunds) but never reduces the storage-credit settlement amount.
...
## All Storage Transitions

The tables below enumerate every storage transition. The baseline gas and refund columns follow the EVM SSTORE schedule defined in [EIP-2200](https://eips.ethereum.org/EIPS/eip-2200) as amended by [EIP-3529](https://eips.ethereum.org/EIPS/eip-3529), with the OSAKA fork enabled, computed from the reference `sstore_dynamic_gas` and `sstore_refund` functions in *Baseline SSTORE gas cost*. TIP-1060's only edit to that baseline is setting `SSTORE_CLEARS_SCHEDULE` to `0`; the TIP-1000 creation component is layered on top for the creation class.

Notation: `0` is the zero value; `X` is the slot's transaction-original nonzero value; `Y`, `Z` are other distinct nonzero values. Columns: **O** = original (start-of-transaction) value, **P** = present value before the write, **N** = new value. Warm access is assumed; add `COLD_SLOAD_COST` (2,100 gas) when the slot is cold. The **EVM refund** column already reflects `SSTORE_CLEARS_SCHEDULE = 0`; values that remain nonzero (19,900 / 2,800) are the preserved restore-to-original refunds. The **TIP-1000 20k** column is the `GAS_STORAGE_SET` residual, charged by the SSTORE gas function under the same `original == present == 0` condition as the base `SSTORE_SET` cost — so it appears only on the clean-creation row, never on a dirty recreation. The TIP-1060 credit classification keys only on the `P -> N` transition (`nonzero -> 0` mints, `0 -> nonzero` is a creation), never on `O`; the 230,000-gas creditable portion (next table) is handled on every creation regardless of `O`.

### Baseline transitions

| O | P | N | Transition | EVM dyn gas | EVM refund | TIP-1000 20k | TIP-1060 class |
|---|---|---|------------|------------:|-----------:|-------------:|----------------|
| 0 | 0 | 0 | no-op                     | 0      | 0      | 0      | none |
| 0 | Y | Y | no-op (same-tx)           | 0      | 0      | 0      | none |
| 0 | Y | Z | dirty overwrite           | 0      | 0      | 0      | none |
| X | X | X | no-op                     | 0      | 0      | 0      | none |
| X | X | Y | clean overwrite           | 2,800  | 0      | 0      | none |
| X | Y | X | dirty restore to original | 0      | 2,800  | 0      | none |
| X | Y | Z | dirty overwrite           | 0      | 0      | 0      | none |
| 0 | Y | 0 | clear of same-tx creation | 0      | 19,900 | 0      | **mint +1** |
| X | X | 0 | clean clear               | 2,800  | 0      | 0      | **mint +1** |
| X | Y | 0 | dirty clear               | 0      | 0      | 0      | **mint +1** |
| 0 | 0 | Y | clean creation            | 19,900 | 0      | 20,000 | **creation** |
| X | 0 | X | recreation to original    | 0      | 2,800  | 0      | **creation** |
| X | 0 | Y | recreation to other value | 0      | 0      | 0      | **creation** |

A `nonzero -> 0` transition mints one credit irrespective of `O` and of mode. A `0 -> nonzero` transition is a creation: its 230,000-gas creditable portion is handled per mode in the next table, while its 20,000-gas residual is charged only on the clean-creation row above. Every other transition neither mints nor consumes a credit.

### Creation handling by mode (`P = 0 -> N = nonzero`)

This table covers only the **230,000-gas creditable portion** of the TIP-1000 component (`230,000` = one storage credit value). The 20,000-gas residual and base `SSTORE_SET` cost are charged separately by the SSTORE gas function — on a clean creation (`O = 0`) only — and are independent of the mode and of the credit balance.

| Mode | Condition | Credit balance | `pending_refund_eligible_creations` | 230k portion charged as gas | Net 230k cost |
|------|-----------|----------------|--------------------------------------|------------------------------|---------------|
| `Refund`   | always (default) | unchanged at write time | +1 | 230,000 upfront | 0 if a credit is available at settlement (230,000 refunded), else 230,000 |
| `Preserve` | always | unchanged | — | 230,000 | 230,000 |
| `Direct`   | `balance > 0` and `budget > 0` | −1 synchronously | — | 0 (covered by credit) | 0; budget −1, auto-switch to `Preserve` if budget reaches 0 |
| `Direct`   | `balance == 0` or `budget == 0` | unchanged | — | 230,000 | 230,000; if `budget == 0`, switch to `Preserve` |

For a clean creation (`O = 0`), add the 20,000-gas residual to each row's net cost; for a dirty recreation (`O = X`), there is no residual and the net cost is exactly the 230k figure above.

Notes:

- A `Direct`-mode creation that consumed a credit (so its 230,000-gas portion was covered by the credit, not charged) MUST NOT later receive any additional refund for that 230,000 gas. `Refund`-mode creations pay the 230,000 upfront, so the 230,000-gas settlement refund is for a cost actually paid earlier in the same transaction.
- A `Direct`-mode creation in the same transaction as a `Refund`-mode creation consumes its own credit synchronously and does NOT increment `pending_refund_eligible_creations`. The two modes' settlement is independent.
- Because credits key only on `P -> N`, a within-transaction `X -> 0 -> Y` sequence mints on `X -> 0` and applies 230k credit handling on `0 -> Y`, so the credit balance nets out. No 20,000-gas residual is charged on that recreation, because the SSTORE gas function does not treat a dirty recreation (`O = X`) as a fresh `SSTORE_SET`.
...
17. The standard EVM refund cap (one fifth of gas used, EIP-3529) still applies to the transaction's ordinary gas refund counter, but it does not affect the `Refund`-mode storage credit return: the storage credit settlement refund (`settled * 230_000` gas per account) is credited in full, outside the cap, regardless of the transaction's gas used.
...
The MUST-preserve rule across both TIPs is that one storage operation cannot receive two refunds: a creation that has its 230,000-gas portion covered by a storage credit (in `Direct` mode) or refunded at settlement (in `Refund` mode) MUST NOT additionally be reimbursed through TIP-1016 state-gas accounting for the same 230,000 gas.
