...
This TIP introduces *storage credits*: per-account, non-transferable credits earned when an account deletes one of its own storage slots and later usable by that same account to offset storage creation costs. A storage credit covers the storage credit value (`STORAGE_CREDIT_VALUE = 230,000` gas) of TIP-1000's storage creation component (`SSTORE_CREATE_COST = 250,000` gas). The residual (`SSTORE_SET_COST = 20,000` gas) is charged by the normal SSTORE gas function only on clean creations, i.e. when `original == present == 0`.
...
2. **The TIP-1000 storage creation cost is split.** TIP-1000's storage creation component (`SSTORE_CREATE_COST`, 250,000 gas) becomes a residual (`SSTORE_SET_COST`, 20,000 gas) plus a creditable portion (`STORAGE_CREDIT_VALUE`, 230,000 gas). The residual is charged by the SSTORE gas function on clean creations only. The creditable portion is either paid as gas or covered by consuming one storage credit, depending on the account's selected mode.
...
- **TIP-1000 storage creation cost**: The state-creation component charged by TIP-1000 (`SSTORE_CREATE_COST = 250,000` gas), separate from the EIP-2929 access cost (2,100 gas cold, 100 gas warm) and any other base SSTORE gas. This TIP splits that component into two portions with different charging rules:
  - the **residual** (`SSTORE_SET_COST = 20,000` gas), charged by the SSTORE gas function itself — i.e. only on a *clean* creation where the transaction-original value is also zero (`original == present == 0`). On a dirty recreation (the slot was nonzero at the start of the transaction, e.g. `X -> 0 -> Y`), the SSTORE gas function charges no residual; and
  - the **creditable portion** (`STORAGE_CREDIT_VALUE = SSTORE_CREATE_COST - SSTORE_SET_COST = 230,000` gas), governed entirely by the storage credit mechanism on the present-to-new transition, independent of the original value.
...
:::note
The `EVM dyn gas` column excludes the 100-gas warm SSTORE/access charge that the opcode charges separately. Thus the clean-creation row lists 19,900 dynamic gas, which together with the 100-gas warm charge equals the `SSTORE_SET_COST` residual.
:::
...
This TIP draws a clean line between two kinds of gas. The baseline SSTORE schedule (EIP-2200 as amended by EIP-3529) measures the *work this block performs* to apply a storage write — it accounts for original, present, and new values and is left intact (other than zeroing `SSTORE_CLEARS_SCHEDULE`). The gas this TIP layers on top of a creation — the TIP-1000 storage-creation component, now `SSTORE_SET_COST` plus `STORAGE_CREDIT_VALUE` — is not block work; it is payment for *long-term storage* and should count toward state gas.
