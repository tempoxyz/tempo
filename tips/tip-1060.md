...
Implementations MUST treat the following transaction-specific slots as non-creditable for clear-side TIP-1060 accounting:
...
3. The TipFeeManager `collected_fees[beneficiary][validator_token]` slot for the current block beneficiary and validator fee token.
...
Fee distribution from TipFeeManager bookkeeping MUST run with TIP-1060 accounting disabled, the same as fee collection and fee refunds. Moving collected protocol fees is protocol accounting, not a user-metered storage creation or deletion, and MUST NOT mint credits, consume credits, or create pending refund-eligible creations.
