...
This exemption is not a discount for arbitrary user storage. It waives only the extra 250,000-gas storage-creation component for the protocol metadata slot that tracks an account's storage-credit state. For contracts, that metadata slot is coupled to an account whose creation already pays the normal account or contract creation overhead; charging another 250,000 gas for the bookkeeping slot would double-count the first per-account footprint. The only EVM-reachable path that creates a backing slot is `setMode`, which is restricted to contracts and protocol precompiles (EOAs and EIP-7702 delegated accounts are rejected; see the *Storage Credits Precompile* behavior). Because each eligible caller is a contract whose creation already paid full contract-creation gas, mass creation of exempt backing slots remains bounded by ordinary contract creation gas, and this path cannot create arbitrary contract-owned storage slots; the exemption is therefore not a state-growth DoS vector. Rejecting EIP-7702 delegated accounts specifically closes the cheap-to-mint path that would otherwise let an attacker create many exempt backing slots for far less than contract-creation cost.
...
interface IStorageCredits {
    ...
    ///         protocol precompiles. EOAs do not hold storage, so a mode
    ///         selection would have no effect. An EIP-7702 delegated account
    ///         is also rejected: its delegation-indicator code does not count
    ///         as contract code here. Such accounts are cheap to mint en
    ///         masse, so permitting them would turn the exempt backing-slot
    ///         write into a state-growth vector.
...
- `setMode(newMode)` MUST revert with `CallerMustBeContract` if `msg.sender` is an externally owned account. For this purpose an account is an EOA if it is not a protocol precompile and either (a) has no associated contract code or (b) its only code is an EIP-7702 delegation indicator. An EIP-7702 delegated account is therefore rejected: its delegation-indicator code does not count as contract code for this check. An account executing its own contract-creation (constructor) frame is treated as a contract even though its code is not yet stored. Only contracts and precompiles may set a mode, both because only they can own the storage the mode governs and because restricting the caller keeps the exempt backing-slot write (see *State*) from becoming a cheap state-growth vector — EIP-7702 delegations in particular are cheap to create en masse.
