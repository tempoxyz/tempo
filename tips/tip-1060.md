...
ai: Remove all mentions about TIP-1016 from tests and put small TIP-1016 section at the end that would explain that TIP-1016 would just change 230k gas from TIP-1060 to be consumed state gas (reservoir), all other requirements in TIP-1060 stay the same. Add explanation that current sstore gas logic should be considered as work needed for this block, and gas added to TIP-1060 represent long term storage and should count towards state gas.
...
ai: rewrite the sentence, to add that current sstore gas mechnanism is preserved and it is modified for two cases: 1. clear refund should be put to zero as credit is introduced for this case (It is done by setting SSTORE_CLEARS_SCHEDULE from eip-3529 to zero) 2. cost of creating new storage should be put from 250k from TIP-1000 to 20k and additional cost could spend credit or additional 230k of gas (This is done by setting GAS_STORAGE_SET to 20k). And that other refunds (for example setting values to original value) are preserved.
...
ai: add current SSTORE gas cost function, and explanation that it takes into account original, present and new values and that it accuratly represent the abount of gas that would cover that block cost. and it is modified by TIP-1000 on new creation cost.
...
ai: Simpify this by a lot, original value of storage should not be checked, credit gets minted on every x->0 change. Remove mentions of CALL/DELEGATECALL/STATIC call and use generic frame.
...
ai: remove mention of transaction-original value, for credit we only care for present and new values. Taking into account original value does not work as creation slot A can use credit from clearing slot B. On every X -> 0 reward the credit, and on every 0->Y consume credit (depending on mode etc)
...
ai: State refund from Refund-mode will skip the lime of one fifth of gas but gas refund is still effected by it.  
...
ai: Reference https://eips.ethereum.org/EIPS/eip-2200 (and followup https://eips.ethereum.org/EIPS/eip-3529) and make a table with all transitions. Include all modes into it. table should be set of origin, present, new and mode values and credit (if credit is zero or not). Use logic from revm to deduce values, for refund https://github.com/bluealloy/revm/blob/6effb301cd836dcaf4848b7ebdb71093c6067bef/crates/context/interface/src/cfg/gas_params.rs#L517 and dynamic gas  https://github.com/bluealloy/revm/blob/6effb301cd836dcaf4848b7ebdb71093c6067bef/crates/context/interface/src/cfg/gas_params.rs#L483 (Assume OSAKA fork is enabled)
...
# Invariants
ai: remove this statement it is wrong now. Credit is minted on every x->0 sstore regardless of original value
...
ai: remove later part of statement, leave just removal of refund addition and subtraction related to state clearing (It is not accurate it is only 0 -> X -> 0)
...
ai: Refund cap is still there, but refund but it is not effecting `Refund`-mode gas return.
