...
Protocol bookkeeping slots that can be cleared during user-metered execution and later recreated by protocol/system writes with TIP-1060 accounting disabled MUST be treated as non-creditable for that transaction. A non-creditable slot is a concrete `(account, storage_slot)` pair recorded transaction-locally. If a non-creditable slot is cleared (`present != 0` and `new == 0`), the clear MUST NOT mint a storage credit. This prevents an unbacked credit: the later disabled-accounting recreation would otherwise restore the slot without consuming a credit or paying `STORAGE_CREDIT_VALUE`.

The non-creditable slot set MUST include any concrete protocol bookkeeping slot that satisfies the clear-and-disabled-recreate condition above. At T7, this includes:

1. the fee payer's TIP-20 fee-token balance slot;
2. the keychain spending-limit slot for the transaction access key, when fee limits are enforced; and
3. the fee manager collected-fees slot for the beneficiary and validator token.

This exclusion affects only the minting side of TIP-1060. A zero-to-nonzero write to one of these slots still follows normal storage-credit consumption rules when it occurs in user-metered execution with TIP-1060 accounting enabled.
...
2. Increments the `storage_credit_balance` field for `A` by one, unless the slot is transaction-locally marked non-creditable as described in [Persistent State](#persistent-state).
...
interface IStorageCredits {
    ...
    ///      The budget is decremented only when a storage credit is actually
    ///      consumed in `Direct` mode.
...
- `modeOf(account)` returns the current transient storage creation mode for `account` within the current transaction, which is `Refund` unless `account` has called `setMode` or `setBudget` earlier in the same transaction (and that call has not been reverted).
...
- `setBudget(creditBudget)` MUST set the transient Direct budget for `msg.sender` to `creditBudget` and set the transient mode to `Direct`. If `creditBudget == 0`, subsequent Direct-mode creations cannot spend credits and therefore charge `STORAGE_CREDIT_VALUE` as gas while `modeOf(msg.sender)` reports `Direct`.
