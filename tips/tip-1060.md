...
interface IStorageCredits {
    ...
    ///         Only contracts (including an account executing its own
    ///         constructor) and protocol precompiles may set a mode. A plain
    ///         EOA never performs its own storage writes, so a mode would have
    ///         no effect. An EIP-7702 delegated account is also rejected: its
    ///         delegation-indicator code does not count as contract code here.
...
- `setMode(newMode)` MUST revert with `CallerMustBeContract` if `msg.sender` is an externally owned account. For this purpose an account is an EOA if it is not a protocol precompile and either (a) has no associated contract code or (b) its only code is an EIP-7702 delegation indicator. An account executing its own contract-creation (constructor) frame is treated as a contract even though its code is not yet stored. A plain EOA never performs its own storage writes, so a mode would have no effect for it. EIP-7702 delegated accounts are rejected as a conservative choice; because the mode is now transient and `setMode` writes no persistent state, this restriction is no longer needed to prevent state-growth abuse and MAY be relaxed by a future revision.
