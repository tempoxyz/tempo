...
3. **FeeManager settles through a direct FX pool.** Before precharging the user, FeeManager uses the aforementioned delayed quote, and checks validator-token liquidity. The user then pays the FX token amount implied by that quote, and the validator receives its USD fee token from the direct pool `(userToken, validatorToken)`.
...
`currentQuote` is the latest pushed quote. `previousQuote` is the only quote used for fee operations. From the fee path's perspective, `previousQuote` is the current usable quote.
