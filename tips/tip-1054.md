...
The fee oracle lives in the TIP-20 precompile. TIP-20 tokens are precompiles, so this TIP adds storage and entry points to the token rather than defining a separate oracle contract.
...
USD tokens MUST reject nonzero fee quote updaters and nonzero fee quote policies. The fee oracle interface MUST NOT make a USD token usable through the FX fee-token path.

A fee quote has two fields: `quoteId` and `pathUsdPerTokenX18`. `quoteId` is an updater-supplied monotonically increasing identifier for the quote. `pathUsdPerTokenX18` is the value of one whole token in `pathUSD`, scaled by `QUOTE_SCALE`. For example, if one token is worth `2.50 pathUSD`, the updater pushes `pathUsdPerTokenX18 = 2.5e18`. The token does not store `updatedAt`; source freshness is the updater's responsibility.
...
`latestQuote` is the newest accepted quote. `usableQuote` is the quote FeeAMM may use. Each quote stores its `quoteId` and price. Each quote block stores the Tempo block in which that quote was accepted. The quote id orders oracle updates; the quote block enforces the one-block delay and freshness checks. A quote becomes usable only after it was pushed in an earlier block.

### Updating and Rolling Quotes
...
The updater MAY call `updateLatestFeeQuote(quoteId, pathUsdPerTokenX18)`. The call MUST revert unless the caller is `feeQuoteUpdater`, the token has a valid fee quote policy, `quoteId` is greater than both `usableQuote.quoteId` and `latestQuote.quoteId`, and `pathUsdPerTokenX18 != 0`.
...
Before `updateLatestFeeQuote` writes the new quote, the token MUST apply `rollIfEligible`. This prevents an older eligible quote from being skipped when no FeeAMM operation read the token in the previous block. The call then writes `{quoteId, pathUsdPerTokenX18}` as `latestQuote`, sets `latestQuoteBlock = block.number`, and emits `FeeQuoteUpdated`.
