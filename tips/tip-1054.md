...
TIP-1054 lets users pay transaction fees on Tempo with non-USD TIP-20 tokens. A non-USD token issuer that wants its token to be used for fees can point the token at a FeeOracle feed that prices the token in USD.
...
The FeeOracle precompile stores shared price feeds used for fee quotes. Payments, charging, and off-protocol oracle operation are outside this TIP. A feed updater MAY be a contract or multisig that implements its own roles, aggregation, billing, or access rules.
...
The quote-age and quote-change constants are maximums. The feed admin MAY set smaller policy values to require fresher quotes or a lower per-update quote change. A feed quote policy is valid only if:
