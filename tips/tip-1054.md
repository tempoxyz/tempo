...
1. FeeManager MUST ask FeeAMM to call `rollFeeQuote()` for `userToken` and to read and return `userToken.previousFeeQuote()`.
2. Fee collection MUST revert if the resolved FX token is not executable in the current block.
3. FeeManager MUST compute `maxUserTokenFee = fxTokenAmountForUsd6(maxFeeUsd6, quote.pathUsdPerTokenX18)`.
4. FeeManager MUST transfer `maxUserTokenFee` from the fee payer through the existing precharge path.
5. FeeAMM MUST compute `reservedValidatorOut = validatorFeeOut(maxFeeUsd6)`.
6. FeeAMM MUST verify that the direct pool has at least `reservedValidatorOut` validator-token reserve, and reserve that amount using the existing FeeManager reservation model.

Post-tx collection MUST use the same quote recorded during pre-tx collection:

1. The protocol MUST recompute `actualFeeUsd6` using existing gas accounting.
2. The protocol MUST compute `actualUserTokenSpend = fxTokenAmountForUsd6(actualFeeUsd6, quote.pathUsdPerTokenX18)`.
3. FeeManager MUST compute `refundUserToken = maxUserTokenFee - actualUserTokenSpend` and refund it to the fee payer.
4. FeeAMM MUST credit the validator with `validatorFeeOut(actualFeeUsd6)` in `validatorToken`.
5. FeeAMM MUST increase `reserve_user_token` by `actualUserTokenSpend` and decrease `reserve_validator_token` by the validator credit.
6. `collect_fee_post_tx` MUST emit `FXFeeSettled` if `actualUserTokenSpend` or `refundUserToken` is nonzero.
