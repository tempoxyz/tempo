...
A non-USD token can be used for fees when the token exposes a `pathUSD` fee quote and maintains the quote cache defined in this TIP. Fee settlement uses the token's `previousFeeQuote`, not a live same-block oracle read.

Each token keeps two quote slots: `previous` and `pending`. `previous` is the quote used for fee settlement. Refreshes write `pending`; an eligible pending quote replaces `previous` no earlier than the next block. Multiple refreshes in the same block overwrite the same pending slot, so only the last refresh from that block can later become usable.
...
This avoids a client-only parent-state oracle read. The last usable quote is ordinary state: fee collection reads `token.previousFeeQuote()`, and tools can also inspect `token.pendingFeeQuote()`.
...
sequenceDiagram
    ...
    FeeManager->>Token: read previous quote
...
- **previous quote** means the latest quote promoted from `pending` into fee-settlement state. It is the quote used for fee settlement and FX FeeAMM operations.
- **pending quote** means the latest sampled quote that is not usable yet.
- **fee-enabled FX token for block `B`** means a fee-quoting FX token whose previous quote is valid for block `B`.
...
It MUST apply any eligible quote promotion and then use `userToken.previousFeeQuote()`. If `userToken` does not have a valid previous quote for the current block, the call MUST revert.
...
7. A quote sampled in block `N` cannot replace `previousFeeQuote` in block `N`.
8. Promoting a quote sets `previousFeeQuote` to the pending quote and clears `pendingFeeQuote`.
9. Fee settlement, FX rebalance, and FX mint use only previous quotes.
10. Pre-tx and post-tx collection for the same transaction use the same previous quote.
...
3. `refreshFeeQuote()` promoting an older eligible pending quote before writing a new pending quote;
4. rejection of same-block promotion when `pendingFeeQuote.sampledBlock == block.number`;
5. multiple same-block refreshes overwriting only `pendingFeeQuote` after any initial eligible promotion;
6. promotion in a later block setting `previousFeeQuote` to the last pending quote and clearing `pendingFeeQuote`;
7. promotion of an invalid pending quote disabling fee use until a later valid quote is promoted;
8. rejection of stale previous quotes even if they were valid when sampled;
...
14. post-tx settlement using the same previous quote used by pre-tx collection even if `pendingFeeQuote` changes during execution;
...
18. `rebalanceSwap` rejection for FX pools and `rebalanceSwapFX` arithmetic, max-input protection, and invalid-previous-quote rejection;
19. first and later `mint()` validation for FX pools, including invalid-previous-quote rejection;
20. unchanged `burn()` behavior for FX pools, including burn after the user token no longer has a valid previous quote; and
