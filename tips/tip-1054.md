...
2. **The token delays quote use by one block.** Each token stores `(previousQuote, previousQuoteBlock)` and `(currentQuote, currentQuoteBlock)`. Fee settlement uses `previousQuote`. A pushed update writes `currentQuote` and which is lazily copied over into `previousQuote` when needed. 
...
The one-block delay avoids same-block fee-accounting races. Without it, a quote update and an FX-fee transaction in the same block could be ordered either way, changing the price used for the fee. This is worse if the quote-update transaction itself pays fees in the FX token, or if a transaction mutates FeeAMM state, because the protocol would need to define whether fees are charged against pre-update or post-update state. 
...
The updater may call `updateCurrentFeeQuote(roundId, pathUsdPerTokenX18)`. Before writing the new quote, the token MUST first roll any eligible existing `currentQuote` into `previousQuote`. The call then writes the new `currentQuote` and sets `currentQuoteBlock = block.number`.

A pushed quote is accepted iff `roundId != 0` and `pathUsdPerTokenX18 != 0`.

`setFeeQuoteUpdater` MUST be callable only by the token's authorized admin and MUST emit `FeeQuoteUpdaterSet`. `updateCurrentFeeQuote` MUST revert if the caller is not the token's `feeQuoteUpdater` or if the pushed quote is not accepted.

The quote is always for one whole token in `pathUSD`, scaled by `QUOTE_SCALE`. For example, if one token is worth `2.50 pathUSD`, the updater pushes `pathUsdPerTokenX18 = 2.5e18`. The updater is responsible for quote freshness; this interface does not carry an `updatedAt` value.
...
`currentQuote` is the latest pushed quote. It is not used for fee settlement in the same block in which it was pushed. When current is rolled into previous, it overwrites the old previous quote.
...
Before any feeAMM operation that needs a quote, FeeAMM MUST ask the token precompile to roll its quote forward. If the current quote is eligible, the token MUST:

1. set `previousQuote = currentQuote`;
2. set `previousQuoteBlock = currentQuoteBlock`.
...
Each `updateCurrentFeeQuote` call MUST roll any eligible existing `currentQuote` into `previousQuote` before it writes the new current quote. This prevents a quote from block `N - 1` from being lost if no fee transaction used it before a new quote arrives in block `N`.

After that roll, the pushed quote writes only `currentQuote`. If `updateCurrentFeeQuote` is called multiple times for the same token in block `N`, each call overwrites `currentQuote` with `currentQuoteBlock = N`, and `previousQuote` remains unchanged for the rest of the same-block updates.


### Previous Quote Validity

A previous quote is valid iff all of the following hold:

1. `previousQuoteBlock != 0`;
2. `previousQuote.roundId != 0`; and
...
2. the transaction is not a Tempo Transaction;
...
For transactions that are not Tempo Transactions, the existing direct-call inference rule is unchanged except that the inferred token MAY now be an FX token.

For Tempo Transactions, inference applies only when:
...
2. have FeeAMM read `userToken.previousFeeQuote()` and return it to FeeManager;
