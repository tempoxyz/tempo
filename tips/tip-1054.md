...
The only new input is a `pathUSD` quote for the user's non-USD token. Because TIP-20 tokens are precompiles, the quote state lives directly in the token precompile. The token admin does not install new oracle contract code through this TIP; it only designates which address may push quote updates.
...
3. **FeeManager settles through a direct FX pool.** The user pays the FX token amount implied by the cached previous quote. The validator receives its USD fee token from the direct pool `(userToken, validatorToken)`.
...
sequenceDiagram
    ...
    FeeManager->>FeeAMM: settle through FX pool
...
This version is narrow. It does not introduce non-USD validator fee-token preferences, signed token-unit fee caps, multi-hop FeeAMM routing, quote-based repricing of USD fee tokens, standardized mempool invalidation rules, or wallet UX requirements.
...
Before writing a new current quote, `updateCurrentFeeQuote` MUST first roll any eligible current quote into `previousQuote`. This prevents an updater from overwriting a quote from an earlier block before it can become usable.
...
- `quote` be `userToken.previousFeeQuote()` after FeeAMM applies any eligible roll; and
