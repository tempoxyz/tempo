...
The updater MAY call `updateLatestFeeQuote(quoteId, pathUsdPerTokenE18)` to push a new quote. The call MUST revert unless all of these conditions hold:
...
abs(pathUsdPerTokenE18 - latestQuote.pathUsdPerTokenE18) * BPS_SCALE
    <= latestQuote.pathUsdPerTokenE18 * maxQuoteChangeBps
```
...
After that roll attempt, the token writes `{quoteId, pathUsdPerTokenE18}` as `latestQuote`, sets `latestQuoteBlock = block.number`, and emits `FeeQuoteUpdated`.

If more than one update is pushed for the same token in block `N`, only the first update can roll an older quote into `usableQuote`. Later same-block updates only overwrite `latestQuote` for later blocks.
...
If no quote has rolled into `usableQuote`, the token has no usable quote. A rolled quote remains valid only while the token has a valid fee quote policy and `block.number - usableQuoteBlock <= maxFeeQuoteAgeBlocks`. If the usable quote is invalid after `rollFeeQuote()`, the FX token is not fee-enabled for the current block and any operation that requires a valid quote MUST revert.
...
- the token is a valid non-USD TIP-20 token which is not paused,
- has a nonzero `feeQuoteUpdater` and valid fee quote policy,
- has a valid usable quote after `rollFeeQuote()`, and,
- can be transferred by the fee payer under the existing token-policy rules. 

Further, the user MUST have sufficient balance of the FX token given the current quote, and the feeAMM MUST have sufficient liquidity of the validator token. 

Pending FX-fee transactions MUST be revalidated when quote state, quote-updater state, token pause state, token policy, or direct-pool reserves change.
...
3. FeeManager records `quote.quoteId` and `quote.pathUsdPerTokenE18` in transaction-scoped fee state, then computes `maxUserTokenFee = fxTokenAmountForUsdE6(maxFeeUsdE6, quote.pathUsdPerTokenE18)`.
4. FeeManager transfers `maxUserTokenFee` from the fee payer through the existing precharge path.
5. FeeAMM computes `reservedValidatorOut = validatorFeeOut(maxFeeUsdE6)`.
...
2. The protocol computes `actualUserTokenSpend = fxTokenAmountForUsdE6(actualFeeUsdE6, quote.pathUsdPerTokenE18)`.
...
The `FXFeeSettled` event records the quote id and price actually used. The precharged FX-token amount is `userTokenIn + userTokenRefund`; the maximum 6-decimal USD fee remains derivable from the transaction's signed gas fields and block base fee.
...
For later mints, FeeAMM MUST call `rollFeeQuote()` and use the usable quote for `userToken` in the current block. Let `U = reserve_user_token`, `V = reserve_validator_token`, `S = totalSupply`, `priceE18` be the usable quote, and `amountValidatorToken` be the caller's deposit. The minted liquidity MUST be:
...
FX fee support relies on the token issuer and its `feeQuoteUpdater` to publish accurate prices. A stale, incorrect, or compromised oracle can cause users to overpay in the FX token or cause LPs in the associated feeAMM pool(s) to receive too little value for their provided liquidity. To limit this risk, each token sets `maxFeeQuoteAgeBlocks` and `maxQuoteChangeBps`, bounded by protocol maximums. The former ensures that old quotes expire while the latter ensures that quote updates cannot move too far from the last accepted quote. These limits reduce the monetary damage from oracle liveness failures or abrupt bad updates, but they do not remove oracle trust. Further, a specific oracle liveness failure may result in the ability of users to pay in a token that has elected to use that oracle. LPs and users should treat fee-enabled FX tokens as carrying the oracle and issuer risk of that token. 
...
The existing 6-decimal USD fee amount remains the source of truth. Validator payout is computed from that 6-decimal USD amount, not by re-valuing the rounded user-token debit. For any valid usable quote `priceE18 = pathUsdPerTokenE18`:
