...
A non-USD token can be used for fees when the token exposes a fee quote as explained in this TIP. Fee settlement reads the token's cached `previousFeeQuote`; it never uses a live same-block oracle read.
...
Tempo already lets users choose among USD-denominated TIP-20 tokens to pay fees. TIP-1054 proposes extending this so that users can pay fees in fee-quoting non-USD tokens. 
...
1. **The token exposes a fee quote.** A fee-quoting non-USD token returns the latest value of one whole token in `pathUSD`. In effect, any non-USD TIP-20 token can choose an oracle price. 
2. **The token stores a delayed quote cache.** `refreshFeeQuote()`  updates the latest quote into `pendingFeeQuote`. A later block may promote that pending quote into `previousFeeQuote`, which is the quote used for fees.
