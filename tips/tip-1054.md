...
A feed can be created only once; `createFeed` sets `msg.sender` as the feed admin and MUST revert if the feed already exists. `feedId` MUST NOT be 0. 
...
Pending FX-fee transactions MUST be revalidated when any of the relevant variables change (e.g., token state, feed state, feed authorization, quote state, or direct-pool reserves).
