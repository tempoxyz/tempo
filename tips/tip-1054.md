...
TIP-1054 lets users pay transaction fees on Tempo with non-USD TIP-20 tokens. A non-USD token issuer that wants its token to be used for fees can point the token at a FeeOracle feed for that token's value in USD.
...
1. **FeeOracle stores shared price feeds.** A feed publishes the USD value of one whole token. Multiple tokens may use the same feed, so several EUR stablecoins can share a EUR/USD oracle if their issuers choose to do so.
2. **A token selects a feed.** A non-USD TIP-20 token opts into FX fees by storing the FeeOracle feed it uses. The token does not store oracle prices itself.
...
sequenceDiagram
    ...
    Updater->>FeeOracle: updateLatestFeeQuote(feedId, ...)
    FeeOracle->>FeeOracle: store quote for a later block
    Note over FeeOracle: later block
    User->>FeeManager: transaction uses FX fee token
    FeeManager->>FeeAMM: get quote and reserve validator-token output
    FeeAMM->>Token: read feeOracleFeedId
    FeeAMM->>FeeOracle: roll latest into usable if eligible
    FeeAMM->>FeeOracle: read usableQuote
...
A non-USD TIP-20 token opts into letting users pay fees in that token by setting a `feeOracleFeedId`. Only the token's authorized admin MAY set or clear it. A successful change MUST emit `FeeOracleFeedSet`. `feeOracleFeedId = 0` means the token is not configured for FX fees.

When a nonzero feed is set, the feed MUST exist and the FeeOracle feed admin MUST have authorized the token to use that feed. USD tokens MUST reject nonzero fee oracle feed ids.

<br>

## FeeOracle Precompile

The FeeOracle precompile stores shared fee quote feeds. This TIP does not define how a feed admin pays, charges for, or operates the oracle off-protocol. A feed updater MAY be a contract or multisig that implements its own roles, aggregation, billing, or access rules.

### Feeds

A feed is keyed by `feedId` and has a feed admin, a feed updater, a fee quote policy, token authorizations, and two quote tuples:
...
`feedId = 0` is reserved and cannot be created. A feed id can be created only once; `createFeed` sets `msg.sender` as the feed admin and MUST revert if the feed already exists.

A fee quote has two fields:
- `quoteId` is assigned by the FeeOracle, starts at `1`, and increases by one for each accepted quote on the feed.
...
The protocol sets maximum values for both fields. The feed admin can choose smaller values to require fresher quotes or smaller per-update quote moves. A cleared policy has both fields set to zero.

Only the feed admin MAY set the feed updater, fee quote policy, or token authorizations. Changing the updater or policy MUST clear both quote tuples and their block fields. A token is not fee-enabled through a feed unless the feed authorizes that token.

### Updating Quotes

The feed updater MAY call `updateLatestFeeQuote(feedId, usdPerTokenE18)` to push a new quote. The call MUST revert unless all of these conditions hold:
- the caller is the feed updater,
- the feed has a valid fee quote policy, and
- `usdPerTokenE18` is nonzero.

Before accepting the new quote, FeeOracle MUST try to roll the existing `latestQuote` into `usableQuote` using the rule below. This prevents an older eligible quote from being skipped when no FeeAMM operation read the feed in the previous block.
...
FeeOracle then increments the feed's quote id, writes `{quoteId, usdPerTokenE18}` as `latestQuote`, sets `latestQuoteBlock = block.number`, and emits `FeeOracleQuoteUpdated`. The updater does not supply `quoteId`. An update MUST revert if the next quote id would overflow.
...
Before FeeAMM uses a feed quote, it MUST call `rollFeeQuote(feedId)` and then read `usableQuote` from FeeOracle.

If no quote has rolled into `usableQuote`, the feed has no usable quote. A rolled quote remains valid only while the feed has a valid fee quote policy and `block.number - usableQuoteBlock <= maxFeeQuoteAgeBlocks`. If the usable quote is invalid after `rollFeeQuote(feedId)`, any operation that requires a valid quote MUST revert.
...
- the token has a nonzero `feeOracleFeedId`,
- the selected feed authorizes the token and has a valid fee quote policy,
- the selected feed has a valid usable quote after `rollFeeQuote(feedId)`, and
...
1. FeeManager asks FeeAMM to read `userToken.feeOracleFeedId()`, call `rollFeeQuote(feedId)`, and read the feed's usable quote.
...
5. FeeAMM verifies that the direct pool has at least `reservedValidatorOut` validator-token reserve, and reserves that amount using the existing FeeManager reservation model.
6. FeeManager transfers `maxUserTokenFee` from the fee payer through the existing precharge path.

Post-tx collection MUST use the transaction-scoped quote recorded during pre-tx collection, not a fresh read from the token or FeeOracle:
...
`rebalanceSwapFX(...)` is the FX-specific rebalance entry point. Before it reads the quote, FeeAMM MUST read `userToken.feeOracleFeedId()`, call `rollFeeQuote(feedId)`, and revert unless the selected feed has a valid usable quote for the current block.
...
For the first `mint(userToken, validatorToken, amountValidatorToken, to)` on an FX direct pool, FeeAMM MUST read `userToken.feeOracleFeedId()`, call `rollFeeQuote(feedId)`, and revert unless the selected feed has a valid usable quote for the current block. The caller deposits only `validatorToken`, `MIN_LIQUIDITY` remains permanently locked, and the bootstrap formula is unchanged. The quote is used only to confirm that the pool's FX token is currently fee-enabled; it is not used in the bootstrap formula.

For later mints, FeeAMM MUST read `userToken.feeOracleFeedId()`, call `rollFeeQuote(feedId)`, and use the selected feed's usable quote in the current block. Let `U = reserve_user_token`, `V = reserve_validator_token`, `S = totalSupply`, `priceE18` be the usable quote, and `amountValidatorToken` be the caller's deposit. The minted liquidity MUST be:
...
BPS_SCALE          = 10000
PROTOCOL_MAX_FEE_QUOTE_AGE_BLOCKS = TBD  // upper bound for feed policy maxFeeQuoteAgeBlocks
...
The protocol constants are maximums. The feed admin MAY set smaller policy values to require fresher quotes or a lower per-update quote change. A feed quote policy is valid only if:
...
The transaction's signed 6-decimal USD fee bound remains the existing bound implied by its gas fields and attodollar gas accounting. `TIP-1007` remains unchanged: `getFeeToken()` continues to expose the resolved fee token for the current transaction.

Except for `FXFeeSettled` and the FeeOracle/TIP-20 events below, this TIP does not standardize additional fee-settlement events, receipt fields, or RPC surfaces.

### Interfaces and Events

```solidity
interface ITIP20FeeOracleConfig {
    event FeeOracleFeedSet(
        address indexed setter,
        bytes32 indexed oldFeedId,
        bytes32 indexed newFeedId
    );

    function feeOracleFeedId() external view returns (bytes32 feedId);

    function setFeeOracleFeedId(bytes32 feedId) external;
}
```
...
interface IFeeOracle {
    ...
    event FeeOracleFeedCreated(
        bytes32 indexed feedId,
        address indexed admin,
        address indexed updater
    );

    event FeeOracleUpdaterSet(
        bytes32 indexed feedId,
        address indexed setter,
    ...
    event FeeOracleTokenAuthorizationSet(
        bytes32 indexed feedId,
        address indexed token,
        bool authorized
    );

    event FeeOracleQuoteUpdated(
        bytes32 indexed feedId,
        address indexed updater,
    ...
    function feeQuotePolicy(bytes32 feedId)
        external
        view
        returns (FeeQuotePolicy memory policy);
    ...
    function rollFeeQuote(bytes32 feedId) external returns (bool rolled);

    function usableFeeQuote(bytes32 feedId)
        external
        view
        returns (FeeQuote memory quote, uint64 quoteBlock);

    function latestFeeQuote(bytes32 feedId)
        external
        view
        returns (FeeQuote memory quote, uint64 quoteBlock);
}
