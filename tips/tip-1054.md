...
An FX fee token is spendable only if, at fee-collection time, the token is a valid non-USD TIP-20 token, is not paused, still has a nonzero `feeQuoteUpdater` and valid fee quote policy, has a valid usable quote after `rollFeeQuote()`, and can be transferred by the fee payer under the existing token-policy rules. The fee payer must also have enough balance for `maxUserTokenFee`.

Balance checks, account-key limits, and transaction-pool cost accounting for an FX fee token MUST use `maxUserTokenFee`, denominated in the FX token. Validator liquidity checks remain denominated in 6-decimal USD and use `maxFeeUsdE6`.
