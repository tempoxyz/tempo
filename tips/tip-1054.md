...
sequenceDiagram
    ...
    FeeAMM->>PriceOracle: read prior-block usable quote
...
Before accepting the new quote, PriceOracle MUST determine the feed's usable quote using the rule in [Usable Quotes](#usable-quotes). If the existing `latestQuoteBlock` is nonzero and older than the current block, PriceOracle MUST copy `latestQuote` into `previousQuote` before overwriting `latestQuote`. This preserves the newest prior-block quote while the new `latestQuote` is ineligible for same-block use.

If no usable quote exists, the accepted quote is trusted as a bootstrap quote. Once a usable quote exists, each later quote MUST satisfy the feed's quote-change limit:
...
Once a usable quote exists, if more than one update is pushed for the same feed in block `N`, only the first update can move an older `latestQuote` into `previousQuote`. Later same-block updates are still compared against that same usable quote, so they cannot walk the price past `maxQuoteChangeBps` within the block.
