...
TIP-1054 lets users pay transaction fees on Tempo with non-USD TIP-20 tokens. Upon adoption of this TIP, a non-USD token issuer who wants their token to be used for fees on Tempo can expose a oracle quote for the token's value denominated in USD. 

Gas accounting does not change. Transactions still use the existing signed gas fields, Tempo still prices gas in attodollars, and validators still receive fees in USD. Users who use non-USD tokens for gas fees are charged based on the latest oracle quote. 
