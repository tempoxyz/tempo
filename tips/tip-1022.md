...
```
[4-byte PREFIX] [8-byte masterId] [8-byte userTag]
= 20 bytes total
```

| Field | Bytes | Description |
|-------|-------|-------------|
| **PREFIX** | 4 | Fixed prefix `0xFDFDFDFD`. Identifies virtual addresses with collision probability ~1/2³². |
| **masterId** | 8 | Deterministic identifier derived from the master address. Supports up to ~1.8×10¹⁹ masters. |
...
- `addr[0:4]` = `VIRTUAL_PREFIX`
- `addr[4:12]` = `masterId` (the raw 8-byte sequence as it appears in the address)
...
### Prefix Selection: `0xFDFDFDFD`

A 4-byte prefix provides collision resistance of ~1/2³². This is an explicit layout tradeoff to allocate 8 bytes to `masterId` while retaining 8 bytes for `userTag`.
...
```
masterId = bytes8(keccak256(abi.encodePacked(msg.sender, salt)))
```
...
In the unlikely event of a masterId collision (two `(address, salt)` pairs mapping to the same 8-byte hash), the second registration reverts with `MasterIdCollision`. The caller can retry with a different salt.
...
- `masterId = bytes8(keccak256(abi.encodePacked(0xABCD...1234, bytes32(0))))` → e.g., `0x07A3B1C2D4E5F607`
- For user #103048, the exchange derives a userTag → e.g., `0xa7c3f19e2b8d4501`

```
Virtual address = 0xFDFDFDFD  07A3B1C2D4E5F607  a7c3f19e2b8d4501
                  ^^^^^^^^    ^^^^^^^^^^^^^^^^  ^^^^^^^^^^^^^^^^
                  prefix (4)  masterId (8)      userTag (8)
```
...
interface IVirtualAddressRegistry {
    ...
    event ForwardingPaused(bytes8 indexed masterId);

    /// @notice Emitted when forwarding is resumed for a master.
    event ForwardingResumed(bytes8 indexed masterId);
    ...
    /// @dev masterId is derived deterministically as bytes8(keccak256(abi.encodePacked(msg.sender, salt))).
    ...
    function registerVirtualMaster(bytes32 salt) external returns (bytes8 masterId);

    // ────────────────── Queries ─────────────────────

    /// @notice Returns the current master address for a given masterId, or address(0) if unregistered.
    function getMaster(bytes8 masterId) external view returns (address);
    ...
    function getMasterIds(address masterAddress)
        external view returns (bytes8[] memory masterIds);
    ...
    function isForwardingActive(bytes8 masterId) external view returns (bool);
    ...
    /// @return masterId The 8-byte master identifier (zero if not virtual).
    /// @return userTag The 8-byte user tag (zero if not virtual).
    function decodeVirtualAddress(address addr)
        external pure returns (bool isVirtual, bytes8 masterId, bytes8 userTag);
    ...
    function getPendingUpdate(bytes8 masterId)
    ...
    function proposeUpdateMaster(bytes8 masterId, address newMaster, uint256 deadline, bytes calldata newMasterSig)
        external;
    ...
    function finalizeUpdateMaster(bytes8 masterId) external;
    ...
    function cancelUpdateMaster(bytes8 masterId) external;
...
|------|-------|-------------|
| `VIRTUAL_PREFIX` | `0xFDFDFDFD` | 4-byte prefix identifying virtual addresses |
...
function resolveRecipient(to: address) -> address:
    // Fast path: check first 4 bytes against VIRTUAL_PREFIX
    if to[0:4] != VIRTUAL_PREFIX:
        return to  // Not virtual, no overhead beyond a 4-byte comparison
...
|------|----------------|
| Non-virtual transfer | One 4-byte prefix comparison; no registry lookup |
...
- Transfer to non-virtual address starting with `0xFD` but not matching full 4-byte prefix → normal transfer (no forwarding).
- Byte parsing conformance: `decodeVirtualAddress` MUST treat address bytes as `[0:4]=prefix,[4:12]=masterId,[12:20]=userTag` independent of host endianness.
- `masterId` collision: two different `(address, salt)` pairs that hash to the same `bytes8` → second registration reverts with `MasterIdCollision`. Retrying with a different salt succeeds.
