...
`setRewardRecipient` is **not** a TIP-20 transfer-path operation and is therefore not subject to TIP-1022 recipient resolution. Implementations SHOULD reject virtual addresses when setting reward recipients so that rewards remain tied to canonical accounts rather than aliases.
...
In the event of a `masterId` collision (two `(address, salt)` pairs mapping to the same 4-byte `masterId`), the second registration reverts with `MasterIdCollision`. The caller can retry with a different valid salt.
...
4. **Address-level recipient check**: if TIP-1028 is active, apply any address-level receive-policy / token-set checks to `effectiveRecipient`, not the literal virtual address.
5. **Apply balance changes**: debit sender, credit `effectiveRecipient`.
6. **Emit events**: per **Event Emission** (two-hop `Transfer` if virtual, single `Transfer` otherwise).
...
3. **Address-level recipient check**: if TIP-1028 is active, apply any address-level receive-policy / token-set checks to `effectiveRecipient`, not the literal virtual address.
4. **Apply balance changes**: credit `effectiveRecipient`.
...
TIP-1022 does not introduce new authorization logic in TIP-403 itself. Instead, TIP-20 transfer and mint logic MUST resolve virtual recipient addresses before invoking the existing TIP-403 / TIP-1015 / TIP-1028 checks.
...
3. If address-level receive controls are active, evaluate them against the resolved master address as receiver.

This preserves view/execution symmetry with the TIP-20 authorization path defined by TIP-1015 and TIP-1028: any internal TIP-20 helper such as `isTransferAuthorized(from, to)` MUST evaluate recipient authorization against the resolved master address when `to` is virtual.
...
TIP-1022 does **not** introduce new transfer-path events. The registry precompile emits `MasterRegistered`, but forwarding itself is represented using **two-hop standard `Transfer` events**: one hop showing funds arriving at the virtual address, and a second hop showing funds moving from the virtual address to the resolved master (or vice versa for `transferThroughVirtual`). This pattern preserves compatibility with existing indexers, explorers, and analytics tools that already understand standard TIP-20 / ERC-20 `Transfer` events.
...
interface ITIP20VirtualSource {
    /// @notice `virtualFrom` does not match the virtual-address format.
    ...
    ///      - match the virtual-address format
...
For `transferThroughVirtual` and `transferThroughVirtualWithMemo`:

1. **Validate source**: `virtualFrom` MUST match the virtual-address format (`InvalidVirtualSender` on failure).
...
5. **Authorization checks**:
   - apply the standard sender authorization rules to `msg.sender` (the resolved master)
   - apply the standard recipient authorization rules to `effectiveRecipient`
   - if TIP-1028 is active, apply address-level receive checks to `effectiveRecipient`
6. **Apply balance changes**: debit `msg.sender` balance, credit `effectiveRecipient`. `virtualFrom` is never debited.
7. **Emit events**: per the rules below. For `transferThroughVirtualWithMemo`, also emit `TransferWithMemo`.
...
The self-transfer cases are specified explicitly to avoid pathological event shapes such as `Transfer(virtualFrom, virtualFrom, amount)` and to keep indexer behavior simple and predictable.
...
## Interaction with TIP-403 and TIP-1028

Virtual address resolution happens **before** TIP-403 / TIP-1015 / TIP-1028 authorization checks. Policy evaluation uses the resolved `masterAddress`, not the literal virtual address.
...
TIP-403 / TIP-1028 operations that accept addresses as policy members MUST reject virtual addresses rather than accepting them silently.
...
Accordingly, TIP-403 / TIP-1028 configuration operations SHOULD reject virtual addresses with explicit errors rather than accepting them.
...
5. **Event consistency**: For standard recipient-forwarding entrypoints, the precompile MUST emit two `Transfer` events: `Transfer(sender, virtualAddress, amount)` followed by `Transfer(virtualAddress, masterAddress, amount)`. For `transferThroughVirtual`, the precompile MUST emit `Transfer(masterAddress, virtualFrom, amount)` followed by one or two hops depending on whether `to` is virtual. `TransferWithMemo` events MUST immediately follow their matching `Transfer` and MUST use `virtualAddress` as the recipient to preserve deposit attribution. `Mint` events MUST use `virtualAddress`.
...
11. **View/execution symmetry**: TIP-20 authorization logic MUST evaluate recipient authorization against the resolved master address when `to` is virtual, matching execution-time recipient resolution semantics.

12. **Policy on master**: TIP-403 / TIP-1015 / TIP-1028 authorization for virtual-forwarded transfers and mints MUST check the resolved `masterAddress`. Policies set on individual virtual addresses MUST be ignored by the TIP-20 transfer path.

13. **Policy-operation rejection**: TIP-403 / TIP-1028 configuration operations that accept literal addresses as policy subjects or members MUST reject virtual addresses.
