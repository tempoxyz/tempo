...
interface IVirtualAddressRegistry {
    ...
    function getMasterId(address masterAddress)
        external view returns (bool isRegistered, bytes8 masterId);
    ...
    function proposeUpdateMaster(address newMaster, uint256 deadline, bytes calldata newMasterSig)
        external;
...
- Event ordering for `transferFromVirtual` matches the rules in **Event Emission**.
