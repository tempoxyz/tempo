...
Zone and provable-contract flows must require `isSet == true`. Once TIP-1092 is active,
`ZonePortal` calls `migrateTransferPolicyIds` before enabling a token, then rejects the token if the
returned `isSet` is still false. Before activation, `ZonePortal` detects the unavailable selector
and preserves the existing token-enablement behavior.
...
Operators manually migrate existing tokens that are already enabled on a running zone. Zone token enablement performs migration before accepting the token. An admin policy update also creates the binding for an otherwise unmigrated token, but does not copy the prior local policy ID because `newPolicyId` replaces it.
