...
Zone and provable-contract flows must require `isSet == true`. `ZonePortal` calls
`migrateTransferPolicyIds` before enabling a token, then rejects the token if the returned `isSet`
is still false.
