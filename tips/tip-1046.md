...
## 3. Key-auth scope gas accounting

**PRs**: [#3595](https://github.com/tempoxyz/tempo/pull/3595), [#3689](https://github.com/tempoxyz/tempo/pull/3689), [#3680](https://github.com/tempoxyz/tempo/pull/3680) · **Authors**: @rakita, @legion2002

Scoped key authorizations need additional intrinsic gas to cover the helper bookkeeping around scope persistence, and repeated scope-set length-slot writes need to be charged like warm resets instead of fresh `SSTORE_SET` rows. T4+ applies the rounded scope surcharge described in [TIP-1011](./tip-1011.md), and also aligns repeated target and selector length-slot updates with the actual storage-touch pattern used by scope persistence.
...
## 5. Check paused state on internal-balance DEX debit paths
...
## 6. Account creation gas deduction and nonce bump for precompile-created accounts

**PR**: [#3207](https://github.com/tempoxyz/tempo/pull/3207) · **Author**: @rakita

Precompile-driven account creation currently installs code without charging the full account-creation costs or reflecting creation semantics on the target account nonce. T4+ treats this path as account creation: it charges the account-creation gas, stores code through an explicit create-account operation, and bumps the newly created account nonce so the resulting state matches the intended creation semantics.
...
**PR**: [#3746](https://github.com/tempoxyz/tempo/pull/3746) · **Author**: @arsenii
...
- **[TIP-1016](./tip-1016.md)**: Exempt Storage Creation from Gas Limits — specifies the state-gas and reservoir-model changes that are tracked under the TIP-1016 rollout rather than this meta TIP.
