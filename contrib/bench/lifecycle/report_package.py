...
def write_package(data, out, chunk_intervals=CHUNK_INTERVALS, full=False):
    ...
    for old in out.iterdir():
        if re.fullmatch(r'(?:context-\d+\.(?:html|json)|(?:block|attempt)-\d+\.html|perfetto(?:-(?:block|attempt)-\d+|-p(?:50|90|99))?\.json)', old.name):
            old.unlink()
    base = {k: v for k, v in data.items() if k not in ('spans', 'transfers', 'blocks')}
    base['population_blocks'] = data['blocks']
    base['packaged'] = True
...
def write_index(data, manifest, out):
    ...
    async_link = '<p><a href="async-tasks.html">Selected async task observations and bounded Perfetto context</a></p>' if data.get('async_tasks_files') else ''
...
if __name__ == '__main__':
    ...
    if args.report.resolve() != (args.out/'lifecycle.json').resolve():
        ...
        for source in args.report.parent.iterdir():
            if re.fullmatch(r'[a-z]\.jsonl|window\.json|async-tasks(?:-index|-\d{4,})?\.json|async-tasks\.html', source.name):
