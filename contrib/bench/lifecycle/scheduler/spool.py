...
def footer(data):
    if len(data) == WAIT_FOOTER.size:
        ...
        version=values.pop()
        if (magic,version) not in ((WAIT_MAGIC,1),(FAULT_MAGIC,2)):raise ValueError('unexpected binary scheduler output')
        wait_enabled=version
    ...
    if wait_enabled:result['wait_reasons']=wait_enabled
