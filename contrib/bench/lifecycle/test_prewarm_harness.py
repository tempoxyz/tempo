...
class PrewarmHarness(unittest.TestCase):
    ...
    def test_actual_workflow_routes_observer_to_nu_not_txgen(self):
        ...
        self.assertIn("BENCH_LIFECYCLE_PREWARM_CPU: ${{ inputs.profiling == 'lifecycle-compare-prewarm-cpu' && 'compare' || 'disabled' }}",workflow)
        for key in ('BENCH_LIFECYCLE:','BENCH_OTLP:','BENCH_VALSCOPE:','BENCH_RUN_SIDE:'):
            self.assertIn('lifecycle-compare-prewarm-cpu',next(line for line in workflow.splitlines() if key in line))
        ...
        environment.update(BENCH_LIFECYCLE='true',BENCH_LIFECYCLE_DETAIL='milestones',BENCH_LIFECYCLE_PREWARM_CPU='compare',BENCH_TRACY='off',BENCH_BENCH_ARGS='--existing-txgen-flag 7')
        args=subprocess.check_output(['bash','-c',script],env=environment).decode().rstrip('\0').split('\0')
        self.assertEqual(args[args.index('--lifecycle-prewarm-cpu')+1],'compare')
    ...
    def test_same_binary_guard_and_invalid_modes_before_work(self):
        ...
        for args in (['--lifecycle-prewarm-cpu','compare'],['--lifecycle','--lifecycle-prewarm-cpu','compare','--run-side','feature'],['--lifecycle','--lifecycle-detail','full','--lifecycle-prewarm-cpu','compare']):
            result=subprocess.run(['nu','bench-e2e.nu','e2e',*args],cwd=ROOT,text=True,capture_output=True,env={**os.environ,'BENCH_BINARY_MODE':'build_v1','BENCH_RUN_CLEANUP':'false'})
    ...
    def test_immutable_same_inputs_and_no_environment_override(self):
        ...
        for extra in ([],['--baseline',sha,'--feature','2'*40],['--baseline',sha,'--feature',sha,'--baseline-env','CUSTOM=1'],['--baseline',sha,'--feature',sha,'--feature-args=--engine.prewarm-threads 32']):
            result=subprocess.run(base+extra,cwd=ROOT,text=True,capture_output=True,env={**os.environ,'BENCH_BINARY_MODE':'build_v1','BENCH_RUN_CLEANUP':'false'})
