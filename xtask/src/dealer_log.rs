...
use commonware_cryptography::{PublicKey, Signer, bls12381::primitives::variant::Variant};
...
impl<P: PublicKey> Read for DealerResult<P> {
    ...
    fn read_cfg(buf: &mut impl Buf, cfg: &Self::Cfg) -> Result<Self, Error> {
        ...
        match tag {
            0 => {
                // Read the map as a vec of (key, value) pairs
                let range_cfg = RangeCfg::from(0..=cfg.get() as usize);
                let entries: Vec<(P, AckOrReveal<P>)> =
                    Read::read_cfg(buf, &(range_cfg, ((), ())))?;
                Ok(Self::Ok(entries))
            }
...
impl<V: Variant, P: PublicKey> InspectableDealerLog<V, P> {
    ...
    pub(crate) fn ack_count(&self) -> usize {
        match &self.results {
            DealerResult::Ok(entries) => entries
                .iter()
                .filter(|(_, aor)| matches!(aor, AckOrReveal::Ack(_)))
                .count(),
    ...
    pub(crate) fn reveal_count(&self) -> usize {
        match &self.results {
            DealerResult::Ok(entries) => entries
                .iter()
                .filter(|(_, aor)| matches!(aor, AckOrReveal::Reveal(_)))
                .count(),
            DealerResult::TooManyReveals => 0,
        }
    }
...
/// A custom SignedDealerLog that exposes the dealer public key directly.
#[derive(Clone, Debug)]
#[allow(dead_code)]
pub(crate) struct InspectableSignedDealerLog<V: Variant, S: Signer> {
    pub dealer: S::PublicKey,
    pub log: InspectableDealerLog<V, S::PublicKey>,
    pub sig: S::Signature,
}

impl<V: Variant, S: Signer> Read for InspectableSignedDealerLog<V, S> {
    type Cfg = NonZeroU32;
...
impl<V: Variant, S: Signer> InspectableSignedDealerLog<V, S> {
    ...
    /// Returns the dealer's public key.
    pub(crate) fn dealer(&self) -> &S::PublicKey {
        &self.dealer
    }
    ...
    /// Returns the number of reveals in this dealer log.
    pub(crate) fn reveal_count(&self) -> usize {
        self.log.reveal_count()
    }

    /// Returns true if this log indicates too many reveals.
    pub(crate) fn has_too_many_reveals(&self) -> bool {
        self.log.has_too_many_reveals()
    }

    /// Returns the player results from the inner log.
    pub(crate) fn player_results(&self) -> Vec<PlayerResult<S::PublicKey>> {
        self.log.player_results()
    }
}
