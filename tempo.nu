...
def generate-summary [
    ...
    let summary_lines = ($config_lines | append [
        ...
        (do $count_row "Expected Validation Reverts" $b_builder_expected_validation_reverts $f_builder_expected_validation_reverts)
        (do $count_row "Expired Tx Reverts" $b_builder_valid_before_reverts $f_builder_valid_before_reverts)
        (do $count_row "Expiring Nonce Replay Reverts" $b_builder_expiring_nonce_replay_reverts $f_builder_expiring_nonce_replay_reverts)
        (do $count_row "Expiring Nonce Set Full Reverts" $b_builder_expiring_nonce_set_full_reverts $f_builder_expiring_nonce_set_full_reverts)
        (do $count_row "Insufficient Funds Reverts" $b_builder_insufficient_funds_reverts $f_builder_insufficient_funds_reverts)
