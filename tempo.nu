...
def run-dev-node [accounts: int, genesis: string, samply: bool, samply_args: list<string>, reset: bool, profile: string, loud: bool, extra_args: list<string>] {
    ...
    let args = (build-base-args $genesis_path $datadir $log_dir 8545 9545 9001)
...
def build-base-args [genesis_path: string, datadir: string, log_dir: string, http_port: int, ws_port: int, reth_metrics_port: int] {
    [
        "node"
        "--chain" $genesis_path
        "--datadir" $datadir
...
def run-consensus-node [
    ...
    print $"  Node ($addr) -> http://localhost:($http_port) ws://localhost:($ws_port) (if $background { '' } else { '(foreground)' })"
...
def build-consensus-node-args [node_dir: string, genesis_path: string, trusted_peers: string, port: int, log_dir: string] {
    let node_index = (port-to-node-index $port)
    let http_port = 8545 + $node_index
    let metrics_port = 9545 + $node_index
    let reth_metrics_port = 9001 + $node_index

    (build-base-args $genesis_path $node_dir $log_dir $http_port $metrics_port $reth_metrics_port)
...
def "main follower" [
    ...
    let args = (build-base-args $genesis_path $follower_dir $log_dir $http_port $ws_port $reth_metrics_port)
        | append [
            "--follow-node" "ws://127.0.0.1:9545"
            "--trusted-peers" $trusted_peers
            "--port" $"($el_p2p_port)"
    ...
    print $"Follower -> http://localhost:($http_port) ws://localhost:($ws_port)"
