...
function metricsWindow(series, node, from, to) {
  ...
  assert.ok(gas.delta > 0, 'no executed gas');
  const deltas = {};
  ...
  const perMgas = value => value === null ? null : value / gas.delta * 1e6;
  const stopReasons = {};
  ...
  return {
    ...
    storage_misses_per_mgas: mixedPaths ? null : perMgas(misses), storage_miss_percent: 100 * misses / (misses + hits),
    execution_seconds: execution, execution_ms_per_mgas: perMgas(execution) * 1000,
    aggregate_execution_mgas_per_second: execution > 0 ? gas.delta / execution / 1e6 : null,
    payload_build_seconds: build, payload_build_ms_per_mgas: build === null ? null : perMgas(build) * 1000,
    ...
    prewarm,
    device_io: { ...disk, reads_per_mgas: perMgas(disk.reads_completed_total ?? null), bytes_per_gas: disk.read_bytes_total === undefined ? null : disk.read_bytes_total / gas.delta, writes_per_mgas: perMgas(disk.writes_completed_total ?? null), written_bytes_per_mgas: perMgas(disk.written_bytes_total ?? null), scope: 'device-wide, not execution-attributed' },
    counters: deltas,
  };
}
