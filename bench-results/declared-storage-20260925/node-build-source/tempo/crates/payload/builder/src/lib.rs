        ...
        StateProviderFactory + ChainSpecProvider<ChainSpec = TempoChainSpec> + Clone + 'static,
{
        ...
        Txs: BestTransactions<Item = BestTransaction> + Send + 'static,
    {
        ...
        // Opt-in bench diagnostics include work discarded by proposal cancellation.
        let time_transaction_attempts =
            std::env::var("TEMPO_BENCH_TX_TIMING").is_ok_and(|value| value == "1");
        ...
        let block_build_stop_reason = loop {
            ...
            let attempt_start = time_transaction_attempts.then(Instant::now);
            if time_transaction_attempts {
                tracing::info!(target: "payload_builder", tx_hash = ?tx.hash(),
                    build_elapsed_us = start.elapsed().as_micros() as u64,
                    "Bench transaction attempt started");
            }
            ...
            if let Some(attempt_start) = attempt_start {
                tracing::info!(target: "payload_builder", tx_hash = ?tx.hash(),
                    execution_us = attempt_start.elapsed().as_micros() as u64,
                    build_elapsed_us = start.elapsed().as_micros() as u64,
                    cancelled = cancel.is_cancelled(), valid = execution_result.is_ok(),
                    "Bench transaction attempt finished");
            }
