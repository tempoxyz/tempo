...
(async()=>{
 const r=JSON.parse(fs.readFileSync(path.join(__dirname,'read-interim.json'))),seen=new Set();let skipped=0;
 const logs=(await readLogs(path.join(__dirname,'events-a.jsonl'))).filter(x=>Date.parse(x.timestamp)<=r.canonical.to_unix_ms+90000).filter(x=>{
 ...
 fs.writeFileSync(path.join(__dirname,'read-latency.json'),JSON.stringify(out,null,2)+'\n');
