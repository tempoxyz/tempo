...
- The standard development mnemonic and 20 prefunded accounts.
...
| `--block-time <duration>` | Set the block interval. Default: `1s`. Examples: `200ms`, `2s`. |
...
docker run --rm -p 9545:8545 \
  ghcr.io/tempoxyz/tempo-localnet:latest
```

The RPC binds to all interfaces inside the container. Do not publish it on a public host.
...
```bash
container_id=$(docker run -d --rm -p 8545:8545 \
  ghcr.io/tempoxyz/tempo-localnet:latest)
