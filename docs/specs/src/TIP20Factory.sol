...
contract TIP20Factory is ITIP20Factory {
    ...
    function createToken(
        ...
        if (keccak256(bytes(currency)) == keccak256(bytes("USD"))) {
            if (
                keccak256(bytes(quoteToken.currency())) !=
                keccak256(bytes("USD"))
            ) {
                revert InvalidQuoteToken();
            }
        ...
        uint64 lowerBytes = uint64(
            bytes8(keccak256(abi.encode(msg.sender, salt)))
        );
        ...
        address tokenAddr = address(
            (uint160(0x20C000000000000000000000) << 64) | uint160(lowerBytes)
        );
