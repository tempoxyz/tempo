...
contract StablecoinExchangeTest is BaseTest {
    ...
    function test_PlaceAskOrder() public {
        ...
        (uint128 askHead, uint128 askTail, uint128 askLiquidity) = exchange
            .getTickLevel(address(token1), 100, false);
    ...
    function test_ImmediateOrderActivation_MultipleOrders(
        uint8 numOrders
    ) public {
        ...
        (uint128 head, , uint128 liquidity) = exchange.getTickLevel(
            address(token1),
            0,
            true
        );
    ...
    function test_ImmediateOrderActivation_MultipleBatches(
        uint8 batch1,
        uint8 batch2
    ) public {
