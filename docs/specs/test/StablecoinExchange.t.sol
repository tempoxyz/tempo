...
contract StablecoinExchangeTest is BaseTest {
    ...
    function testFuzz_Cancel_ValidationRules(uint128 orderId, address caller) public {
        ...
        vm.prank(alice);
        uint128 validOrderId = exchange.place(address(token1), MIN_ORDER_AMOUNT, true, 100);
    ...
    function test_Routing_RevertIf_NoPathExists() public {
        ...
        try exchange.quoteSwapExactAmountIn(
            address(token1), address(isolatedToken), MIN_ORDER_AMOUNT
        ) {
            revert CallShouldHaveReverted();
        } catch {
    ...
    function testFuzz_Routing_TokenPairCombinations(
        ...
        try exchange.quoteSwapExactAmountIn(tokenIn, tokenOut, MIN_ORDER_AMOUNT) returns (
            uint128 amountOut
        ) {
    ...
    function test_Routing_RevertIf_InvalidPath() public view {
        ...
        try exchange.quoteSwapExactAmountIn(invalidToken, address(token1), MIN_ORDER_AMOUNT) {
            revert CallShouldHaveReverted();
        } catch {
        ...
        try exchange.quoteSwapExactAmountIn(address(token1), invalidToken, MIN_ORDER_AMOUNT) {
            revert CallShouldHaveReverted();
        } catch {
            // Successfully reverted
        }
    ...
    {
        vm.expectEmit(true, true, true, true);
        emit OrderPlaced(exchange.pendingOrderId() + 1, user, address(token1), amount, true, tick);
    ...
    {
        vm.expectEmit(true, true, true, true);
        emit OrderPlaced(exchange.pendingOrderId() + 1, user, address(token1), amount, false, tick);
