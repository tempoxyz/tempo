...
contract FeeAMMTest is BaseTest {
    ...
    function setUp() public override {
        ...
        userToken = TIP20(
            factory.createToken("User", "USR", "USD", pathUSD, admin)
        );
        validatorToken = TIP20(
            factory.createToken("Validator", "VAL", "USD", pathUSD, admin)
        );
    ...
    function test_MintWithValidatorToken_InitialLiquidity_RevertsIf_TooSmall()
        public
    {
    ...
    function test_MintWithValidatorToken_RevertsIf_InvalidInputs() public {
            ...
            )
        {
            ...
            assertEq(
                err,
                abi.encodeWithSelector(IFeeAMM.IdenticalAddresses.selector)
            );
        }
            ...
            )
        {
            ...
            assertEq(
                err,
                abi.encodeWithSelector(IFeeAMM.InvalidToken.selector)
            );
        }
        ...
        TIP20 eurToken = TIP20(
            factory.createToken("Euro", "EUR", "EUR", pathUSD, admin)
        );
            ...
            )
        {
            ...
            assertEq(
                err,
                abi.encodeWithSelector(IFeeAMM.InvalidCurrency.selector)
            );
        }
    ...
    function test_MintWithValidatorToken_RevertsIf_ZeroLiquidityOnSubsequent()
        public
    {
    ...
    function test_MintWithValidatorToken_SubsequentDeposit() public {
        ...
        assertEq(
            amm.liquidityBalances(poolId, alice),
            aliceBalanceBefore + liquidity
        );
    }
    ...
    function test_Burn_RevertsIf_InsufficientLiquidity() public {
            ...
            )
        {
            ...
            assertEq(
                err,
                abi.encodeWithSelector(IFeeAMM.InsufficientLiquidity.selector)
            );
        }
    }
    ...
    function test_Burn_Succeeds() public {
        ...
        vm.prank(alice);
        (uint256 amountU, uint256 amountV) = amm.burn(
            address(userToken),
            address(validatorToken),
            aliceLiquidity,
            alice
        ...
        assertEq(
            validatorToken.balanceOf(alice),
            aliceValidatorBefore + amountV
        );
    }
    ...
    function test_RebalanceSwap_Succeeds() public {
        ...
        vm.prank(alice);
        uint256 amountIn = amm.rebalanceSwap(
            address(userToken),
            address(validatorToken),
            100e18,
            alice
