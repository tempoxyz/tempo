...
You can only place orders on pairs between a token and its designated quote token. All TIP-20 tokens specify a quote token for trading pairs. [PathUSD](/documentation/protocol/exchange/pathUSD) can be used as a simple choice for quote token.
