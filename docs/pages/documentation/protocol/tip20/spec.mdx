...
interface ITIP20 {
    ...
    /// @notice Emitted when a reward stream is canceled and remaining funds are refunded
    /// @param funder The account that originally funded the reward stream
    /// @param id The identifier of the reward stream that was canceled
    /// @param refund The amount of unclaimed rewards that were refunded
    ...
    /// @notice Emitted when a new reward stream is scheduled
    /// @param funder The account funding the reward stream
    /// @param id The identifier of the reward stream
    /// @param amount The total amount of tokens allocated to the reward stream
    /// @param durationSeconds The duration of the reward stream in seconds
    ...
    /// @notice The caller is not the recorded funder of the referenced reward stream
    error NotStreamFunder();

    /// @notice The configured transfer policy denies authorization for the sender or recipient
    error PolicyForbids();

    /// @notice The referenced reward stream is inactive, does not exist, or has already ended
...
## System Functions
System level functions `systemTransferFrom`, `transferFeePreTx`, and `transferFeePostTx` are only callalbe by other Tempo protocol precompiles. These entrypoints power transaction fee collection, refunds, and internal accounting within the Fee AMM and StablecoinExchange. They must not be callable by general contracts or users.
