...
Add validator token to the pool to receive LP tokens representing your share. The first liquidity provider to a new pool must burn 1,000 units of liquidity. This costs approximately 0.002 USD and prevents attacks on pool reserves. Learn more in the [Fee AMM specification](/documentation/protocol/fees/spec-fee-amm).
...
You can rebalance pools by swapping validator tokens for accumulated user tokens at a fixed rate. Rebalancing restores validator token reserves and enables continued fee conversions. Learn more [here](/documentation/protocol/fees/spec-fee-amm#swap-mechanisms).
...
<Card.Container>
  ...
  <Card.Link
    ...
    href="/documentation/protocol/fees/fee-amm"
    icon={LucideDroplet}
