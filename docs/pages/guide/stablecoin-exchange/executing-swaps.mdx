...
Execute swaps between stablecoins on the exchange. Swaps execute immediately against existing orders in the order book, providing instant liquidity for cross-stablecoin payments.
...
Quote requests will fail with an `InsufficientLiquidity` error if there isn't enough liquidity in the order book to satisfy the requested amount.
...
<Card.Container>
  <Card.Link
    description="Learn about how the Stablecoin Exchange interacts with your balances"
