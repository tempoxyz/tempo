...
The notable difference between RPC nodes and validator nodes is the emission of the `--follow` argument and the addition of the `--consensus.signing-key` and `--consensus.fee-recipient` arguments.
