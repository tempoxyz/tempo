...
* validator wallet - the recipient of transaction fees in your validators' proposed blocks
...
The notable difference between RPC nodes and validator nodes is the omission of the `--follow` argument and the addition of the `--consensus.signing-key` and `--consensus.fee-recipient` arguments.
...
The `consensus_engine_dkg_manager_ceremony_successes_total` metric should be greater than 0.
