...
We endorse [this proposed ERC](https://github.com/ethereum/ERCs/pull/1220) to 
...
>
  <p>
    We strongly recommend using <a href="/guide/quickstart/integrate-tempo#tempo-transactions">Tempo Transactions</a> over legacy Ethereum transactions if you have control over the transaction submission, in order to leverage features like native gas sponsorship.
  </p>
</Card.Notice>
...
As specified in the preference system above, the simplest way to specify the fee token for a user is to set the user default fee token. Read about how to do that [here](http://localhost:5173/documentation/protocol/fees/spec-fee#account-level) on behalf of an account.
...
On the Tempo Testnet, PathUSD is available to be deposited from the [faucet](/guide/quickstart/faucet).
