...
Rewards can be distributed by anyone on any TIP-20 token, and claimed by any holder who has opted in. This guide covers both the issuer and token holder use cases: in general issuers distribute rewards for holders to claim. While the demo below uses a token you create, the same principles apply to any token.

## Demo

Try out the complete rewards flow: issue a token, opt in to receive rewards on it, create a reward for yourself, and claim it.
...
If you would like to distribute rewards on a token you have issued, follow the [Create a Stablecoin](/guide/issuance/create-a-stablecoin) guide to deploy your token.
...
Anyone can start a reward distribution that allocates tokens to all opted-in holders proportionally based on their balance. In general, this will be done by the issuer.
