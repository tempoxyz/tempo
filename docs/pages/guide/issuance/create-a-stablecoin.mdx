...
Before we send off a transaction to deploy our stablecoin to Tempo, we need to make sure our account is funded with a stablecoin to cover the transaction fee. 

As we have configured our project to use `DONOTUSE` (`0x20c000…33abb6ac7d235e5`) 
as the [default fee token](/quickstart/integrate-tempo#default-fee-token), we will need to add some `DONOTUSE` to our account.

Luckily, the built-in Tempo faucet supports funding accounts with `DONOTUSE`.
