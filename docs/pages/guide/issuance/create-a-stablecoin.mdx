...
We can use the `isPending` property from the `useCreateSync` hook to show the pending state to the user on our "Create" button.
...
We can use the `error` property from the `useCreateSync` hook to show the error state to the user.
