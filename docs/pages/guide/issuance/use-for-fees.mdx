...
Before users can pay fees with your token, ensure there is sufficient liquidity in the fee pool. On the mainnet candidate, validators expect fees in DONOTUSE, so you need to add DONOTUSE liquidity to your token's fee pool.
...
const validatorToken = '0x20c000000000000000000000033abb6ac7d235e5' // DONOTUSE token

const mintFeeLiquidity = Hooks.amm.useMintSync() // [!code hl]

// Add 100 DONOTUSE of liquidity to the fee pool // [!code hl]
...
address validatorToken = 0x20c000000000000000000000033abb6ac7d235e5; // DONOTUSE

// Add 100 DONOTUSE of liquidity to the fee pool
feeAmm.mint(
  yourToken,
  validatorToken,
...
const { data: pool } = Hooks.amm.usePool({
  userToken: yourToken,
  validatorToken: '0x20c0000000000000000000000000000000000001', // DONOTUSE token
})
...
export function PayWithIssuedToken() {
  ...
  const paymentToken = '0x20c000000000000000000000033abb6ac7d235e5' // DONOTUSE
...
```solidity [Solidity]
ITIP20 token = ITIP20(0x20c000000000000000000000033abb6ac7d235e5); // DONOTUSE
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // DONOTUSE
    let validator_token = address!("0x20c000000000000000000000033abb6ac7d235e5"); // [!code focus]
...
Before promoting fee payments with your token, thoroughly test the flow:
