...
Before you can send a payment, you need to fund your account. In this guide you will be sending `DONOTUSE` (`0x20c000…0001`).

The built-in Tempo faucet funds accounts with `DONOTUSE`.

<Demo.Container name="Add Funds" footerVariant="balances" showBadge={false} tokens={[Demo.DONOTUSE]}>
...
Now that you have `DONOTUSE` you are ready to add logic to send a payment with an optional memo.

<Demo.Container name="Send Payment" footerVariant="balances" showBadge={false} tokens={[Demo.DONOTUSE]}>
    ...
    <form onSubmit={
      (event) => {
        ...
        sendPayment.mutate({ // [!code hl]
          ...
          token: '0x20c000000000000000000000033abb6ac7d235e5', // [!code hl]
...
const { receipt } = await client.token.transferSync({
  ...
  token: '0x20c000000000000000000000033abb6ac7d235e5', // DONOTUSE
})
```
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let token = ITIP20::new( // [!code focus]
        address!("0x20c000000000000000000000033abb6ac7d235e5"), // DONOTUSE // [!code focus]
        &provider, // [!code focus]
    ); // [!code focus]
...
const calls = [
  {
    to: '0x20c000000000000000000000033abb6ac7d235e5', // DONOTUSE address
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let token_address: Address = address!("0x20c000000000000000000000033abb6ac7d235e5"); // [!code focus]
