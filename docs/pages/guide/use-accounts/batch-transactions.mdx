...
One of the most powerful features of the [Tempo Transaction](/documentation/protocol/transactions/spec-tempo-transaction) type is batching multiple operations into a single transaction. This allows you to execute several actions atomically (i.e., they all succeed or all fail together). This helps reduce gas costs, prevents partial failures, and creates better user experiences by combining multiple steps into one transaction.

<Callout type="info">
  Batch transactions are a feature of Tempo's [TempoTransaction type](/documentation/protocol/transactions/spec-tempo-transaction). See the TempoTransaction spec for complete details on batching and other features.
</Callout>
