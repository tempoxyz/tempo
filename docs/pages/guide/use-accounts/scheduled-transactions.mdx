...
Scheduled transactions are enabled by Tempo's [Account Abstraction transaction type](/documentation/protocol/transactions/spec-account-abstraction), which includes optional timestamp fields. These fields are validated by the protocol when the transaction is submitted.
...
For complete specifications on timestamp validation, see the [Account Abstraction](/documentation/protocol/transactions/spec-account-abstraction) transaction type specification.
