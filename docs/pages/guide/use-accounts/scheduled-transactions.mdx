...
Scheduled transactions are enabled by the [Tempo Transaction](/documentation/protocol/transactions/spec-tempo-transaction) type, which includes optional timestamp fields. These fields are validated by the protocol when the transaction is submitted.
...
For complete specifications on timestamp validation, see the [Tempo Transaction](/documentation/protocol/transactions/spec-tempo-transaction) type specification.
