...
:::info
The code samples in this guide will place orders on the `DONOTUSE` / `pathUSD` pair.

The "spend" token is the token that will be spent to place the order.
- buying `DONOTUSE` spends `pathUSD`
- selling `DONOTUSE` spends `DONOTUSE`
:::
...
function ApproveSpend(props: { orderType: 'buy' | 'sell' }) {
  ...
  // buying DONOTUSE requires we spend pathUSD // [!code hl]
  const spendToken = orderType === 'buy' ? pathUsd : DONOTUSE // [!code hl]
  // [!code hl]
...
function PlaceOrder(props: { orderType: 'buy' | 'sell' }) {
  const { orderType } = props
  // buying DONOTUSE requires we spend pathUSD
  const spendToken = orderType === 'buy' ? pathUsd : DONOTUSE
...
const { data: metadata } = Hooks.token.useGetMetadata({ // [!code focus]
  token: '0x20c0000000000000000000000000000000000001', // DONOTUSE // [!code focus]
}) // [!code focus]
