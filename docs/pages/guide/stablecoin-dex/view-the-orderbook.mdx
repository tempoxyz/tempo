...
Find the highest bid prices (buyers) for DONOTUSE. This query filters out fully filled and cancelled orders, groups by price level (tick), and shows the top 5 bid prices with their total liquidity.

<IndexSupplyQuery
  title={'Best Bid Prices for DONOTUSE'}
  query={`SELECT
    tick as price,
    SUM(amount) as total_liquidity
  FROM orderplaced
  WHERE token = '0x20c000000000000000000000033abb6ac7d235e5'
    AND "isBid" = true
    AND NOT EXISTS (
...
Find the lowest ask prices (sellers) for DONOTUSE. The spread is the difference between the highest bid and lowest ask price.

<IndexSupplyQuery
  title={'Best Ask Prices for DONOTUSE'}
  query={`SELECT
    tick as price,
    SUM(amount) as total_liquidity
  FROM orderplaced
...
This query inspects the details of the most recent order for DONOTUSE. It shows when the order was created, at what price (tick), the order size, and who placed it.
...
Check if an order has been cancelled. This query returns an order for DONOTUSE that was explicitly cancelled by the maker before being fully filled.
...
<IndexSupplyQuery
  ...
  WHERE op.token = '0x20c000000000000000000000033abb6ac7d235e5'
...
<IndexSupplyQuery
  title={'Recent Trade Prices for DONOTUSE'}
  query={`SELECT
    ofl.block_num,
    ofl."orderId",
    o.tick as price,
  ...
  WHERE o.token = '0x20c000000000000000000000033abb6ac7d235e5'
