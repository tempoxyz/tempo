...
It also consolidates onchain liquidity into a single system that simplifies routing and makes it easy for users to trade between stablecoins, or send cross-stablecoin payments. This eliminates the need for external DEX infrastructure and provides optimal pricing for stablecoin conversions.

<Card.Notice title="Exchange Specification" icon={LucideArrowLeftRight} inline>
  Learn more in the [exchange specification](/protocol/exchange).
</Card.Notice>
