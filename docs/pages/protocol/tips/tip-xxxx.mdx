...
interface IValidatorConfigV2 {
    ...
    /// @notice Migrate validators from V1 to V2 (owner only)
    /// @dev Can be called multiple times to batch migration. Each call migrates validators
    ///      from startIndex to endIndex (exclusive). Does not set initialized flag.
    ...
    /// @param startIndex Start index in V1 validators array (inclusive)
    /// @param endIndex End index in V1 validators array (exclusive)
    function migrateValidators(uint64 startIndex, uint64 endIndex) external;

    /// @notice Finalize migration and enable V2 (owner only)
    /// @dev Can only be called after all validators have been migrated via migrateValidators.
    ///      Sets initialized=true. After this call, CL reads from V2 instead of V1.
    ///      Also copies owner and nextDkgCeremony from V1 if not already set.
...
2. **Manual migration**: Admin migrates validators in batches, then flips the
   `initialized` flag. CL reads from V1 until the flag is set.
...
V2 uses manual migration where the admin explicitly migrates validators in
batches and then flips the `initialized` flag. The `initialized` bit (bit 255
of slot 0) tracks whether migration is complete.
...
**`migrateValidators(startIndex, endIndex)`**:
- Reads validators from V1 at indices `[startIndex, endIndex)`
- For each V1 validator:
...
- Can be called multiple times to batch migration across multiple transactions
...
- **Batched migration**: Can split migration across multiple transactions
- **Owner controlled**: Only admin can migrate and finalize
- **No tx gas limit risk**: Migration can be batched to fit within gas limits
...
```
Before Fork           Post-Fork (V2 not init)    Admin Migration           After finalizeMigration
     │                      │                          │                          │
     │  CL reads V1         │  CL reads V1             │  migrateValidators()     │  CL reads V2
     │                      │  (isInitialized=false)   │  (batched txs)           │  isInitialized=true
     │                      │                          │                          │
─────┴──────────────────────┴──────────────────────────┴──────────────────────────┴───────────────►
                            │                          │                          │
                      allegroTime              migrateValidators()      finalizeMigration()
```
...
4. **Admin migrates validators** by calling `migrateValidators(start, end)` in batches
   - Can split across multiple transactions to stay within gas limits
   - Example: `migrateValidators(0, 10)`, `migrateValidators(10, 20)`, etc.
5. **Admin finalizes migration** by calling `finalizeMigration()`
...
28. **migrateValidators imports validators**: Calling `migrateValidators(0, n)`
    correctly imports validators from V1 at those indices
29. **migrateValidators is batched**: Multiple calls to `migrateValidators` with
    non-overlapping ranges correctly imports all validators
30. **migrateValidators owner only**: Non-owner cannot call `migrateValidators`
31. **All validators imported on migration**: Both V1 active and inactive validators
    are imported; inactive ones have `addedAtHeight == deletedAtHeight == hardforkHeight`
32. **addedAtHeight set correctly**: Imported validators have
    `addedAtHeight == hardforkHeight` at migration time
33. **finalizeMigration sets flag**: After `finalizeMigration()`, `isInitialized()` returns true
34. **finalizeMigration copies owner**: V2 `owner()` matches V1 after `finalizeMigration()`
35. **finalizeMigration copies DKG ceremony**: V2 `getNextFullDkgCeremony()` matches V1
    after `finalizeMigration()`
36. **finalizeMigration owner only**: Non-owner cannot call `finalizeMigration`
37. **isInitialized returns correct value**: Returns false before finalize, true after
...
fields, array push, pubkey mapping). With the batched `migrateValidators` approach,
this is no longer a blocking issue:

```
validatorsPerTx = min(txGasLimit, blockGasLimit) / (11 * SSTORE_COST)
```

With current gas costs (~20,000 gas per cold SSTORE), each transaction can migrate
roughly `30,000,000 / (11 * 20,000) ≈ 136` validators. For networks with more
validators, simply call `migrateValidators` multiple times with different ranges.

## Migration Timing

The migration must be completed (including `finalizeMigration()`) before an epoch
...
3. Complete all `migrateValidators` calls and `finalizeMigration` atomically if
   possible, or ensure sufficient time buffer
