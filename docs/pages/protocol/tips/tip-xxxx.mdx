...
interface IValidatorConfigV2 {
    ...
    function migrateValidator(uint64 valConfigV1Idx) external;
...
**`migrateValidator(valConfigV1Idx)`**:
...
4. **Admin migrates validators** by calling `migrateValidator(valConfigV1Idx)` for each validator
