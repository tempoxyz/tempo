...
interface IValidatorConfigV2 {
    ...
    ///      Reverts if the validator at valIdx has already been migrated (valIdx != validatorsArray.length).
    ...
    /// @param valIdx Index of the validator in V1 validators array (must equal current validatorsArray.length)
...
- Reverts if `valIdx != validatorsArray.length` (ensures sequential migration)
