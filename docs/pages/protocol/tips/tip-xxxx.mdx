...
- **Validator address provided**: The V2 validator address is provided as a parameter, not copied from V1
...
29. **migrateValidator uses provided address**: The V2 validator uses `valAddress` parameter,
    not the address from V1
30. **migrateValidator reverts on duplicate**: Calling `migrateValidator(i, addr)` reverts
