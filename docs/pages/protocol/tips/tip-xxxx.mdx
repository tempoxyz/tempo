...
interface IValidatorConfigV2 {
    ...
    /// @param ingress Address where other validators can connect (format: `<hostname|ip>:<port>`)
    /// @param egress IP Address from which this validator will dial, e.g. for firewall whitelisting (format: `<ip>:<port>`, no hostnames)
    ...
    /// @param ingress The validator's inbound address `<hostname|ip>:<port>` for incoming connections
    ...
    /// @param ingress The new validator's inbound address `<hostname|ip>:<port>` for incoming connections
...
- **ingress**: Must be in `<hostname|ip>:<port>` format. Hostnames and IP
  addresses are both accepted.
- **egress**: Must be in `<ip>:<port>` format. Only IP addresses are
