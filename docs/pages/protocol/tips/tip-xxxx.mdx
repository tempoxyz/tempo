...
interface IValidatorConfigV2 {
    ...
    ///          <NAMESPACE>, contractAddress, ingress, egress
    ///      )),
    ...
    function initializeIfMigrationComplete() external;
...
13. **Atomic rotation**: `rotateValidator` atomically deletes the old validator
    (setting its `deactivatedAtHeight`) and adds a new validator entry. Both
...
─────┴──────────────────────┴──────────────────────────┴──────────────────────────┴───────────────►
                            │                          │                          │
                      hardforkTime           migrateValidator() x N         initialize()
```
...
19. **RotateValidator new pubkey exists**: Cannot rotate to existing public key
20. **RotateValidator invalid signature**: Rejects invalid signature for rotation
...
25. **addedAtHeight correctness**: Validators have correct `addedAtHeight` set
    at creation
26. **deactivatedAtHeight correctness**: Deleted validators have correct
