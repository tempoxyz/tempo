...
By recording `addedAtHeight` and `deactivatedAtHeight`, nodes can determine DKG players for any epoch using only current state. When preparing for a DKG ceremony in epoch `E+2`, a node reads the contract at `boundary(E)` and filters:
...
interface IValidatorConfigV2 {
    ...
    ///      keccak256(abi.encodePacked(<NAMESPACE>, contractAddress, validatorAddress, ingress, egress)),
    ///      where `<NAMESPACE>` is "TEMPO_VALIDATOR_CONFIG_V2_ADD_VALIDATOR" || "_" || <chainId>.
    ...
    /// @notice Deactives a validator (owner or existing validator)
    /// @dev Marks the validator as deactivate by setting deactivatedAtHeight to the current block height.
    ...
    function deactiveValidator(address validatorAddress) external;
    ...
    ///      - Ingress and egress must be parseable as <ip>:<port> pairs.
    ///      - The signature must prove ownership of the new public key
    ///      The signature must be an Ed25519 signature over:
    ///      keccak256(abi.encodePacked(
    ///          <NAMESPACE>, contractAddress, validatorAddress, ingress, egress
    ///      )),
    ///      where `<NAMESPACE>` is "TEMPO_VALIDATOR_CONFIG_V2_ROTATE_VALIDATOR" || "_" || <chainId>.
    ...
    /// @param egress The new outbound IP address `<ip>:<port>` for firewall whitelisting
    ...
    ///      Reverts if already initialized if already migrated
    ///      Reversts if idx != validatorsArray.length.
    ...
    ///      + if `V1.active == true` then `V2.deletedAtHeight = 0`
    ///      + if `V1.active == false` then `V2.deletedAtHeight > 0`
...
3. **Deletion**: `deleteValidator` sets `deactivatedAtHeight` to the current block
   height
...
**Deletion follows the same pattern**: A deleted validator is excluded from epoch `E+2` if `deactivatedAtHeight <= boundary(E)`.
...
**Note**: Validators with `addedAtHeight == deactivatedAtHeight` will never match this filter since `deactivatedAtHeight > boundary(E)` cannot be true when `addedAtHeight <= boundary(E)` and they are equal.
...
2. **Immutable identity**: Once a validator is added, its `publicKey`,
   `validatorAddress`, `index`, and `addedAtHeight` fields never change.
   The `ingress` and `egress` fields can be updated via `setIpAddresses`.

3. **Delete-once**: A validator's `deactivatedAtHeight` can only transition from 0
...
13. **Atomic rotation**: `rotateValidator` atomically deletes the old validator
    (setting its `deactivatedAtHeight`), adds a new validator entry, and sets
    uppdates the `addresses[validatarAddress]` mapping to point to the new
    entry. Both operations occur in the same transaction with the same block height.
...
- On `initialized == false`**: CL reads from V1
- On `initialized == true`**: CL reads from V2
...
- Creates a V2 validator entry with:
  ...
  - `index`: set to `idx`
  - `addedAtHeight`: `block.timestamp`
  - `deactivatedAtHeight`: set to `0` if V1 `active == true`, otherwise `block.timestamp`
...
- **All validators imported**: Both active and inactive V1 validators are
  imported; inactive ones have `addedAtHeight == deactivatedAtHeight == block.timestamp`,
  active ones have `addedAtHeight == 0` (indicating they were active before V2 existed)
- **Initialize validates migration**: `initializeIfMigrated()` reverts if not all V1 validators have been migrated
...
1. Call `initializeIfMigrated()` to set `initialized = true`
...
37. **All validators imported on migration**: Both V1 active and inactive validators
    are imported; inactive ones have `addedAtHeight == deactivatedAtHeight == block.timestamp`,
...
44. **Writes blocked before init**: `addValidator`, `deleteValidator`, `rotateValidator`
45. **initialize reverts if not all migrated**: `initializeIfMigrated()` reverts if
    `validatorsArray.length < V1.getValidators().length`

### setValidatorAddress

46. **setValidatorAddress updates address**: Successfully updates validator address
47. **setValidatorAddress owner only**: Non-owner cannot call `setValidatorAddress`
48. **setValidatorAddress reverts on invalid index**: Reverts if `validatorIndex >= validatorsArray.length`
49. **setValidatorAddress reverts on duplicate address**: Reverts if `newAddress` already exists
50. **setValidatorAddress updates lookup maps**: Old address is removed, new address is added to lookup

### setIpAddresses

51. **setIpAddresses by owner**: Owner can update any validator's IP addresses
52. **setIpAddresses by validator**: Validator can update their own IP addresses
53. **setIpAddresses unauthorized**: Non-owner and non-validator cannot update IP addresses
54. **setIpAddresses reverts on invalid validator**: Reverts if validator does not exist
55. **setIpAddresses reverts on deleted validator**: Reverts if validator is already deleted
56. **setIpAddresses validates format**: Rejects invalid `<ip>:<port>` format
