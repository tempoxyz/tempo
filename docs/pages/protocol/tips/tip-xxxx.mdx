...
10. **Owner authorization**: Only the owner can call `addValidator`,
    `deleteValidator`, `transferOwner`, `setNextFullDkgCeremony`, and `transferValidatorOwnership`.
...
56. **setIpAddresses reverts on deleted validator**: Reverts if validator is already deleted
