...
You can only place orders on pairs between a token and its designated quote token. All TIP-20 tokens specify a quote token for trading pairs. [PathUSD](/protocol/exchange/pathUSD) can be used as a simple choice for quote token.
...
1. **Placement**: When you call `place()` or `placeFlip()`:
   ...
   - The order is inserted into the orderbook at the chosen tick
   - The function returns an order ID

2. **Filling**: As swaps execute against your order:
   - Your order fills partially or completely in price‑time priority
   - Proceeds are credited to your DEX balance
   - If a flip order fills completely, it creates a new resting order on the opposite side at `flipTick`
...
Any unfilled portion of your order is refunded to your [DEX balance](/protocol/exchange/exchange-balance).
...
For complete details on checking balances, depositing, withdrawing, and best practices for managing your exchange balance, see the [Exchange Balance](/protocol/exchange/exchange-balance) page.
