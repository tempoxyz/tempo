...
After setting a validator token preference, all fees collected in blocks the validator proposes will be automatically converted to the chosen token (if needed). Validators can receive their accumulated fees at any time by calling `distributeFees()`.
...
Validators can receive their accumulated fees at any time by calling `distributeFees(validator)` on the FeeManager. This transfers their fee balance to their address and emits a `FeesDistributed` event.
...
9. Validator calls `distributeFees()` to receive their accumulated 0.9970 USDT
