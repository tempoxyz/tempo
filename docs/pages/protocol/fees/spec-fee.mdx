...
3. TIP-20 contract (if the transaction is calling `transfer`, `transferWithMemo`, or `startReward` on a TIP-20 contract, the transaction uses that token as its fee token)
...
For [Tempo transactions](/documentation/protocol/transactions/spec-account-abstraction.mdx), this rule applies only if _all_ top-level calls are to the same TIP-20 contract, and each such call is to one of the functions listed above, with `fee_payer == tx.origin`.
