...
TIP-20 Rewards is a built-in mechanism that allows token issuers to efficiently distribute rewards to token holders proportional to their holdings. The system supports both streaming rewards (distributed linearly over time) and instant rewards, all while maintaining low gas costs at scale and complying with [TIP-403 transfer policies](/protocol/tip403/spec).
