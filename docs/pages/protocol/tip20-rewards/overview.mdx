...
Note: Time-based streaming rewards are planned for a future upgrade. Until then, attempting to create a timed distribution will revert (calling `distributeReward(amount, seconds_)` with `seconds_ > 0` reverts with `ScheduledRewardsDisabled()`).
