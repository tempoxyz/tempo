...
Current spec *intentionally excludes garbage collection* for nonces.
...
2. **Gas Model Addresses State Growth**
Creating new nonce keys incurs 22,100 gas (SSTORE set cost), which provides natural economic pressure against state bloat. We can also introduce a protocol limit of 32 or 64 nonce keys for each account if needed.
...
**Worst Case Scenario:**
- An attacker could create many nonce keys by paying 22,100 gas per new key
- Example: Creating 100 nonce keys would require cumulative gas costs of ~2,210,000 gas


**Practical Usage:**
- Most users will use 1-5 parallel nonce keys for typical parallel transaction patterns
- The fixed cost per new key (22,100 gas) is reasonable for legitimate use cases
