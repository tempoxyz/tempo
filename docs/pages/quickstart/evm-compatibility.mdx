...
<Card.Container>
  <Card.Link
    description="How Tempo handles wallet compatibility, balance checks, and native token representation."
  ...
  />
  <Card.Link
    description="VM-level differences including balance opcodes and JSON-RPC compatibility."
...
We endorse [this proposed ERC](https://github.com/ethereum/ERCs/pull/1220) to allow wallets to indicate to apps that they are able to pay for gas fees using sources other than the chain's native gas token.
...
>
  <p>
    We strongly recommend using <a href="/quickstart/integrate-tempo#tempo-transactions">Tempo Transactions</a> over legacy Ethereum transactions if you have control over the transaction submission, in order to leverage features like native gas sponsorship.
  </p>
</Card.Notice>
...
This preference system is specified [here](/documentation/protocol/fees/spec-fee#fee-token-preferences) in detail. 

#### Consideration 1: Setting a user default fee token

As specified in the preference system above, the simplest way to specify the fee token for a user is to set the user default fee token. Read about how to do that [here](/documentation/protocol/fees/spec-fee#account-level) on behalf of an account.
...
:::info
We are exploring transaction level introspection for Tempo Transactions, with an ability to declare things like `tx.fee_tokens` and `tx.fee_payer` in Solidity.
:::
