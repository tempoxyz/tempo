...
Tempo does not have a native gas token. Instead, fees are denominated in USD and fees can be paid in a stablecoin. For Tempo Transactions, the `fee_token` field can be set to any TIP-20 token, and fees are paid in that token.
