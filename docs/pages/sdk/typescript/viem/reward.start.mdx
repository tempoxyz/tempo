...
type ReturnType = {
  ...
  funder: Address
  /** Unique stream ID (0 for immediate distributions, >0 for streaming) */
  id: bigint
  /** Transaction receipt */
...
The duration in seconds. Use `0` for immediate distribution, or `>0` for linear streaming over time.
