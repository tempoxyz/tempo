...
You can only place orders on pairs between a token and its designated quote token. All TIP-20 tokens specify a quote token for trading pairs. [PathUSD](/protocol/specs/stablecoin-exchange/pathUSD) can be used as a simple choice for quote token.
...
For complete details on checking balances, depositing, withdrawing, and best practices for managing your exchange balance, see the [Exchange Balance](/protocol/specs/stablecoin-exchange/exchange-balance) page.
